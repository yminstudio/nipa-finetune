...
## Record ID
01_유형분류_기준_structure_0006

## Question
자산별 적용지수를 주식형에 적용하는 기준은 무엇인가?
...
```json
{
  ...
  "generated_question": "자산별 적용지수를 주식형에 적용하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "적용 기준 템플릿"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "자산별 적용지수를 주식형에 적용하는 기준은 무엇인가?",
...
## Assistant Response
```text
주식형(주식자산)에는 **자산별 적용지수**로 **Capital Return 지수**를 적용합니다. 

이 기준은 주식 BM이 **Total Return이 아닌 Capital Return** 기준이기 때문이며, 주식 운용 펀드는 **배당수익이 별도로 발생**해 BM 대비 성과가 높게 나올 수 있어 형평성을 맞추기 위한 취지입니다.
```
