...
```json
{
  ...
  "qa_type": "definition",
  "question_template": "BM의 설정기준 세부적용 방법 정의"
}
```
...
## Assistant Response
```text
BM의 설정기준 세부적용 방법은, 펀드의 **국내/해외 여부**와 **환헷지 여부**에 따라 BM(벤치마크)을 어떤 기준으로 적용할지 정하는 적용 규칙입니다. 

1) 유형 BM, 펀드 BM의 별도 적용   
- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 **별도 펀드 BM을 적용하지 않고 유형 BM 적용**  
- 해외펀드: **펀드 BM을 별도 적용**  
