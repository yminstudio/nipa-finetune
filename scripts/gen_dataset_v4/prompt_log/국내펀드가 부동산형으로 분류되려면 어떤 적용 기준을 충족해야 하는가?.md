...
## Question
국내펀드가 부동산형으로 분류되려면 어떤 적용 기준을 충족해야 하는가?
...
```json
{
  ...
  "generated_question": "국내펀드가 부동산형으로 분류되려면 어떤 적용 기준을 충족해야 하는가?",
  "qa_type": "application",
  "question_template": "Application_DomesticFundType"
}
```
...
```json
{
  ...
  "subsection": "1) 국내펀드",
  "seed_title": "1) 국내펀드",
  "question": "국내펀드가 부동산형으로 분류되려면 어떤 적용 기준을 충족해야 하는가?",
...
## Assistant Response
```text
국내펀드가 “부동산형”으로 분류되려면, 제로인은 먼저 투자설명서상의 투자목적과 투자전략에 따라 펀드 유형을 정하고, 국내투자펀드는 자산유형을 기준으로 운용전략/스타일 관점에서 판단합니다.  

그 결과, 국내 유형 중 “부동산형”은 아래의 부동산 관련 자산/전략(소유형) 중 하나(또는 이에 준하는 구성)로 분류될 때 해당합니다. 
