...
```json
{
  ...
  "target_total": 100,
  "generated_question": "정보비율(IR)의 판단 기준은 무엇으로 결정되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 정보비율 ; Information Ratio(IR)",
  "question": "정보비율(IR)의 판단 기준은 무엇으로 결정되는가?",
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과인 초과수익률(펀드 주간 로그수익률 평균 − 벤치마크 주간 로그수익률 평균)을 적극적 활동에 따른 변동성(추적오차, Tracking Error)으로 나눈 값이며   
- 제로인은 IR의 판단 기준을 “이 비율이 높을수록 더 좋은 투자활동으로 판단”하는 것으로 결정합니다
```
...
## Assistant Response
```text
정보비율(IR)은 적극적인 투자활동 결과의 “초과수익률”과 그 활동으로 인해 발생하는 “변동(추적오차)”의 관계를 판단하기 위한 지표입니다.   
- IR의 판단 기준은 IR 값의 크기이며, 비율이 높을수록 더 좋은 투자활동으로 판단합니다.   
- IR은 (펀드의 주간 로그수익률 평균 − 벤치마크(BM)의 주간 로그수익률 평균)을 (펀드와 BM의 추적오차, TE)로 나눈 값으로 결정됩니다.   
- 즉, 분자는 초과수익률 성격의 수익 차이, 분모는 추적오차(변동)로 구성됩니다.
```
...
## Assistant Response
```text
정보비율(IR)은 적극적인 투자활동의 결과(성과)를 판단하기 위한 지표이며, 초과수익률과 그에 따른 변동성(추적오차)로 결정됩니다. 

- 판단의 “보상(Reward)”은 펀드의 주간 로그수익률 평균에서 벤치마크(시장/BM)의 주간 로그수익률 평균을 뺀 초과수익률입니다. 
- 판단의 “변동성(Variability)”은 펀드와 벤치마크(시장/BM)의 추적오차(Tracking Error)입니다. 
- 따라서 IR은 (초과수익률) / (추적오차)로 산출되며, 이 값이 높을수록 더 좋은 투자활동으로 판단합니다.
```
...
## Assistant Response
```text
정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 초과수익률이 “얼마나 일관되게” 나타났는지를 판단하기 위한 지표입니다. 

- 판단의 핵심은 초과수익률(펀드 주간 로그수익률 평균 − 시장(또는 BM) 주간 로그수익률 평균)입니다.   
- 그 초과수익률을 추적오차(TE: 펀드와 시장(또는 BM) 간 Tracking Error)로 나누어 변동성/불일치 정도를 반영합니다.   
- 따라서 IR 값(= 초과수익률 ÷ 추적오차)이 높을수록 더 좋은 투자활동으로 판단합니다.
```
...
## Assistant Response
```text
정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 성과를 판단하기 위한 지표입니다. 

- 판단 기준은 IR 값 자체이며, IR이 높을수록 더 좋은 투자활동으로 판단합니다. 
- IR은 (펀드 주간 로그수익률 평균 − 벤치마크(BM) 주간 로그수익률 평균)인 초과수익 성격의 분자와, 펀드-벤치마크 간 추적오차(Tracking Error, TE)를 분모로 하는 비율로 결정됩니다. 
- 따라서 “초과수익(보상)”과 “추적오차(변동성)”의 상대적 크기가 IR을 통해 판단 기준으로 반영됩니다.
```
