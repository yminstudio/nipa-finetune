...
## Question
MM’s RAP 평가에서 위험지표로 표준편차를 사용하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MM’s RAP 평가에서 위험지표로 표준편차를 사용하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_metric_usage"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "9) MM’s RAP ; Modigliani & Modigliani ‘s Risk Adjusted Performance",
  "question": "MM’s RAP 평가에서 위험지표로 표준편차를 사용하는가?",
...
## Assistant Response
```text
- MM’s RAP( Modigliani & Modigliani ’s Risk Adjusted Performance )은 시장(또는 BM) 표준편차(σ^M)와 펀드 표준편차(σ^F)를 사용합니다.   
- 따라서 MM’s RAP 평가에서 위험지표로 표준편차를 사용합니다.
```
...
## Assistant Response
```text
MM’s RAP은 위험을 반영해(위험조정) 펀드의 초과성과를 판단하기 위한 지표입니다.   
예, MM’s RAP에서 위험지표로 표준편차(총위험/변동성)를 사용합니다.   
구체적으로 RAP^{F,M}는 시장(또는 BM)의 표준편차 σ^M를 곱하고, 펀드의 표준편차 σ^F로 나누는 형태로 정의됩니다.
```
...
## Assistant Response
```text
- MM’s RAP는 CAPM에서 시장(혹은 BM) 위험이 주어졌을 때의 펀드 “가상 수익률”을 보여주기 위한 위험조정 성과지표입니다.   
- 네, 표준편차를 위험지표로 사용합니다. MM’s RAP는 σ^M(시장/BM의 표준편차)를 곱하고, 샤프비율 구성에서 σ^F(펀드의 표준편차)로 나누는 형태로 계산됩니다.   
- 따라서 MM’s RAP 평가에서는 펀드의 표준편차와 BM(시장)의 표준편차가 모두 위험으로 반영됩니다.
```
...
## Assistant Response
```text
MM’s RAP 평가는 위험을 반영한 성과(위험조정 성과)를 판단하기 위한 지표입니다.   
- 사용되는 위험지표는 표준편차이며, 펀드의 표준편차(σF)가 분모에 들어갑니다.   
- 또한 시장(또는 BM)의 표준편차(σM)가 곱해져 함께 반영됩니다.   
- 따라서 MM’s RAP는 RAP^F,M = σM × ( r̄F − r̄rf ) / σF 형태로 계산됩니다.
```
...
## Assistant Response
```text
MM’s RAP은 위험을 반영해 조정된 성과를 판단하기 위한 지표입니다. 

- MM’s RAP 산식에 위험 관련 항으로 표준편차가 포함됩니다.   
- 펀드 위험지표로는 펀드의 표준편차(σ^F)를 분모에 사용합니다.   
- 또한 시장(또는 BM)의 표준편차(σ^M)를 곱해 CAPM의 시장위험(BM의 위험)이 주어진 상황에서의 가상 수익률을 나타내도록 설계되어 있습니다.
```
