...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Drawdown의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "11) Maximum Drawdown",
  "question": "Maximum Drawdown의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 펀드 수익률의 고점에서 다음 저점까지의 하락폭이 가장 큰 구간의 하락비율이 Maximum Drawdown의 판단 기준입니다.   
- 누적 수익률 기준으로, 과거 거래에서 발생한 최악의 수익률(특정 시점 투자자의 최대손실가능성)을 의미합니다.   
- 계산값은 각 시점의 Drawdown \(D_t\) 가운데 최댓값으로 판단하며, \(MD=\max_{0\le t\le n}D_t\) 입니다.
```
...
## Assistant Response
```text
Maximum Drawdown은 펀드의 “누적 수익률 기준 최악의 하락(최대손실가능성)”을 판단하기 위한 지표입니다. 

- 펀드 수익률의 고점(과거 누적 수익률의 최고값)에서 다음 저점(해당 시점까지의 누적 수익률)까지의 하락폭 중 “가장 큰 구간”의 하락비율을 최대낙폭으로 봅니다. 
- 각 시점 t에서 Drawdown은 다음처럼 정의합니다.  
  D_t = (고점 대비 t시점 누적수익률 하락액) / 고점 
- Maximum Drawdown(MD)은 전체 기간에서 D_t의 최댓값으로 산정합니다.  
  MD = max(0 ≤ t ≤ n) D_t
```
...
## Assistant Response
```text
- Maximum Drawdown은 펀드 수익률의 고점에서 다음 저점까지 하락폭이 가장 큰 구간의 하락비율(최악의 누적손실)을 판단하기 위한 지표입니다.   
- 각 시점 t에서 Drawdown은 0~t까지의 누적수익지수 최고값과 0~t까지의 누적수익지수의 차이를, 그 최고값으로 나눈 비율로 판단합니다(D_t).   
- Maximum Drawdown은 전체 기간(0~n)에서 D_t 값들 중 최댓값으로 판단합니다(MD = max D_t).
```
...
## Assistant Response
```text
Maximum Drawdown은 펀드의 과거 구간에서 나타나는 최악의 누적손실(최대손실가능성)을 판단하기 위한 지표입니다. 

- 펀드 수익률의 고점에서 다음 저점까지의 하락폭이 가장 큰 구간의 하락비율을 Maximum Drawdown으로 판단합니다.   
- 각 시점 t에서 Drawdown은 누적 수익률의 직전 최고치 대비 현재 누적수익률이 얼마나 낮아졌는지로 정의하며,  
  D_t = (max_{0≤s≤t}(1+r_0^s) − (1+r_0^t)) / max_{0≤s≤t}(1+r_0^s) 입니다.   
- Maximum Drawdown은 0≤t≤n 구간에서의 D_t 최댓값, 즉 MD = max_{0≤t≤n} D_t로 결정합니다.
```
...
## Assistant Response
```text
Maximum Drawdown은 펀드의 누적 수익률 기준으로 “고점 대비 다음 저점까지” 하락이 가장 크게 발생한 구간의 하락비율을 판단합니다. 

- 특정 시점 t의 Drawdown(Dt): 과거(0~t) 누적수익률 고점 대비 현재(0~t) 누적수익률 저점까지의 하락비율   
- Maximum Drawdown(MD): 기간(0~n) 내 Drawdown(Dt) 값들 중 가장 큰 값   
- 수식: MD = max(0≤t≤n) Dt
```
