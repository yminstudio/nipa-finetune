...
## Question
Modified Sharpe Ratio의 판단 기준에서 값이 양수일 때 우수한 위험조정성과를 시사하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Modified Sharpe Ratio의 판단 기준에서 값이 양수일 때 우수한 위험조정성과를 시사하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_2"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 수정샤프지수 ; Modified Sharpe Ratio",
  "question": "Modified Sharpe Ratio의 판단 기준에서 값이 양수일 때 우수한 위험조정성과를 시사하는가?",
...
## Assistant Response
```text
- 수정샤프지수는 초과수익률이 (+)인 경우에는 샤프지수와 같은 값을 갖도록 설계되어, 이때는 “수익률은 높을수록, 위험은 낮을수록 좋은 성과”로 해석하는 방향을 따른다.   
- 따라서 수정샤프지수 값이 양수라면 우수한 위험조정성과를 시사한다.
```
...
## Assistant Response
```text
- 수정샤프지수(MS, Modified Sharpe Ratio)는 샤프지수에서 무위험초과수익률이 음수일 때 해석이 반대로 나타나는 문제를 보완해 위험조정 성과를 판단하기 위한 지표입니다.   
- MSR은 무위험초과수익률이 (+)이면 샤프지수와 같은 값을, (–)이면 “수익률이 높을수록, 위험이 낮을수록” 좋은 성과로 읽히도록 보정합니다.   
- 따라서 MSR 값이 양수일 때는, 이 보정이 목표로 하는 방향(수익 대비 위험이 유리한 쪽)으로 위험조정 성과가 우수함을 시사한다고 이해할 수 있습니다.
```
...
## Assistant Response
```text
- 수정샤프지수는 무위험 초과수익을 위험과 함께 묶어 위험조정 성과를 판단하기 위한 지표입니다.   
- 방법론에서는 초과수익률이 (+)인 경우 수정샤프지수가 샤프지수와 같은 값으로 해석되도록 설계한다고 설명합니다.   
- 따라서 수정샤프지수가 양수이면(초과수익이 (+)인 경우) 샤프지수의 해석처럼 값이 높을수록 투자성과가 성공적이라는 맥락에서 우수한 위험조정성과를 시사합니다.
```
...
## Assistant Response
```text
- 수정샤프지수는 무위험 초과수익(펀드 수익률에서 무위험수익률을 뺀 값)이 음수인 경우 샤프지수 해석이 뒤집히는 문제를 보완해 위험조정 성과를 보이기 위한 지표입니다.   
- 무위험 초과수익이 (+)인 경우에는 수정샤프가 샤프지수와 같은 값의 성격을 갖게 하도록 설계되어 있습니다.   
- 샤프지수는 값이 높을수록 투자성과가 성공적이라고 설명되므로, 수정샤프지수가 양수라면(무위험 초과수익이 +) 우수한 위험조정성과를 시사한다고 볼 수 있습니다.
```
...
## Assistant Response
```text
- Modified Sharpe Ratio는 샤프지수가 무위험 초과수익이 음(-)일 때 해석이 뒤집히는 문제를 보완해, 위험조정 성과를 일관되게 읽기 위한 지표다.   
- 초과수익이 (+)인 경우 Modified Sharpe Ratio는 샤프지수와 같은 값을 가지므로, 값이 양수라면 샤프지수가 높을수록 투자성과가 성공적이라는 해석 방향에서 우수한 위험조정성과를 시사한다고 볼 수 있다.
```
