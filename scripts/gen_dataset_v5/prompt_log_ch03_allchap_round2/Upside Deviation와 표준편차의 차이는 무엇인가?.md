...
## Question
Upside Deviation와 표준편차의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Upside Deviation와 표준편차의 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_상승편차_표준편차"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) 상승편차 ; Upside Deviation",
  "question": "Upside Deviation와 표준편차의 차이는 무엇인가?",
...
## Assistant Response
```text
- 표준편차는 투자기간 동안 펀드수익률이 평균수익률과 대비해 변동한 범위를 측정해, 값이 클수록 변동성(위험)이 크다는 지표입니다.   
- 상승편차(Upside Deviation)는 펀드수익률이 벤치마크(BM) 대비 상승한 시기의 표준편차(변동성)로, 플러스 수익률에 대한 위험지표입니다.   
- 차이의 핵심은 “평균 대비 변동성(표준편차)”인지 “BM 대비 상승한 구간의 변동성(상승편차)”인지의 기준/대상 구간이 다르다는 점입니다.
```
...
## Assistant Response
```text
- 표준편차는 펀드 수익률이 평균수익률과 대비해 변동한 범위를 측정하는 통계량으로, 값이 클수록 변동성(총위험)이 크다고 봅니다.   
- 상승편차(Upside Deviation)는 펀드 수익률이 벤치마크(BM/시장) 대비 “상승한 시기”의 변동성을 보는 지표로, 플러스 구간(상방)과 연관된 위험을 나타냅니다.   
- 즉, 표준편차는 평균 대비 전체 변동을 보는데 비해 상승편차는 BM을 초과한 기간만을 대상으로 하여 상방(+) 국면의 변동성을 요약한다는 점에서 차이가 있습니다.
```
...
## Assistant Response
```text
같은 “수익률의 변동(위험/변동성)”을 수치화한다는 공통 맥락에서, 표준편차와 상승편차는 변동을 어떤 구간/기준에 대해 보느냐가 다릅니다.  

- 표준편차: 투자기간 동안 펀드수익률이 평균수익률과 대비해 변동한 범위를 측정(전체 구간 기준)하며, 값이 클수록 위험이 큽니다. 
- 상승편차(Upside Deviation): 벤치마크(BM) 대비 상승한 시기의 표준편차(변동성)로, ‘플러스’ 구간(BM 초과)에서의 변동을 위험지표로 봅니다. 
- 따라서 표준편차는 “평균 대비 전 기간 변동성”을, 상승편차는 “BM 초과(상승) 시점의 변동성”을 각각 강조합니다.
```
...
## Assistant Response
```text
- 두 지표 모두 수익률의 “변동성(위험)”을 나타내는 위험지표라는 공통 맥락이 있습니다.    
- 표준편차는 펀드 수익률이 평균수익률 대비 변동한 범위 전체를 측정하는 반면, Upside Deviation은 벤치마크(BM) 대비 상승한 시기만을 대상으로(초과분만, 그 외는 0 처리) 변동성을 측정합니다.    
- 따라서 표준편차는 “총위험/전 구간”의 변동성을 의미하는 데 비해, Upside Deviation은 “BM을 초과해 플러스가 난 구간”의 변동성(플러스 수익률에 대한 위험)을 지표로 삼습니다.
```
...
## Assistant Response
```text
- 공통 맥락: 둘 다 펀드 수익률의 “변동성(위험)”을 수치로 나타내는 위험지표입니다.    
- 차이 1(기준점): 표준편차는 펀드 수익률이 “평균수익률”과 얼마나 달라지는지로 변동을 측정합니다.  반면 Upside Deviation은 “벤치마크(BM) 대비 상승한 시기”만을 대상으로 변동성을 측정합니다.   
- 차이 2(포착하는 구간의 성격): 표준편차는 전체 변동성(전반적 위험) 관점이고, Upside Deviation은 BM 대비 “상승 국면의 변동성(플러스 수익률에 대한 위험)” 관점입니다.    
- 차이 3(해석 방향의 초점): 표준편차는 값이 클수록 전체 변동성이 커 위험이 크다는 해석에 초점이 있고, Upside Deviation은 값이 클수록 BM 대비 상승 시점의 위험이 더 크다는 해석에 초점이 있습니다.
```
