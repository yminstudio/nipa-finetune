...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Sortino Ratio의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "10) Sortino Ratio",
  "question": "Sortino Ratio의 정의는 무엇인가?",
...
## Assistant Response
```text
- Sortino Ratio는 “무위험수익률을 초과하는 펀드 수익률”을 “하방편차(Downside Risk)”로 나눈 값입니다.   
- 샤프지수가 양측 표준편차를 사용하는 데 비해, Sortino Ratio는 수익률이 마이너스일 경우의 하락편차만을 사용하여 마이너스 수익률에 대한 위험만을 고려합니다.   
- 산식: \(SR_D^{F,BM}=\dfrac{\overline{R_A^F}-\overline{R_A^{rf}}}{\sigma_D^{F,BM}}\).
```
...
## Assistant Response
```text
- Sortino Ratio는 무위험수익률을 초과하는 펀드 수익률을 하방편차로 나눈 값입니다.   
- 샤프지수는 양측 변동성을 쓰는 반면, Sortino Ratio는 수익률이 마이너스일 때의 하락편차만을 사용해 마이너스 수익률에 대한 위험만을 고려합니다.   
- 따라서 하방위험 1단위당 초과수익(무위험초과)을 의미하며, 수치가 클수록 큰 손실 위험이 적다고 해석됩니다.
```
...
## Assistant Response
```text
- Sortino Ratio는 무위험수익률을 초과하는 펀드 수익률을 하방편차(하락편차)로 나눈 값입니다.   
- 즉, 샤프지수처럼 양측 변동성을 쓰는 것이 아니라 수익률이 마이너스일 때의 하락편차만 사용해 마이너스 수익률에 대한 위험만을 고려합니다.   
- 산식은 다음과 같습니다. SR_D^{F,BM} = ( \overline{R_A^F} - \overline{R_A^{rf}} ) / σ_D^{F,BM}.
```
...
## Assistant Response
```text
- Sortino Ratio는 무위험수익률을 초과하는 펀드 수익률을 하방편차(Downside Deviation)로 나눈 값입니다.   
- Sharpe Ratio가 양측 변동성(양의/음의 표준편차)을 모두 쓰는 것과 달리, Sortino Ratio는 수익률이 마이너스인 경우의 하락편차만을 사용하여(즉, 마이너스 수익률에 대한 위험만 고려) 하방위험 1단위당 초과수익을 의미합니다.
```
...
```text
- Sortino Ratio는 무위험수익률을 초과하는 펀드 수익률을 하방편차로 나눈 값입니다.   
- 샤프지수처럼 양측 변동성을 쓰기보다, 수익률이 마이너스일 경우의 하락편차(하방편차)만 사용해 마이너스 수익률에 대한 위험만을 고려합니다.   
- 즉, 하방위험 1단위당 펀드의 초과수익을 의미합니다.
```
