...
## Question
Maximum Drawdown에서 판단하는 하락 구간은 어떤 구간인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Drawdown에서 판단하는 하락 구간은 어떤 구간인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "11) Maximum Drawdown",
  "question": "Maximum Drawdown에서 판단하는 하락 구간은 어떤 구간인가?",
...
## Assistant Response
```text
- Maximum Drawdown에서의 하락 구간은 펀드 수익률의 고점에서 다음 저점까지의 구간 중, 하락폭(하락비율)이 가장 큰 구간입니다.   
- 해당 “Maximum Drawdown이 발생한 기간”은 Maximum Drawdown Start Date(고점 시점)부터 End Date(저점 시점)까지의 기간입니다.
```
...
## Assistant Response
```text
Maximum Drawdown에서 판단하는 하락 구간은 “펀드 수익률의 고점에서 다음 저점까지” 중 하락폭이 가장 큰 구간을 뜻합니다. 

- 각 시점 t에서 0~t까지의 누적수익률(수익지수) 최대값(고점)과 현재 누적수익률(저점에 해당하는 값) 사이의 하락비율을 Drawdown D_t로 계산합니다.   
- 그 D_t가 최대가 되는 순간의 “고점(0~t까지의 최대 누적수익률) → 다음 저점(해당 시점 누적수익률)” 구간이 Maximum Drawdown에서 말하는 하락 구간입니다.
```
...
## Assistant Response
```text
Maximum Drawdown에서의 “하락 구간”은 최대 누적손실(최대손실가능성)을 판단하기 위한 피크-트로프(고점-저점) 구간입니다. 

- 누적 수익률 기준으로 수익률의 “고점(이전 최고 누적수익률 시점)”부터 “다음 저점(현재 시점 누적수익률 시점)”까지의 하락 구간입니다.   
- 그 하락폭이 가장 큰 고점→저점까지의 하락비율이 Maximum Drawdown으로 판단됩니다.   
- 수학적으로는 임의의 시점 t에서, 0~t 사이 누적수익률의 최고치 대비 t시점의 누적수익률 하락폭(=Drawdown)을 사용해 구간을 특정합니다.
```
...
## Assistant Response
```text
Maximum Drawdown이 하락위험(최대 누적손실)을 판단하는 데, 하락 구간은 누적 수익률이 “고점에서 저점으로 내려가는” 구간을 뜻합니다. 

- 하락 구간의 시작: 누적수익률이 도달한 과거의 고점(해당 시점까지의 최대 누적수익률)   
- 하락 구간의 끝: 그 고점 이후 더 낮은 누적수익률이 나타난 다음 저점(해당 시점의 누적수익률)   
- 그중 선택 기준: 위와 같은 “고점→저점” 형태의 하락폭 중 가장 큰 하락비율(= 최대 드로우다운)에 해당하는 구간
```
...
## Assistant Response
```text
Maximum Drawdown이 “하락 구간”을 판단하는 목적은, 과거 누적수익률 흐름에서 가장 큰 하락(최대 누적손실)을 찾아내기 위함입니다. 

- 하락 구간은 펀드 수익률의 고점(해당 시점까지의 누적 수익률 중 최대가 형성된 시점)에서 다음 저점(그 고점 이후 누적 수익률이 그만큼 낮아진 시점)까지로 봅니다.   
- 각 시점 t에 대해, “t까지의 누적수익률 최대값”에서 “t 시점 누적수익률”까지의 하락비율(Drawdown)을 계산하고 그 하락폭이 가장 큰 구간을 Maximum Drawdown이 됩니다.   
- 이때 해당 최대낙폭이 발생한 하락 구간의 시작일과 종료일을 각각 Maximum Drawdown Start Date, Maximum Drawdown End Date로 잡습니다(하락 구간의 길이는 두 날짜의 차).
```
