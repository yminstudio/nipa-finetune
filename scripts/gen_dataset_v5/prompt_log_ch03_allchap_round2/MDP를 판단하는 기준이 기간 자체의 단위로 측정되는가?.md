...
## Question
MDP를 판단하는 기준이 기간 자체의 단위로 측정되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MDP를 판단하는 기준이 기간 자체의 단위로 측정되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "16) Maximum Drawdown Periods",
  "question": "MDP를 판단하는 기준이 기간 자체의 단위로 측정되는가?",
...
## Assistant Response
```text
- MDP(Maximum Drawdown Period)는 Maximum Drawdown이 발생한 구간의 기간을 “Maximum Drawdown End Date - Maximum Drawdown Start Date”로 계산하며, 이때 기간 단위가 주, 월로 제시됩니다.   
- 따라서 MDP를 판단하는 기준은 기간 자체의 단위(주, 월)로 측정됩니다.
```
...
## Assistant Response
```text
MDP(최대손실 구간의 기간)는 펀드에서 최대 드로우다운이 발생한 “구간의 길이”를 판단하기 위한 지표입니다.   
MDP는 Maximum Drawdown의 시작일과 종료일의 차로 계산되며, 이때 측정 단위는 “주 또는 월”로 제시됩니다.   
따라서 MDP 판단/산출의 핵심 기준은 기간 자체의 시간 단위(주·월)로 측정됩니다.
```
...
## Assistant Response
```text
MDP는 펀드의 Maximum Drawdown이 실제로 나타난 구간이 얼마나 지속됐는지를 보기 위한 지표입니다. 

- MDP는 Maximum Drawdown 발생의 시작일과 종료일의 차이로 계산합니다.   
- 이때 MDP의 단위는 주(주간) 또는 월(월간)로 제시됩니다.   
- 따라서 MDP를 판단하는 기준은 ‘기간 자체의 단위(주/월)’로 측정됩니다.
```
...
## Assistant Response
```text
MDP는 최대손실(MDD)이 발생한 “기간의 길이”를 판단하기 위한 지표입니다.   
- MDP^F = Maximum Drawdown End Date - Maximum Drawdown Start Date로, 시작일과 종료일 사이의 기간을 계산합니다.   
- 따라서 MDP 판단 기준은 기간 자체의 단위(주, 월)로 측정됩니다.   
- 예시에서도 MDP가 “3 (주)”처럼 주 단위로 나타납니다.
```
...
## Assistant Response
```text
MDP(최대낙폭 발생 기간) 판단 기준은 “최대 Drawdown이 발생한 구간”의 길이를 보려는 것입니다. 

- MDP는 Maximum Drawdown End Date에서 Maximum Drawdown Start Date를 빼서 산출합니다.   
- 이때 MDP의 단위는 “주, 월”로 정의됩니다.   
- 따라서 MDP 판단 기준은 기간 자체의 단위(주/월)로 측정됩니다.
```
