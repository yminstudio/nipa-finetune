...
```json
{
  ...
  "target_total": 100,
  "generated_question": "추적오차와 Tracking Error는 같은 개념인가?",
  "qa_type": "comparison",
  "question_template": "ComparisonQuestionTemplate"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "8) 추적오차 ; Tracking Error",
  "question": "추적오차와 Tracking Error는 같은 개념인가?",
...
## Assistant Response
```text
- 제로인 방법론에서는 Tracking Error(트래킹에러)를 “(지수)추적오차”라고도 부르므로, 두 용어는 같은 개념으로 사용됩니다.   
- 또한 펀드평가에서 추적오차/Tracking Error는 “펀드 기간수익률과 벤치마크 지표 수익률의 차이”에 대한 변동성(초과수익률의 표준편차)으로 측정합니다.
```
...
## Assistant Response
```text
- 제로인 방법론에서는 Tracking Error(트래킹에러)와 추적오차를 같은 개념으로 보고, 트래킹에러를 “(지수)추적오차”라고도 표기합니다 .  
- 따라서 용어만 달리 부르는 것이고, 펀드평가에서는 펀드 기간수익률과 벤치마크 지표 수익률의 차이 변동성(초과수익률의 표준편차)이라는 동일한 측정을 의미합니다 .
```
...
## Assistant Response
```text
- 제로인 펀드평가 방법론에서는 트래킹에러를 “(지수)추적오차”라고도 부르므로, 두 용어는 같은 개념으로 사용됩니다   
- 펀드평가에서는 펀드 기간수익률과 벤치마크 지표 수익률의 차이를 변동성(초과수익률의 표준편차)으로 측정한다는 점에서도 동일한 지표로 정리됩니다   
- 결론적으로 “추적오차”와 “Tracking Error”는 제로인 기준으로 동일하다고 보면 됩니다
```
...
## Assistant Response
```text
- 제로인에서는 트래킹에러(Tracking Error)를 추적오차로 보고, “(지수)추적오차”라는 표현도 함께 씁니다.   
- 두 용어 모두 일정기간 펀드 수익률과 대응하는 지수(벤치마크) 수익률의 차이에 대한 변동성(초과수익률의 표준편차)을 의미합니다.   
- 결론적으로 추적오차와 Tracking Error는 같은 개념이라고 보면 됩니다(용어만 다르게 부르는 경우).
```
...
## Assistant Response
```text
- 제로인 방법론에서는 Tracking Error(트래킹에러)를 “(지수)추적오차”라고도 부르므로, 용어상 같은 개념으로 사용합니다.   
- 공통 맥락은 “일정기간 펀드 수익률과 이에 대응하는 지수(벤치마크) 수익률 간 차이”를 측정하는 지표라는 점입니다.   
- 제로인에서는 이 추적오차(Tracking Error)를 “초과수익률(펀드-지수)로 본 표준편차”로 계산합니다.
```
