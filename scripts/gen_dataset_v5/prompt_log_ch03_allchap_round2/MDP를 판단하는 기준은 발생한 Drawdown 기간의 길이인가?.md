...
## Question
MDP를 판단하는 기준은 발생한 Drawdown 기간의 길이인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MDP를 판단하는 기준은 발생한 Drawdown 기간의 길이인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "16) Maximum Drawdown Periods",
  "question": "MDP를 판단하는 기준은 발생한 Drawdown 기간의 길이인가?",
...
## Assistant Response
```text
- MDP(Maximum Drawdown Period)는 Maximum Drawdown이 발생한 구간의 “기간(주, 월)” 길이로 판단합니다.   
- 계산 기준은 Maximum Drawdown End Date − Maximum Drawdown Start Date 입니다.
```
...
## Assistant Response
```text
MDP(Maximum Drawdown Period)는 펀드에서 Maximum Drawdown이 “발생한 구간”이 얼마나 오래 지속됐는지를 판단하기 위한 지표입니다.   
MDP 판단 기준은 그 Maximum Drawdown 구간의 기간(길이) 자체이며, MDP는 Maximum Drawdown End Date - Maximum Drawdown Start Date로 정의됩니다.   
따라서 MDP 값은 해당 구간의 길이를 주, 월 단위로 나타내는 형태입니다.
```
...
## Assistant Response
```text
MDP는 “최대낙폭(Maximum Drawdown)이 실제로 발생한 구간이 얼마나 지속됐는지”를 판단하기 위한 기간 지표입니다. 

- MDP의 판단 기준은 그 최대낙폭이 나타난 구간의 길이입니다.   
- 계산은 “Maximum Drawdown Start Date부터 End Date까지의 차이(주, 월)”로 정해집니다.   
- 즉, MDP는 최대낙폭의 크기가 아니라 해당 최대낙폭 구간의 지속기간을 길이로 나타낸 값입니다.
```
...
## Assistant Response
```text
MDP(Maximum Drawdown Period)는 펀드에서 Maximum Drawdown이 발생한 구간의 “길이”를 판단하기 위한 지표입니다.   
MDP는 Maximum Drawdown의 종료일에서 시작일을 뺀 값으로 계산하며, 주(weeks) 또는 월(months) 단위로 산정합니다.   
따라서 MDP를 판단하는 기준은 발생한 Drawdown(최대낙폭) 기간의 길이에 해당합니다.
```
...
## Assistant Response
```text
MDP(Maximum Drawdown Periods)는 최대낙폭(Maximum Drawdown)이 나타난 하락이 얼마나 오래 지속됐는지를 판단하기 위한 지표입니다.   
MDP의 판단(산출) 기준은 “최대낙폭이 발생한 기간의 길이”입니다.   
구체적으로는 최대낙폭의 시작일과 종료일의 차이(주, 월)로 MDP^F를 계산합니다.
```
