...
```json
{
  ...
  "section": "9) MM’s RAP ; Modigliani & Modigliani ‘s Risk Adjusted Performance",
  "subsection": "",
  "seed_title": "9) MM’s RAP ; Modigliani & Modigliani ‘s Risk Adjusted Performance",
  "record_target": 2,
  ...
  "target_total": 100,
  "generated_question": "MM’s RAP의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "definition_question_template"
}
```
...
[핵심 명사]
MM, RAP, Modigliani, Risk, Adjusted, Performance, 펀드의, 샤프비율에, 시장, 혹은, BM, 표준편차를
...
## Assistant Response
```text
- MM’s RAP(Modigliani & Modigliani’s Risk Adjusted Performance)는 펀드의 샤프비율에 시장(혹은 BM) 표준편차를 곱해, CAPM에서 시장위험(BM의 위험)이 주어졌을 때의 펀드 “가상 수익률”을 보여주는 지표입니다.   
- 정의식: RAP^{F,M} = σ^M × ( ( \overline{r^F} - \overline{r^{rf}} ) / σ^F ).
```
...
## Assistant Response
```text
MM’s RAP(Modigliani & Modigliani’s Risk Adjusted Performance)는 펀드의 샤프비율에 시장(혹은 BM)의 표준편차를 곱해, CAPM에서 시장위험(BM의 위험)이 주어졌을 때의 “가상 수익률”을 나타내는 지표입니다. 

RAP^{F,M} = σ^M × ( (r̄^F − r̄^{rf}) / σ^F )
```
...
## Assistant Response
```text
MM’s RAP( Modigliani & Modigliani Risk Adjusted Performance )은 펀드의 샤프비율에 시장(또는 BM) 표준편차를 곱해, CAPM에서 시장위험(BM의 위험)이 주어졌을 때의 펀드 “가상 수익률”을 나타내는 위험조정 성과지표입니다. 

- 정의식: RAP^{F,M} = σ^M × ( (r̄^F − r̄^{rf}) / σ^F ) 
- 여기서 r̄^F, r̄^{rf}는 각각 펀드와 무위험이자율의 주간 로그수익률 평균(연환산은 하지 않음)이며, σ^M은 시장(또는 BM), σ^F는 펀드의 표준편차입니다.
```
...
## Assistant Response
```text
- MM’s RAP(Modigliani & Modigliani’s Risk Adjusted Performance)은 펀드의 샤프비율에 시장(또는 BM) 표준편차를 곱해, 시장위험(BM의 위험)이 주어졌을 때의 펀드 “가상 수익률”을 보여주는 지표입니다.   
- 정의식: RAP^{F,M} = σ^M × ( ( r̄^F − r̄^{rf} ) / σ^F ).   
- 여기서 r̄^F는 주간 로그수익률 평균(연환산 하지 않음), r̄^{rf}는 무위험이자율 주간 로그수익률 평균이며(제로인: CD91 사용), σ^M은 시장(또는 BM) 표준편차, σ^F는 펀드 표준편차입니다.
```
...
## Assistant Response
```text
- MM’s RAP(Modigliani & Modigliani’s Risk Adjusted Performance)는 펀드의 샤프비율에 시장(또는 BM) 표준편차를 곱해, “시장위험(=시장/BM의 위험)이 주어졌을 때의 펀드 가상 수익률”을 나타내는 위험조정 성과지표입니다.   
- 정의식: RAP^{F,M} = σ^M × ( (평균 r^F − 평균 r^{rf}) / σ^F )   
- 여기서 r^F는 펀드의 주간 로그수익률 평균, r^{rf}는 무위험이자율의 주간 로그수익률 평균, σ^M는 시장(또는 BM)의 표준편차, σ^F는 펀드의 표준편차입니다.
```
