...
## Question
Bull/Bear Beta Ratio의 정의를 한 문장으로 서술하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Bull/Bear Beta Ratio의 정의를 한 문장으로 서술하시오.",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "11) Bull/Bear Beta Ratio",
  "question": "Bull/Bear Beta Ratio의 정의를 한 문장으로 서술하시오.",
...
## Assistant Response
```text
- Bull/Bear Beta Ratio는 상승베타(Bull Beta)를 하락베타(Bear Beta)로 나눈 값으로, BR^{F,M} = BL_B^{F,M} / BR_B^{F,M} 이다.   
- 값이 클수록 상승장에서 상대적으로 변동폭이 큰 펀드이고, 값이 작을수록 하락장에서 변동폭이 큰 펀드이다.
```
...
## Assistant Response
```text
Bull/Bear Beta Ratio는 상승국면의 Bull Beta(펀드와 시장(혹은 BM) 수익률과 Bull Beta)인 BL\_B^{F,M}를 하락국면의 Bear Beta(펀드와 시장(혹은 BM) 수익률과 Bear Beta)인 BR\_B^{F,M}로 나눈 값으로, BR^{F,M} = BL\_B^{F,M} / BR\_B^{F,M} 입니다.   
값이 클수록 상승장에서 상대적으로 변동폭이 큰 펀드이고, 값이 작을수록 하락장에서 변동폭이 큰 펀드입니다.
```
...
## Assistant Response
```text
- Bull/Bear Beta Ratio는 상승베타(Bull Beta)를 하락베타(Bear Beta)로 나눈 값이다.   
- 값이 클수록 상승장에서 상대적으로 변동폭이 큰 펀드이고, 값이 작을수록 하락장에서 변동폭이 큰 펀드이다.
```
...
## Assistant Response
```text
- Bull/Bear Beta Ratio는 상승장에서의 베타(상승베타)를 하락장에서의 베타(하락베타)로 나눈 값이며, BR^{F,M} = BL_B^{F,M} / BR_B^{F,M} 로 정의한다.   
- 값이 클수록 상승장에서 상대적으로 변동폭이 큰 펀드이고, 값이 작을수록 하락장에서 변동폭이 큰 펀드로 해석한다.
```
