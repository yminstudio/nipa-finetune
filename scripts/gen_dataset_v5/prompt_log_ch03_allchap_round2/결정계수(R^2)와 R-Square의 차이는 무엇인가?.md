...
```json
{
  ...
  "target_total": 100,
  "generated_question": "결정계수(R^2)와 R-Square의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "7) 결정계수 ; R-Square($R^2$)",
  "question": "결정계수(R^2)와 R-Square의 차이는 무엇인가?",
...
## Assistant Response
```text
- 제로인 방법론에서 결정계수는 “R-Square($R^2$)”로 함께 표기되므로, R^2와 R-Square는 같은 통계량을 뜻합니다   
- 즉, R-Square는 명칭이고 R^2는 그 값을 나타내는 표기(기호)로 이해하면 됩니다
```
...
## Assistant Response
```text
- 제로인 방법론에서 결정계수는 “R-Square($R^2$)”로 함께 표기되어, R^2와 R-Square는 같은 통계(결정계수)를 가리킵니다   
- 차이는 표현 방식인데, R^2는 계산식/기호로 쓰는 표기이고, R-Square는 그 통계를 부르는 명칭(용어)입니다
```
...
## Assistant Response
```text
- 제로인 방법론에서는 “결정계수”를 “R-Square”라고 부르고, 그 표기를 “$R^2$”로 함께 사용합니다.   
- 따라서 차이의 핵심은 값/의미의 차이라기보다, 명칭(R-Square)과 수식 기호($R^2$)의 표기 방식 차이입니다.   
- 결정계수(R-Square, $R^2$)는 “종속변수 분산 중 독립(설명)변수로 설명되는 비율”로 설명됩니다.
```
...
## Assistant Response
```text
- 제로인 기준에서 결정계수는 “R-Square($R^2$)”로 함께 표기되며, 같은 개념(같은 값)을 가리킵니다.   
- 차이는 의미가 아니라 표기에 가깝습니다: R^2는 회귀/통계 식에서 쓰는 기호, R-Square는 해당 기호가 의미하는 결정계수라는 용어명입니다.   
- 둘 다 종속변수 분산 중 설명변수(독립변수)가 설명하는 비율로 보고, 0~1(또는 백분율) 범위로 이해합니다.
```
...
## Assistant Response
```text
- 공통 맥락으로, 제로인이 말하는 결정계수는 “R-Square($R^2$)”로 함께 표기되는 동일한 통계량입니다.   
- 차이(비교축)는 표기와 이름의 구분입니다.  
- R^2는 그 결정계수 값을 나타내는 기호(표기)이고, R-Square는 그 결정계수의 명칭(영문 표현)입니다.
```
