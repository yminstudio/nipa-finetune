...
## Question
Recovery Periods의 판단 기준으로 사용되는 기간의 명칭은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Recovery Periods의 판단 기준으로 사용되는 기간의 명칭은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_RP_02"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "question": "Recovery Periods의 판단 기준으로 사용되는 기간의 명칭은 무엇인가?",
...
## Assistant Response
```text
- Recovery Periods의 판단 기준에서 사용되는 기간의 명칭은 “Recovery Period End Date(총기간)”의 “총기간”입니다.
```
...
## Assistant Response
```text
Recovery Periods는 “Maximum Drawdown이 발생한 이후에도 성과가 회복되는 데 걸린 정도”를 판단하기 위한 지표입니다.   
- 판단 기준으로 사용되는 기간의 명칭: Maximum Drawdown이 발생한 후의 잔여기간(Recovery Period)   
- 기준에 쓰이는 종료 시점 명칭: Recovery Period End Date(총기간)   
- 기준에 쓰이는 시작 시점 명칭: Maximum Drawdown Start Date
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown 발생 이후의 “잔여기간”을 판단하기 위한 지표입니다.   
- 판단 기준으로 사용되는 기간의 명칭: Recovery Period End Date(총기간)   
- Recovery Period End Date(총기간)는 Recovery Period 계산에서 Maximum Drawdown Start Date와의 차이로 잔여기간을 산출하는 기준이 됩니다.
```
...
## Assistant Response
```text
Recovery Periods는 최대손실(최대낙폭)이 발생한 이후의 ‘잔여기간’을 판단하기 위한 기간 기준입니다. 

- 시작 기준 명칭: Maximum Drawdown Start Date(최대낙폭 시작일)   
- 종료 기준 명칭: Recovery Period End Date(총기간)(회복기간 종료일, 총기간)   
- Recovery Periods 산출은 “Recovery Period End Date(총기간) - Maximum Drawdown Start Date”의 형태로 정의됩니다.
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown 발생 이후의 잔여기간(회복 기간)을 판단하기 위한 지표입니다. 

- 판단 기준으로 사용되는 기간의 명칭: Recovery Period End Date(총기간) 
- 이는 Recovery Period의 종료 시점을 “총기간”으로 표기한 것입니다.
```
