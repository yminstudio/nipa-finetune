...
## Question
Recovery Periods의 주된 판단 축은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Recovery Periods의 주된 판단 축은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_RP_03"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "question": "Recovery Periods의 주된 판단 축은 무엇인가?",
...
## Assistant Response
```text
- Recovery Periods의 주된 판단 축은 Maximum Drawdown이 발생한 시점 이후의 “잔여기간”입니다.   
- 구체적으로는 Recovery Period End Date(총기간)에서 Maximum Drawdown Start Date를 차감한 기간(= RP^F)로 판단합니다.
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown이 발생한 이후 펀드가 회복(또는 반등 국면 진입)하는 데 걸리는 시간의 길이를 판단하기 위한 지표입니다. 

- 기준이 되는 축은 Maximum Drawdown이 시작된 날짜(End/Start 중 “Start Date”)입니다.
- 종료 시점은 Recovery Period End Date(총 측정기간의 종료일)입니다.
- 결국 Recovery Period는 “총기간 종료일 − Maximum Drawdown 시작일”로 계산되는 잔여기간(기간의 길이)이 주된 판단 축입니다.
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown 이후 펀드가 회복(원상 수준으로의 회복을 전제한 기간)되기까지 걸리는 시간, 즉 잔여기간을 판단하기 위한 지표입니다. 

- Maximum Drawdown이 발생한 “후”의 잔여기간을 기준으로 합니다. 
- Recovery Period는 Recovery Period End Date(총기간)에서 Maximum Drawdown Start Date를 차감해 산출합니다. 
- 즉, 주된 판단 축은 “Maximum Drawdown 시작일 이후 회복기간 종료일까지의 기간 길이”입니다.
```
...
## Assistant Response
```text
Recovery Periods는 특정 기간 동안 Maximum Drawdown 이후 얼마나 남은(지속된) 시간 구간이 있는지를 판단하기 위한 지표입니다. 

- 기준 시점: Maximum Drawdown Start Date(최대낙폭 발생 시작일)   
- 종료 시점: Recovery Period End Date(총기간)   
- 산출 축: Recovery Period = Recovery Period End Date(총기간) − Maximum Drawdown Start Date
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown이 발생한 뒤 펀드의 잔여(회복/지속) 기간이 얼마나 되는지를 판단하기 위한 지표입니다. 

- Maximum Drawdown Start Date(최대낙폭 시작일) 이후 구간의 길이   
- Recovery Period End Date(회복기간 종료일, 총기간)까지의 잔여기간을 기준으로 산출   
- 산식: Recovery Period End Date(총기간) - Maximum Drawdown Start Date
```
