...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Kurtosis가 말해주는 내용은 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3.3. 기타 지표",
  "question": "Kurtosis가 말해주는 내용은 무엇인가?",
...
## Assistant Response
```text
- 첨도(Kurtosis)는 펀드 수익률이 최빈값 주변에 얼마나 밀집되어 있는지를 말해주는 지표입니다.   
- 최빈값 주변에 자료가 많이 분포하면 분포곡선의 정점이 더 뾰족한 형태가 나타납니다.   
- Kurtosis 값이 0보다 크면 정규분포보다 최빈값 주변에 몰려 있으므로 봉우리 부분이 더 뾰족하게 나타납니다.   
- Kurtosis 값이 0보다 작으면 정규분포에 비해 봉우리 경사가 완만해지는(더 평평한) 형태가 나타납니다.
```
...
- 정상적인 정규분포 대비 최빈값 주변에 자료가 더 많이 몰려 있으면 분포의 정점(봉우리)이 더 뾰족한 형태가 됩니다.   
- Kurtosis 값이 0보다 크면 최빈값 주변에 더 집중되어 있어 분포곡선의 봉우리 부분이 더 뾰족하다는 뜻입니다.   
- Kurtosis 값이 0보다 작으면 정규분포에 비해 봉우리 경사가 완만한(평평한) 형태라는 뜻입니다.   
- 즉, 수익률 분포의 “봉우리의 뾰족함/평평함”을 통해 최빈값 주변 밀집도를 파악합니다.
```
...
## Assistant Response
```text
- 제로인 방법론에서 첨도(Kurtosis)는 “펀드 수익률이 최빈값 주변에 얼마나 밀집되어 있는지”를 말해주는 지표입니다.   
- 정상적인 정규분포보다 최빈값 주변에 자료가 더 많이 분포하면 분포곡선의 정점이 보다 뾰족한 형태가 되며, 이때는 Kurtosis 값이 0보다 큽니다.   
- 반대로 최빈값 주변에 자료가 덜 분포하면 정규분포 곡선보다 평평한 형태가 되며, 이때는 Kurtosis 값이 0보다 작습니다.
```
...
## Assistant Response
```text
Kurtosis는 펀드 수익률 분포가 “최빈값(가장 자주 나타나는 값) 주변에 얼마나 밀집돼 있는지”를 나타내는 척도입니다. 

- 정의: Kurtosis는 다음의 식으로 계산되며, 값의 크기는 분포의 봉우리 형태(정점의 뾰족함/평평함)와 관련해 해석합니다.   
  Kurtosis = n(n+1)/((n-1)(n-2)(n-3)) × Σ[( (R_i^F − 평균(R^F)) / σ^F )^4] − 3(n−1)^2/((n−2)(n−3)) 
- 의미: 정상적인 정규분포보다 최빈값 주변에 자료가 많이 분포하면 분포곡선의 정점이 더 뾰족한 형태가 되고, 그렇지 않으면 정규분포 곡선보다 더 평평한 형태가 됩니다. 
- 해석(부호): Kurtosis가 0보다 크면 최빈값 주변에 몰려 있어 분포곡선이 뾰족해지고, 0보다 작으면 정규분포에 비해 봉우리 경사가 완만해집니다.
```
...
## Assistant Response
```text
- 첨도(Kurtosis)는 펀드 수익률 분포가 최빈값 주변에 얼마나 밀집되어 있는지를 말해주는 지표입니다.   
- 정상적인 정규분포보다 최빈값 주변에 자료가 많이 분포하면 분포 곡선의 정점이 더 “뾰족”해지고, 최빈값 주변에 덜 몰리면 정규분포보다 더 “평평”해집니다.   
- Kurtosis 값이 0보다 크면 최빈값 주변에 더 몰려 있어 정점이 뾰족해집니다.   
- Kurtosis 값이 0보다 작으면 정규분포에 비해 정점 경사가 완만해집니다(더 평평해짐).   
- (계산식 관점에서) 최빈값 주변의 “뾰족함/평평함”은 주간 log 수익률을 평균과 표준편차로 표준화한 값의 4제곱 합을 기반으로 산출됩니다.
```
