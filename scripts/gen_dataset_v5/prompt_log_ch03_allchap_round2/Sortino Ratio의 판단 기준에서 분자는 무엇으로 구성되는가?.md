...
## Question
Sortino Ratio의 판단 기준에서 분자는 무엇으로 구성되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Sortino Ratio의 판단 기준에서 분자는 무엇으로 구성되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "10) Sortino Ratio",
  "question": "Sortino Ratio의 판단 기준에서 분자는 무엇으로 구성되는가?",
...
## Assistant Response
```text
- Sortino Ratio의 분자 = 무위험수익률을 초과하는 펀드 수익률(초과수익률) = \overline{R_A^F} − \overline{R_A^{rf}}   
- 여기서 \overline{R_A^F}는 펀드 주간 log수익률 평균의 연환산수익률, \overline{R_A^{rf}}는 무위험수익률(CD) log수익률 평균의 연환산수익률로 구성됩니다
```
...
## Assistant Response
```text
Sortino Ratio는 무위험수익률을 초과하는 펀드의 “보상(초과수익)”을 하방편차(하방위험)로 나누어 판단하기 위한 지표이며, 분자가 바로 그 초과수익 부분입니다. 

- Sortino Ratio 분자 = \overline{R_A^F} - \overline{R_A^{rf}}   
- \overline{R_A^F}는 펀드 주간 log수익률 평균의 연환산수익률(= \overline{R^F} × 52)   
- \overline{R_A^{rf}}는 무위험수익률(CD) 주간 log수익률 평균의 연환산수익률
```
...
## Assistant Response
```text
Sortino Ratio는 무위험수익률을 초과하는 펀드의 수익을 하방편차 대비 얼마나 내는지를 판단하기 위한 지표입니다. 

- 분자 = 펀드의 무위험수익률 초과수익(초과분)  
- 구체적으로는 (펀드 주간 log수익률 평균의 연환산수익률 − 무위험수익률(CD) log수익률 평균의 연환산수익률)로 구성됩니다.
```
...
## Assistant Response
```text
- Sortino Ratio는 무위험수익률을 초과하는 펀드의 수익 성과가 하방편차(하락위험)에 비해 어느 정도인지를 판단합니다.   
- 분자(초과수익 부분)는 펀드의 연환산 평균수익률에서 무위험수익률의 연환산 평균수익률을 뺀 값으로 구성됩니다.   
- 구체적으로는  $\overline{R_A^F} - \overline{R_A^{rf}}$ 이며, 여기서 $\overline{R_A^F}$는 펀드 주간 log수익률 평균의 연환산수익률, $\overline{R_A^{rf}}$는 무위험수익률(CD) 주간 log수익률 평균의 연환산수익률입니다.
```
...
## Assistant Response
```text
Sortino Ratio는 무위험수익률을 초과하는 펀드 수익률을 하방편차로 나누어 판단합니다. 

분자(초과수익률 부분)는 다음 2가지로 구성됩니다.
- 펀드의 연환산 수익률 평균(주간 log수익률 평균의 연환산) 
- 무위험수익률(CD) 연환산 수익률 평균(주간 log수익률 평균의 연환산)
```
