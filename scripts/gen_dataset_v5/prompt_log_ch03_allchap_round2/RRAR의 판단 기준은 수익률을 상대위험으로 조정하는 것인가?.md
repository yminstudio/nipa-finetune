...
## Question
RRAR의 판단 기준은 수익률을 상대위험으로 조정하는 것인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "RRAR의 판단 기준은 수익률을 상대위험으로 조정하는 것인가?",
  "qa_type": "criteria",
  "question_template": "RRAR_criteria_2"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "question": "RRAR의 판단 기준은 수익률을 상대위험으로 조정하는 것인가?",
...
## Assistant Response
```text
- RRAR의 판단 기준은 수익률을 상대위험(동일유형 초과수익률의 변동성)으로 조정한 “상대위험 조정후 수익률”에 있습니다.   
- 즉, 펀드 평균수익률과 동일유형 평균수익률의 차이를 동일유형 초과수익률의 표준편차(σ^{F-P})로 나누어 상대위험만큼 수익률을 조정하는 방식입니다.
```
...
## Assistant Response
```text
RRAR은 펀드 성과가 “얼마나 더 벌었는가”를 상대적 위험(추가로 부담한 위험)까지 반영해 평가하기 위한 기준입니다.   
RRAR은 (동일유형 대비) 펀드의 평균수익률 초과분을 동일유형 초과수익률의 표준편차로 나누는 방식으로, 수익률을 상대위험으로 조정해 산출됩니다.   
즉, 분모에 사용되는 σ^{F-P}(동일유형 초과수익률의 변동성) 때문에 “상대위험으로 조정된 수익률” 자체가 판단 기준이 됩니다.
```
...
## Assistant Response
```text
RRAR은 펀드가 동일유형의 위험/수익 수준을 바탕으로, 추가적인(상대적) 위험 부담에 대해 얼마나 보상받는 수익을 얻었는지를 평가하기 위한 지표입니다. 

- 판단 기준은 “수익률을 상대위험으로 조정”하는 형태입니다.   
- RRAR은 (펀드 평균수익률 − 동일유형 평균수익률) 을 동일유형 초과수익률의 표준편차(상대위험)로 나눠 산출합니다.   
- 따라서 수익률 차이를 상대위험(초과수익률의 변동성) 단위로 환산해 나타나는 값이 RRAR의 판단 기준이 됩니다.
```
...
## Assistant Response
```text
RRAR은 펀드의 성과(수익)를 상대적 위험 부담과 함께 평가하기 위한 지표입니다.   
- RRAR의 판단 기준은 “수익률 격차(펀드 평균수익률 − 동일유형 평균수익률)”를 “상대위험(동일유형 초과수익률의 표준편차)”으로 나누는 형태입니다.   
- 즉, 수익률을 상대위험으로 조정한 값이며, 수식은 RRAR = ( \overline{R^F} − \overline{R^P} ) / σ^{F-P} 입니다.   
- 여기서 σ^{F-P}는 동일유형 초과수익률의 표준편차로, 상대적 위험을 반영하는 분모 역할을 합니다.
```
...
## Assistant Response
```text
RRAR는 펀드가 “위험(변동성) 부담 대비” 어느 정도로 수익을 더 얻었는지를 보려는 상대위험 조정 성과척도입니다.   
- 판단 기준은 펀드의 평균 수익률에서 동일유형 평균 수익률(=상대적 초과수익)을 뺀 값을  
- 동일유형 초과수익률의 표준편차(상대위험)로 나누는 방식입니다.   
- 따라서 RRAR는 수익률을 상대위험으로 조정해 산출하는 지표라고 볼 수 있습니다.
```
