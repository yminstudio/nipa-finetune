...
## Question
Maximum Drawdown의 비교 축으로 삼는 항목은 '하락비율'인가, '하락폭'인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Drawdown의 비교 축으로 삼는 항목은 '하락비율'인가, '하락폭'인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "11) Maximum Drawdown",
  "question": "Maximum Drawdown의 비교 축으로 삼는 항목은 '하락비율'인가, '하락폭'인가?",
...
## Assistant Response
```text
- Maximum Drawdown은 “고점에서 다음 저점까지의 하락폭이 가장 큰 구간”을 보되, 비교하는 값은 그 구간의 하락비율입니다.   
- 따라서 Maximum Drawdown의 비교 축은 하락비율입니다.
```
...
## Assistant Response
```text
제로인에서 Maximum Drawdown은 “고점에서 다음 저점까지의 하락폭이 가장 큰 구간”을 찾되, 그 값을 그대로 쓰는 비교축은 하락폭이 아니라 하락비율입니다.   
- 하락비율: Maximum Drawdown의 산출/표현 단위로 사용됩니다. (최고 누적 대비 저점 누적의 비율 형태)   
- 하락폭: “어느 구간이 최악인지(하락폭이 가장 큰 구간)를 고르는 기준”으로만 쓰이고, 최종 비교값은 아닙니다.
```
...
## Assistant Response
```text
Maximum Drawdown은 “펀드 수익률의 고점에서 다음 저점까지의 하락폭이 가장 큰 구간”을 찾는 방식으로 언급되지만, 실제 비교(산출)되는 값은 그 구간의 “하락비율”입니다. 

- 비교축(최종 수치): 하락비율(Drawdown 비율)   
- 하락폭(표현): “가장 큰 구간”을 지칭하는 데 쓰임
```
...
## Assistant Response
```text
- 제로인에서 Maximum Drawdown은 “고점에서 다음 저점까지의 하락폭이 가장 큰 구간”을 찾되, 그 구간의 “하락비율”을 값으로 삼습니다.   
- 따라서 비교 축(지표로 산출·표현되는 축)은 하락폭이 아니라 하락비율입니다.
```
...
## Assistant Response
```text
Maximum Drawdown은 “고점→저점”으로 이어지는 구간에서 하락이 가장 큰 구간을 찾되, 최종적으로 비교(측정)하는 값은 그 구간의 하락비율(D_t)입니다. 

- 하락폭: “하락폭이 가장 큰 구간”을 고르는 데 쓰이는 표현(구간 선정의 기준)
- 하락비율: Maximum Drawdown의 실제 비교 축(하락비율의 최대값을 사용)
```
