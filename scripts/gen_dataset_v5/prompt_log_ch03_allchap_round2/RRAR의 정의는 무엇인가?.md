...
```json
{
  ...
  "target_total": 100,
  "generated_question": "RRAR의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
RRAR(상대위험 조정후 수익률, Relative Risk Adjusted Return)은 샤프지수 산출식에서 무위험수익률 대신 펀드 전체의 평균수익률을 사용해 상대적으로 위험을 조정한 수익률을 나타내는 척도입니다.   
정의는 다음과 같습니다.  
...
## Assistant Response
```text
RRAR(상대위험 조정후 수익률, Relative Risk Adjusted Return)은 샤프지수에서 무위험수익률 대신 동일유형 평균수익률을 사용해, “동일한 위험(상대 변동성) 부담 대비 추가로 얻은 수익 보상”을 상대적으로 평가하는 지표로 정의됩니다.   

RRAR = ( (평균 펀드수익률) - (평균 동일유형수익률) ) / (동일유형 초과수익률의 표준편차 ) = ( \overline{R^F} - \overline{R^P} ) / \sigma^{F-P}   

여기서 \overline{R^F}는 펀드 주간 로그수익률 평균, \overline{R^P}는 해당 펀드와 동일유형 주간 로그수익률 평균, \sigma^{F-P}는 동일유형 초과수익률의 표준편차입니다.
```
...
## Assistant Response
```text
- 상대위험 조정후 수익률(RRAR)은 샤프지수 산출식에서 무위험수익률 대신 펀드의 “동일유형 평균수익률”을 사용해, 상대적으로 추가 위험 부담 대비 초과로 달성한 수익에 대한 보상을 평가하는 지표입니다.   
- 정의(산식): RRAR = ( \overline{R^F} - \overline{R^P} ) / \sigma^{F-P}   
- 여기서 \overline{R^F}, \overline{R^P}는 각각 펀드 및 동일유형의 주간 로그수익률 평균(연환산)이며, \sigma^{F-P}는 동일유형 초과수익률의 표준편차입니다.
```
...
## Assistant Response
```text
상대위험 조정후 수익률(Relative Risk Adjusted Return, RRAR)은 펀드의 수익률을 동일유형 대비 위험까지 반영해 평가하기 위해, 펀드 평균수익률과 동일유형 평균수익률의 차를 동일유형 초과수익률의 표준편차로 나눈 지표입니다. 

RRAR = ( \overline{R^F} - \overline{R^P} ) / \sigma^{F-P}   
- \overline{R^F} : 펀드 주간 로그수익률 평균(연환산)   
- \overline{R^P} : 동일유형 주간 로그수익률 평균(연환산)   
...
## Assistant Response
```text
RRAR(상대위험 조정후 수익률; Relative Risk Adjusted Return)은 샤프지수에서 무위험수익률 대신 “해당 펀드의 동일유형 평균수익률”을 기준으로 상대적인 위험-수익 보상을 평가하는 지표입니다. 

RRAR의 정의는 다음과 같습니다.   
- RRAR = (평균수익률(F) − 평균수익률(P)) / 동일유형 초과수익률의 표준편차   
- 여기서  평균수익률(F)=펀드 주간 로그수익률 평균(연환산) , 평균수익률(P)=동일유형 주간 로그수익률 평균(연환산)   
- σ^{F-P}=동일유형 초과수익률의 표준편차(연환산)
```
