...
## Question
평균손실과 Average Loss 표현은 동일한 정의를 가지는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "평균손실과 Average Loss 표현은 동일한 정의를 가지는가?",
  "qa_type": "comparison",
  "question_template": "비교-표현동일성"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "9) 평균손실 ; Average Loss",
  "question": "평균손실과 Average Loss 표현은 동일한 정의를 가지는가?",
...
## Assistant Response
```text
- 네. 제로인의 기준에서 “평균손실 ; Average Loss”는 펀드가 손실을 볼 경우 “평균적으로 한 주에 어느 정도 손실을 기록하는지”를 같은 의미로 나타내며, 동일하게 Avg_L^F = (∑ min(r_i^F, 0)) / n 로 정의됩니다.   
- 따라서 “평균손실”과 “Average Loss”는 같은 정의(동일 산식과 대상: 주간 실현수익률 r_i^F의 손실구간만 사용)에 대응합니다.
```
...
## Assistant Response
```text
- 네. 제로인 방법론에서 “평균손실”은 “Average Loss”와 동일 지표로 표기되며, 같은 의미(손실이 발생한 구간에서 평균적으로 한 주에 어느 정도의 손실을 기록하는지)를 가집니다.   
- 또한 계산식도 동일하게 제시됩니다: 평균손실/​Average Loss = (∑ min(r_i^F, 0)) / n.
```
...
## Assistant Response
```text
- 제로인 펀드평가 방법론에서 “평균손실”과 “Average Loss”는 같은 지표로 함께 표기되며, 동일한 정의(손실 구간에서의 평균 손실 크기)를 갖습니다.   
- 해당 지표는 “펀드가 손실을 볼 경우, 평균적으로 한 주에 어느 정도 손실을 기록하는지”로 설명되고, 계산식도 Avg\_L^F = (∑ min(r\_i^F, 0))/n로 동일합니다.   
- 따라서 용어만 한국어/영어로 달라졌을 뿐, 정의는 동일하다고 보면 됩니다.
```
...
## Assistant Response
```text
공통 맥락에서 제로인 방법론은 손실(마이너스 수익)이 발생한 구간을 기준으로 “평균손실”을 측정하며, 이 지표의 영문 표기를 “Average Loss”로 함께 제시합니다. 

따라서 “평균손실(Avg_L^F)”과 “Average Loss”는 동일한 정의(손실인 경우만 합산해 n으로 나누는 방식)와 같은 산식을 의미합니다.
```
...
## Assistant Response
```text
- 제로인 방법론에서 “평균손실”과 “Average Loss”는 같은 지표로, 펀드의 주간 실현수익률 중 손실 부분만을 골라 평균낸다는 정의가 동일합니다.   
- 수식 또한 같아서, 평균손실/ Average Loss는 \(Avg\_L^F=\frac{\sum_{i=1}^n \min(r_i^F,0)}{n}\)처럼 손실(음수) 구간만 사용해 한 주당 평균 손실을 나타냅니다.
```
