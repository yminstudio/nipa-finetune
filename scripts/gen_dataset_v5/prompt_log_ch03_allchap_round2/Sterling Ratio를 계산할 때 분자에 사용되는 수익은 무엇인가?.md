...
## Question
Sterling Ratio를 계산할 때 분자에 사용되는 수익은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Sterling Ratio를 계산할 때 분자에 사용되는 수익은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "SterlingRatio_분자_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "13) Sterling Ratio",
  "question": "Sterling Ratio를 계산할 때 분자에 사용되는 수익은 무엇인가?",
...
## Assistant Response
```text
- Sterling Ratio 분자에는 Annualized Rate of Return(연복리수익률)이 사용됩니다.   
- Annualized Rate of Return은 단순수익률의 연환산이며, 로그수익률은 사용하지 않습니다.
```
...
## Assistant Response
```text
- Sterling Ratio는 최대손실(평균 Maximum Drawdown) 대비 수익을 얼마나 내는지를 판단하는 위험조정 성과지표입니다.   
- 이때 분자에 사용되는 수익은 “펀드의 Annualized Rate of Return(연복리/연환산 수익률)”입니다.   
- 제로인 기준에서는 이 Annualized Rate of Return을 “단순수익률의 연환산”으로 사용하며, “로그수익률은 사용하지 않음”이라고 명시돼 있습니다.
```
...
## Assistant Response
```text
Sterling Ratio는 최대손실 수준(평균 최대 드로다운)에 비해 위험 대비 수익이 얼마나 좋은지를 판단하는 지표입니다.   

- 분자에 사용되는 수익은 펀드의 연복리수익률, 즉 Annualized Rate of Return입니다.   
- 이 Annualized Rate of Return은 “단순수익률의 연환산”을 사용하며, “로그수익률”은 사용하지 않습니다.
```
...
## Assistant Response
```text
Sterling Ratio는 위험(평균 최대낙폭에서 10%를 뺀 값) 대비 수익 성과를 판단하기 위한 지표입니다.   

- 분자에 사용되는 수익은 Annualized Rate of Return(연환산 수익률)입니다.   
- 이 연환산 수익률은 단순수익률을 연환산한 값이며, 로그수익률은 사용하지 않습니다.
```
...
## Assistant Response
```text
Sterling Ratio는 위험(최대낙폭)을 감안한 수익률을 보기 위한 지표이므로, 분자에는 “연복리(연환산) 수익률”이 사용됩니다.   

이 연환산 수익률은 로그수익률이 아니라 “단순수익률의 연환산” 값입니다.
```
