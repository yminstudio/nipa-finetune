...
## Question
MDP를 판단하는 기준에 End 시점의 포함 여부가 반영되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MDP를 판단하는 기준에 End 시점의 포함 여부가 반영되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "16) Maximum Drawdown Periods",
  "question": "MDP를 판단하는 기준에 End 시점의 포함 여부가 반영되는가?",
...
## Assistant Response
```text
- MDP는 “Maximum Drawdown End Date - Maximum Drawdown Start Date”로 정의되어, 계산에서 End Date가 Start Date와의 차이로 반영됩니다.   
- 다만 End 시점이 “포함/미포함”되는지에 대한 별도 규정(판단 기준)은 명시되어 있지 않습니다.
```
...
## Assistant Response
```text
MDP(최대낙폭 기간)는 최대낙폭이 발생한 구간의 길이를 보기 위한 지표입니다. 

- 제로인 방법에서는 MDP^F를 “Maximum Drawdown End Date - Maximum Drawdown Start Date”로 정의해, End 시점과 Start 시점이 모두 기간 산정에 사용됩니다.   
- 다만, End 시점을 “포함/미포함(예: 당일 포함)”으로 처리하는지에 대한 명시적 규칙은 MDP 정의 부분에 별도로 기재돼 있지 않습니다.
```
...
## Assistant Response
```text
MDP는 Maximum Drawdown이 발생한 기간의 길이를 판단하기 위한 지표입니다. 

- MDP는 “Maximum Drawdown End Date - Maximum Drawdown Start Date”로 계산합니다.   
- 별도로 End 시점을 “포함(+1 등)”하는 보정은 식/정의에 제시되어 있지 않습니다.   
- 예시에서도 “6주 - 3주 = 3주”로 산출되어, End 시점이 별도 포함 처리 없이 차감 방식으로 반영됩니다.
```
...
## Assistant Response
```text
- MDP는 “Maximum Drawdown이 발생한 기간(주, 월)”의 길이를 판단하기 위한 지표입니다.   
- MDP는 Maximum Drawdown End Date(종료 시점)와 Maximum Drawdown Start Date(시작 시점)의 차이로 정의되므로, End 시점 포함 여부가 반영됩니다.   
- 즉, End 시점은 기간 산출에 직접 쓰여 MDP 값에 영향을 줍니다.
```
...
- MDP 산식은 “Maximum Drawdown End Date − Maximum Drawdown Start Date”로 정의됩니다.   
- 따라서 기간 길이는 End 시점을 ‘별도로 +1 하여 셈’하는 방식이 아니라, End Date와 Start Date의 차이로 산정됩니다.   
- 예시에서도 MDP를 “6주 − 3주 = 3주”처럼 날짜 차이로 계산합니다.
```
