...
## Question
Recovery Periods의 판단 기준은 무엇으로 정해지는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Recovery Periods의 판단 기준은 무엇으로 정해지는가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_RP_01"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "question": "Recovery Periods의 판단 기준은 무엇으로 정해지는가?",
...
## Assistant Response
```text
- Recovery Periods는 “기간 동안 Maximum Drawdown이 발생한 후의 잔여기간”으로 정의된다.   
- 산정의 시작 기준은 Maximum Drawdown Start Date(최대낙폭 시작일)이다.   
- 산정의 종료 기준은 Recovery Period End Date(총기간)(총 측정기간의 종료일)이다.
```
...
## Assistant Response
```text
Recovery Periods는 “Maximum Drawdown이 발생한 뒤” 남아있는 기간을 무엇을 기준으로 잡아 측정하는지로 정해집니다.   
- 시작 기준: Maximum Drawdown Start Date(최대낙폭 시작일)   
- 종료 기준: Recovery Period End Date(총기간의 종료일)   
- 판정 내용: 기간 동안 Maximum Drawdown 발생 후의 잔여기간
```
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown이 발생한 이후로 “얼마나 더 지속되는지(잔여기간)”를 판단하기 위한 지표입니다. 

- Recovery Periods는 “Maximum Drawdown이 발생한 후의 잔여기간”으로 정의됩니다.   
- 판단 기준은 Recovery Period End Date(총기간의 종료일)와 Maximum Drawdown Start Date(최대낙폭 시작일)입니다.   
- 산식은 Recovery Period End Date(총기간) − Maximum Drawdown Start Date로 정해집니다.
```
...
## Assistant Response
```text
Recovery Periods는 “최대손실 구간(Maximum Drawdown)이 발생한 뒤 회복에 걸리는 잔여 기간”을 판단하기 위한 지표입니다. 

- 기준은 Maximum Drawdown이 시작된 시점(Recovery Period 계산의 기점: Maximum Drawdown Start Date)입니다.   
- 회복 기간의 종료는 “총 측정기간의 종료일”(Recovery Period End Date, 총기간)로 정해집니다.   
- 이에 따라 Recovery Period는 “총기간 종료일 − Maximum Drawdown Start Date”로 산출됩니다.
```
...
## Assistant Response
```text
Recovery Periods는 최대손실(Maximum Drawdown) 발생 이후 펀드가 회복되기까지의 “남은 기간(잔여기간)”을 판단하기 위한 기준입니다.   
- 기간 동안 Maximum Drawdown이 발생한 후의 잔여기간을 Recovery Periods로 봅니다.   
- 산정 기준은 Recovery Period End Date(총기간)에서 Maximum Drawdown Start Date를 차감하는 형태로 정해집니다.   
- 따라서 Recovery Periods의 길이는 “Maximum Drawdown이 시작된 시점”을 기준으로 “총 측정기간의 종료 시점”까지의 기간으로 결정됩니다.
```
