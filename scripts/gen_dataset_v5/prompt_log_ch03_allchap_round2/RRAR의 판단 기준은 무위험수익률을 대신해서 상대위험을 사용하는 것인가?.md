...
## Question
RRAR의 판단 기준은 무위험수익률을 대신해서 상대위험을 사용하는 것인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "RRAR의 판단 기준은 무위험수익률을 대신해서 상대위험을 사용하는 것인가?",
  "qa_type": "criteria",
  "question_template": "RRAR_criteria_1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "question": "RRAR의 판단 기준은 무위험수익률을 대신해서 상대위험을 사용하는 것인가?",
...
## Assistant Response
```text
- RRAR은 샤프지수 산출식에서 무위험수익률을 쓰지 않고, 펀드 평균수익률과 동일유형 평균수익률(초과수익 \overline{R^F}-\overline{R^P})을 사용해 상대적 보상을 구성합니다.   
- 위험요소는 무위험수익률을 “상대위험으로 대체”하는 방식이라기보다, 동일유형 초과수익률의 표준편차 \sigma^{F-P}(상대위험)를 분모로 사용합니다.   
- 결론적으로 RRAR의 판단은 무위험수익률 대신 동일유형 대비 초과수익과 그 변동성(상대위험)으로 이루어집니다.
```
...
## Assistant Response
```text
RRAR은 위험(표준편차) 대비 수익을 “상대적으로” 평가하기 위한 지표입니다.   
- RRAR은 샤프지수처럼 무위험수익률을 쓰지 않고, 무위험수익률 대신 펀드(해당 동일유형)의 평균수익률(동일유형 주간 로그수익률 평균)을 사용합니다.   
- 분모의 위험도 무위험 자산 대비의 절대 위험이 아니라 “동일유형 초과수익률의 표준편차”인 σ^{F-P}를 사용합니다.   
- 따라서 질문처럼 무위험수익률을 ‘상대위험’으로 직접 대체한다기보다는, 기준(무위험수익률)을 동일유형 평균수익률로 바꾸고 위험을 동일유형 대비 변동성(상대위험)으로 측정하는 구조입니다.
```
...
## Assistant Response
```text
RRAR은 펀드의 위험-수익 관계를 “유형 내 추가적인 위험” 기준으로 평가하기 위한 위험조정 성과지표입니다. 

- RRAR의 분자에는 무위험수익률이 아니라 “동일유형(펀드의 기준) 평균수익률”이 들어가며, 펀드 평균수익률에서 동일유형 평균수익률을 뺍니다.   
- RRAR의 분모는 무위험수익률 관련 위험이 아니라 “동일유형 초과수익률의 표준편차(σ^{F-P})”를 사용합니다.   
- 따라서 RRAR의 판단기준은 무위험수익률을 그대로 쓰는 형태를 유지하는 것이 아니라, 무위험수익률 대신 유형 내 기준을 사용하고 그에 대한 상대적 위험(초과수익의 변동성)을 반영하는 구조입니다.
```
...
## Assistant Response
```text
RRAR은 위험-수익의 상충관계를 반영해 성과를 판단하기 위한 지표입니다.   
- RRAR은 샤프지수의 산출식에서 무위험수익률을 펀드(해당 유형) 평균수익률로 대체해 상대적인 초과수익을 만듭니다.   
- 그리고 ‘상대위험’은 분모에서 펀드-유형 간(동일유형 초과수익률)의 표준편차(σ^{F-P})로 사용됩니다.   
따라서 “무위험수익률을 대신해서 상대위험을 사용”한다기보다는, 무위험수익률은 평균수익률로 대체되고 상대위험은 분모의 표준편차로 반영됩니다.
```
...
## Assistant Response
```text
RRAR는 펀드가 부담하는 위험(상대적으로 초과수익을 만들기 위한 변동)에 대해, 그 대가로 얻는 수익이 동종(동일유형) 평균 대비 얼마나 되는지를 판단하기 위한 지표입니다.   
- RRAR는 샤프지수 산출식에서 무위험수익률을 쓰지 않고, 무위험수익률 대신 펀드(전체)의 평균수익률(동일유형 평균수익률)을 사용합니다.   
- 따라서 판단 기준은 (펀드 평균수익률 − 동일유형 평균수익률) / 동일유형 초과수익률의 표준편차(상대위험, σ^{F-P}) 형태로 정해집니다.
```
