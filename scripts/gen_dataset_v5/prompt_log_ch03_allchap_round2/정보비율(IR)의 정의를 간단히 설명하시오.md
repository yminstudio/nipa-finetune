...
```json
{
  ...
  "target_total": 100,
  "generated_question": "정보비율(IR)의 정의를 간단히 설명하시오.",
  "qa_type": "definition",
  "question_template": "정보비율 정의 템플릿"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 정보비율 ; Information Ratio(IR)",
  "question": "정보비율(IR)의 정의를 간단히 설명하시오.",
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과 나타나는 초과수익률을, 적극적인 활동에 따른 수익률(초과수익률)의 표준편차로 나눈 비율이다.   
- 이 비율은 Reward-to-Variability Ratio(RVR)라고도 한다.
```
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 초과수익률(벤치마크 대비 성과)과, 그 적극적 활동에 따른 수익률의 표준편차(변동성)의 비율을 말합니다.   
- 제로인에서는 이를 보통 Reward-to-Variability Ratio(RVR)로도 부릅니다.   
- 벤치마크를 가정하는 경우 정보비율은 IR = (펀드의 평균수익률 − 벤치마크의 평균수익률) / 추적오차(TE) 로 표시됩니다.
```
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 초과수익률을, 적극적인 활동에 따른 수익률의 표준편차(변동성)로 나눈 비율을 말한다.   
- 이 비율은 Reward-to-Variability Ratio(RVR)라고도 하며, 벤치마크를 가정하는 경우 IR은 초과수익률을 추적오차(Tracking Error)로 나눈 형태로 표시된다.
```
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 초과수익률을, 적극적인 활동에 따른 수익률의 표준편차(변동성)로 나눈 비율이다.   
- 이 비율은 Reward-to-Variability Ratio(RVR)라고도 부른다.   
- 펀드의 벤치마크를 가정할 때 IR은 다음처럼 표시한다.  \(IR^{F,M}=\dfrac{\overline{R^F}-\overline{R^M}}{TE^{F,M}}\)
```
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과로 나타나는 초과수익률과, 그 적극적인 활동에 따른 수익률의 표준편차(변동성)의 비율을 말하며 Reward-to-Variability Ratio(RVR)라고도 합니다.   
- 펀드의 벤치마크를 가정하면 IR은 IR^{F,M} = (주간 로그수익률 평균의 펀드 − 주간 로그수익률 평균의 시장(또는 BM)) / 추적오차(TE^{F,M})로 표시합니다.
```
