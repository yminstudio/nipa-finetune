...
## Question
샤프지수(Sharpe Ratio)를 위험조정 성과지표로 사용할 때의 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "샤프지수(Sharpe Ratio)를 위험조정 성과지표로 사용할 때의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_샤프지수_판단"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3.2. 위험조정 성과지표",
  "question": "샤프지수(Sharpe Ratio)를 위험조정 성과지표로 사용할 때의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 샤프지수는 펀드의 위험 1단위에 대한 초과수익의 정도를 나타내며, 샤프지수가 높을수록 투자성과가 성공적(우수)이라고 판단합니다 .  
- 비교 목적으로 사용할 때는 측정 기간과 투자에 대한 동일 유형이어야 비교의 의미가 있습니다 .  
- 단, 펀드의 초과수익률이 음의 값을 가지는 경우에는 샤프지수의 해석이 어렵다고 봅니다(해석할 수 없는 문제) .
```
...
## Assistant Response
```text
샤프지수(Sharpe Ratio)는 “위험 1단위 부담에 대해 얻는 초과수익의 정도”를 나타내는 위험조정 성과지표입니다. 

- 샤프지수 값의 방향: 샤프지수가 높을수록 투자성과가 성공적(우수)이라고 판단합니다. 
- 지표가 판단하는 대상: 위험 1단위 대신 얻은 대가인 “초과수익”이 얼마나 되는지를 측정하는 것으로 봅니다. 
- 비교 가능 조건(해석의 전제): 샤프지수를 비교해 의미를 두려면 측정 기간과 투자(유형)가 동일 유형이어야 합니다.
```
...
## Assistant Response
```text
샤프지수(Sharpe Ratio)는 위험조정 성과지표로, “위험 1단위에 대해 얻은 초과수익이 얼마나 되는지”를 보는 지표입니다. 

- 샤프지수는 무위험 초과수익(수익률에서 무위험 수익률을 뺀 값) 대비 위험(표준편차)의 정도를 나타내며, 지수가 높을수록 투자성과가 성공적이라고 판단합니다. 
- 샤프지수 “비교”의 의미는 측정 기간과 투자(유형)가 동일해야만 성립합니다. 
- 초과수익률이 음의 값을 가지는 경우에는 샤프지수를 그대로 해석할 수 없습니다.
```
...
## Assistant Response
```text
- 샤프지수는 “위험 1단위당 초과수익(무위험수익률을 뺀 수익)”의 정도를 나타내는 위험조정 성과지표이므로, 지수가 높을수록 투자성과가 성공적이라고 판단합니다.   
- 샤프지수의 비교는 측정 기간과 투자(유형)가 동일해야만 의미가 있으므로, 서로 다른 기간·유형 간에는 단순 비교 판단에 주의합니다.   
- 샤프지수는 초과수익률이 음의 값을 가지는 경우 해석할 수 없다는 문제가 있으므로, 초과수익이 (-)인 상황에서는 샤프지수를 그대로 근거로 판단하기 어렵습니다.   
- 이 경우를 보완하기 위해 초과수익률이 (+)인 경우는 샤프지수와 같은 방식으로 두고, (-)인 경우에는 “표준편차에 대한 지수 형태의 보정”을 적용하는 수정샤프지수로 대응합니다.
```
...
## Assistant Response
```text
- 샤프지수는 “펀드의 위험 1단위 부담”에 대해 “얼마나 초과수익을 얻었는지”를 나타내는 위험조정 성과지표이므로, 샤프지수가 높을수록 투자성과가 성공적이라고 판단합니다   
- 초과수익은 무위험수익률 대비 초과분(무위험초과수익률)을 기준으로 보며, 이는 위험을 부담해 얻은 수익의 의미가 무위험자산에 투자해 얻는 수익을 초과하는 데에 있기 때문입니다   
- 샤프지수의 “해석/비교”는 측정 기간과 투자(운용) 유형이 동일해야만 비교의 의미가 있으므로, 이 조건이 다르면 같은 지표라도 단순 우열로 판단하지 않습니다
```
