...
## Question
Average Gain를 계산할 때 손실 기간의 이익은 0으로 처리되어 평균에 반영되지 않는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Average Gain를 계산할 때 손실 기간의 이익은 0으로 처리되어 평균에 반영되지 않는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "10) 평균이익 ; Average Gain",
  "question": "Average Gain를 계산할 때 손실 기간의 이익은 0으로 처리되어 평균에 반영되지 않는가?",
...
## Assistant Response
```text
- Average Gain(Avg_G^F)은 각 주간 실현수익률 r_i^F에 대해 max(r_i^F, 0)을 사용하므로, 손실(음수) 기간은 이익이 0으로 처리됩니다.   
- 이 값들은 분자에서 더해지지 않지만(양수 부분만 합산), 평균 계산은 n으로 나누어 전체 기간을 기준으로 0 기여분이 평균에 반영됩니다.
```
...
## Assistant Response
```text
Average Gain은 전체 관측기간에서 주당 이익의 “평균 크기”를 보기 위한 지표입니다. 

- 계산식에서 각 기간의 기여값은 max(rᵢ, 0)로 정해집니다.   
- 따라서 손실 기간(rᵢ < 0)은 max(rᵢ, 0)=0이 되어 이익 합계에는 0으로 반영됩니다.   
- 이 합계를 n으로 나누므로(전체 기간으로 나눔), 손실 기간은 “0”으로 평균에 포함됩니다.
```
...
## Assistant Response
```text
Average Gain은 “이익이 발생한 구간에서의 평균 이익”을 보기 위한 지표입니다.   
제로인은 Average Gain을 Avg_G^F = (∑ max(r_i^F, 0)) / n 으로 계산하므로, 손실 기간(i.e., r_i^F<0)은 max로 인해 해당 항목이 0으로 처리됩니다.   
다만 이 0은 합계에 포함되며 분모 n으로 나누기 때문에, 손실 기간은 “평균에서 제외”되는 것이 아니라 평균에 0 기여분으로 반영됩니다.
```
...
## Assistant Response
```text
Average Gain은 이익(플러스 수익)을 낸 구간에서 “한 주에 어느 정도 이익을 기록했는지”를 보려는 지표입니다. 

- 계산식은 Avg_G^F = (∑ max(r_i^F, 0)) / n 형태로, 각 주의 수익률을 max(r_i^F, 0)로 처리합니다.   
- 따라서 손실 기간(음수 수익률)은 max(r_i^F,0)에 의해 0으로 바뀌어 분자에는 더해지지 않습니다.  
- 다만 분모 n은 전체 기간 개수이므로, 손실 기간의 0이 평균의 크기를 낮추는 방식으로 평균에 반영됩니다.
```
...
## Assistant Response
```text
Average Gain은 “이익이 발생한 주의 평균 이익 규모”를 보되, 계산은 전체 기간 기준으로 산정되는 지표입니다.   
Avg_G^F는 max(r_i^F, 0)을 더해 분자에선 손실(음수) 구간이 0으로 처리됩니다.   
다만 평균을 낼 때는 분모를 n(전체 관측치 수)로 두기 때문에, 손실 기간의 값이 0으로 들어가 평균에 반영됩니다.
```
