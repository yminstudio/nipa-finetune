...
## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0033

## Question
Sterling Ratio의 정의는 무엇인가?
...
```json
{
  ...
  "section": "13) Sterling Ratio",
  "subsection": "",
  "seed_title": "13) Sterling Ratio",
  "record_target": 2,
  ...
  "target_total": 100,
  "generated_question": "Sterling Ratio의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "Sterling_Definition_Template"
}
```
...
[핵심 명사]
) Sterling Ratio, Sterling Ratio, Sterling, Ratio, 펀드의, 연복리수익률을, Maximum, Drawdown에서, 값으로, 나눈, 비율, Calmar
...
```json
{
  ...
  "subsection": "",
  "seed_title": "13) Sterling Ratio",
  "question": "Sterling Ratio의 정의는 무엇인가?",
...
## Assistant Response
```text
- Sterling Ratio는 펀드의 연복리수익률을 Average Maximum Drawdown에서 10%를 뺀 값으로 나눈 비율입니다.   
- 산식: Sterling Ratio = Annualized Rate of Return / |Average Maximum Drawdown - 10%|   
- 여기서 Annualized Rate of Return은 단순수익률의 연환산이며, Log수익률을 사용하지 않습니다.
```
...
## Assistant Response
```text
- Sterling Ratio는 펀드의 연복리(연환산) 수익률을, 평균 Maximum Drawdown에서 10%를 뺀 값(절댓값)으로 나눈 위험조정 지표입니다.   
- 정의식: Sterling Ratio = Annualized Rate of Return / | Average Maximum Drawdown − 10% |   
- 여기서 Annualized Rate of Return은 단순수익률의 연환산을 사용합니다.
```
...
## Assistant Response
```text
Sterling Ratio는 펀드의 연복리(연환산) 수익률을 Maximum Drawdown(평균)에서 10%를 뺀 값의 절댓값으로 나눈 비율입니다. 

Sterling Ratio = Annualized Rate of Return ÷ |Average Maximum Drawdown − 10%| 

여기서 Annualized Rate of Return은 단순수익률의 연환산이며, 로그수익률은 사용하지 않습니다.
```
...
## Assistant Response
```text
- Sterling Ratio는 펀드의 연복리(연간) 수익률을 Maximum Drawdown에서 10%를 뺀 값으로 나눈 비율입니다.   
- 정의식: Sterling Ratio = Annualized Rate of Return ÷ |Average Maximum Drawdown − 10%|   
- 여기서 Annualized Rate of Return은 단순수익률의 연환산을 사용하며, 로그수익률은 사용하지 않습니다.
```
...
## Assistant Response
```text
- Sterling Ratio는 펀드의 연복리(연환산) 수익률을 “평균 최대손실(MDD)에서 10%를 뺀 값”으로 나눈 위험조정 성과 지표입니다.   
- 정의식: Sterling Ratio = Annualized Rate of Return / |Average Maximum Drawdown − 10%|   
- 여기서 Annualized Rate of Return은 로그수익률이 아닌 단순수익률의 연환산을 사용합니다.
```
