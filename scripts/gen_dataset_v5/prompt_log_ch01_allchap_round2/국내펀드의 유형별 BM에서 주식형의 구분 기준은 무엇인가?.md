...
## Question
국내펀드의 유형별 BM에서 주식형의 구분 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "국내펀드의 유형별 BM에서 주식형의 구분 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "국내펀드의 유형별 BM에서 주식형의 구분 기준은 무엇인가?",
...
## Assistant Response
```text
- 국내펀드의 유형별 BM에서 주식형은 대유형(주식형) 아래 소유형으로 구분하며, 소유형은 일반주식(순수·자산배분), 중소형주식, 배당주식, 테마주식, KOSPI200 인덱스, 기타인덱스로 나뉩니다.   
- 각 소유형에 대응하는 BM이 설정되며, 일반주식은 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50을 사용합니다.   
- 테마주식, KOSPI200 인덱스, 기타인덱스는 KOSPI200을 소유형 BM으로 사용합니다.
```
...
## Assistant Response
```text
국내펀드에서 유형별 BM을 적용할 때, 주식형은 “주식 투자 비중/성격”에 따라 소유형으로 나뉘고 각 소유형에 해당하는 BM이 연결됩니다. 

- 주식형 자체 구분: 약관상 최고 주식투자한도가 2/3를 초과하는 펀드   
- 일반주식: 약관상 최고 주식투자한도가 2/3를 넘으면서, 운용 중 평균 주식 편입비가 2/3~100%인 펀드(순수주식/자산배분 포함)   
- 중소형주식: 투자설명서(투자목적)상 중소형주에 주식 자산의 50% 이상을 투자하는 펀드   
- 배당주식: 투자설명서(투자목적)상 고배당주에 주식 자산의 50% 이상을 투자하는 펀드   
- 테마주식: 투자설명서(투자목적)상 코스닥 및 특정업종/특정그룹주 등 테마성 주식에 전체자산의 50% 이상을 투자하는 펀드   
- KOSPI200 인덱스: KOSPI200지수를 100% 복제·추종하거나 복제전략을 사용하되 소폭의 초과수익을 추구하는 패시브 주식펀드   
- 기타인덱스: KOSPI200을 제외한 특정지수를 100% 복제·추종하거나 복제전략을 사용하되 소폭의 초과수익을 추구하는 패시브 주식펀드
```
...
## Assistant Response
```text
국내펀드의 유형별 BM(벤치마크)에서 주식형은 ‘소유형’별로 나누어 각 소유형에 해당하는 지수를 BM으로 둡니다. 

- 일반주식(순수, 자산배분): KOSPI200   
- 중소형주식: 중소형지수   
- 배당주식: 코스피 고배당 50   
- 테마주식, KOSPI200 인덱스, 기타인덱스: 모두 KOSPI200
```
...
## Assistant Response
```text
국내펀드의 유형별 BM을 정할 때, 주식형은 “대유형 주식형” 아래 “소유형”으로 다시 나누고 각 소유형에 대응되는 주식 BM을 배정합니다. 

- 주식형 자체의 구분 기준: 약관상 최고 주식투자한도가 2/3를 초과하는 펀드 
- 주식형의 유형별 BM(소유형) 구분은 다음 소유형과 소유형 BM 매칭으로 이뤄집니다   
  - 일반주식 → KOSPI200  
  - 중소형주식 → 중소형지수  
  - 배당주식 → 코스피 고배당 50  
  - 테마주식 → KOSPI200  
  - KOSPI200 인덱스 → KOSPI200  
  - 기타인덱스 → KOSPI200
```
...
## Assistant Response
```text
국내펀드의 유형별 BM에서 주식형은 대유형(주식형) 안의 소유형으로 나뉘며, 소유형에 따라 주식형 BM(비교지수)이 구분됩니다. 

- 일반주식(순수, 자산배분): BM = KOSPI200   
- 중소형주식: BM = 중소형지수   
- 배당주식: BM = 코스피 고배당 50   
- 테마주식 / KOSPI200 인덱스 / 기타인덱스: BM = KOSPI200
```
