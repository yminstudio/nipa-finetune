...
```json
{
  ...
  "target_total": 100,
  "generated_question": "유형 BM을 적용하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM_유형적용_기준"
}
```
...
- 해외펀드: 투자설명서 또는 규약에 명시된 비교지수가 있으면 이를 펀드 BM으로 설정하되, 제로인이 보유하지 않거나(희망 BM을 대체할 유사 BM이 없음), BM이 없거나 제로인이 보유한 지수 중 유사 BM이 존재하지 않아 확신하기 어려운 경우 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
펀드 성과평가에서 벤치마크(BM)는 펀드별 “펀드 BM”을 우선 적용하되, 적용 조건에 따라 “유형 BM”을 사용합니다. 

- 국내펀드: 테마펀드와 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다. 
- 국내펀드(테마/기타인덱스): 투자설명서 또는 규약의 비교지수를 펀드 BM으로 설정하되, 비교지수가 없으면 유형 BM을 설정합니다. 
- 해외펀드: 투자설명서 또는 규약의 비교지수를 펀드 BM으로 설정합니다. 
- 해외펀드(비교지수 부재 시): 운용사가 희망하는 BM을 먼저 적용하고, 제로인이 보유하지 않은 경우 희망 BM과 가장 유사한 BM을 적용합니다. 다만 BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
유형 BM은 펀드의 비교지수(BM)를 “유형 기준”으로 정해야 하는 경우에 펀드 BM으로 적용하는 방식입니다. 

- 국내펀드: 테마펀드 및 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 투자설명서 또는 규약에 명시된 비교지수가 있으면 그 비교지수를 펀드 BM으로 적용하되, 펀드 BM이 없거나 제로인이 보유한 지수 중 유사한 BM을 찾기 어려운 경우(적합한 BM이 없거나 확신하기 힘든 경우)에는 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
유형 BM은 펀드의 벤치마크(BM)를 설정할 때, 별도의 펀드 BM 대신 “해당 유형의 표준 BM”을 펀드 BM으로 삼는 경우를 말합니다. 

- 국내펀드: 테마펀드와 기타인덱스를 제외한 경우에는 별도의 펀드 BM을 적용하지 않고 유형 BM을 펀드 BM으로 적용합니다. 
- 국내펀드(테마/기타인덱스): 투자설명서 또는 규약에 명시된 비교지수가 있으면 그 비교지수를 펀드 BM으로 두고, 비교지수가 없으면 유형 BM을 적용합니다. 
- 해외펀드: 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 두되, 펀드 BM이 없거나 제로인이 보유한 지수 중 가장 유사한 BM을 찾기 어려운 경우(유사 BM 부재 등) 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
펀드의 벤치마크(BM)를 어떤 기준으로 잡아 “유형 BM”을 쓸지 정하는 맥락에서, 유형 BM 적용 기준은 아래와 같습니다. 

- 국내펀드는 테마펀드와 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않으며, 유형 BM을 펀드 BM으로 설정합니다.   
- 해외펀드는 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 설정하지만, 비교지수가 없거나(또는 제로인이 보유한 지수 중) 적합한 유사 BM을 찾기 어려운 경우에는 유형 BM을 적용합니다.
```
