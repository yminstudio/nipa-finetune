...
## Question
국내펀드의 유형별 BM 맥락에서 주식형의 정의를 간단히 서술하라.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "국내펀드의 유형별 BM 맥락에서 주식형의 정의를 간단히 서술하라.",
  "qa_type": "definition",
  "question_template": "정의_주식형"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "국내펀드의 유형별 BM 맥락에서 주식형의 정의를 간단히 서술하라.",
...
## Assistant Response
```text
- 주식형은 약관상 최고 주식투자한도가 2/3를 초과하는 국내펀드로 정의한다.   
- 국내펀드는 테마펀드와 기타인덱스를 제외하고는 별도의 펀드 BM 대신 유형 BM을 적용한다.   
- 따라서 주식형은 소유형에 따라 유형 BM이 달라지며, 예를 들어 일반주식은 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50을 유형 BM으로 둔다.
```
...
## Assistant Response
```text
제로인에서는 국내펀드의 유형별 벤치마크(BM)를 적용할 때, 주식형을 먼저 “주식 비중 기준”으로 정의해 해당 유형의 BM을 기준으로 비교합니다.  

- 정의: 주식형은 약관상 최고 주식투자한도가 2/3를 초과하고, 운용 중 평균주식 편입비가 2/3~100%인 펀드입니다.   
- 유형(소유형)으로는 일반주식(순수/자산배분), 중소형주식, 배당주식, 테마주식, KOSPI200 인덱스, 기타인덱스가 포함됩니다.   
- BM 맥락: 국내펀드는 테마펀드와 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용하며, 주식형은 그 소유형에 대응하는 유형 BM(예: 일반주식=KOSPI200 등)을 사용합니다.
```
...
## Assistant Response
```text
국내펀드에서는 테마/기타를 제외하면 유형별 BM을 펀드 BM으로 적용하며, 그 유형 중 “주식형”은 주식 편입 기준으로 정의된다. 

- 정의: 주식형은 “약관상 최고 주식투자한도가 2/3를 초과”하는 펀드로 본다.   
- BM 맥락: 주식형의 펀드 BM은 소유형에 따라 유형별로 달리 설정되며, 예를 들어 일반주식(순수·자산배분)은 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50, 테마주식은 KOSPI200을 BM으로 둔다.
```
...
## Assistant Response
```text
국내펀드를 유형별 BM으로 구분할 때, 주식형은 “주식 편입 비중”을 기준으로 분류된다.   
- 정의: 주식형은 약관상 최고 주식투자한도가 2/3를 초과하고, 운용 중 평균 주식 편입비가 2/3~100%인 펀드를 말한다.   
- 유형별 BM(소유형 BM)은 주식형 내부 소유형에 따라 적용되며, 예를 들면 일반주식(순수/자산배분)은 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50이다.   
- 또한 테마주식, KOSPI200 인덱스, 기타인덱스는 KOSPI200을 소유형 BM으로 둔다.
```
...
## Assistant Response
```text
국내펀드는 유형별 벤치마크(BM)를 펀드BM으로 삼아 평가하므로, ‘주식형’은 이 유형별 BM 체계에서 정해지는 주식 투자 비중 기준으로 정의된다. 

- 정의: 주식형은 약관상 최고 주식투자한도가 2/3를 넘으면서 운용중 평균주식 편입비가 2/3~100%인 펀드로, 프리스타일(액티브) 펀드에 해당한다. 
- BM 맥락: 주식형의 유형BM은 소유형에 따라 달라지며, 예를 들어 일반주식은 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50을 유형BM으로 둔다.
```
