...
```json
{
  ...
  "target_total": 100,
  "generated_question": "소유형 BM을 결정하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "소유형_BM_선정_템플릿"
}
```
...
```json
{
  ...
  "subsection": "해외펀드의 유형별 BM",
  "seed_title": "해외펀드의 유형별 BM",
  "question": "소유형 BM을 결정하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 소유형 BM은 소유형(세부유형)에 대응해 유형별 BM 표에서 정해진 지수로 설정된다.   
- 국내펀드는 테마펀드·기타인덱스를 제외하면 별도 펀드 BM 없이 유형 BM(= 소유형 BM)을 적용한다.   
- 해외펀드는 투자설명서·규약에 명시된 비교지수를 펀드 BM으로 우선 적용하되, 명시된 비교지수가 없으면 가장 유사한 BM을 적용하고 적합 BM이 없거나 확신이 어려우면 유형 BM(= 소유형 BM)을 적용한다.
```
...
## Assistant Response
```text
소유형 BM은 펀드를 ‘대유형-소유형’으로 구분했을 때, 그 소유형에 대응되는 벤치마크를 BM으로 두는 방식으로 결정됩니다. 

- 국내펀드는(테마 및 기타인덱스 제외) 별도의 펀드 BM을 두지 않고 유형 BM을 펀드 BM으로 적용하므로, 결과적으로 소유형에 매칭된 유형 BM이 소유형 BM 역할을 합니다. 
- 국내에서 테마 및 기타 유형은 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 설정하며, 그 비교지수가 없으면 유형 BM으로 설정합니다. 
- 해외펀드는 소유형 BM 자체보다 투자설명서/규약의 비교지수를 펀드 BM으로 설정하고, 비교지수가 없으면 가장 유사한 BM을, 유사 BM이 없거나 확신하기 어렵다면 유형 BM을 사용합니다.
```
...
## Assistant Response
```text
소유형 BM은 펀드를 특정 “소유형”으로 분류한 뒤, 그 소유형에 대응되는 벤치마크를 BM으로 쓰는 방식으로 결정됩니다. 

- 국내펀드는(테마펀드, 기타인덱스 제외) 별도의 펀드 BM을 두지 않고 유형 BM을 펀드 BM으로 적용하며, 이 유형 BM이 소유형 BM에 해당합니다.   
- 해외펀드는 투자설명서/규약의 비교지수를 펀드 BM으로 적용하되, 명시된 비교지수가 없으면 펀드 투자전략과 가장 유사한 BM을 우선 검토합니다.   
- 유사한 BM을 확신하기 어렵거나 적합한 BM이 없으면 유형 BM(즉, 소유형 BM)을 적용합니다.
```
...
## Assistant Response
```text
펀드 평가에서 ‘소유형 BM’은 펀드를 대유형-소유형으로 분류했을 때, 그 소유형에 대응시키는 벤치마크(유형별 BM)를 뜻합니다. 

- 국내 펀드는 원칙적으로(테마펀드, 기타인덱스를 제외) 별도 펀드 BM을 두지 않고 유형 BM을 적용하며, 이때 유형 BM이 소유형 BM 역할을 합니다.   
- 다만 테마 및 기타인덱스는 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 설정하고, 그 비교지수가 없으면 유형 BM(=소유형 BM)을 적용합니다.   
- 해외 펀드는 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 설정하되, 명시된 비교지수가 없으면 투자전략과 가장 유사한 BM을 우선 적용하고, 적합한 BM이 없거나 확신하기 어려우면 유형 BM(=소유형 BM)을 적용합니다.   
- 소유형 BM 자체는 소유형(예: 글로벌주식, 유럽주식 등)별로 미리 정해진 지수로 구성됩니다(유형별 BM 매핑).
```
...
## Assistant Response
```text
소유형 BM은 펀드가 어떤 ‘소유형’으로 분류되는지에 따라 그 소유형에 대응하는 벤치마크를 정해 성과평가의 기준으로 삼는 개념입니다. 

- 펀드의 유형·세부유형(소유형) 자체는 투자설명서상 투자목적과 투자전략에 따라 먼저 분류합니다.   
- 국내펀드는(테마펀드·기타인덱스 제외) 별도의 펀드 BM을 적용하지 않고 유형 BM을 펀드 BM으로 설정하므로, 소유형 BM이 곧 펀드 BM으로 반영됩니다.   
- 소유형 BM은 소유형-대응 BM 체계(유형별 BM 설정)에 따라 결정됩니다. 또한 해당 표에서 볼드체 BM은 해당 소유형이 속한 대유형의 BM과 동일합니다.   
- 해외펀드는 투자설명서/규약의 비교지수를 우선 펀드 BM으로 설정하되, 비교지수가 없거나 제로인이 보유한 유사 BM이 없어 불확실한 경우에는 유형 BM(=소유형 BM)으로 설정합니다.
```
