...
```json
{
  ...
  "target_total": 100,
  "generated_question": "주식형 펀드에 적용되는 BM의 예시를 제시하시오.",
  "qa_type": "application",
  "question_template": "application_주식형BM적용예시"
}
```
...
```json
{
  ...
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "주식형 펀드에 적용되는 BM의 예시를 제시하시오.",
...
## Assistant Response
```text
- 국내 주식형 펀드(테마펀드, 기타인덱스 제외)는 별도의 펀드 BM 없이 유형 BM을 펀드 BM으로 적용한다.   
- 해외 주식형 펀드의 유형별 BM 예시: 글로벌주식은 MSCI ACWI, 유럽주식은 MSCI EUROPE, 중국주식은 MSCI CHINA를 BM으로 사용한다.   
- 해외펀드 BM 적용 전환 규칙: 투자설명서/규약에 비교지수가 명시되면 이를 펀드 BM으로, 없으면 투자전략과 가장 유사한 BM을 적용하며, 적합한 BM이 없거나 확신하기 어려우면 유형 BM을 적용한다.
```
...
## Assistant Response
```text
주식형 펀드는 유형별로 정해진 BM(벤치마크) 지수를 예시처럼 적용합니다. 

- 국내 주식형(일반주식/테마주식/기타인덱스 등): KOSPI200 
- 국내 주식형(배당주식): 코스피 고배당 50 
- 국내 주식형(중소형주식): 중소형지수 
- 해외 주식형(유형별 예시): 글로벌주식 MSCI ACWI, 유럽주식 MSCI EUROPE, 중국주식 MSCI CHINA
```
...
## Assistant Response
```text
제로인에서 주식형 펀드의 BM은 국내는 유형BM을 기본으로, 해외는 투자설명서/규약의 비교지수를 펀드BM으로 설정하는 방식으로 정해진다. 

- 국내 주식형(일반주식): KOSPI200   
- 국내 주식형(배당주식): 코스피 고배당 50   
- 해외 주식형(글로벌주식): MSCI ACWI   
- 해외 주식형(유럽주식): MSCI EUROPE   
- 해외 주식형(중국주식): MSCI CHINA
```
...
## Assistant Response
```text
국내 주식형 펀드는 평가 시 사용할 BM을 유형(또는 비교지수) 기준으로 정해 적용합니다.   
- 국내 펀드는(테마펀드·기타인덱스 제외) 기본적으로 유형BM을 펀드BM으로 설정합니다.   
- 테마 및 기타 유형은 투자설명서/규약의 비교지수를 펀드BM으로 설정하며, 그 비교지수가 없으면 유형BM으로 전환합니다.   
- 주식형(유형BM) 예시: 일반주식(순수, 자산배분)·테마주식·K200 인덱스·기타인덱스는 KOSPI200을 사용합니다.   
- 주식형 예시: 중소형주식은 중소형지수를 BM으로 사용합니다.   
- 주식형 예시: 배당주식은 코스피 고배당 50을 BM으로 사용합니다.
```
...
## Assistant Response
```text
주식형 펀드는 펀드 성과를 비교하기 위해 지역/유형에 따라 BM(벤치마크)을 정해 사용한다. 

- 국내펀드: 테마펀드·기타인덱스를 제외하면 별도 펀드 BM을 두지 않고 유형 BM을 적용한다.   
- 국내펀드(예외): 테마 및 기타 유형은 투자설명서(또는 규약)의 비교지수를 펀드 BM으로 설정한다.   
- 해외펀드: 투자설명서(또는 규약)에 명시된 비교지수를 펀드 BM으로 설정하고, 없으면 가장 유사한 BM을 적용한다.   
- 유사 BM도 없거나 확신이 어려우면 유형 BM을 적용한다.   
  - 예시(해외주식형 유형별 BM): 글로벌주식=MSCI ACWI, 유럽주식=MSCI EUROPE, 중국주식=MSCI CHINA
```
