...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대유형 BM과 소유형 BM의 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교문_BM_대소유형"
}
```
...
```json
{
  ...
  "subsection": "해외펀드의 유형별 BM",
  "seed_title": "해외펀드의 유형별 BM",
  "question": "대유형 BM과 소유형 BM의 차이는 무엇인가?",
...
## Assistant Response
```text
- 대유형 BM은 펀드의 상위 분류(대유형)를 기준으로 설정되는 벤치마크입니다.  
- 소유형 BM은 대유형 안에서 더 세분화된 소유형별로 설정되는 벤치마크입니다(유형별 BM에서 소유형 BM이 별도로 제시).   
- 다만 소유형 BM 중에는 “해당 소유형이 속한 대유형의 BM과 동일”하게 표시되는 경우가 있습니다.   
- 국내펀드는(테마펀드·기타 제외) 기본적으로 별도 펀드 BM 대신 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
대유형과 소유형은 해외펀드를 BM(벤치마크) 설정을 위해 계층(대분류→세분류)으로 나눈 뒤, 그 계층별 BM을 연결해 두는 구조입니다. 

- 대유형 BM: 상위 분류(예: 해외주식형/해외채권형/해외부동산형 등)에 대응되는 BM(상위 기준 BM)입니다.   
- 소유형 BM: 하위 분류(예: 글로벌주식/유럽주식/중국주식 등)에 대응되는 BM으로, 소유형별로 특정 지수(예: MSCI ACWI, MSCI EUROPE, MSCI CHINA 등)가 부여됩니다.   
- 관계(차이의 핵심): 소유형 BM 중 “대유형 BM과 동일”한 경우가 별도로 표시되며, 해당 소유형의 BM은 대유형 BM과 같은 값으로 간주됩니다.
```
...
## Assistant Response
```text
제로인은 펀드를 유형별로 나눠(대유형→소유형) 상대성과 비교가 가능하도록 벤치마크(BM)를 설정합니다. 

- 대유형 BM: 펀드의 “대분류”에 대응되는 BM(유형 체계의 상위 레벨 기준)입니다. 
- 소유형 BM: 대유형 안에서 더 세분화된 “소분류”별로 대응되는 BM입니다. 예를 들어 해외주식형의 소유형별로 글로벌주식(MSCI ACWI), 유럽주식(MSCI EUROPE)처럼 소유형 BM이 달라집니다. 
- 연결되는 차이(동일 여부): 문서에서는 “볼드체로 표기된 소유형 BM은, 해당 소유형이 속한 대유형의 BM과 동일”하다고 명시합니다.
```
...
## Assistant Response
```text
대유형 BM과 소유형 BM은 제로인에서 펀드의 “유형별 벤치마크(BM)”를 정할 때, 같은 유형 체계라도 상위(대유형)와 하위(소유형) 중 어디까지 세분해 BM을 둘지에 따라 구분됩니다. 

- 대유형 BM: 자산군/대분류 수준(예: 해외주식형, 해외채권형 등)에 대응해 설정되는 BM입니다.   
- 소유형 BM: 대유형 아래의 더 구체적인 하위 분류(예: 해외주식형의 글로벌주식, 유럽주식, 중국주식 등)에 대응해 설정되는 BM입니다.   
- 비교 포인트: 소유형 BM은 대체로 더 구체적인 지수(예: MSCI EUROPE, MSCI CHINA 등)로 제시되며, 소유형 중 일부는 해당 소유형이 속한 대유형 BM과 “동일”하게 표시됩니다(문서상 ‘볼드체 BM’ 기준).
```
...
## Assistant Response
```text
대유형-소유형 체계로 펀드를 분류하고, 그 분류에 대응하는 벤치마크(BM)를 설정한다는 점을 전제로 합니다. 

- 대유형 BM: 상위 분류(예: 해외주식형/해외채권형/해외부동산형) 수준의 BM이 기준이 되며, 표에서 표시되는 일부 소유형 BM은 “해당 소유형이 속한 대유형의 BM과 동일”하게 적용됩니다(볼드체 BM). 
- 소유형 BM: 대유형의 하위 분류(예: 해외주식형 내 글로벌주식/유럽주식/중국주식 등)마다 부여되는 BM으로, 해당 소유형에 대응하는 별도 벤치마크가 제시됩니다(예: 글로벌주식=MSCI ACWI, 유럽주식=MSCI EUROPE, 중국주식=MSCI CHINA). 
- 요약 비교: 소유형 BM은 대유형 BM을 그대로 쓰는 경우도 있지만(동일 표시), 일반적으로는 소유형별로 더 세분화된 BM이 매칭됩니다.
```
