...
## Question
해외펀드의 BM에서 소유형 BM으로 분류되기 위한 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "해외펀드의 BM에서 소유형 BM으로 분류되기 위한 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM_소유형BM_판단기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) 해외펀드의 유형별 BM",
  "question": "해외펀드의 BM에서 소유형 BM으로 분류되기 위한 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드의 BM은 우선 투자설명서/규약(또는 설정 당시 운용사)에서 정한 비교지수(희망 BM)를 “펀드 BM”으로 적용한다.    
- 제로인이 해당 펀드 BM을 보유하지 않는 경우에는 희망 BM과 “가장 유사한 BM”을 적용한다.   
- 다만 BM이 없거나, 보유 지수 중 유사 BM이 없거나, 적합한 BM이 없거나 확신하기 힘든 경우에는 “유형 BM(= 소유형 BM)”으로 분류한다.
```
...
## Assistant Response
```text
해외펀드에서 BM은 우선 ‘펀드 BM’을 쓰되, 특정 조건이면 ‘유형 BM’으로 정해지며 이때 소유형 BM 체계로 매핑됩니다. 

- 설정 당시 운용사가 희망하는 BM(투자설명서/규약의 비교지수)이 있으면 이를 펀드 BM으로 적용합니다.   
- 제로인이 해당 희망 BM을 보유하지 않는 경우에는 희망 BM과 가장 유사한 BM을 적용합니다.   
- 위 조건과 달리 “BM이 없거나”, “제로인이 보유한 지수 중 유사한 BM이 존재하지 않는 경우”에는 유형 BM으로 전환되며, 유형 BM은 소유형별로 정의된 소유형 BM으로 매핑됩니다.
```
...
## Assistant Response
```text
해외펀드는 BM을 “펀드가 제시한 비교지수(펀드 BM)”로 둘지, “소유형이 제공하는 유형 BM(소유형 BM)”으로 둘지 결정해 평가합니다. 

- 해외펀드의 BM은 설정 시 운용사가 희망하는 BM(또는 투자설명서/규약에 명시된 비교지수)을 펀드 BM으로 설정합니다.   
- 제로인이 해당 BM을 보유하지 않은 경우에는, 희망 BM과 가장 유사한 BM을 적용합니다.   
- BM이 없거나(비교지수 부재), 제로인이 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용합니다.   
- 이때 유형 BM은 해외펀드의 소유형에 대응하는 소유형 BM(예: 글로벌주식–MSCI ACWI 등)으로 정해집니다.
```
...
## Assistant Response
```text
해외펀드는 BM을 어떻게 설정하느냐에 따라 “소유형 BM”에 해당하는 기준을 적용받게 됩니다. 

- 해외펀드의 BM은 투자설명서 또는 규약에 명시된 비교지수를 우선 펀드 BM으로 설정합니다.   
- 비교지수가 없으면, 펀드의 투자전략과 가장 유사한 BM을 펀드 BM으로 설정합니다.   
- 다만 유사한 BM이 없거나(적합한 BM이 없음), 어떤 BM이 맞는지 확신하기 어려운 경우에는 유형 BM으로 설정합니다.   
- 이때 유형 BM은 해외펀드의 소유형별로 제시된 “소유형 BM(예: 글로벌주식- MSCI ACWI 등)”에 해당하는 값을 적용합니다.
```
...
## Assistant Response
```text
해외펀드의 BM은 먼저 펀드가 참조하는 비교지수를 보되, 그게 불명확할 때는 제로인 유형 체계의 BM(소유형 BM)으로 정해집니다. 

- 해외펀드에서 투자설명서/규약에 비교지수가 명시돼 있으면 그 비교지수를 펀드 BM으로 설정합니다.   
- 비교지수가 없으면 펀드의 투자전략과 가장 유사한 BM을 펀드 BM으로 설정합니다.   
- 다만 “적합한 BM이 없거나 확신하기 힘든 경우”에는 유형 BM을 사용하며, 이때 유형 BM은 소유형에 대응되는 BM으로 봅니다(해외펀드 유형별 소유형 BM이 매핑됨).    
- 소유형 자체는 해외펀드 유형분류 기준인 “투자지역, 투자섹터”에 따라 결정됩니다.
```
