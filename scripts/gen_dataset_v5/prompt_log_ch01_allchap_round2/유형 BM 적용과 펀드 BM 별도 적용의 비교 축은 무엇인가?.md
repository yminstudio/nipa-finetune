...
## Question
유형 BM 적용과 펀드 BM 별도 적용의 비교 축은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "유형 BM 적용과 펀드 BM 별도 적용의 비교 축은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "유형 BM 적용과 펀드 BM 별도 적용의 비교 축은 무엇인가?",
...
## Assistant Response
```text
- 비교 축은 “별도 적용할 BM을 펀드BM으로 둘지, 유형BM으로 둘지”를 가르는 기준이며, 문서에서는 이를 국내펀드/해외펀드로 구분해 설명합니다.   
- 유형 BM 적용: 국내펀드는(테마펀드·기타인덱스 제외) 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 펀드 BM 별도 적용: 해외펀드는 펀드 BM을 별도로 적용합니다(운용사 희망 BM→제로인이 보유한 유사 BM→불가 시 유형 BM).
```
...
## Assistant Response
```text
제로인 방법론에서 “유형 BM을 적용”하는 경우와 “펀드 BM을 별도 적용”하는 경우를 가르는 기준(비교 축)을 묻는 질문입니다. 

- 첫 번째 비교 축은 펀드의 투자지역 구분입니다: 국내펀드는 별도 펀드 BM을 적용하지 않고 유형 BM을 적용하는 반면, 해외펀드는 펀드 BM을 별도 적용합니다.   
- 두 번째 비교 축은 국내펀드 내부의 예외 여부입니다: 국내펀드는 테마펀드·기타인덱스를 제외하고는 유형 BM을 적용합니다.   
- 세 번째 비교 축은 해외펀드에서 펀드 BM을 “우선 설정할 수 있는지”입니다: 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용하되, 제로인이 보유하지 않으면 희망 BM과 가장 유사한 BM을 적용하며, BM이 없거나 제로인 보유 지수 중 유사 BM도 없으면 유형 BM으로 전환합니다.
```
...
## Assistant Response
```text
BM의 설정기준을 적용할 때 ‘유형 BM 적용’과 ‘펀드 BM 별도 적용’은 주로 BM을 어느 기준(펀드별 vs 유형별)으로 붙이느냐를 가르는 구분축이 “국내펀드냐, 해외펀드냐”입니다. 

- 국내펀드(테마펀드, 기타인덱스를 제외): 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용   
- 해외펀드: 펀드 BM을 별도로 적용(설정 시 운용사가 희망하는 BM을 우선 적용하며, 제로인이 보유하지 않는 등 예외 상황에서는 유사 BM 또는 유형 BM으로 감)
```
...
## Assistant Response
```text
질문은 제로인에서 벤치마크(BM)를 “유형 기준으로 적용”하는 경우와 “펀드별로 별도 적용”하는 경우를 무엇을 기준으로 구분하는지 묻고 있습니다. 

비교 축은 펀드의 적용 범위(국내/해외)와 BM을 고르는 우선순위(유형 BM vs 펀드 BM)입니다. 
- 국내펀드: 별도의 펀드 BM을 두지 않고 유형 BM을 적용 
- 해외펀드: 펀드 BM을 별도로 적용(설정 당시 운용사가 희망하는 BM 우선) 
- 해외펀드의 펀드 BM이 적용되더라도, 제로인이 보유하지 않은 경우 ‘가장 유사한 BM’으로 대체하며, BM이 없거나 유사 BM도 없으면 유형 BM으로 전환
```
...
## Assistant Response
```text
질문은 제로인에서 BM을 “유형 BM”으로 볼지, “펀드 BM을 별도로 적용”할지 가르는 핵심 구분축을 묻고 있습니다. 

- 첫 번째 비교축은 적용 대상의 구분(국내펀드 vs 해외펀드)입니다.   
- 유형 BM 적용(국내펀드): 테마펀드·기타인덱스 등을 제외하면 별도의 펀드 BM 없이 유형 BM을 적용합니다.   
- 펀드 BM 별도 적용(해외펀드): 투자설명서/규약에 명시된 비교지수(또는 설정 당시 운용사가 희망하는 BM)를 펀드 BM으로 적용합니다.   
- 다만 해외펀드에서 원하는 BM을 제로인이 보유하지 않거나 BM이 부재/유사 BM이 없으면, 그때의 대체 규칙으로 유형 BM으로 전환되는지 여부가 비교축의 일부로 작동합니다.
```
