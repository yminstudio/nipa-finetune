...
## Question
소유형 BM이 적용될 수 있는 펀드 유형을 하나 제시하라.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "소유형 BM이 적용될 수 있는 펀드 유형을 하나 제시하라.",
  "qa_type": "application",
  "question_template": "적용_소유형BM"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "소유형 BM이 적용될 수 있는 펀드 유형을 하나 제시하라.",
...
## Assistant Response
```text
- 해외펀드 중 해외주식형의 소유형이 ‘글로벌주식’인 펀드는 소유형 BM이 적용될 수 있습니다.   
- 적용 조건: 해외펀드는 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 두되, 제로인이 보유하지 않거나 유사한 BM이 없어 BM이 없을 때에는 유형 BM으로 전환합니다.   
- 이때 ‘글로벌주식’의 소유형 BM은 MSCI ACWI입니다.
```
...
## Assistant Response
```text
제로인은 펀드의 BM을 대유형·소유형 매칭으로 정하되, 국내의 경우 원칙적으로 유형 BM을 펀드 BM으로 설정합니다. 

- 적용 순서: (1) 국내펀드인지 확인  → (2) 해당 펀드의 소유형을 유형 내에서 특정  → (3) 소유형에 대응하는 소유형 BM을 펀드 BM으로 사용  
  예: 국내 MMF형(소유형 MMF)은 소유형 BM으로 ‘제로인 MMF 지수’를 적용 
- 적용 조건: 국내펀드가 테마펀드 및 기타(기타인덱스) 유형이 아닐 때, 소유형 BM 매핑이 그대로 적용됩니다. 
- 전환 규칙: 국내에서 테마 및 기타 유형은 비교지수를 펀드 BM으로 설정하되, 비교지수가 없으면 다시 유형 BM(소유형 BM 매핑)으로 설정합니다.
```
...
## Assistant Response
```text
제로인 방법론에서는 펀드를 대유형/소유형으로 구분하고, 소유형에 해당하는 소유형 BM으로 성과를 비교합니다. 

- 대상 펀드 유형(소유형): 국내 주식형 중 배당주식  
- 적용되는 소유형 BM: 코스피 고배당 50   
- 전제: 국내펀드는 테마펀드, 기타인덱스를 제외하면 유형BM(=소유형별 BM 포함)을 적용합니다.
```
...
## Assistant Response
```text
해외펀드에서 소유형 BM은 소유형(예: 글로벌주식)에 대응되는 기준지수로 적용되는 방식입니다. 

- 적용 대상(펀드 유형 1개): 해외주식형-글로벌주식 소유형 펀드(소유형 BM: MSCI ACWI) 
- 적용 순서: 해외펀드는 설정 당시 투자설명서/규약에 명시된 비교지수(펀드 BM)를 우선 펀드 BM으로 적용합니다. 
- 적용 조건/전환 규칙: 제로인이 보유한 BM이 없으면 희망 BM과 가장 유사한 BM을 적용하고, BM이 없거나 유사한 BM이 존재하지 않으면 유형 BM을 적용합니다. 
- 전환 시 소유형 BM 연결: 이때 ‘글로벌주식’의 유형 BM에 해당하는 소유형 BM은 MSCI ACWI입니다.
```
...
## Assistant Response
```text
해외펀드는 BM을 먼저 “펀드 BM”으로 설정하되, 비교지수 설정이 불명확하면 “유형 BM”으로 전환합니다. 

- 펀드 유형(하나): 해외주식형–글로벌주식(소유형)   
- 적용 순서/조건: 해외펀드는 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정   
- 전환 규칙: 명시된 비교지수가 없으면 투자전략과 가장 유사한 BM을 설정하되, 적합한 BM이 없거나 확신하기 어려우면 유형 BM으로 설정   
- 이때 유형(소유형) BM: 글로벌주식의 소유형 BM은 MSCI ACWI
```
