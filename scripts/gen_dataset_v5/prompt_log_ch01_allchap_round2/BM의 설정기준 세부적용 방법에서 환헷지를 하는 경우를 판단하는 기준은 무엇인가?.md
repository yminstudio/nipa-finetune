...
## Question
BM의 설정기준 세부적용 방법에서 환헷지를 하는 경우를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM의 설정기준 세부적용 방법에서 환헷지를 하는 경우를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM_설정기준_세부적용_환헷지_판단기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "BM의 설정기준 세부적용 방법에서 환헷지를 하는 경우를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드에서 환헷지를 “하는 경우”는 투자설명서(또는 규약)에 근거해 판단하며, 환헷지가 가능하다고 명시되면 해당으로 본다   
- “필요한 경우 환헷지를 할 수 있다”처럼 현재/조건부 환헷지 실행이 가능한 문구면 환헷지를 하는 경우로 인정   
- “지금은 환헷지 수단이 없지만, 후에 환헷지 수단이 생기면 환헷지 할 수 있다”는 문구면 투자하는 지역의 통화 및 상황에 따라 환헷지 가능 여부로 판단   
- 판정 예: 인도(현재 환헷지 불가) / 러시아(현재 환헷지 가능) / 브라질(ADR에 주로 투자해 USD와 환헷지 인정, 브라질 Local 투자면 환헷지 불가)   
- 환헷지를 하는 경우로 판정되면 로컬지수를 적용한다
```
...
## Assistant Response
```text
해외펀드의 BM을 설정할 때 환헷지 여부를 가르며, 여기서 “환헷지를 하는 경우”로 판단하는 기준은 투자설명서의 환헷지 가능 문구와 투자 지역의 통화·상황이다. 

- 투자설명서에 ‘필요한 경우 환헷지를 할 수 있다’고 명시된 경우 환헷지를 하는 경우로 인정한다.   
- ‘지금은 환헷지 수단이 없지만, 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다’처럼 사후 가능성만 있는 경우에는 투자하는 지역의 통화 및 상황에 따라 판단한다(예: 인도는 현재 환헷지 불가로 봄, 러시아는 현재 환헷지 가능으로 봄).   
- 브라질의 경우 ‘ADR에 주로 투자하면 USD와 환헷지하는 것으로 인정’하되, ‘브라질 Local에 투자하면 환헷지 할 수 없음’으로 판단한다.   

환헷지를 하는 경우로 판단되면 로컬지수를 적용한다.
```
...
## Assistant Response
```text
해외펀드의 BM을 설정할 때, 환헷지를 “하는 경우”로 판단하는 기준은 투자설명서(또는 운용/규약)상 환헷지 가능 문구와, 그 문구가 의미하는 실제 환헷지 가능 여부(투자지역의 통화·상황)에 달려 있습니다. 

- 투자설명서에 “필요한 경우 환헷지를 할 수 있다”라고 명시된 경우 → 환헷지를 하는 경우로 인정 
- “현재는 환헷지 수단이 없지만, 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다”처럼 ‘향후 가능’인 경우 → 투자하는 지역의 통화 및 상황에 따라(현재 시점에 환헷지가 가능한지) 판단 
- 투자지역/통화 상황에 따른 판단 예시  
  - 인도: 현재 환헷지 할 수 없음(환헷지 하는 경우로 보지 않음)   
  - 러시아: 현재 환헷지 할 수 있음   
  - 브라질: ADR에 주로 투자하면 USD와의 환헷지로 인정(단, USD-BRL 환헷지는 아님) / 브라질 Local에 투자하면 환헷지 할 수 없음
```
