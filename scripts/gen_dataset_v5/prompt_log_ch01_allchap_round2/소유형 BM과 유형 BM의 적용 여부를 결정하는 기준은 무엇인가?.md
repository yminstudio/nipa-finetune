...
## Question
소유형 BM과 유형 BM의 적용 여부를 결정하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "소유형 BM과 유형 BM의 적용 여부를 결정하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM 적용 여부 결정 기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "소유형 BM과 유형 BM의 적용 여부를 결정하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 국내펀드는 테마펀드·기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용한다(유형 BM 체계는 대유형-소유형-소유형 BM으로 구성).    
- 국내 테마펀드·기타인덱스는 투자설명서/규약의 비교지수를 펀드 BM으로 설정한다.   
- 해외펀드는 투자설명서/규약의 비교지수(또는 설정 당시 희망 BM→제로인 미보유 시 유사 BM)로 펀드 BM을 적용하며, BM이 없거나 제로인이 보유한 지수 중 유사 BM이 없으면 유형 BM을 적용한다.
```
...
## Assistant Response
```text
펀드의 성과를 비교할 때는 먼저 “펀드 BM(비교지수)”을 쓸 수 있는지 보고, 그에 따라 “유형 BM(=소유형별로 정해진 소유형 BM)” 적용 여부가 정해집니다. 

- 국내펀드: 테마펀드·기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 투자설명서/규약의 비교지수(또는 설정 시 운용사가 희망하는 BM)를 펀드 BM으로 적용하되, BM이 없거나(또는 제로인이 보유한 지수 중) 유사한 BM이 없어 펀드 BM 설정이 곤란한 경우 유형 BM을 적용합니다.   
- 여기서 유형 BM은 소유형별로 매핑된 소유형 BM 값(예: 해외주식형의 소유형별 BM 등)으로 설정됩니다.
```
...
## Assistant Response
```text
펀드의 벤치마크(BM)는 국내/해외, 그리고 별도 “펀드 BM”을 적용할 수 있는지 여부에 따라 “유형(소유형별) BM” 적용으로 결정됩니다. 

- 유형별 BM 체계는 대유형-소유형-소유형 BM으로 구성되어, 유형 BM을 적용할 때 그 소유형별로 정해진 BM에 따르게 됩니다. 
- 국내펀드는 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다. 
- 해외펀드는 펀드 BM을 별도 적용하되, 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 하고(제로인이 보유하지 않으면 유사 BM 적용), BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 존재하지 않는 경우에는 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
국내·해외 펀드에서 벤치마크(BM)를 무엇으로 둘지(소유형 BM/유형 BM 체계 vs 별도 펀드 BM) 는 “별도 펀드 BM을 적용하느냐” 여부로 갈립니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 투자설명서(또는 규약)의 비교지수가 있으면 이를 펀드 BM으로 적용하되, 운용사가 희망하는 BM을 Zeroone이 보유하지 않거나(가장 유사한 BM도 불가), BM이 없거나 유사 BM이 존재하지 않으면 유형 BM을 적용합니다.   
- 이때 유형 BM은 대유형·소유형 조합에 따라 소유형 BM으로 매칭되는 체계(소유형별 소유형 BM)에서 선택됩니다.
```
...
## Assistant Response
```text
펀드의 벤치마크(BM)는 “펀드별 BM(비교지수)을 별도로 두는지”에 따라 유형 BM과 소유형 BM 가운데 적용 대상이 달라집니다. 

- 국내펀드: 테마펀드, 기타인덱스는 예외적으로 “별도의 펀드 BM”을 적용하고, 그 외 국내펀드는 별도 펀드 BM이 없으므로 “유형 BM”을 적용합니다.   
- 해외펀드: 투자설명서/규약에 명시된 비교지수가 있으면 “펀드 BM”을 적용하고, 그 비교지수가 없거나 제로인이 보유한 지수에서 유사한 BM을 찾기 어려운 경우에 “유형 BM”을 적용합니다.   
- 유형 BM이 적용되는 경우에는, 유형(대유형-소유형) 체계에서 소유형에 대응해 제시된 “소유형 BM”이 그 유형 BM으로 사용됩니다.
```
