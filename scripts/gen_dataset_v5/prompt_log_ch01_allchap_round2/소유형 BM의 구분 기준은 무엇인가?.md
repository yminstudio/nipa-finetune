# 질문 프롬프트 로그

## Stage
questions

## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0007

## Question
소유형 BM의 구분 기준은 무엇인가?

## Model
gpt-5-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "record_target": 7,
  "requested_question_count": 9,
  "call_requested_question_count": 5,
  "call_allowed_types": [
    "criteria"
  ],
  "call_type_targets": {
    "criteria": 5
  },
  "target_total": 100,
  "generated_question": "소유형 BM의 구분 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_basic"
}
```

## System Prompt
```text
당신은 제로인 방법론 기반 QA 데이터셋의 질문 생성기다.
반드시 한국어로만 답하고, 제공된 재료 범위 안에서만 질문을 만든다.
질문은 짧고 단일 쟁점 중심이어야 한다.
일반 금융상식, 투자조언, 문서 바깥 배경지식 질문은 금지한다.
업로드 문서를 보지 않고도 일반 금융 상식만으로 답할 수 있는 질문은 생성하지 않는다.
질문은 반드시 문서 안의 특정 정의, 기준, 비교, 적용 중 하나에 직접 매핑되어야 한다.
정답은 문서의 핵심 정보와 의미 연결을 위한 최소 설명만으로 성립해야 하며, 문서 밖 지식 확장을 요구하는 질문은 만들지 않는다.
정답이 자연스러운 문장으로 연결될 수는 있어도, 그 연결을 위해 새로운 정보가 필요해지는 질문은 만들지 않는다.
```

## User Prompt
```text
아래 재료만 사용해서 자연스러운 질문 후보를 생성해줘.

[단원 제목]
1. 유형분류 기준

[절 제목]
1.2 유형별 벤치마크

[소절 제목]
국내펀드의 유형별 BM

[핵심 제목]
국내펀드의 유형별 BM

[핵심 명사]
국내펀드의 유형별 BM, 대유형, 소유형, 소유형 BM, 국내펀드의, 유형별, BM, 주식형, 일반주식, 순수, 자산배분, KOSPI200

[질문 유형 목표]
criteria 5개, definition 0개, comparison 0개, application 0개

[이번 호출에서 허용되는 질문 유형]
criteria

[이번 호출의 생성 목표]
criteria 질문만 생성한다. 다른 유형은 금지한다.

규칙:
- 질문은 짧고 단일 쟁점 중심
- 문서 범위를 벗어난 질문 금지
- 제목을 그대로 복붙한 문장 금지
- 일반 금융상식, 투자조언, 시장전망 질문 금지
- 업로드 문서를 보지 않고도 일반 금융 상식만으로 답할 수 있는 질문은 생성하지 않는다
- 답이 표 전체 요약이나 장문 설명이 되도록 만드는 질문은 금지하고, 특정 기준 하나만 묻는 질문으로 만든다
- 질문 하나에는 하나의 판단축만 남긴다. 정의와 기준, 기준과 예외를 한 문장에 함께 묻지 않는다
- 정답은 문서의 핵심 정보와 의미 연결을 위한 최소 설명만으로 완결될 수 있어야 한다
- 정답이 자연스러운 문장으로 연결되더라도 새로운 정보 추가 없이 답할 수 있어야 한다
- 질문이 새로운 판단 기준, 질문에 없는 대상, 불필요한 비교를 답변에 끌어오게 만들면 폐기한다
- definition 질문은 정의문 하나로 직접 답할 수 있어야 하며 절차·비교·예외를 동시에 요구하지 않는다
- criteria 질문은 판단 기준만 묻게 하고 적용 절차나 다른 대상 비교를 함께 요구하지 않는다
- comparison 질문은 필요한 비교축만 묻게 하고 각 대상의 정의나 절차 설명까지 요구하지 않는다
- application 질문은 적용 순서나 전환 조건을 묻되 정의/비교를 한 문장에 함께 묻지 않는다
- 질문 유형은 definition, criteria, comparison, application 중 하나만 사용
- 허용되지 않은 질문 유형은 절대 생성하지 않는다
- 최대 5개까지만 생성

생성 후 자체 점검:
- 이 질문은 문서의 특정 표, 기준, 정의, 절차가 없으면 답하기 어려운가?
- 이 질문에 일반 금융 상식으로 그럴듯하게 답할 수 있으면 폐기한다
- 질문이 길거나 복합적이면 더 짧은 한 쟁점 질문으로 다시 쓴다
- 이 질문에 답하려고 할 때 질문에 없는 대상/비교/예외를 덧붙이게 되면 폐기한다

반환 형식:
순수 JSON 배열만 출력한다.
[
  {
    "question": "질문 내용",
    "qa_type": "definition|criteria|comparison|application",
    "question_template": "템플릿 이름"
  }
]

```

---

## Stage
answers

## Record ID
v5_q_0049

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "소유형 BM의 구분 기준은 무엇인가?",
  "answer_policy": "chapter1_summary"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
반드시 한국어로만 답한다.
업로드된 문서에 근거가 없는 기준, 수치, 예외를 만들지 않는다.
질문에 필요한 최소 범위까지만 답한다.
문서 밖 일반 지식으로 빈칸을 메우지 않는다.
문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준에 한한다.
답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
답변은 짧고 구조적으로 유지한다.
문서의 핵심 용어와 판단 기준어는 가능하면 유지한다.
질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
새로운 판단 기준, 규칙, 예외, 질문에 없는 대상이나 비교는 추가하지 않는다.
출처/원문/문서/장/절을 직접 언급하지 않는다.
일반 금융 상식으로 확장하지 않는다.
```

## User Prompt
```text
[질문]
소유형 BM의 구분 기준은 무엇인가?

[답변 스타일]
1단원 요약형으로 답한다. 2~5개 불릿 또는 2~4문장으로 정리하고, 분류 원리와 핵심 비교만 남긴다. 문맥 연결을 위한 최소 설명만 허용하고 표 전체를 장황하게 나열하지 않는다.
- 답변 길이는 공백 포함 최대 900자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이와 생략된 주어/관계 보완 수준에 한한다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 기준형: 답변은 판단 기준만 제시한다
- 기준형: 적용 절차나 비교 설명은 포함하지 않는다
- 질문에 직접 필요한 기준만 남기고 예시, 배경설명, 장황한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
- 소유형 BM은 대유형 내 소유형을 투자설명서/약관상 투자비중·투자대상 특성으로 먼저 나눈 뒤, 그 소유형에 대응되는 소유형 BM을 적용한다.   
- 주식형 소유형 구분: 최고 주식투자한도 2/3 초과, 평균주식 편입비 2/3~100%(일반주식), 중소형주 50% 이상(중소형주식), 고배당주 50% 이상(배당주식).   
- 혼합형/채권형 소유형 구분: 혼합형은 약관상 주식투자한도 10%~2/3(예: 위험자산 40%~2/3로 평균주식편입비가 거의 변하지 않는 일반주식혼합, 위험자산 편입비를 0~100% 사이에서 움직이며 평균주식편입비가 35~65%일 것으로 추정되는 공격적자산배분).  채권형은 주식투자가 불가능하고 채권·CP 등 채권형 자산에만 투자하며, 신용도/타겟듀레이션(0.5년 미만, 2~4년 등)으로 구분한다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "chapter1_summary",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Stage
answers

## Record ID
v5_q_0049

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "소유형 BM의 구분 기준은 무엇인가?",
  "answer_policy": "chapter1_summary"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
소유형 BM의 구분 기준은 무엇인가?

[답변 스타일]
1단원 요약형으로 답한다. 먼저 질문의 대상이 놓인 기본 맥락을 1문장 이내로 짚고, 이어서 핵심 정의/기준/비교를 2~5개 불릿 또는 2~4문장으로 정리한다. 문맥 연결을 위한 최소 설명은 허용하되 표 전체를 장황하게 나열하지 않는다.
- 답변 길이는 공백 포함 최대 1100자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 기준형: 답변은 판단 기준을 중심으로 제시하되, 그 기준이 무엇을 판단하기 위한 것인지 짧게 밝혀 독립적으로 이해 가능하게 한다
- 기준형: 적용 절차나 비교 설명은 포함하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
소유형 BM은 국내펀드를 대유형-소유형으로 먼저 나눈 뒤, 그 소유형에 해당하는 기준지수를 BM으로 적용하는 방식으로 구분됩니다. 

- 주식형의 소유형(일반주식(순수·자산배분)) → 소유형 BM: KOSPI200 
- 주식형의 소유형(중소형주식) → 소유형 BM: 중소형지수 
- 주식형의 소유형(배당주식) → 소유형 BM: 코스피 고배당 50 
- 주식형의 소유형(테마주식) → 소유형 BM: KOSPI200 
- 주식형의 소유형(K200 인덱스·기타인덱스) → 소유형 BM: KOSPI200
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "chapter1_summary",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Stage
answers

## Record ID
v5_q_0049

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "소유형 BM의 구분 기준은 무엇인가?",
  "answer_policy": "chapter1_summary"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
소유형 BM의 구분 기준은 무엇인가?

[답변 스타일]
1단원 요약형으로 답한다. 먼저 질문의 대상이 놓인 기본 맥락을 1문장 이내로 짚고, 이어서 핵심 정의/기준/비교를 2~5개 불릿 또는 2~4문장으로 정리한다. 문맥 연결을 위한 최소 설명은 허용하되 표 전체를 장황하게 나열하지 않는다.
- 답변 길이는 공백 포함 최대 1100자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 기준형: 답변은 판단 기준을 중심으로 제시하되, 그 기준이 무엇을 판단하기 위한 것인지 짧게 밝혀 독립적으로 이해 가능하게 한다
- 기준형: 적용 절차나 비교 설명은 포함하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Stage
answers

## Record ID
v5_q_0049

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "소유형 BM의 구분 기준은 무엇인가?",
  "answer_policy": "chapter1_summary"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
소유형 BM의 구분 기준은 무엇인가?

[답변 스타일]
1단원 요약형으로 답한다. 먼저 질문의 대상이 놓인 기본 맥락을 1문장 이내로 짚고, 이어서 핵심 정의/기준/비교를 2~5개 불릿 또는 2~4문장으로 정리한다. 문맥 연결을 위한 최소 설명은 허용하되 표 전체를 장황하게 나열하지 않는다.
- 답변 길이는 공백 포함 최대 1100자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 기준형: 답변은 판단 기준을 중심으로 제시하되, 그 기준이 무엇을 판단하기 위한 것인지 짧게 밝혀 독립적으로 이해 가능하게 한다
- 기준형: 적용 절차나 비교 설명은 포함하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Stage
answers

## Record ID
v5_q_0049

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "1. 유형분류 기준",
  "section": "1.2 유형별 벤치마크",
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "소유형 BM의 구분 기준은 무엇인가?",
  "answer_policy": "chapter1_summary"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
소유형 BM의 구분 기준은 무엇인가?

[답변 스타일]
1단원 요약형으로 답한다. 먼저 질문의 대상이 놓인 기본 맥락을 1문장 이내로 짚고, 이어서 핵심 정의/기준/비교를 2~5개 불릿 또는 2~4문장으로 정리한다. 문맥 연결을 위한 최소 설명은 허용하되 표 전체를 장황하게 나열하지 않는다.
- 답변 길이는 공백 포함 최대 1100자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 기준형: 답변은 판단 기준을 중심으로 제시하되, 그 기준이 무엇을 판단하기 위한 것인지 짧게 밝혀 독립적으로 이해 가능하게 한다
- 기준형: 적용 절차나 비교 설명은 포함하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
소유형 BM은 대유형을 구성하는 세부 구분(소유형)에 따라, 해당 소유형에 대응되는 벤치마크를 정해 구분합니다. 

- 국내펀드: 테마펀드·기타인덱스를 제외하면 별도 펀드 BM 없이 유형 BM을 적용하며, 그 유형 BM이 소유형별로 KOSPI200(일반주식 등), 중소형지수(중소형주식), 코스피 고배당 50(배당주식) 등으로 구분됩니다. 
- 해외펀드: 소유형별로 해당 지역/시장 지수를 유형 BM(=소유형 BM)으로 매칭하며, 예를 들어 글로벌주식은 MSCI ACWI, 유럽주식은 MSCI EUROPE, 중국주식은 MSCI CHINA로 구분합니다. 
- 채권형 소유형도 듀레이션/종류에 따라 소유형 BM이 달라지며(예: 일반채권 2년 90%, 초단기채권 6개월 90% 등), 소유형 분류가 곧 BM 배정의 기준이 됩니다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "chapter1_summary",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Assistant Response
```text
소유형 BM은 대유형 내에서 펀드의 “소유형”이 무엇인지에 따라 적용하는 벤치마크(소유형 BM)를 정하는 기준입니다. 

- 국내 주식형의 소유형별 소유형 BM은 다음처럼 구분합니다: 일반주식→KOSPI200, 중소형주식→중소형지수, 배당주식→코스피 고배당 50, 테마주식→KOSPI200, K200 인덱스→KOSPI200, 기타인덱스→KOSPI200   
- 다만 국내에서는 테마펀드와 기타인덱스를 제외하고는 별도 펀드 BM을 두지 않고 유형 BM을 적용하며, 테마/기타는 투자설명서(또는 규약)의 비교지수를 펀드 BM으로 설정합니다.   
- 테마/기타의 비교지수가 없으면 유형 BM으로 설정합니다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "chapter1_summary",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Assistant Response
```text
소유형 BM은 국내펀드 평가에서 대유형 아래의 “소유형”을 어떻게 분류했는지에 따라, 그 소유형에 대응하는 비교지수로 구분합니다. 

- 구분 기준: 소유형(예: 일반주식, 중소형주식, 배당주식, 테마주식, K200 인덱스, 기타인덱스 등)별로 소유형 BM이 정해짐 
- 주식형 소유형 BM: 일반주식·테마주식·K200 인덱스·기타인덱스는 KOSPI200, 중소형주식은 중소형지수, 배당주식은 코스피 고배당 50 
- 주식혼합형/채권혼합형/채권형 소유형 BM: 주식혼합형은 KOSPI200 50% + 매경 BP 종합채권 1년 50%, 채권혼합형은 KOSPI200 25% + 매경 BP 종합채권 1년 75%, 채권형은 일반채권 매경 BP 종합채권 2년 90%·초단기채권 KAP CD 6개월 90%·MMF 제로인 MMF 지수로 구분되며, 부동산형은 제로인 대안투자기대수익지수 5Y
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "chapter1_summary",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```
