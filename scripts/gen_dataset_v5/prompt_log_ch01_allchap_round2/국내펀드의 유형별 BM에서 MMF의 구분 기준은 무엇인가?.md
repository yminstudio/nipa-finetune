...
## Question
국내펀드의 유형별 BM에서 MMF의 구분 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "국내펀드의 유형별 BM에서 MMF의 구분 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "국내펀드의 유형별 BM에서 MMF의 구분 기준은 무엇인가?",
...
## Assistant Response
```text
- 국내펀드에서 MMF는 가중평균 잔존만기 90일 이내인 채권형 자산에 투자하는 펀드로 구분한다.   
- MMF형(소유형)은 CP, CD, Call 등 1년 미만의 단기금융상품에만 투자하는 것으로 설명된다.   
- 해당 MMF의 유형별 BM은 제로인 MMF 지수로 설정한다.
```
...
## Assistant Response
```text
국내펀드의 유형별 BM에서 MMF는 “MMF형”으로 분류되는 펀드에 해당 BM을 적용합니다. 

- MMF형 구분 기준: 가중평균 잔존만기가 90일 이내인 채권형 자산에 투자하는 펀드 
- 소유형(MMF) 구분 기준: CP, CD, Call 등 “1년 미만”의 단기금융상품에만 투자 
- 위 기준으로 MMF(소유형 MMF)로 설정된 경우의 유형별 BM: 제로인 MMF 지수
```
...
## Assistant Response
```text
국내펀드는 유형별 BM을 설정할 때, MMF에 해당하는지 여부를 먼저 “MMF형” 기준으로 구분한 뒤 그에 맞는 BM을 적용합니다. 

- MMF의 구분 기준(유형 기준): 가중평균 잔존만기가 90일 이내인 채권형 자산에 투자하는 펀드(= MMF형)   
- 편입대상 범위: CP, CD, Call 등 1년 미만의 단기금융상품에만 투자   
- 유형별 BM에서의 매핑: 국내펀드 유형별 BM 표에서 MMF 소유형의 BM은 “제로인 MMF 지수”로 설정
```
...
## Assistant Response
```text
국내펀드의 유형별 벤치마크(BM)를 설정할 때, MMF는 ‘MMF형’으로 구분해 BM을 적용합니다. 

- MMF형 구분 기준: 가중평균 잔존만기가 90일 이내인 채권형 자산에 투자하는 펀드입니다.   
- MMF형의 투자대상(예시/범위): CP, CD, Call 등 1년 미만의 단기금융상품에만 투자하는 것으로 설명됩니다.   
- MMF형의 유형별 BM(소유형 BM): 제로인 MMF 지수입니다.
```
...
## Assistant Response
```text
국내펀드의 유형별 BM을 설정할 때, MMF는 “MMF형”으로 구분되는 요건을 만족하는 펀드에 한해 해당 BM이 적용됩니다. 

- MMF 구분 기준: 가중평균 잔존만기가 90일 이내인 채권형 자산에 투자하는 펀드 
- 세부 투자대상 기준: CP, CD, Call 등 1년 미만의 단기금융상품에만 투자 
- (BM 관점) 국내 유형별 BM에서 MMF형은 제로인 MMF 지수로 설정됩니다.
```
