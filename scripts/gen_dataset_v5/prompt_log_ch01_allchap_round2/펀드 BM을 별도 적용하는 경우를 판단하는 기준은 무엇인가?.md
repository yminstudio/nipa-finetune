...
## Question
펀드 BM을 별도 적용하는 경우를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 BM을 별도 적용하는 경우를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "펀드 BM 별도 적용 여부 기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "펀드 BM을 별도 적용하는 경우를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드는 펀드 BM을 별도 적용한다.   
- 펀드 BM은 설정 당시 운용사가 희망하는 BM을 우선 적용한다.   
- 제로인이 해당 희망 BM을 보유하지 않으면 희망 BM과 가장 유사한 BM을 적용한다.   
- BM이 없거나, 제로인이 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용한다.   
- 참고로 국내펀드는 테마펀드, 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용한다.
```
...
## Assistant Response
```text
펀드 BM을 “유형 BM” 대신 “펀드 BM(해당 펀드 고유 비교지수)”으로 별도 적용할지는, 펀드가 국내/해외 중 어디에 투자하는지와 유형(테마/기타 등) 및 비교지수의 존재 여부로 구분합니다. 

- 국내펀드: 테마펀드 및 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용 
- 국내펀드(테마·기타): 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정하되, 비교지수가 없으면 유형 BM으로 설정 
- 해외펀드: 펀드 BM을 별도 적용(설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용) 
- 해외펀드에서 BM이 없거나(또는 제로인이 보유한 유사 BM이 존재하지 않아) 유형 BM을 적용하는 경우
```
...
## Assistant Response
```text
해당 기준은 펀드를 “국내/해외”로 나눠, 어떤 벤치마크(BM)를 펀드 수준에서 별도로 적용할지(펀드 BM vs 유형 BM)를 정하는 데 쓰입니다. 

- 국내펀드: 테마펀드 및 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 펀드 BM을 별도로 적용합니다.   
- (해외펀드 내 펀드 BM의 내용) 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용합니다.   
- (예외로 유형 BM으로 전환) 제로인이 보유하지 않은 경우에는 희망 BM과 가장 유사한 BM을 적용하며, BM이 없거나 제로인이 보유한 지수 중 유사한 BM도 존재하지 않으면 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
제로인에서는 펀드 BM을 “별도 적용”할지 여부를 국내/해외 구분 및 국내 유형(테마·기타인덱스 해당 여부)로 판단합니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 펀드 BM을 별도로 적용합니다.   
- (국내 예외에 해당) 테마 및 기타 유형: 투자설명서 또는 규약에 있는 비교지수를 펀드 BM으로 설정합니다.
```
...
## Assistant Response
```text
펀드의 벤치마크(BM)를 유형 BM으로 둘지, 펀드 BM을 별도로 둘지는 펀드가 국내에 투자하는지/해외에 투자하는지에 따라 달라집니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다. 
- 해외펀드: 펀드 BM을 별도 적용합니다. 
- 해외펀드(펀드 BM 산정의 기준): 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용합니다. 
- 해외펀드(대체 기준): 제로인이 보유하지 않은 경우 희망 BM과 가장 유사한 BM을 적용하고, BM이 없거나 보유 지수 중 유사한 BM이 없으면 유형 BM을 적용합니다.
```
