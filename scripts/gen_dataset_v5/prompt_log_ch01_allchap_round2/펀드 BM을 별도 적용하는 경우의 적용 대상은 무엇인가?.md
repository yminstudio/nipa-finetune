...
## Question
펀드 BM을 별도 적용하는 경우의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 BM을 별도 적용하는 경우의 적용 대상은 무엇인가?",
  "qa_type": "application",
  "question_template": "적용_펀드BM별도적용"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "펀드 BM을 별도 적용하는 경우의 적용 대상은 무엇인가?",
...
## Assistant Response
```text
- 적용 조건을 먼저 구분합니다. 국내펀드는 테마펀드, 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 국내 예외에 해당하면(테마펀드/기타인덱스) 비교지수(투자설명서/규약)를 펀드 BM으로 설정합니다.   
- 해외펀드는 펀드 BM을 별도 적용합니다. 설정 당시 운용사가 희망하는 BM을 우선 적용하고, 제로인이 보유하지 않으면 가장 유사한 BM을 적용하며, BM이 없거나 유사한 BM이 없으면 유형 BM으로 전환합니다.
```
...
## Assistant Response
```text
펀드 BM과 유형 BM 중 어떤 것을 쓰는지에 따라, “펀드 BM을 별도 적용”하는 대상과 그 전환 규칙이 달라집니다. 

- 적용 대상(펀드 BM 별도 적용):  
  - 해외펀드   
  - 국내펀드 중 테마펀드, 기타인덱스(테마 및 기타 유형)   

- 적용 순서/조건 및 전환 규칙  
  - 해외펀드: 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용. 제로인이 해당 BM을 보유하지 않으면 “희망 BM과 가장 유사한 BM”을 적용. BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 없으면 유형 BM으로 전환.   
  - 국내(테마·기타): 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정. 비교지수가 없으면 유형 BM으로 전환.
```
...
## Assistant Response
```text
BM 설정기준에서 ‘유형 BM’과 ‘펀드 BM’을 구분할 때, 펀드 BM을 별도로 적용하는 대상이 정해져 있습니다. 

- 적용 대상: 해외에 투자하는 펀드(해외펀드)   
  (국내펀드는 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.) 
- 적용 순서/조건(해외펀드의 펀드 BM 별도 적용)
  1) 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용   
  2) 제로인이 보유하지 않은 경우, 희망 BM과 가장 유사한 BM을 적용 
- 전환 규칙
  - BM이 없거나, 제로인이 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용
```
...
## Assistant Response
```text
펀드 BM을 “별도”로 적용하는지는 유형 BM과의 구분 규칙에 따라 정해집니다. 

- 적용 대상(조건): 해외펀드에는 펀드 BM을 별도 적용합니다.   
- 적용 순서: 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용합니다.   
- 전환 규칙: 제로인이 해당 희망 BM을 보유하지 않으면 희망 BM과 가장 유사한 BM을 적용합니다.   
- 최종 전환 규칙: BM이 없거나, 제로인이 보유한 지수 중 유사 BM이 존재하지 않으면 유형 BM으로 전환합니다.
```
...
## Assistant Response
```text
국내/해외 구분에 따라 BM을 “유형 BM”으로 쓸지 “펀드 BM(별도)”으로 쓸지가 달라집니다. 

펀드 BM을 별도 적용하는 적용 대상은 해외펀드입니다.   
- 적용 순서/조건: 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용   
- 적용 전환 규칙 1: 제로인이 보유하지 않은 경우, 희망 BM과 가장 유사한 BM으로 적용   
- 적용 전환 규칙 2: BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용
```
