...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대유형 BM과 소유형 BM 간의 차이점은 무엇인가?",
  "qa_type": "comparison",
  "question_template": "comparison_대유형_vs_소유형"
}
```
...
```json
{
  ...
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "대유형 BM과 소유형 BM 간의 차이점은 무엇인가?",
...
## Assistant Response
```text
- 대유형 BM은 펀드의 대분류(대유형) 기준으로 매칭되는 BM입니다.   
- 소유형 BM은 대유형 내부의 세부 분류(소유형)별로 매칭되는 BM이며, 표에서 소유형에 따라 다른 지수(소유형 BM)가 제시됩니다.   
- 다만 표에서 “볼드체 BM”으로 표시된 소유형 BM은, 해당 소유형이 속한 대유형 BM과 동일합니다.
```
...
## Assistant Response
```text
대유형 BM과 소유형 BM은 제로인 펀드평가에서 “유형”을 기준으로 벤치마크(BM)를 어떻게 나누어 적용·표기하는지에 해당합니다. 

- 대유형 BM: 대유형(큰 분류)에 대응해 기준으로 쓰이는 BM을 의미합니다.   
- 소유형 BM: 같은 대유형 안에서도 더 세부 분류인 소유형별로 제시(매핑)되는 BM을 의미합니다.   
- 관계(차이의 핵심): 소유형 BM 중 일부는 “해당 소유형이 속한 대유형의 BM과 동일”하도록 처리되며, 이 경우 표에서는(볼드체로) 동일하게 표시합니다.
```
...
## Assistant Response
```text
질문은 제로인 펀드평가에서 벤치마크(BM)를 정할 때 쓰는 ‘대유형’ 수준 BM과 ‘소유형’ 수준 BM이 무엇이 다르고 어떻게 연결되는지 묻고 있습니다. 

- 대유형 BM은 상위(큰) 분류의 기준이 되는 BM이고, 소유형 BM은 하위(세부) 분류에 대응해 더 세분화된 BM입니다.   
- 소유형별로 지정된 BM이 존재하며, 그 BM이 “해당 소유형이 속한 대유형의 BM과 동일”하다고 표시(볼드체)된 경우에는 소유형 BM이 사실상 대유형 BM과 같은 역할을 합니다.   
- 국내펀드는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용하므로, 대유형/소유형 체계에서 정해진 BM이 펀드 BM으로 연결됩니다.
```
...
## Assistant Response
```text
제로인 방법론은 펀드를 대유형(대분류)–소유형(소분류)로 나눠, 유형 내 비교가 가능하도록 벤치마크(BM)를 설정합니다. 

- 대유형 BM: 상위 유형(예: 국내 주식형, 해외주식형 등) 단위에서 적용되는 BM입니다.   
- 소유형 BM: 대유형을 더 세분한 소유형(예: 중소형주식, 배당주식 / 해외에서는 글로벌주식, 유럽주식 등) 단위에서 적용되는 BM입니다.    
- 관계/정합성: 특정 소유형의 BM이 해당 소유형이 속한 대유형의 BM과 동일한 경우가 있으며, 이 경우(표기 기준) 두 BM은 같은 것으로 봅니다.   
- 실무 적용 측면(국내/해외): 국내펀드는(테마펀드·기타인덱스를 제외하고) 별도 펀드 BM 없이 유형 BM을 적용하고, 해외펀드는 펀드 BM을 우선 적용하되 필요한 BM이 없으면 유형 BM(대유형 수준)을 적용합니다.
```
...
## Assistant Response
```text
대유형과 소유형은 펀드를 상위 분류(대분류)와 하위 분류(소분류)로 나눈 뒤, 각각에 대응하는 벤치마크(BM)를 두는 체계입니다. 

- 대유형 BM: 대분류(상위 유형)에 대응하는 BM입니다.  
- 소유형 BM: 소분류(세부 유형)에 대응하는 BM입니다.   
- 비교상 핵심 차이: 소유형 BM은 원칙적으로 소분류에 맞게 구체화되어 제시되며, 다만 일부 소유형의 BM(볼드체)은 “해당 소유형이 속한 대유형의 BM과 동일”하게 설정됩니다.
```
