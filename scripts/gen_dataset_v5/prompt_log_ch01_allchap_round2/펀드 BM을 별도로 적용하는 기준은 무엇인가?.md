...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 BM을 별도로 적용하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM_펀드별도적용_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "펀드 BM을 별도로 적용하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 국내펀드는 테마펀드, 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드는 펀드 BM을 별도로 적용합니다.   
- 별도 적용 시에는 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 하되, 제로인이 보유하지 않은 경우 희망 BM과 가장 유사한 BM, 그것도 없거나 유사한 BM이 없으면 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
제로인 펀드평가에서 BM은 펀드 성과를 비교하는 기준인데, “펀드 BM을 별도로 적용”한다는 것은 유형 BM이 아니라 해당 펀드에 맞춘 비교지수를 BM으로 쓰는 경우를 뜻합니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 펀드 BM을 별도로 적용합니다.   
- (해외) 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용합니다.   
- (해외) 제로인이 해당 BM을 보유하지 않은 경우 희망 BM과 가장 유사한 BM을 적용하며, BM이 없거나 보유한 지수 중 유사한 BM이 존재하지 않으면 유형 BM을 적용합니다.
```
...
## Assistant Response
```text
국내·해외 펀드를 평가할 때 “유형 BM”이 기본이지만, 일부 경우에는 “펀드 BM”을 별도로 적용합니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드: 펀드 BM을 별도로 적용합니다.   
- 해외펀드의 펀드 BM 선정 기준(별도 적용 시 내부 규칙):  
  - 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용   
  - 제로인이 보유하지 않은 경우 희망 BM과 가장 유사한 BM 적용   
  - BM이 없거나, 제로인이 보유한 지수 중 유사한 BM이 없는 경우 유형 BM 적용
```
...
## Assistant Response
```text
펀드의 벤치마크(BM)는 기본적으로 유형 BM을 쓰되, 국내/해외와 BM 존재 여부에 따라 펀드 BM을 별도로 적용합니다. 

- 국내펀드: 테마펀드, 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
  - 다만 테마 및 기타 유형은 투자설명서 또는 규약의 비교지수를 펀드 BM으로 설정하며, 명시 비교지수가 없으면 유형 BM으로 설정합니다. 
- 해외펀드: 펀드 BM을 별도로 적용합니다.   
  - 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 적용   
  - 제로인이 보유하지 않은 경우, 희망 BM과 가장 유사한 BM을 적용   
  - BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 존재하지 않는 경우, 유형 BM을 적용
```
...
## Assistant Response
```text
펀드 BM을 “유형 BM”이 아니라 “펀드 BM”으로 따로 설정해 성과를 비교하는 기준을 묻는 것으로 이해하면 됩니다. 

- 국내 펀드는 테마펀드, 기타인덱스를 제외하면 별도의 펀드 BM을 두지 않고 유형 BM을 적용합니다. 
- 해외 펀드는 펀드 BM을 별도로 적용합니다. 
- 해외 펀드의 펀드 BM은 설정 시점에 운용사가 희망하는 BM을 우선 적용합니다. 
- 제로인이 해당 희망 BM을 보유하지 않으면, 희망 BM과 가장 유사한 BM을 적용합니다. 
- 해외 펀드에서 BM이 없거나, 제로인이 보유한 지수 중 유사한 BM이 없으면 유형 BM을 적용합니다.
```
