...
## Question
환경(Environment) 축은 ESG 평가의 구성 축에 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "환경(Environment) 축은 ESG 평가의 구성 축에 포함되는가?",
  "qa_type": "criteria",
  "question_template": "환경축포함판단"
}
```
...
```json
{
  ...
  "subsection": "가) ESG평가란?",
  "seed_title": "가) ESG평가란?",
  "question": "환경(Environment) 축은 ESG 평가의 구성 축에 포함되는가?",
...
## Assistant Response
```text
- ESG 평가의 구성 축은 기업의 지속가능성에 영향을 미치는 요소를 어떤 항목들로 구분해 평가하는지를 뜻한다.   
- 그 구성 축에는 환경(Environment)이 포함된다.
```
