...
## Question
ESG 펀드 스코어링 방법론에서 제외 여부를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ESG 펀드 스코어링 방법론에서 제외 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "exclusion-criteria"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) ESG 펀드 스코어링 방법론",
  "question": "ESG 펀드 스코어링 방법론에서 제외 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
펀드 ESG 스코어링에서 “무엇을 계산에 포함하고(스코어링 입력으로 쓰고) 무엇을 제외하는지”를 정하기 위한 기준입니다.

- 대상 포트폴리오 자산에서 ETF는 제외하고, 주식종목(우선주 포함)만 대상으로 합니다   
- ESG 평가 점수가 있는 종목만을 대상으로 하며, ESG 점수가 없는 종목은 종목 비중 재산출 과정에서 제외됩니다   
- 동일유형내 펀드 포트폴리오 ESG점수의 절대순위 및 %순위 산출은 “펀드 내 Cover 비중이 50% 이상인 종목”을 대상으로 합니다
```
