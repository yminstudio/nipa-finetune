...
## Question
펀드 ESG 스코어 산출의 대상 유형에 ETF가 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 ESG 스코어 산출의 대상 유형에 ETF가 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "나) 펀드 ESG 스코어의 산출",
  "seed_title": "나) 펀드 ESG 스코어의 산출",
  "question": "펀드 ESG 스코어 산출의 대상 유형에 ETF가 포함되는가?",
...
## Assistant Response
```text
펀드 ESG 스코어 산출에서 ETF 포함 여부는 “펀드 ESG 스코어 산출 시 대상 포트폴리오 자산”을 무엇으로 잡는지에 대한 판단입니다.   
- 펀드 ESG 스코어의 대상 포트폴리오 자산은 주식종목(우선주 포함)입니다.   
- 대상에서 ETF는 제외됩니다.   
- 다만 ETF의 경우 ETF를 그대로 둘지, ETF 구성 종목을 배분할지에 대한 결정 필요성이 함께 명시돼 있습니다.
```
