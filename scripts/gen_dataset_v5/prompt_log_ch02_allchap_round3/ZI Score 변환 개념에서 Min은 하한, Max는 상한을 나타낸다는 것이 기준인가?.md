...
## Question
ZI Score 변환 개념에서 Min은 하한, Max는 상한을 나타낸다는 것이 기준인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score 변환 개념에서 Min은 하한, Max는 상한을 나타낸다는 것이 기준인가?",
  "qa_type": "criteria",
  "question_template": "criteria_MinMax_의미"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "ZI Score 변환 개념",
  "question": "ZI Score 변환 개념에서 Min은 하한, Max는 상한을 나타낸다는 것이 기준인가?",
...
## Assistant Response
```text
ZI Score 변환은 ZI 값이 어느 정도 “위치”에 있는지(최솟값~최댓값 범위 내) 점수(0~100)로 정규화하기 위한 것입니다. 

- 예, Min은 ZI 값의 최소값으로서 변환 구간의 하한을 의미합니다(Score 0).   
- Max는 ZI 값의 최대값으로서 변환 구간의 상한을 의미합니다(Score 100).   
- 또한 ZI 값이 증가할수록 Score가 증가하며, Min→Max 범위에서 0→100으로 정규화합니다.
```
