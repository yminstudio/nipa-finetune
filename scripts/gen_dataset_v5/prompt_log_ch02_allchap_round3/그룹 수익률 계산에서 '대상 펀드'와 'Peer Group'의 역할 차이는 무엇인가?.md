...
## Question
그룹 수익률 계산에서 '대상 펀드'와 'Peer Group'의 역할 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "그룹 수익률 계산에서 '대상 펀드'와 'Peer Group'의 역할 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_대상펀드와PeerGroup역할"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 그룹(Peer Group) 수익률 계산",
  "question": "그룹 수익률 계산에서 '대상 펀드'와 'Peer Group'의 역할 차이는 무엇인가?",
...
## Assistant Response
```text
- 공통 맥락: 그룹수익률은 특정 펀드들을 하나의 “그룹(=Peer Group)”으로 묶어 일별 수익률을 계산·누적해 산출합니다.   
- 대상 펀드: 그 그룹수익률 계산에 실제로 포함(또는 평가/집계)되는 개별 펀드의 ‘선정 대상’입니다(예: 총 운용규모 10억원 이상, 설정 후 2주 이상 운용, 일임펀드 제외, 유형/테마 관련 제외 등). fileciteturn0file1turn0file0  
- Peer Group: 같은 운용사 유형 또는 펀드유형에 속한 펀드 전체를 “하나의 펀드(Composite)”로 보고, 그 집합을 단위로 그룹수익률을 측정하는 ‘집계 단위’입니다.
```
