...
## Question
ZI Score Calculation Flow에서 펀드의 기간별 성과를 계산하는 것이 평가 기준에 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score Calculation Flow에서 펀드의 기간별 성과를 계산하는 것이 평가 기준에 포함되는가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_A"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "<ZI Score Calculation Flow>",
  "question": "ZI Score Calculation Flow에서 펀드의 기간별 성과를 계산하는 것이 평가 기준에 포함되는가?",
...
## Assistant Response
```text
펀드 성과를 기반으로 ZI Score를 산출하는 과정이 평가를 위한 기준에 해당합니다.   

- ZI Score Calculation Flow에서는 펀드의 1년/3년/5년(분석기간) “기간별 성과”를 먼저 계산한 뒤 이를 표준화해 ZI(i,j)를 산출합니다.   
- 이 기간별 성과는 CE(i,j)로 계산된 후 CE를 표준화한 ZI(i,j)로 이어지며, 이후 기간별 ZI를 가중 평균해 최종 ZI Score(ZI_i)를 만듭니다.
```
