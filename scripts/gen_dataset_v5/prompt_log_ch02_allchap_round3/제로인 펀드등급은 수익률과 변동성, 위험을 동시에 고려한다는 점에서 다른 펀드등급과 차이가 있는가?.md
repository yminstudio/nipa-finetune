...
## Question
제로인 펀드등급은 수익률과 변동성, 위험을 동시에 고려한다는 점에서 다른 펀드등급과 차이가 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "제로인 펀드등급은 수익률과 변동성, 위험을 동시에 고려한다는 점에서 다른 펀드등급과 차이가 있는가?",
  "qa_type": "comparison",
...
[핵심 명사]
. 제로인 펀드등급, [CE 식의 유도], 펀드등급, 성과등급, Performance, Rating, 제로인은, 수익률과, 변동성, 위험, 동시에, 고려한
