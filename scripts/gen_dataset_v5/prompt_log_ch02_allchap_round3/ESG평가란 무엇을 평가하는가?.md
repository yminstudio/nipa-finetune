...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ESG평가란 무엇을 평가하는가?",
  "qa_type": "definition",
...
## Assistant Response
```text
ESG평가란 기업의 지속가능성에 영향을 미치는 요소를 평가하는 것을 정의합니다.   
구체적으로는 환경(Environment), 사회책임(Social Responsibility), 지배구조(Governance)와 같은 항목을 대상으로 평가합니다.
```
