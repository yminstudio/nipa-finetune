...
## Question
제로인 펀드등급 평가의 판단 기준에서 성과등급(Performance, Rating)의 역할을 가리키는 용어는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "제로인 펀드등급 평가의 판단 기준에서 성과등급(Performance, Rating)의 역할을 가리키는 용어는 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_performance_role"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2.2. 제로인 펀드등급",
  "question": "제로인 펀드등급 평가의 판단 기준에서 성과등급(Performance, Rating)의 역할을 가리키는 용어는 무엇인가?",
...
## Assistant Response
```text
성과등급(Performance Rating)은 수익률과 변동성(위험)을 동시에 고려해 펀드의 성과 수준을 판단하기 위한 등급입니다.   
이 성과등급의 산출 근거(역할)를 가리키는 용어는 수익률과 위험의 관계를 반영한 위험조정수익률이며, 즉 효용값(확실성 등가; CE)입니다.
```
