...
## Question
CE 식의 유도가 성능등급의 판단 기준으로 문서에 명시되어 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CE 식의 유도가 성능등급의 판단 기준으로 문서에 명시되어 있는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[참고 : CE 식의 유도]",
  "seed_title": "[참고 : CE 식의 유도]",
  "question": "CE 식의 유도가 성능등급의 판단 기준으로 문서에 명시되어 있는가?",
...
## Assistant Response
```text
- 성능등급(펀드등급)은 수익률과 변동성(위험)을 함께 고려한 위험조정수익률인 CE를 바탕으로, 유형내 표준화점수(ZI Score)로 산출합니다.   
- 문서에는 CE 식의 유도 내용이 별도로 “참고” 형태로 명시되어 있으며, CE를 펀드등급 산출을 위한 기초값으로 두고 위험회피계수(λ) 및 CE 산식을 유도/정의합니다.   
- 이렇게 정의된 CE(CE i,j)를 계산해 ZI를 산출하고, ZI 백분율순위(%Rank)를 기초로 등급을 부여합니다.
```
