...
## Question
ZI Score 변환 개념의 기준은 변환 범위가 Min에서 Max 사이로 한정된다는 것인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score 변환 개념의 기준은 변환 범위가 Min에서 Max 사이로 한정된다는 것인가?",
  "qa_type": "criteria",
  "question_template": "criteria_범위_한정"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "ZI Score 변환 개념",
  "question": "ZI Score 변환 개념의 기준은 변환 범위가 Min에서 Max 사이로 한정된다는 것인가?",
...
## Assistant Response
```text
ZI Score는 ZI 값(정확히는 ZI 값을 점수화해 등급 산출에 쓰기 위한 환산)을 “점수(Score)”로 변환하기 위한 것입니다. 

- 변환의 기준은 ZI 값이 최소값(Min)부터 최대값(Max)까지의 범위를 가진다는 점이며, 이 Min→Max 구간에서 Score를 0→100으로 정규화합니다.   
- 또한 ZI 값이 커질수록 Score가 커지며, 구간 기반의 선형 보간 방식으로 Score가 계산됩니다.
```
