...
```json
{
  ...
  "target_total": 100,
  "generated_question": "성능등급의 판단 기준으로 CE 식의 유도가 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[참고 : CE 식의 유도]",
  "seed_title": "[참고 : CE 식의 유도]",
  "question": "성능등급의 판단 기준으로 CE 식의 유도가 포함되는가?",
...
## Assistant Response
```text
성능등급은 CE(확실성 등가)라는 위험조정 성과값을 등급 산출의 기초로 쓰기 위한 판단 기준입니다. 

- 성능등급 산출에는 CE 값이 사용되며, CE는 CE = 평균수익 − λ × σ²(효용값) 형태로 제시됩니다.   
- 다만 “CE 식의 유도”는 성능등급 판단을 위한 계산 단계로 사용된다기보다, CE 및 λ 식을 이해하기 위한 설명(참고)로 별도 표기되어 있습니다.
```
