...
## Question
펀드의 순자산 총액 산출에 적용되는 NAV의 종류를 하나의 판단 기준으로 제시한다면 어떤 NAV를 선택하는 것이 되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 순자산 총액 산출에 적용되는 NAV의 종류를 하나의 판단 기준으로 제시한다면 어떤 NAV를 선택하는 것이 되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "바) 펀드의 순자산 총액 산출 관련",
  "seed_title": "바) 펀드의 순자산 총액 산출 관련",
  "question": "펀드의 순자산 총액 산출에 적용되는 NAV의 종류를 하나의 판단 기준으로 제시한다면 어떤 NAV를 선택하는 것이 되는가?",
...
## Assistant Response
```text
순자산 총액 산출(운용규모·AUM을 의미하는 “순자산총액 NAV”)을 판단하기 위한 기준은, 그 NAV가 제로인이 보정해 산출한 순자산총액이어야 한다는 점입니다. 

- 보정 NAV를 선택합니다.   
- 보정 NAV는 제로인 산출 순자산총액으로, 과거 협회 제공 순자산총액의 오류를 보완하고 결산익일 재투자분으로 인한 순자산총액 감소 현상을 보정해 반영합니다.
```
