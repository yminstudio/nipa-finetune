...
## Question
그룹의 일별 BM 수익률 계산에서 분모로 사용되는 기준은 MDNAV인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "그룹의 일별 BM 수익률 계산에서 분모로 사용되는 기준은 MDNAV인가?",
  "qa_type": "criteria",
  "question_template": "bm_daily_return_criteria_denominator"
}
```
...
```json
{
  ...
  "subsection": "다) 그룹의 일별 BM 수익률",
  "seed_title": "다) 그룹의 일별 BM 수익률",
  "question": "그룹의 일별 BM 수익률 계산에서 분모로 사용되는 기준은 MDNAV인가?",
...
## Assistant Response
```text
그룹의 일별 BM 수익률은 각 그룹이 BM 대비 어느 정도 성과를 내는지를 보는 계산입니다.  
- 네, 일별 BM 수익률의 분모는 MDNAV(수정순자산총액)의 합입니다.   
- 즉, i일 P그룹의 일 BM수익률은 분모가 각 j펀드의 MDNAV_{i,j} = NAV_{i,j} / R_{i,j} (R_{i,j}: i일 j펀드 일수익률+1)의 합으로 산출됩니다.
```
