...
## Question
BM 수익률 계산에서 일수익률 산출을 판단 기준으로 삼는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM 수익률 계산에서 일수익률 산출을 판단 기준으로 삼는가?",
  "qa_type": "criteria",
  "question_template": "BM_수익률_일수익률_기준"
}
```
...
```json
{
  ...
  "subsection": "다) BM 수익률",
  "seed_title": "다) BM 수익률",
  "question": "BM 수익률 계산에서 일수익률 산출을 판단 기준으로 삼는가?",
...
## Assistant Response
```text
BM 수익률을 계산할 때 무엇을 기준으로 일별 성과를 정하느냐에 대한 질문입니다. 

- 제로인에서는 펀드별 BM(또는 유형별 BM)에 대해 BM의 일수익률을 산출하는 것을 기준으로 삼습니다.   
- BM의 일수익률은 “기준일의 전영업일 종가 / 그 전영업일의 종가 − 1”로 정의합니다.   
- 따라서 BM 수익률 계산에서 일수익률 산출은 판단 기준(산출의 출발점)으로 사용됩니다.
```
