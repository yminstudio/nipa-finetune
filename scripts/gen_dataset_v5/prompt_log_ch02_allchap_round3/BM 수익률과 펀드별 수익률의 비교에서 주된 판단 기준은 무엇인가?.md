...
## Question
BM 수익률과 펀드별 수익률의 비교에서 주된 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM 수익률과 펀드별 수익률의 비교에서 주된 판단 기준은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "다) BM 수익률",
  "seed_title": "다) BM 수익률",
  "question": "BM 수익률과 펀드별 수익률의 비교에서 주된 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- BM 수익률과 펀드별 수익률의 비교에서는, 특정 기간 동안 펀드가 BM을 얼마나 초과(또는 하회)했는지의 BM대비 초과수익률(또는 초과손실)이 주된 판단 기준입니다.   
- BM대비 초과수익률은 자산배분효과, 자산배분타이밍효과, 종목선정효과로 나뉘어 설명되므로, 비교 결과를 해석할 때는 이 3가지 요인이 초과/하회의 원천인지 확인하는 흐름이 함께 사용됩니다.
```
