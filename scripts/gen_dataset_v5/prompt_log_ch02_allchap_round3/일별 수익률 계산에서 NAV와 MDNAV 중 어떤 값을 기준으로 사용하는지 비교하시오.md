...
## Question
일별 수익률 계산에서 NAV와 MDNAV 중 어떤 값을 기준으로 사용하는지 비교하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "일별 수익률 계산에서 NAV와 MDNAV 중 어떤 값을 기준으로 사용하는지 비교하시오.",
  "qa_type": "comparison",
  "question_template": "Comparison_NAV_MDNAV기준"
}
```
...
```json
{
  ...
  "subsection": "나) 일별 수익률",
  "seed_title": "나) 일별 수익률",
  "question": "일별 수익률 계산에서 NAV와 MDNAV 중 어떤 값을 기준으로 사용하는지 비교하시오.",
...
## Assistant Response
```text
일별 수익률(그룹 수익률)을 계산할 때는 NAV와 MDNAV를 함께 사용합니다. 

- NAV: 일별 수익률 산식에서 분자(각 펀드의 NAV 합)로 사용됩니다.   
- MDNAV: 같은 산식에서 분모(각 펀드의 MDNAV 합)로 사용됩니다.   
- 또한 MDNAV는 NAV를 일수익률로 나눈 “수정순자산총액” 형태이므로, NAV가 그대로 들어가는 값이라기보다 일별수익률 반영을 위해 보정된 값으로 분모에 적용됩니다.
```
