...
## Question
현재 산출 대상 Hedged 지수에 포함된 지수군의 예시를 하나 들시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수에 포함된 지수군의 예시를 하나 들시오.",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수에 포함된 지수군의 예시를 하나 들시오.",
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수의 지수군은 ‘현재 산출 대상 Hedged 지수’에 포함된 지수들로, 예를 들어 임의로 부여한 헤지비율에 따라 각각 산출되는 지수군을 뜻한다.   
예시로 MSCI ACWI가 현재 산출 대상 Hedged 지수 지수군에 포함된다.
```
