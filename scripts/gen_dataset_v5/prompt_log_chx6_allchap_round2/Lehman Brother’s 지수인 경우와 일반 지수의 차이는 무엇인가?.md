...
## Question
Lehman Brother’s 지수인 경우와 일반 지수의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Lehman Brother’s 지수인 경우와 일반 지수의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Lehman Brother’s 지수인 경우와 일반 지수의 차이는 무엇인가?",
...
## Assistant Response
```text
- Lehman Brother’s 지수인 경우 헤지비율 H_t를 Yield_t에 연동해 계산하며, H_t = (1 + (Yield_t / 100)/2)^(1/6)로 산정합니다.   
- 일반 지수의 경우 H_t는 투자자가 적용할 헤지비율을 임의의 고정값(예: 0.5, 0.9, 1 등)으로 두고 사용합니다.
```
...
## Assistant Response
```text
두 지수 모두 KRW Hedged 지수 산출에서 헤지비율(Ht)이 핵심 입력으로 들어갑니다. 

- 일반 지수: 헤지비율 Ht를 “임의의 비율”로 정해 상수로 적용합니다(예: Ht=0.5, 0.9, 1).   
- Lehman Brother’s 지수: Ht를 Yieldt에 연동해 산출합니다. 즉 Ht = (1 + (Yieldt/100)/2)^(1/6).   
- 결과적으로 비교축은 “Ht를 고정값으로 쓰는지” vs “Ht를 수익률(Yield) 기반으로 계산해 변동시키는지”의 차이입니다.
```
...
## Assistant Response
```text
- 공통 맥락으로, 제로인의 KRW Hedged 지수 산출에서는 헤지비율 H_t를 먼저 정한 뒤 이를 반영해 지수를 계산합니다.   
- 일반 지수(예: MSCI 등)의 경우 H_t는 “임의의 비율”로 상수를 적용하며, 예시로 H_t=0.5, 0.9, 1 등이 언급됩니다.   
- 반면 Lehman Brother’s 지수인 경우 H_t는 Yield_t(=Yield to Worst)를 이용해 (1 + (Yield_t/100)/2)^(1/6) 형태로 산출합니다.
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 헤지비율 H_t가 동일하게 사용되지만, “Lehman Brother’s 지수인 경우”와 “일반 지수(기타 제공 지수)”는 H_t를 만드는 방식이 다릅니다. 

- 일반 지수인 경우: 헤지비율 H_t를 임의의 비율로 “상수”처럼 적용(예: 0.5, 0.9, 1).   
- Lehman Brother’s 지수인 경우: H_t가 Yield_t로부터 계산되는 값으로 정해짐( H_t = (1 + (Yield_t/100)/2)^(1/6) ).   
- 즉, 차이는 헤지비율 H_t의 적용 방식(상수 적용 vs Yield 기반 산출)에 있습니다.
```
