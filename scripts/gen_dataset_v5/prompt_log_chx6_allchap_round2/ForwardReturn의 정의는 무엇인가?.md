...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ForwardReturn의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "ForwardReturn의 정의는 무엇인가?",
...
## Assistant Response
```text
- ForwardReturn_t는 헤지비용을 반영한 Forward 거래 기반의 반환으로, 다음과 같이 정의됩니다.   
  ForwardReturn_t = FFR_0/FXR_0^R − ((FFR_t − FXR_t^R)/FXR_0^R) × (OddDay_t/T)
- 여기서 FFR_0는 전월 말일의 KRW/USD 1 Month Forward 환율, FFR_t는 해당 t일의 KRW/USD 1 Month Forward 환율입니다.   
- OddDay_t는 해당 월에서 t일의 잔여일수(= T − t일 경과일수(당일 포함)), T는 해당 월의 총일수입니다.
```
...
## Assistant Response
```text
- 제로인 KRW Hedged 지수 산출에서의 ForwardReturn_t는 “Forward 거래(1 Month Forward)로부터 계산되는 수익률”로, 다음과 같이 정의된다.   
- 정의식:  
  ForwardReturn_t = (FFR_0 / FXR_0^R) − ((FFR_t − FXR_t^R) / FXR_0^R) × (OddDay_t / T)
```
...
## Assistant Response
```text
ForwardReturn_t는 제로인 KRW 헤지 지수 산출에서 헤지비용(Forward 거래)을 반영하기 위해 사용하는 “Forward 환율 관련 수익률”입니다. 

- 정의(산식):  
  ForwardReturn_t = FFR_0 / FXR_0^R − ((FFR_t − FXR_t^R) / FXR_0^R) × (OddDay_t / T)   
- 여기서 FFR_0는 전월 말일 KRW/USD 1 Month Forward 환율, FFR_t는 해당 t일의 KRW/USD 1 Month Forward 환율이며, FXR^R는 해당 수익률(Yield to Worst) 관련 값, OddDay_t는 해당 월에서 t일의 잔여일수, T는 해당 월 총일수입니다.
```
...
## Assistant Response
```text
- ForwardReturn_t는 KRW Hedged 지수 산출에서 환헤지 운용성과 중 “Forward 거래(1 Month Forward) 관련” 성분을 나타내는 값이며, HReturn_t의 구성요소로 쓰인다( HReturn_t = ForwardReturn_t − CurrencyReturn_t ).   
- 정의(산출식)는 다음과 같다.  
  ForwardReturn_t = FFR_0 / FXR_0^R − ((FFR_t − FXR_t^R) / FXR_0^R) × (OddDay_t / T)   
- 여기서 FFR_0는 전월 말일의 KRW/USD 1 Month Forward 환율, FFR_t는 t일의 KRW/USD 1 Month Forward 환율, FXR_t^R·FXR_0^R는 기준환율(해당 월/기준 시점의 값), OddDay_t는 해당 월에서 t일의 잔여일수, T는 해당 월 총일수이다.
```
