...
## Question
Hedged 지수 산출 대상의 구성에 따라 KRW로 환헤지되어 산출되는 지수의 예시는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Hedged 지수 산출 대상의 구성에 따라 KRW로 환헤지되어 산출되는 지수의 예시는 무엇인가?",
  "qa_type": "application",
  "question_template": "Application_Template_A8"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "Hedged 지수 산출 대상의 구성에 따라 KRW로 환헤지되어 산출되는 지수의 예시는 무엇인가?",
...
## Assistant Response
```text
- 산출 대상 구성에 “MSCI ACWI”가 포함되면, 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 KRW로 환헤지되어 산출되는 지수로 적용   
- 산출 대상 구성에 “MSCI WI”가 포함되면, 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 KRW로 환헤지되어 산출되는 지수로 적용   
- 산출 대상 구성에 “MSCI EM”이 포함되면, 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 KRW로 환헤지되어 산출되는 지수로 적용   
- 산출 대상 구성에 “Lehman Brother’s US Aggregate”가 포함되면, KRW로 환헤지되어 산출되는 지수로 적용   
- 산출 대상 구성에 “JACI, JACI IG”가 포함되면, 임의로 부여한 헤지비율(예: 95%, 100% 등)에 따라 KRW로 환헤지되어 산출되는 지수로 적용
```
...
## Assistant Response
```text
USD지수를 기반으로 Forward 거래를 통한 헤지비용을 감안하고, 헤지비율(H_t)을 반영해 KRW로 환헤지된 지수를 산출합니다 .

- MSCI ACWI: 임의로 부여한 헤지비율(예: 50%, 100%)에 따라 KRW로 환헤지되어 산출   
- MSCI WI: 임의로 부여한 헤지비율(예: 50%, 100%)에 따라 KRW로 환헤지되어 산출   
- MSCI EM: 임의로 부여한 헤지비율(예: 50%, 100%)에 따라 KRW로 환헤지되어 산출   
- Lehman Brother’s US Aggregate: KRW로 환헤지되어 산출 대상에 포함   
- JACI, JACI IG: 임의로 부여한 헤지비율(예: 95%, 100%)에 따라 KRW로 환헤지되어 산출
```
...
## Assistant Response
```text
KRW로 환헤지되어 산출되는 “Hedged 지수”는 아래처럼 산출 대상(지수 종류)에 따라 헤지비율(Hedge Ratio) 산정 규칙이 달라지고, 그 헤지비율을 적용해 KRW Hedged 지수로 전환해 산출합니다. 

- MSCI ACWI / MSCI WI / MSCI EM: 헤지비율을 “임의로 부여”(예: 50%, 100%)하는 규칙으로 정한 뒤, 해당 헤지비율로 KRW 환헤지 지수로 전환·산출 
- Lehman Brother’s US Aggregate: 헤지비율을 “Yield to Worst” 기반의 규칙으로 산정한 뒤(수식 기반), 그 헤지비율로 KRW 환헤지 지수로 전환·산출  
- JACI / JACI IG: 헤지비율을 “임의로 부여”(예: 95%, 100%)하는 규칙으로 정한 뒤, 해당 헤지비율로 KRW 환헤지 지수로 전환·산출
```
...
## Assistant Response
```text
Hedged 지수 산출에서 KRW로 환헤지되어 산출되는 “산출 대상 Hedged 지수” 예시는 아래와 같으며, 적용 조건은 각 지수에 부여되는 헤지비율(Hedge Ratio)에 따라 KRW Hedged 지수를 각각 산출하는 것이다. 

- MSCI ACWI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- MSCI WI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- Lehman Brother’s US Aggregate: 산출 대상에 포함   
- JACI, JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100%)
```
