...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Where와 frac의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Where와 frac의 차이는 무엇인가?",
...
## Assistant Response
```text
- Where: 수식에 사용된 기호(예: 평균수익률, 무위험이자율, 표준편차 등)를 “정의”해 적는 부분입니다.   
- \frac: 수식에서 “분수(분자/분모)” 형태로 값을 계산하는 수학 표기(나눗셈 구조)입니다.
```
...
## Assistant Response
```text
공통 맥락은 제로인 펀드평가 방법론의 수식 표기에서입니다.   

- Where: 수식(등식) 뒤에 나오는 “기호/변수 정의”를 붙이는 구분(예: R^F_i, R^M_i 등).   
- frac: 수식 안에서 분수 형태(분자/분모)를 나타내는 표기. 예를 들어 \frac{분자}{분모}처럼 계산식을 구성합니다.
```
...
## Assistant Response
```text
공통 맥락은 수식(예: β, CE, IR 등)을 제시한 뒤 기호 의미를 붙이는 문서 표기입니다. 

- Where: 수식에 쓰인 각 기호의 의미(정의/값이 무엇을 뜻하는지)를 나열하는 구문입니다.   
- frac: 분수 형태(분자/분모)를 수식 안에서 표현하기 위한 표기(LaTeX의 분수 서식)입니다. 예컨대 β 식에서 처럼 분수를 구성하는 데 쓰입니다.  
- 정리하면, Where는 “수식의 기호가 무엇인지”를 설명하는 표지이고, frac는 “수식에서 분수를 어떻게 써서 나타낼지”를 담당합니다.
```
...
## Assistant Response
```text
공통 맥락: 제로인 방법론의 수식 표기에서, 수식 자체(계산식)와 변수 의미(정의)를 구분해 나타낼 때 Where와 \frac가 함께 쓰입니다. 

- Where: 수식이 끝난 뒤 나오는 “변수/기호의 정의” 파트로, 예를 들어 $R^F$ 같은 항이 무엇을 뜻하는지 구체적으로 적습니다.   
- \frac: 수식 “계산식” 안에서 분자/분모를 나누는 형태(분수)로 쓰이며, 예를 들어 $S^F=\frac{\overline{R^F}-\overline{R^{rf}}}{\sigma^F}$처럼 값의 산출 자체를 표현합니다.
```
