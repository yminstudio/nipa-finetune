...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HedgedIndex의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HedgedIndex의 정의는 무엇인가?",
...
## Assistant Response
```text
- HedgedIndex는 t일의 “최종 KRW Hedged Index”를 의미하며, 산출은 전월 말의 HedgedIndex_0를 기준으로 갱신된다.   
- 산출식은 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 이며, HedgedIndexR_t는 “KRW Hedged 누적수익률”로 정의된다.   
- HedgedIndexR_t는 (I_t/I_0) × (FXR_t^R/FXR_0^R) - 1 과 H_t × HReturn_t의 합으로 구성되고, HReturn_t = ForwardReturn_t - CurrencyReturn_t 이다.
```
...
## Assistant Response
```text
HedgedIndex는 환헤지를 반영해 산출하는 지수로, 해당 t일의 최종 KRW Hedged Index를 의미한다. 

- 정의식: HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)   
- 여기서 HedgedIndex_0는 전월 말일의 KRW Hedged Index이다.   
- HedgedIndexR_t는 전월 말일~해당 t일 까지의 KRW Hedged 누적수익률이다.
```
...
## Assistant Response
```text
HedgedIndex는 해외투자 성과측정을 위해 산출하는 “KRW Hedged 지수”로, 전월 말일의 KRW Hedged Index에서 시작해 해당 t일의 최종 KRW Hedged Index로 업데이트되는 지수입니다 .

- 정의(누적 산출): HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)   
- HedgedIndexR_t(누적수익률): HedgedIndexR_t = (I_t/I_0) × (FXR_t^R/FXR_0^R) − 1 + H_t × HReturn_t   
- 환헤지 반영 항: HReturn_t = ForwardReturn_t − CurrencyReturn_t, 여기서 CurrencyReturn_t = FXR_t^R/FXR_0^R
```
...
## Assistant Response
```text
- HedgedIndex_t는 해당 t일의 최종 KRW Hedged Index(원화 헤지 반영 지수)로, 전월 말일의 KRW Hedged Index 값( HgedhedIndex_0 )을 기초로 산출한 지수입니다.   
- HedgedIndex_0는 전월 말일의 KRW Hedged Index입니다.   
- HedgedIndexR_t는 전월 말일 ~ 해당 t일 까지의 KRW Hedged 누적수익률로, HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 연결해 산출됩니다.
```
