...
## Question
Hedged 지수의 산출 대상에 포함되는 지수의 예시는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Hedged 지수의 산출 대상에 포함되는 지수의 예시는 무엇인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A5"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "Hedged 지수의 산출 대상에 포함되는 지수의 예시는 무엇인가?",
...
## Assistant Response
```text
Hedged 지수의 산출 대상에는 제시된 “현재 산출 대상 Hedged 지수”가 포함되며, 그 예시는 다음과 같습니다.   
- MSCI ACWI   
- MSCI WI   
- MSCI EM   
...
## Assistant Response
```text
Hedged 지수의 산출 대상은 제로인에서 “현재 산출 대상 Hedged 지수”로 지정한 지수들(헤지비율을 임의로 부여해 각각 산출되는 항목 포함)입니다. 
...
## Assistant Response
```text
Hedged 지수의 산출 대상은 임의로 부여한 헤지비율에 따라 각각 Hedged(환헤지 반영) 성격으로 산출하는 지수들입니다.   
- MSCI ACWI   
- MSCI WI   
- MSCI EM   
- Lehman Brother’s US Aggregate
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 “산출 대상 Hedged 지수”란, 현재 제로인에서 헤지드 지수로 산출·적용하기 위해 포함하는 기준 지수들을 뜻합니다.   
- MSCI ACWI   
- MSCI WI   
- MSCI EM   
