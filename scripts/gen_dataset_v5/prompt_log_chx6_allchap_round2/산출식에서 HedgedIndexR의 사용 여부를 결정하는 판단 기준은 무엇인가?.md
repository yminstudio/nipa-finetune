...
## Question
산출식에서 HedgedIndexR의 사용 여부를 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "산출식에서 HedgedIndexR의 사용 여부를 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "산출식에서 HedgedIndexR의 사용 여부를 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외투자 펀드 중 환헷지를 하지 않는 경우에는 원화환산 BM지수 산출식을 적용하므로 HedgedIndexR은 사용하지 않는다.   
- KRW Hedged 지수 산출에서는 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 형태로 KRW Hedged 누적수익률 계산을 위해 HedgedIndexR_t를 사용한다.
```
...
## Assistant Response
```text
환헤지를 반영해 ‘KRW Hedged 지수’를 산출하는지 여부를 기준으로 HedgedIndexR을 산출식에 적용할지 결정한다. 

- KRW Hedged 지수 산출 구간에서는 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 형태로, 산출식에 HedgedIndexR_t를 사용한다. 
- 환헷지를 하지 않는 해외투자에 적용하는 경우에는 원화환산 BM지수(= Local(or USD)지수 × (원화/로컬)환율) 산출식을 사용하며, 이 경우에는 HedgedIndexR_t를 쓰지 않는다.
```
...
## Assistant Response
```text
HedgedIndexR는 KRW Hedged 지수의 t일 누적수익률(헤지 반영 성과분)을 계산에 반영할지 여부를 가르는 요소입니다. 

- KRW Hedged 지수 산출(환헤지를 반영해 KRW Hedged 지수를 BM/기준으로 쓰는 경우)에서는 HedgedIndexR_t를 산출식에 사용해 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 최종 지수를 계산합니다.    
- HedgedIndexR_t는 USD지수 변화 항과 H_t(헤지비율) × HReturn_t(헤지 리턴 누적수익률) 항으로 구성되므로, Forward 거래 기반의 헤지비용/헤지리턴을 반영하는 KRW Hedged 지수 산출에서만 해당됩니다.   
- (반대로) KRW Hedged 지수 산출이 아닌 경우에는 위와 같은 KRW Hedged 산출 구조에서 HedgedIndexR_t를 전제로 한 계산을 하지 않습니다.
```
...
## Assistant Response
```text
HedgedIndexR_t는 전월 말일 대비 t일까지의 KRW Hedged 누적수익률을 의미하며, HedgedIndex_t를 업데이트할 때 이를 반영하기 위해 사용합니다. 

- 산출 대상 Hedged 지수가 “Lehman Brother’s”인지를 기준으로, HedgedIndexR_t를 구성하는 헤지 관련 항의 정의가 달라집니다. 
- Lehman Brother’s 지수인 경우: H_t를 Yield_t로 산출하고, HReturn_t를 “전월 말일~해당 t일”의 Hedge Return 누적수익률로 두어 HedgedIndexR_t를 산출합니다. 
- Lehman Brother’s가 아닌 경우(예: MSCI ACWI, JACI, JACI IG 등): H_t를 임의의 상수 헤지비율로 두고, HReturn_t를 ForwardReturn_t − CurrencyReturn_t로 정의한 뒤 HedgedIndexR_t를 산출합니다.
```
