...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CurrencyReturn의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "CurrencyReturn의 정의는 무엇인가?",
...
## Assistant Response
```text
- CurrencyReturn_t의 정의는 다음과 같습니다: CurrencyReturn_t = FXR_t^R / FXR_0^R   
- 이 CurrencyReturn_t는 HReturn_t를 산출할 때 ForwardReturn_t에서 차감되는 항으로 사용됩니다
```
...
## Assistant Response
```text
CurrencyReturn은 KRW Hedged 지수를 산출할 때, 환헤지의 환율 관련 성과 항으로 사용되는 값입니다. 

CurrencyReturn_t의 정의는 다음과 같습니다.   
- CurrencyReturn_t = FXR_t^R / FXR_0^R
```
...
## Assistant Response
```text
- CurrencyReturn_t는 KRW Hedged 지수 산출에 사용되는 환율요소로, t일의 CurrencyReturn은 다음과 같이 정의된다: CurrencyReturn_t = FXR_t^R / FXR_0^R   
- 여기서 FXR_0^R는 전월 말일의 KRW/USD 1 Month Forward 환율, FXR_t^R는 해당 t일의 KRW/USD 1 Month Forward 환율이다
```
...
## Assistant Response
```text
- 제로인 방법론에서 CurrencyReturn_t는 환헤지 지수 산출에서의 “통화(환율) 요소”에 해당하는 수치이며, 다음과 같이 정의됩니다.   
- 정의식: CurrencyReturn_t = FXR_t^R / FXR_0^R   
- 또한 HedgeReturn_t는 ForwardReturn_t에서 CurrencyReturn_t를 차감하여 산출됩니다.
```
