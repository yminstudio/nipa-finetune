...
## Question
현재 산출 대상 Hedged 지수에서 KRW 통화 헤지 여부를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수에서 KRW 통화 헤지 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수에서 KRW 통화 헤지 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 환헷지를 하는 경우: 로컬지수 적용(= KRW 통화 헤지로 판단)   
- 환헷지를 하지 않는 경우: 원화환산 지수 적용(단, 적극적으로 밝힌 경우만 인정)   
- 투자설명서 기준 인정: ‘필요한 경우 환헷지를 할 수 있다’ → 환헷지 하는 경우로 인정, ‘지금은 환헷지 수단이 없지만 후에… 생기는 경우 환헷지를 할 수 있다’ → 투자지역/상황에 따라 인도는 현재 환헷지 불가, 러시아는 현재 환헷지 가능으로 판단(브라질은 ADR에 투자하면 USD와 환헷지로 인정하나, USD- BRL 환헷지는 안 됨)
```
...
## Assistant Response
```text
KRW 통화 헤지 여부는 해당 Hedged 지수가 “USD 노출을 KRW 기준으로 헤지한(=KRW Hedged 성격을 가진)” 지수인지 구분하기 위한 기준입니다.

- KRW Hedged 지수로 판단하는 핵심은 KRW/USD 1 Month Forward 환율(FFR_0, FFR_t)을 헤지비용(ForwardReturn_t) 및 환율 성과(CurrencyReturn_t) 계산에 사용해 헤지 반영 수익률(HReturn_t=ForwardReturn_t−CurrencyReturn_t)을 산출하는지 여부입니다.   
- 동시에 헤지비율 H_t를 적용해 헤지 강도(예: 0.5, 0.9, 1)를 “상수 또는 임의로 부여한 헤지비율(예: 50%, 100%, 95%)” 형태로 설정하는지 여부로 결정합니다.    
- 즉, Hedged 지수 산출식에서 H_t×HReturn_t 항이 반영되어 KRW 헤지 수익률이 구성되는 경우를 KRW 통화 헤지 적용으로 봅니다.
```
...
## Assistant Response
```text
KRW 통화 헤지 여부(환헷지 적용/미적용)는 해외펀드에 대해 BM을 어떤 기준으로 둘지 정하기 위한 판단으로, 투자설명서에서의 환헷지 가능/현재 수단 유무와 투자지역의 환헷지 가능 상황을 근거로 가릅니다. 

- 투자설명서상 “필요한 경우 환헷지를 할 수 있다”로 명시된 경우: 환헷지를 하는 경우로 봅니다.   
- “지금은 환헷지 수단이 없지만, 후에 수단이 생기면 환헷지할 수 있다”로 명시된 경우: 투자하는 지역/통화 및 현재 상황에 따라 환헷지 가능 여부를 판단합니다.   
  - 예: 인도는 현재 환헷지할 수 없음, 러시아는 현재 환헷지할 수 있음으로 봅니다.   
  - 예: 브라질을 Local에 투자하면 환헷지할 수 없음, 브라질에 ADR에 주로 투자하면 USD와 환헷지하는 것으로 인정합니다(단, USD와 BRL 간 환헷지는 안 함).   

(참고로, 환헷지를 하는 경우 로컬지수를 적용하고, 환헷지를 하지 않는 경우 원화환산 지수를 적용합니다. )
```
...
## Assistant Response
```text
해외펀드의 “KRW 통화 헤지 여부”는, 환헷지를 하는지 여부를 구분하는 기준에 따라 판단합니다. 

- 환헷지를 하는 경우: 로컬지수를 적용합니다.   
- 환헷지를 하지 않는 경우: 원화환산 지수를 적용합니다.   
- 다만, “환헷지 수단이 현재 없다”는 취지 등은 원칙적으로 원화환산 지수 적용으로 인정하되, 투자설명서에서 적극적으로 밝힌 경우에만 인정합니다.
```
