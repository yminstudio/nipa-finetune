...
## Question
이 문서에서 KRW Hedged 지수를 산출하기 위해 대상으로 삼는 지수는 어떤 것들인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "이 문서에서 KRW Hedged 지수를 산출하기 위해 대상으로 삼는 지수는 어떤 것들인가?",
  "qa_type": "application",
  "question_template": "Application_Template_A3"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "이 문서에서 KRW Hedged 지수를 산출하기 위해 대상으로 삼는 지수는 어떤 것들인가?",
...
## Assistant Response
```text
- MSCI ACWI: 임의로 부여한 헤지비율에 따라 각각 산출 대상   
- MSCI WI: 임의로 부여한 헤지비율에 따라 각각 산출 대상   
- MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출 대상   
- Lehman Brother’s US Aggregate: KRW Hedged 지수 산출을 위한 대상   
- JACI, JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출 대상
```
...
## Assistant Response
```text
- MSCI ACWI: 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 산출   
- MSCI WI: 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 산출   
- MSCI EM: 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 산출   
- Lehman Brother’s US Aggregate: 해당 지수를 기반으로 헤지비율을 산출(문서 내 별도 헤지비율 산식 적용)   
- JACI, JACI IG: 임의로 부여한 헤지비율(예: 95%, 100% 등)에 따라 산출
```
...
## Assistant Response
```text
- KRW Hedged 지수 산출에 앞서, “현재 산출 대상 Hedged 지수”에 포함된 지수를 대상으로 삼습니다.   
- MSCI 계열(ACWI, WI, EM): 임의로 부여한 헤지비율에 따라 각각 산출되며(예: 50%, 100% 등), 이후 산출 과정에서 해당 헤지비율이 반영됩니다.   
- Lehman Brother’s US Aggregate: 해당 지수의 경우 헤지비율 H_t를 Yield to Worst를 이용한 정해진 식으로 산출하여 반영합니다.   
- JACI 계열(JACI, JACI IG): 임의로 부여한 헤지비율에 따라 각각 산출되며(예: 95%, 100% 등), 이후 산출 과정에서 해당 헤지비율이 반영됩니다.
```
...
## Assistant Response
```text
KRW Hedged 지수를 산출할 때 현재 “산출 대상 Hedged 지수”로는 아래 지수들을 사용하며, 대부분은 헤지비율(Hedge Ratio)을 임의로 부여해 산출합니다. 다만 Lehman Brother’s 지수는 Yield to Worst를 이용해 헤지비율을 정하는 방식이 적용됩니다. 

- MSCI ACWI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- MSCI WI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- Lehman Brother’s US Aggregate: 헤지비율은 Yield to Worst를 통해 산출(지수인 경우 H_t 산출식 적용)   
