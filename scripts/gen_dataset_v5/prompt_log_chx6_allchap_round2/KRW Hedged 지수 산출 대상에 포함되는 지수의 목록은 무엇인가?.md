...
## Question
KRW Hedged 지수 산출 대상에 포함되는 지수의 목록은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "KRW Hedged 지수 산출 대상에 포함되는 지수의 목록은 무엇인가?",
  "qa_type": "application",
  "question_template": "Application_Template_A2"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "KRW Hedged 지수 산출 대상에 포함되는 지수의 목록은 무엇인가?",
...
## Assistant Response
```text
KRW Hedged 지수 산출 시 현재 산출 대상에 포함되는 Hedged 지수는 아래와 같습니다. 

- MSCI ACWI: 임의로 부여한 헤지비율로 각각 산출(예: 50%, 100% 등). 이 경우 헤지비율은 상수로 적용    
- MSCI WI: 임의로 부여한 헤지비율로 각각 산출(예: 50%, 100% 등)   
- MSCI EM: 임의로 부여한 헤지비율로 각각 산출(예: 50%, 100% 등)   
- Lehman Brother’s US Aggregate: 다른 지수와 달리 Lehman Brother’s 지수인 경우의 헤지비율(H_t) 산출식을 적용    
- JACI, JACI IG: 임의로 부여한 헤지비율로 각각 산출(예: 95%, 100% 등). 이 경우 헤지비율은 상수로 적용
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에 사용되는 Hedged 지수(현재 산출 대상)는 아래 지수들로 구분되며, 헤지비율 산출 방식이 지수별로 달라집니다. 

- 1) MSCI ACWI / MSCI WI / MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 0.5, 0.9, 1)하고, 해당 방식으로 Hedged 지수 산출을 진행    
- 2) Lehman Brother’s US Aggregate: 지수가 Lehman Brother’s인 경우로 전환하여 헤지비율 H_t를 Yield to Worst 기반 식으로 산출한 뒤 해당 헤지비율을 반영해 산출    
- 3) JACI / JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100% 등)하고, 해당 방식으로 Hedged 지수 산출을 진행
```
...
## Assistant Response
```text
KRW Hedged 지수 산출 대상(현재 산출 대상 Hedged 지수)으로는 아래 지수들이 포함되며, 선택된 지수에 대해 헤지비율 적용 방식에 따라 각각 산출한다. 

- MSCI ACWI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) 
- MSCI WI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) 
- MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) 
- Lehman Brother’s US Aggregate, JACI, JACI IG:  
  - Lehman Brother’s US Aggregate는 지수인 경우 헤지비율 H_t를 Yield to Worst 기반 식으로 산출   
  - JACI, JACI IG는 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100% 등)
```
