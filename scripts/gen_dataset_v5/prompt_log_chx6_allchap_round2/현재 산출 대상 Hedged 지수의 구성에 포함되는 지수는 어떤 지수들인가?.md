...
## Question
현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 어떤 지수들인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 어떤 지수들인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A4"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 어떤 지수들인가?",
...
## Assistant Response
```text
정의: 현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 아래 지수들이다.   
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 다음과 같습니다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는, 헤지비율을 임의로 적용해 각각 산출하는 “원지수(산출 대상 Hedged 지수의 기반 지수)”들이다.  
해당 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI 및 JACI IG이다.
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수의 구성에 포함되는 지수는 아래 지수들입니다. 
