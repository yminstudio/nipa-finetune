...
```json
{
  ...
  "target_total": 100,
  "generated_question": "frac의 적용 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "frac의 적용 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 제공된 방법론 문서에서는 ‘frac’(또는 ‘FRAC’)의 적용 여부를 판단하는 기준이 명시되어 있지 않습니다.   
- 따라서 해당 기준만 근거로 정리해 드릴 수 없습니다.
```
...
## Assistant Response
```text
- ‘frac’는 수식에서 분수 형태를 쓰는 표기 방식이며, 문서에서는 이것을 쓰는지 여부를 가르는 별도의 판단대상을 정의하지 않습니다.   
- ‘frac’는 예시 수식들에서 단순히 분수 표기로 사용됩니다(예: CD 듀레이션, Modified Sharpe Ratio 등).    
- 따라서 문서에 ‘frac의 적용 여부’를 판단하기 위한 조건(기준/규칙/예외)은 별도로 제시되어 있지 않습니다.
```
...
## Assistant Response
```text
- ‘frac’가 무엇을 의미하는지(예: 특정 지표/조정항목명인지, 수식 표기(\frac) 용어인지) 먼저 짚기 위한 질문입니다.  
- 현재 확인 가능한 제로인 펀드평가 방법론 내용에서는 ‘frac의 적용 여부’를 판단하기 위한 별도의 기준이 명시돼 있지 않습니다 .  
- 따라서 ‘frac’가 가리키는 대상(문서 내에서의 원문 문구/문맥)을 알려주시면, 그에 해당하는 적용 기준이 있는지 확인해 드릴 수 있습니다.
```
...
## Assistant Response
```text
‘frac’는 수식에서 특정 값을 “분자/분모” 형태의 분수(나눗셈)로 표기할지 여부를 정하는 개념입니다. 

- 분자와 분모로 구성된 비율(예: 듀레이션 계산식 중 D_CD처럼)로 나타내야 하는 부분에는 분수 표기로 사용합니다. 
- 문서에서는 ‘frac를 쓸지 말지’를 가르는 별도의 독립 규칙(조건/예외)은 따로 제시되어 있지 않고, 수식 구성상 분수 형태가 필요한 곳에 표기로 쓰는 방식입니다.
```
