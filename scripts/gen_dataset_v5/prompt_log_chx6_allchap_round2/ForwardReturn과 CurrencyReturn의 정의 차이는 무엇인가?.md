...
## Question
ForwardReturn과 CurrencyReturn의 정의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ForwardReturn과 CurrencyReturn의 정의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "ForwardReturn과 CurrencyReturn의 정의 차이는 무엇인가?",
...
## Assistant Response
```text
- ForwardReturn_t는 KRW/USD 1 Month Forward 환율(FXR가 아닌 FFR)인 FFR_0, FFR_t를 사용하고, 잔여일수 가중치인 (OddDay_t/T)도 함께 반영하는 값으로 정의됩니다.   
- CurrencyReturn_t는 FFR이 아니라 FXR_t^R, FXR_0^R의 비율(= FXR_t^R / FXR_0^R)로만 정의됩니다.   
- 즉, 정의 차이는 “Forward(Forward 환율·잔여일 가중)”을 쓰는가와 “Currency(환율지수 비율만)”을 쓰는가의 구분입니다.
```
...
## Assistant Response
```text
- 공통 맥락: ForwardReturn과 CurrencyReturn은 헤지수익(HReturn)을 만드는 구성요소로 함께 쓰이며, HReturn_t = ForwardReturn_t − CurrencyReturn_t 관계로 정의됩니다.   
- ForwardReturn: 전월말 KRW/USD 1 Month Forward 환율(FFR_0)을 기준으로, 해당 시점의 (FFR_t − FXR_t^R)을 잔여일수 비중(OddDay_t/T)만큼 반영해 산출합니다.   
- CurrencyReturn: 통화(환율) 관련 항은 FXR의 비율로만 산출되며, CurrencyReturn_t = FXR_t^R / FXR_0^R 입니다.
```
...
## Assistant Response
```text
- 두 지표는 KRW Hedged 지수 산출에서 Hedge Return이 HReturn_t = ForwardReturn_t - CurrencyReturn_t 로 분해될 때 함께 사용됩니다   
- ForwardReturn_t는 KRW/USD 1 Month Forward 환율(FFR_0, FFR_t)과 기준 환율(FXR^R, FXR_0^R, FXR_t^R) 간의 차이를 OddDay_t/T(잔여일수 가중)로 반영해 정의됩니다   
- CurrencyReturn_t는 통화 변동만을 FXR_t^R / FXR_0^R로 단순 비율로 정의됩니다
```
...
## Assistant Response
```text
- 공통 맥락: KRW Hedged 지수 산출에서 Hedge Return은 ForwardReturn과 CurrencyReturn의 차이로 정의됩니다(HReturn_t = ForwardReturn_t − CurrencyReturn_t).   
- ForwardReturn: 1 Month Forward 환율(FFR_0, FFR_t)과 잔여일수 비중(OddDay_t/T)을 함께 반영해 산출됩니다.   
- CurrencyReturn: FXR_t^R와 FXR_0^R의 비율로만 산출되어, forward 환율 및 OddDay_t/T 같은 항이 직접 포함되지 않습니다.
```
