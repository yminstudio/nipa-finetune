...
## Question
다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI WI와 MSCI EM
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI WI와 MSCI EM",
  "qa_type": "comparison",
  "question_template": "Comparison_Template_A5"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI WI와 MSCI EM",
...
## Assistant Response
```text
공통으로, 현재 산출 대상 Hedged 지수 목록에는 두 지수가 모두 포함됩니다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수에는 MSCI WI와 MSCI EM이 모두 포함됩니다.   
두 지수는 공통으로 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등)되는 방식으로 제시돼 있습니다.
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수에는 MSCI WI와 MSCI EM이 함께 포함됩니다. 
