...
## Question
현재 산출 대상 Hedged 지수의 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI(및 JACI IG)로 한정된다.   
- 위 지수들은 “임의로 부여한 헤지비율”에 따라 각각 산출한다(예: 50%, 100% 등; JACI/JACI IG는 예: 95%, 100% 등).
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는, KRW Hedged 지수를 만들기 위해 적용하는 “대상 지수”와 그 지수에 사용할 헤지비율(H_t) 산정 방식으로 정해집니다. 

- MSCI ACWI, MSCI WI, MSCI EM: 임의로 부여한 헤지비율에 따라(예: 50%, 100%) 각각 산출 대상에 포함   
- JACI, JACI IG: 임의로 부여한 헤지비율에 따라(예: 95%, 100%) 각각 산출 대상에 포함   
- Lehman Brother’s US Aggregate: 해당 지수는 헤지비율 H_t를 ‘전월 말일~해당 t일까지의 Yield to Worst’에 기반한 산식으로 결정
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 “현재 산출 대상”으로 삼는 Hedged 지수를 정하기 위한 기준은, 어떤 지수를 쓰고(대상 지수), 헤지비율을 어떤 방식으로 적용하느냐(헤지비율 방식)입니다. 

- MSCI ACWI: 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 각각 산출 
- MSCI WI, MSCI EM: 임의로 부여한 헤지비율(예: 50%, 100% 등)에 따라 각각 산출 
- Lehman Brother’s US Aggregate: 헤지비율 \(H_t\)를 \(H_t=\left(1+\frac{Yield_t/100}{2}\right)^{1/6}\)으로 산출 
- JACI, JACI IG: 임의로 부여한 헤지비율(예: 95%, 100% 등)에 따라 각각 산출
```
...
## Assistant Response
```text
Hedged 지수의 “산출 대상”을 정하기 위한 판단 기준은, 어떤 기초지수에 헤지비율을 적용해 KRW Hedged 지수를 만들 것인지에 있습니다. 

- MSCI ACWI / MSCI WI / MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100%)   
- JACI / JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100%)   
- Lehman Brother’s US Aggregate: 헤지비율 Ht를 Yield to Worst를 사용한 식으로 산출
```
