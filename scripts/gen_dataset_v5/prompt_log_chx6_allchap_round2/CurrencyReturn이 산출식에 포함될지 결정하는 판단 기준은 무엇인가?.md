...
## Question
CurrencyReturn이 산출식에 포함될지 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CurrencyReturn이 산출식에 포함될지 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "CurrencyReturn이 산출식에 포함될지 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- CurrencyReturn_t는 KRW Hedged 지수 산출에서 Hedge Return을 구하기 위한 $HReturn_t = ForwardReturn_t - CurrencyReturn_t$에 포함되는 값이므로, KRW Hedged 지수를 산출할 때 CurrencyReturn이 산출식에 포함됩니다.   
- 해외펀드에서 환헷지를 하지 않는 경우에는 원화환산 BM지수(= 해당 Local(or USD)지수 × (원화/로컬)환율)를 적용하므로, 해당 산출에서 CurrencyReturn_t 차감 항은 사용되지 않습니다.
```
...
## Assistant Response
```text
CurrencyReturn은 “헤지수익률(HReturn_t)을 어떤 구성요소로 정의해 계산하는지”를 판단할 때, 그 구성상 통화요인 항을 ForwardReturn_t에서 차감하는 형태라면 산출식에 포함됩니다. 

- KRW Hedged 지수 산출에서 HReturn_t는 ForwardReturn_t에서 CurrencyReturn_t를 차감하도록 정의됩니다( HReturn_t = ForwardReturn_t − CurrencyReturn_t ).   
- 이때 CurrencyReturn_t는 FXR_t^R / FXR_0^R 로 정의됩니다.
```
...
## Assistant Response
```text
- CurrencyReturn이 산출식에 포함되는지는 “헤지비용을 감안한 KRW Hedged 지수”의 일별 수익률을 계산할 때, HReturn을 만들기 위해 CurrencyReturn을 차감항으로 쓸지 여부를 판단하기 위한 기준입니다.   
- KRW Hedged 지수 산출에서는 HReturn_t = ForwardReturn_t - CurrencyReturn_t로 정의되므로 CurrencyReturn_t가 HReturn 계산에 포함됩니다.   
- 이때 CurrencyReturn_t는 CurrencyReturn_t = FXR_t^R / FXR_0^R로 정해져, HReturn에서 ForwardReturn에서 빼는 구성요소가 됩니다.   
- 반대로 환헷지를 하지 않는 경우에는 원화환산 BM지수처럼 “지수 × 환율” 방식으로 산출하므로, CurrencyReturn을 별도 차감항으로 쓰는 형태는 아닙니다.
```
...
## Assistant Response
```text
- CurrencyReturn을 산출식에 포함할지는 “환헷지 여부에 따라 적용하는 BM(원화환산 vs KRW Hedged)”를 구분하는 데서 결정됩니다. 
- 환헷지를 하는 경우(= KRW Hedged 지수 산출)에는 Hedge Return을 만들기 위해 HReturn_t가 ForwardReturn_t에서 CurrencyReturn_t를 차감하는 형태로 정의되어 CurrencyReturn이 포함됩니다. 
- 환헷지를 하지 않는 경우(= 원화환산 BM지수 산출)에는 Local(or USD)지수에 원화/로컬 환율을 곱해 환산하며, 해당 산식에는 CurrencyReturn 항이 나타나지 않습니다.
```
