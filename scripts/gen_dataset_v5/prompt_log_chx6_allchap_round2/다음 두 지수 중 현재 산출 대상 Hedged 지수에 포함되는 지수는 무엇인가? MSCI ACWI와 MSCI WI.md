...
## Question
다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI ACWI와 MSCI WI
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI ACWI와 MSCI WI",
  "qa_type": "comparison",
  "question_template": "Comparison_Template_A1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "다음 두 지수 중 현재 산출 대상 Hedged 지수에 포함되는 지수는 무엇인가? MSCI ACWI와 MSCI WI",
...
- MSCI WI: 현재 산출 대상 Hedged 지수에 포함됩니다.
```
...
## Assistant Response
```text
공통 맥락으로, 제로인의 “현재 산출 대상 Hedged 지수” 목록에는 두 지수 모두 포함됩니다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 헤지비율을 임의로 부여해 각각 산출하는 지수들을 의미하며, 그 범위에 두 지수 모두 포함됩니다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수 기준으로 보면, 두 지수 모두 포함됩니다. 
