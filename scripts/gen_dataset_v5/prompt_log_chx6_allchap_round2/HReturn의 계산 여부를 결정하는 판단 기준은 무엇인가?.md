...
## Question
HReturn의 계산 여부를 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HReturn의 계산 여부를 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HReturn의 계산 여부를 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 제로인에서 HReturn은 KRW Hedged 지수의 산출식에서 사용되는 경우(즉 HedgedIndexR_t에 포함될 때) 계산 대상으로 봅니다.   
- KRW Hedged에 “해당 지수가 제공되는지 여부”가 기준이 되며, 제공업체가 KRW Hedged 지수를 제공·판매하지 않는 경우에는 USD 지수를 이용해 자체적으로 KRW Hedged와 유사한 지수를 산출하는 프로세스를 정의하므로 이때 HReturn(ForwardReturn_t − CurrencyReturn_t)을 사용합니다.
```
...
## Assistant Response
```text
HReturn은 환헷지 지수 산출에서 헤지비용(환헤지 효과)을 반영하기 위해 계산해 포함할지/어떤 값으로 반영할지를 결정하는 항목입니다. 

- HReturn_t는 ForwardReturn_t에서 CurrencyReturn_t를 차감한 값으로 정의하며, 즉 HReturn_t = ForwardReturn_t − CurrencyReturn_t로 계산합니다. 
- 계산된 HReturn_t는 HedgedIndexR_t 산식에서 헤지비율 H_t와 함께 “H_t × HReturn_t” 항으로 반영됩니다. 
- 따라서 HedgedIndexR_t를 구성할 때 해당 헤지비율 항이 적용되는 경우에 HReturn_t의 계산값이 사용됩니다.
```
...
## Assistant Response
```text
HReturn은 KRW Hedged 지수를 산출할 때(산출식에서 HReturn_t = ForwardReturn_t - CurrencyReturn_t로 정의) 포함되는 구성요소를 말한다. 

- 해외펀드의 BM을 정할 때 “환헷지 여부”가 판단 기준이다. 
- 환헷지를 하는 경우에는 로컬지수를 적용한다. 
- 환헷지를 하지 않는 경우에는 원화환산 지수를 적용하되, 적극적으로 밝힌 경우만 인정한다.
```
...
## Assistant Response
```text
HReturn은 KRW Hedged 지수의 수익률을 계산할 때, 헤지(환헤지) 효과를 반영하기 위한 구성요소를 구하는지 여부를 판단합니다. 

- 헤지비율(H_t)을 반영해 HedgedIndexR_t에 “H_t × HReturn_t” 항이 포함되는 경우에 HReturn을 계산합니다.   
- HReturn_t는 ForwardReturn_t에서 CurrencyReturn_t를 차감해 산출하므로, HedgedIndexR_t 계산에서 이 차감이 필요한 경우에 HReturn을 계산합니다.
```
