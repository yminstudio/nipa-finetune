...
## Question
Lehman Brother’s 지수인 경우를 구분하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Lehman Brother’s 지수인 경우를 구분하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Lehman Brother’s 지수인 경우를 구분하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- Hedged 지수 산출 대상 지수가 “Lehman Brother’s US Aggregate”(Lehman Brother’s 지수)로 설정된 경우   
- 이 경우에 한해 Hedge 비율(H_t)을 Lehman Brother’s 지수용 식   
- 따라서 지수 제공업체/지수명이 Lehman Brother’s로 명시된 경우를 “Lehman Brother’s 지수인 경우”로 구분
```
...
## Assistant Response
```text
- 이는 KRW Hedged 지수 산출에서 ‘Lehman Brother’s 지수인 경우’로 분류해 H_t(헤지 관련 항)를 어떤 식으로 계산할지를 구분하기 위한 기준입니다.  
- 헤지드 지수(Hedged index)가 Lehman Brother’s US Aggregate로 설정(해당 지수를 사용)되는 경우에 “Lehman Brother’s 지수인 경우”로 구분합니다.   
- 이 경우 H_t는 \(H_t=\left(1+\frac{Yield_t/100}{2}\right)^{1/6}\)로 산출합니다.
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 헤지비용(헤지비율) 계산식이 어떤 지수 기준으로 적용되는지를 구분하기 위한 기준이다.

- 현재 산출 대상 Hedged 지수가 Lehman Brother’s US Aggregate로 잡히는 경우 
- 이 경우 헤지비율 H_t를 H_t = (1 + (Yield_t / 100)/2)^(1/6)로 계산한다
```
...
## Assistant Response
```text
Lehman Brother’s 지수 구분은 KRW Hedged 지수 산출 시 적용하는 USD Hedged 지수(헤지드 지수)의 지수 유형이 무엇인지에 따라 판단합니다. 

- 적용 대상 “Hedged 지수”가 Lehman Brother’s US Aggregate로 설정되는 경우   
- 이때 헤지비율(H_t)은 Yield to Worst(Yield_t)를 사용해 \(H_t=\left(1+\frac{Yield_t/100}{2}\right)^{1/6}\)로 산출합니다.
```
