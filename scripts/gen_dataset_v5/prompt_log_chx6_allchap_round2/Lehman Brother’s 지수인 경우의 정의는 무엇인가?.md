...
## Question
Lehman Brother’s 지수인 경우의 정의는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Lehman Brother’s 지수인 경우의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Lehman Brother’s 지수인 경우의 정의는 무엇인가?",
...
## Assistant Response
```text
- Lehman Brother’s 지수인 경우, 헤지비율 H_t는 다음과 같이 정의한다: H_t = (1 + (Yield_t/100)/2)^(1/6)   
- 여기서 Yield_t는 전월 말일의 Yield to Worst이다   
- H_t는 해당 t일의 헤지비율(Hedge Ratio)이다
```
...
## Assistant Response
```text
Lehman Brother’s 지수인 경우, KRW Hedged 지수 산출에서 헤지비용(수익률) 반영을 위한 계수 H_t를 다음처럼 정의합니다. 

- Lehman Brother’s 지수인 경우의 정의:  
...
## Assistant Response
```text
Lehman Brother’s 지수인 경우에는 헤지비율(H_t)을 다음과 같이 정의한다.   
- Lehman Brother’s 지수의 헤지비율: H_t = (1 + (Yield_t / 100) / 2)^(1/6)   
- 여기서 Yield_t는 전월 말일의 Yield to Worst 이다.
```
...
## Assistant Response
```text
Lehman Brother’s 지수인 경우, 헤지비율(H_t)은 다음과 같이 정의한다. 
