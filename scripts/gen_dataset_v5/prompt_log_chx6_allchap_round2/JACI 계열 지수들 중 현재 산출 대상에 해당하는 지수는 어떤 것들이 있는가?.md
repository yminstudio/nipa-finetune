...
## Question
JACI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "JACI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?",
  "qa_type": "application",
  "question_template": "Application_Template_A7"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "JACI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?",
...
## Assistant Response
```text
- JACI: 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100%)   
- JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100%)
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 “현재 산출 대상 Hedged 지수”로 포함되는 JACI 계열 지수는 아래와 같습니다. 

- 적용: JACI 계열을 현재 산출 대상 Hedged 지수 목록에서 확인한 뒤, 산출 대상 지수로 JACI, JACI IG를 선택합니다. 
- 적용 조건: 두 지수 모두 “임의로 부여한 헤지비율”에 따라 각각 산출됩니다(예: 95%, 100%). 
- 전환 규칙: 같은 지수라도 헤지비율이 달라지면 해당 헤지비율별로 별도로 산출합니다.
```
...
## Assistant Response
```text
JACI 계열 중 ‘현재 산출 대상 Hedged 지수’에 포함되는 것은 다음 2개이며, 적용은 부여된 헤지비율에 따라 각각 산출하는 방식으로 진행됩니다. 

- JACI  
  적용 순서: JACI를 포함한 항목 중 하나로서 ‘부여된 헤지비율’에 따라 산출   
  적용 조건: 임의로 부여한 헤지비율(예: 95%, 100% 등)에 따라 각각 산출   
  전환 규칙: 다른 지수로 넘어가기 전, JACI는 독립적으로 산출 

- JACI IG  
  적용 순서: JACI 이후, 동일한 방식으로 ‘부여된 헤지비율’에 따라 산출   
  적용 조건: 임의로 부여한 헤지비율(예: 95%, 100% 등)에 따라 각각 산출   
  전환 규칙: JACI와 별개로 각각 산출 

- 공통 적용(헤지비율 기반)  
  적용 조건: JACI 계열 내 해당 지수들은 임의로 부여된 헤지비율에 따라 ‘각각’ 산출
```
...
## Assistant Response
```text
- 적용 순서 1: “현재 산출 대상 Hedged 지수”에 해당하는 JACI 계열 중 JACI를 선택하고, 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100% 등)   
- 적용 순서 2: 동일한 기준으로 JACI IG도 선택하며, 임의로 부여한 헤지비율에 따라 각각 산출(예: 95%, 100% 등)   
- 적용 조건: JACI, JACI IG 산출 시에는 “임의의 비율” 케이스로 헤지비율 Ht를 상수로 적용(예: Ht=0.5, 0.9, 1 등)   
- 전환 규칙: 산출된 헤지비율을 KRW Hedged 지수 산식에 반영해 HedgedIndext = HedgedIndex0 × (1 + HedgedIndexRt)로 전환하며, HedgedIndexRt에서 Ht × HReturnt 항으로 반영
```
