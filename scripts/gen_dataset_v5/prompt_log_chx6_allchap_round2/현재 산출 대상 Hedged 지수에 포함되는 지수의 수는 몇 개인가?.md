...
## Question
현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 몇 개인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 몇 개인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A8"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 몇 개인가?",
...
## Assistant Response
```text
- 정의: 현재 산출 대상 Hedged 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG로 구성된 지수군을 의미한다   
- 따라서 포함되는 지수의 수는 총 6개이다
```
...
## Assistant Response
```text
- ‘현재 산출 대상 Hedged 지수’는 아래에 제시된 헤지드(환헤지) 지수들을 대상으로 한다: MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG   
- 따라서 현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 총 6개이다
```
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수는 KRW Hedged 지수 산출 시 적용하는 “현재 산출 대상 Hedged 지수” 항목들이다.   
- 해당 항목에 포함되는 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG로 총 6개이다.
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는, 헤지드 지수 산출 시 참고하는 “지수의 목록”을 말합니다. 해당 목록에 포함되는 지수는 총 6개입니다.
```
