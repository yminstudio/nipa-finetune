...
## Question
HedgedIndex와 HedgedIndexR의 정의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HedgedIndex와 HedgedIndexR의 정의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HedgedIndex와 HedgedIndexR의 정의 차이는 무엇인가?",
...
## Assistant Response
```text
- HedgedIndex(= HedgedIndex_t)는 해당 t일의 “최종 KRW Hedged Index” 값(지수 레벨)이며, 전월 말일의 HedgedIndex_0에 HedgedIndexR_t를 반영해 산출합니다.    
- HedgedIndexR(= HedgedIndexR_t)는 전월 말일 ~ 해당 t일까지의 “KRW Hedged 누적수익률”(수익률)입니다.   
- 즉, HedgedIndex는 지수 값, HedgedIndexR은 누적수익률로서 HedgedIndex를 (1+HedgedIndexR) 형태로 변환하는 역할에 해당합니다.
```
...
## Assistant Response
```text
KRW Hedged 지수를 산출할 때, HedgedIndex와 HedgedIndexR은 역할이 다릅니다. 

- HedgedIndex는 시점 t의 “지수 값(최종 KRW Hedged Index)”으로, HedgedIndex_0에서 시작해 HedgedIndexR_t에 의해 증감된 결과입니다. 
- HedgedIndexR은 시점 t까지의 “누적수익률(KRW Hedged 누적수익률)”로, HedgedIndex_t를 계산하기 위해 (1 + HedgedIndexR_t) 형태로 지수에 반영됩니다.
```
...
## Assistant Response
```text
- 두 정의는 KRW Hedged 지수 산출에서 “지수 값”과 “해당 지수의 누적수익률”을 나눠 표현한다는 공통 맥락을 가진다.   
- HedgedIndexR: 전월 말일~해당 t일까지의 KRW Hedged 누적수익률이다.   
- HedgedIndex: 해당 t일의 최종 KRW Hedged Index(지수 값)이다.   
- 관계식으로 보면 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 이므로, HedgedIndexR은 HedgedIndex를 만드는 입력(누적수익률)이고 HedgedIndex는 그 결과(지수 값)다.
```
...
## Assistant Response
```text
- 공통 맥락: 두 값은 KRW Hedged 지수 산출에서 함께 쓰이며, HedgedIndex는 지수 “수준”, HedgedIndexR는 그 지수의 “누적수익률(변화율)”에 해당합니다.   
- HedgedIndex는 “해당 t일의 최종 KRW Hedged Index(지수 값)”으로, 초기값 HedgedIndex_0에 HedgedIndexR의 반영 결과가 더해져 산출됩니다.   
- HedgedIndexR는 “전월 말일~해당 t일 까지의 KRW Hedged 누적수익률”로 정의되며, HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 관계로 HedgedIndex를 변화시키는 항입니다.
```
