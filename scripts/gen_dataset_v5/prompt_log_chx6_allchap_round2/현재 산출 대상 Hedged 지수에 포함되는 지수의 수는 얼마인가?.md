...
## Question
현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 얼마인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 얼마인가?",
  "qa_type": "application",
  "question_template": "Application_Template_A4"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 수는 얼마인가?",
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수에 포함되는 지수는 총 6개입니다: MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG
```
...
## Assistant Response
```text
- 적용 대상: 현재 산출 대상 Hedged 지수는 “현재 산출 대상 Hedged 지수”에 열거된 구성 지수를 집계한다   
- 적용 조건: 집계는 열거된 항목(MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG)을 그대로 산입한다   
- 적용 결과(포함되는 지수의 수): 총 6개   
- 전환 규칙: 별도 전환/대체 규칙은 해당 구성 항목의 개수 산정에 명시되어 있지 않으므로, 현재 열거된 항목 수(6개)를 사용한다
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수에 포함되는 지수는 총 6개입니다. 
...
```text
- 현재 산출 대상 Hedged 지수에 포함되는 지수는 총 6개입니다.   
- 포함 지수(카운트 기준)는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG 입니다.
```
