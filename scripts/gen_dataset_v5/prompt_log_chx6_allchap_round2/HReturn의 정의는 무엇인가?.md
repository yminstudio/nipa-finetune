...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HReturn의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- HReturn_t의 정의는 “HReturn_t = ForwardReturn_t - CurrencyReturn_t”이다.   
- 또한 HReturn_t는 “전월 말일 ~ 해당 t일 까지의 Hedge Return의 누적수익률”이다.
```
...
## Assistant Response
```text
- HReturn_t는 전월 말일 ~ 해당 t일 까지의 Hedge Return의 누적수익률로 정의된다.   
- 제로인에서는 HReturn_t를 “ForwardReturn_t - CurrencyReturn_t”로 산출한다.   
- 즉 HReturn_t = ForwardReturn_t(Forward 거래로 인한 수익) − CurrencyReturn_t(통화/환 변동으로 인한 수익)이다.
```
...
## Assistant Response
```text
- HReturn은 환헤지 성과 관련 항목으로, t시점까지의 Hedge Return 누적수익률을 뜻하며 , 정의는 다음과 같습니다.   
- HReturn_t = ForwardReturn_t − CurrencyReturn_t   
  여기서 CurrencyReturn_t = FXR_t^R / FXR_0^R 입니다.
```
...
## Assistant Response
```text
- HReturn은 헤지수익률(Hedge Return)을 누적한 수익률로, 전월 말일부터 해당 시점 t일까지의 Hedge Return 누적수익률을 뜻합니다.   
- 정의식은 HReturn_t = ForwardReturn_t - CurrencyReturn_t 입니다.
```
