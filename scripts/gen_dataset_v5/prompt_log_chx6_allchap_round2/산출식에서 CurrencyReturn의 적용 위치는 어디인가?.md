...
## Question
산출식에서 CurrencyReturn의 적용 위치는 어디인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "산출식에서 CurrencyReturn의 적용 위치는 어디인가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "산출식에서 CurrencyReturn의 적용 위치는 어디인가?",
...
## Assistant Response
```text
- CurrencyReturn_t는 HReturn_t를 계산할 때 적용되며, HReturn_t = ForwardReturn_t - CurrencyReturn_t 형태로 “ForwardReturn_t에서 차감”됩니다.   
- 산출된 HReturn_t는 HedgedIndexR_t에서 “헤지비율 H_t에 곱해지는 항”인 H_t × HReturn_t로 반영됩니다.   
- CurrencyReturn_t 자체는 FXR_t^R / FXR_0^R로 산출하여 위 차감에 사용됩니다.
```
...
## Assistant Response
```text
- KRW Hedged 지수 산출에서 CurrencyReturn_t는 먼저 단독 산출한다( FXR_t^R / FXR_0^R ).   
- 그 다음 단계에서 CurrencyReturn_t는 HReturn_t를 만들기 위한 전환(조정) 과정에 적용된다. 즉, HReturn_t = ForwardReturn_t − CurrencyReturn_t 로, ForwardReturn_t에서 차감하는 형태로 들어간다.   
- 이어서 HReturn_t는 HedgedIndexR_t 계산에서 헤지비율 H_t와 곱해져 반영된다( H_t × HReturn_t ).   
- 최종적으로 HedgedIndex_t는 (1 + HedgedIndexR_t)로 누적 계산되며, CurrencyReturn_t의 영향은 앞 단계들(차감→곱셈→HedgedIndexR_t 반영)을 거쳐 이 최종 수익률 항에 반영된다.
```
...
## Assistant Response
```text
- KRW Hedged 지수 산출 과정에서 CurrencyReturn은 먼저 통화환율 항(FXR)로부터 계산되어 CurrencyReturn_t = FXR_t^R / FXR_0^R 형태로 “HReturn” 구성요소가 되도록 준비됩니다.   
- 그 다음 CurrencyReturn_t는 HReturn_t의 분해식에서 ForwardReturn_t에서 “차감” 형태로 적용됩니다: HReturn_t = ForwardReturn_t − CurrencyReturn_t.   
- 이후 HReturn_t는 HedgedIndexR_t 산출식의 헤지비율 항에 들어가며, HedgedIndexR_t = ( … ) + H_t × HReturn_t 형태로 곱해져 반영됩니다.   
- 최종적으로 HedgedIndex_t는 HedgedIndex_0 × (1 + HedgedIndexR_t)로 산출되므로, CurrencyReturn의 영향은 HReturn_t를 통해 HedgedIndexR_t에 반영된 뒤 지수 증분으로 전환됩니다.
```
...
## Assistant Response
```text
- t일에 먼저 CurrencyReturn_t = FXR_t^R / FXR_0^R 로 산출한다.   
- 이후 Hedge Return(HReturn_t)은 ForwardReturn_t 에서 CurrencyReturn_t 를 차감하는 형태로 계산한다( HReturn_t = ForwardReturn_t - CurrencyReturn_t ).   
- HReturn_t는 HedgedIndexR_t 산출식에서 헤지비율 H_t와 곱해져 반영된다(… + H_t × HReturn_t …).   
- 최종적으로 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 업데이트할 때, CurrencyReturn의 영향은 위의 HReturn_t 경로를 통해 HedgedIndexR_t에 반영된 값으로 전환된다.
```
