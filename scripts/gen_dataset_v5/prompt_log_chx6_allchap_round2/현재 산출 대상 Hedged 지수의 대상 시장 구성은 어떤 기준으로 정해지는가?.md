...
## Question
현재 산출 대상 Hedged 지수의 대상 시장 구성은 어떤 기준으로 정해지는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 대상 시장 구성은 어떤 기준으로 정해지는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 대상 시장 구성은 어떤 기준으로 정해지는가?",
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수의 대상 시장 구성은 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG를 대상으로 정해진다.   
- MSCI ACWI, MSCI WI, MSCI EM, JACI, JACI IG는 임의로 부여한 헤지비율에 따라 각각 산출된다.
```
...
## Assistant Response
```text
KRW Hedged 지수의 산출 대상 시장 구성은 해외투자 성과측정 및 상대적 비교를 위한 벤치마크로 적용할 “해당 지수(시장 대표)”를 정하는 방식으로 결정된다. 

- 주식 시장은 MSCI ACWI, MSCI WI, MSCI EM을 대상으로 하며, 각 지수에 임의로 부여한 헤지비율에 따라 산출한다.   
- 채권 시장은 Lehman Brother’s US Aggregate를 대상으로 한다.   
- 추가로 JACI 및 JACI IG도 대상으로 하며, 마찬가지로 임의로 부여한 헤지비율에 따라 산출한다.
```
...
## Assistant Response
```text
KRW Hedged 지수의 성과를 상대적으로 비교하기 위한 “대상 시장”은, 헤지드 지수 산출에 사용되는 기초 시장지수를 정해 그 지수들에 해당한다. 

- 현재 산출 대상 Hedged 지수의 기초 시장 구성은 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG로 정해진다. 
- MSCI ACWI / MSCI WI / MSCI EM / JACI / JACI IG는 임의로 부여한 헤지비율에 따라 각각 산출되도록 구성된다(예: 50%, 95%, 100% 등). 
- Lehman Brother’s US Aggregate는 위 목록에 포함된 별도의 산출 대상 시장으로 설정된다.
```
...
## Assistant Response
```text
성과측정에 적용할 헤지드 벤치마크(“KRW Hedged 지수”)를 만들기 위해, 산출 대상 시장 구성은 USD지수 기반으로 정해집니다. 

- MSCI ACWI: 임의로 부여한 헤지비율에 따라 산출   
- MSCI WI: 임의로 부여한 헤지비율에 따라 산출(예: 50%, 100%)   
- MSCI EM: 임의로 부여한 헤지비율에 따라 산출(예: 50%, 100%)   
- Lehman Brother’s US Aggregate, JACI/JACI IG: 해당 지수들을 산출 대상으로 포함(예: JACI, JACI IG는 임의 헤지비율 적용)
```
