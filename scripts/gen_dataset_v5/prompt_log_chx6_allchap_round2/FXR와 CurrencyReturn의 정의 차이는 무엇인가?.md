...
## Question
FXR와 CurrencyReturn의 정의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "FXR와 CurrencyReturn의 정의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "FXR와 CurrencyReturn의 정의 차이는 무엇인가?",
...
## Assistant Response
```text
- FXR는 HedgedIndexR_t 및 ForwardReturn_t 산출식에서 FXR_t^R, FXR_0^R 형태로 직접 사용되는 입력 변수입니다.   
- CurrencyReturn은 FXR를 기준일 대비 비율로 정리한 값으로, CurrencyReturn_t = FXR_t^R / FXR_0^R 로 정의됩니다.   
- 따라서 FXR(원값)은 산식에 직접 들어가고, CurrencyReturn은 그 FXR의 상대적 변화를 나타내는 비율입니다.
```
...
## Assistant Response
```text
- 공통 맥락: KRW Hedged 지수 산출에서 FXR는 HedgedIndexR_t의 항과 ForwardReturn_t의 구성요소에 함께 사용되고, CurrencyReturn으로도 연결됩니다.   
- FXR: 산식에 그대로 등장하는 FXR_t^R(해당 t일 값)과 FXR_0^R(기준 시점 값)처럼 “FXR 값(레벨)” 자체를 의미합니다.   
- CurrencyReturn: FXR의 값을 수익률(변화율) 형태로 정규화한 값으로, CurrencyReturn_t = FXR_t^R / FXR_0^R 로 정의됩니다.
```
...
## Assistant Response
```text
- 공통 맥락: KRW Hedged 지수 산출에서 통화 관련 항으로 FXR_t^R(및 FXR_0^R)이 사용되고, 그로부터 CurrencyReturn_t가 정의됩니다 .  
- FXR: CurrencyReturn_t를 만들기 위한 입력으로서, 시점 t의 FXR_t^R와 기준시점의 FXR_0^R가 식에 직접 등장하는 값입니다 .  
- CurrencyReturn: FXR 값을 “수익률 형태”로 변환한 정의값으로, CurrencyReturn_t = FXR_t^R / FXR_0^R 입니다 .
```
...
## Assistant Response
```text
- HedgedIndex 산출에서 FXR는 “FXR_t^R” 형태로 쓰이며, “t일의 값”과 “기준일(전월 말일) 값” “FXR_0^R”가 같이 등장합니다.   
- CurrencyReturn은 별도의 값을 다시 정의해 “CurrencyReturn_t = FXR_t^R / FXR_0^R”로, FXR_t^R를 기준일 대비 비율(정규화된 수익률/변화율)로 만든 값입니다.   
- 즉, 같은 FXR_t^R를 쓰더라도 FXR은 산식에 투입되는 원천 값(기호)이고, CurrencyReturn은 그 원천 값을 FXR_0^R로 나누어 수익률 형태로 표현한 지표입니다.
```
