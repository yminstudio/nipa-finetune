...
## Question
산출식에 FXR이 반영될지 여부를 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "산출식에 FXR이 반영될지 여부를 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "산출식에 FXR이 반영될지 여부를 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드는 환헷지를 하는 경우 로컬지수를 적용하고, 환헷지를 하지 않는 경우 원화환산 지수를 적용하는지로 판단한다.   
- 환헷지는 투자설명서상 ‘필요한 경우 환헷지를 할 수 있다’는 경우 환헷지를 하는 경우로 인정한다.   
- ‘지금은 환헷지 수단이 없지만, 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다’는 경우에는 투자하는 지역의 통화·상황에 따라 판단하며(예: 인도는 현재 환헷지 불가, 러시아는 현재 환헷지 가능, 브라질은 ADR의 경우 USD와 환헷지 인정/Local은 환헷지 불가), 해당 판단 결과에 따라 FXR이 포함되는 산출식 적용 여부가 결정된다(참고로 FXR은 KRW Hedged 지수 산출식의 CurrencyReturn_t 등에 반영됨).
```
...
## Assistant Response
```text
산출식에 FXR이 반영될지는, 해당 펀드가 “환헷지를 하는 경우”로 보아 KRW Hedged 지수를 쓸지(이때 FXR이 사용됨) “환헷지를 하지 않는 경우”로 볼지(이때 FXR 없이 다른 BM을 씀)를 판단하기 위한 기준입니다. 

- 투자설명서에서 “필요한 경우 환헷지를 할 수 있다”처럼 환헷지를 할 수 있는 것으로 제시되면 환헷지를 하는 경우로 인정하여 FXR이 반영되는 산출( KRW Hedged 지수 )에 적용합니다.  
- “지금은 환헷지 수단이 없지만, 후에 환헷지 수단이 생길 수 있다”처럼 미래 가능성이 포함된 경우에는 투자 지역 및 상황에 따라 환헷지 가능 여부를 판단합니다(예: 인도는 환헷지 불가, 러시아는 환헷지 가능, 브라질 Local 투자면 환헷지 불가). 
- 환헷지를 하지 않는 것으로 판단되면 원화환산 BM지수를 적용하며(해당 Local(or USD)지수 × 원화/로컬 환율), 이 경우 FXR 반영 산출이 아니라서 FXR은 적용되지 않습니다.
```
...
## Assistant Response
```text
FXR이 산출식에 반영되는지의 판단 기준은, 해외펀드 BM 산출에서 해당 통화에 대해 “환헷지를 하는지/하지 않는지”를 구분하기 위한 것입니다. 

- 투자설명서에 “필요한 경우 환헷지를 할 수 있다”처럼 환헷지를 할 수 있다는 취지가 있으면, 환헷지를 하는 경우로 인정합니다. 
- “현재 환헷지 수단이 없지만 후에 생길 수 있다”는 문구는 투자하는 지역의 통화 및 상황에 따라 현재 환헷지 가능 여부로 판단하며, 예로 인도는 현재 환헷지 할 수 없음, 러시아는 현재 환헷지 할 수 있음으로 봅니다. 
- 환헷지를 하는 경우에는 로컬지수, 환헷지를 하지 않는 경우에는 원화환산 지수를 적용하며, 환헷지 분류에 따라 FXR(예: FXR_t^R/FXR_0^R)이 포함되는 KRW Hedged 지수 산출식이 적용됩니다.
```
...
## Assistant Response
```text
산출식에 FXR(환율 관련 항)이 반영되는지는, 원화환산/헤지 지수를 어떤 방식으로 적용하는지(환헷지에 해당하는지) 판단하기 위한 기준입니다. 

- 환헷지를 “하는 경우”로 인정되면 로컬지수를 적용하며, 이 경우 Hedged 지수 산출식의 FXR 항( FXR_t^R/FXR_0^R )이 쓰이지 않습니다.    
- 환헷지를 “하지 않는 경우”로 분류되면 원화환산 지수를 적용하며(이때 Hedged 지수 산출식에 FXR이 포함됨), 다만 원화환산 적용은 적극적으로 밝힌 경우만 인정합니다.    
- 환헷지 가능 여부는 투자설명서의 환헷지 관련 문구와 투자 지역의 통화/상황에 따라 판단합니다(예: ‘필요한 경우 환헷지를 할 수 있다’는 환헷지 하는 경우로 인정).
```
