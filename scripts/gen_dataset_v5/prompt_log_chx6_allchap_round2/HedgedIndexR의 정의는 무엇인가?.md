...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HedgedIndexR의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HedgedIndexR의 정의는 무엇인가?",
...
## Assistant Response
```text
- HedgedIndexR은 “전월 말일 ~ 해당 t일”의 KRW Hedged 누적수익률로, HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 를 만족하는 값이다.   
- HedgedIndexR_t = (I_t/I_0) × (FXR_t^R/FXR_0^R) − 1 + H_t × HReturn_t 이다.   
- 여기서 HReturn_t = ForwardReturn_t − CurrencyReturn_t 이다.
```
...
## Assistant Response
```text
HedgedIndexR는 KRW로 헤지된 지수의 누적수익률(전월 말일~해당 t일)로, HedgedIndex 산출식의 증가율 항으로 정의됩니다. 

- 정의(산출식): HedgedIndexR_t = (I_t/I_0) × (FXR_t^R/FXR_0^R) − 1 + H_t × HReturn_t   
- 또한 HReturn_t = ForwardReturn_t − CurrencyReturn_t 로 두며, ForwardReturn_t와 CurrencyReturn_t는 동일 문서의 세부 식으로 정의됩니다.
```
...
## Assistant Response
```text
- HedgedIndexR_t는 전월 말일의 KRW Hedged 지수에서 t일의 KRW Hedged 지수로 가는 누적수익률(전월 말일~t 구간)의 구성값이며, HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 정의됩니다.   
- 정의식: HedgedIndexR_t = (I_t / I_0) × (FXR_t^R / FXR_0^R) − 1 + H_t × HReturn_t.   
- 여기서 HReturn_t = ForwardReturn_t − CurrencyReturn_t 입니다.
```
...
## Assistant Response
```text
- HedgedIndexR_t는 전월 말일 ~ 해당 t일 까지의 KRW Hedged 누적수익률을 의미한다.   
- 산출식은 다음과 같다:  
  HedgedIndexR_t = (I_t / I_0) × (FXR_t^R / FXR_0^R) − 1 + H_t × HReturn_t   
- 여기서 HReturn_t는 ForwardReturn_t − CurrencyReturn_t로 정의한다.
```
