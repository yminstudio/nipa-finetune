...
## Question
산출식에서 HedgedIndex의 적용 여부를 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "산출식에서 HedgedIndex의 적용 여부를 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "산출식에서 HedgedIndex의 적용 여부를 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드의 환헷지 구분은 “환헷지를 하는 경우”와 “환헷지를 하지 않는 경우”로 나누며, 환헷지를 하는 경우 로컬지수를 적용하고 환헷지를 하지 않는 경우 원화환산 지수를 적용한다(단, 원화환산 지수 적용은 적극적으로 밝힌 경우만 인정).   
- 투자설명서에 “필요한 경우 환헷지를 할 수 있다”라고 기재된 경우에는 환헷지를 하는 경우로 인정한다.   
- “현재는 환헷지 수단이 없지만, 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다”는 내용은 투자하는 지역의 통화 및 상황에 따라 판단한다.
```
...
## Assistant Response
```text
HedgedIndex를 산출식에 반영할지의 판단 기준은, 해당 해외펀드가 환헷지를 “하는지/하지 않는지”를 어떻게 인정할지입니다. 

- 환헷지를 하는 것으로 인정되면(환헷지 수행) HedgedIndex가 적용됩니다.   
- 환헷지를 하지 않는 것으로 인정되면(환헷지 미수행) HedgedIndex는 적용하지 않고 원화환산 지수를 적용합니다(단, 적극적으로 밝힌 경우만 인정).   
- 투자설명서 문구로 환헷지 여부를 인정하며, 예를 들어 “필요한 경우 환헷지를 할 수 있다”는 환헷지를 하는 경우로 인정합니다. 또 “지금은 수단이 없지만 후에 수단이 생기면 환헷지를 할 수 있다”는 투자 지역의 통화·상황에 따라 판단합니다(인도: 현재 환헷지 불가, 러시아: 현재 환헷지 가능, 브라질: ADR이면 USD와 환헷지 인정, 브라질 Local이면 환헷지 불가).
```
...
## Assistant Response
```text
HedgedIndex 적용 여부는 해외투자 펀드의 BM에서 환헷지 효과를 반영할지 여부를 판단하기 위한 기준이다. 

- 환헷지를 하는 것으로 판단되면(환헷지를 하는 경우) HedgedIndex 적용 대상이 되고, 환헷지를 하지 않는 것으로 판단되면 적용하지 않는다(환헷지를 하지 않는 경우에는 원화환산 지수 적용). 
- 이때 “환헷지 여부”는 투자설명서 문구로 인정하며, 예를 들어 ‘필요한 경우 환헷지를 할 수 있다’는 환헷지를 하는 경우로 인정한다. 
- ‘현재는 환헷지 수단이 없지만 후에 수단이 생기면 할 수 있다’처럼 문구가 있는 경우에는 투자지역의 통화 및 상황에 따라 현재 환헷지 가능 여부로 판단한다(인도: 현재 환헷지 불가, 러시아: 현재 환헷지 가능, 브라질: ADR에 주로 투자하면 USD 환헷지로 인정하나 브라질 Local에 투자하면 환헷지 불가).
```
...
## Assistant Response
```text
HedgedIndex 적용 여부는 해외펀드 벤치마크를 환헷지 반영 방식(환헷지 시 로컬지수, 환헷지 미반영 시 원화환산 지수)으로 정하기 위한 판단 기준이다. 

- 투자설명서상 환헷지가 가능한 것으로 명시되어 “환헷지를 하는 경우”로 인정되는지(예: ‘필요한 경우 환헷지를 할 수 있다’). 
- 환헷지 수단이 “현재는 없지만, 후에 생기는 경우 환헷지를 할 수 있다”처럼 향후 가능으로만 적혀 있는 경우에는, 투자하는 지역의 통화 및 상황에 따라 판단하는지. 
- 지역별 판단 예: 인도는 현재 환헷지 불가(비적용), 러시아는 현재 환헷지 가능(적용), 브라질은 ADR에 주로 투자하면 USD와 환헷지로 인정(적용), 브라질 Local 투자이면 환헷지 불가(비적용).
```
