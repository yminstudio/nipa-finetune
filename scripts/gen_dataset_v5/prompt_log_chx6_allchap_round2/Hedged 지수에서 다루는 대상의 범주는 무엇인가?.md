...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Hedged 지수에서 다루는 대상의 범주는 무엇인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A3"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "Hedged 지수에서 다루는 대상의 범주는 무엇인가?",
...
## Assistant Response
```text
Hedged 지수에서 다루는 대상의 범주는 “현재 산출 대상 Hedged 지수”로, KRW Hedged와 유사한 지수를 산출하기 위해 정해진 지수군이다. 
...
## Assistant Response
```text
Hedged 지수는 KRW 헤지 효과를 반영해 산출하는 지수로, “현재 산출 대상 Hedged 지수”에 해당하는 벤치마크를 대상으로 합니다. 

- MSCI 계열 지수(MSCI ACWI, MSCI WI, MSCI EM) 
- Lehman Brother’s US Aggregate 
- JACI 계열 지수(JACI, JACI IG)
```
...
## Assistant Response
```text
제로인 방법론에서 Hedged 지수는 환헤지 비용/효과를 반영해 산출되는 “현재 산출 대상” 기준 지수의 범주를 의미합니다. 

- MSCI ACWI
- MSCI WI
- MSCI EM
- Lehman Brother’s US Aggregate, JACI 및 JACI IG
```
...
## Assistant Response
```text
Hedged 지수( KRW Hedged 지수 )는 해외투자 성과측정에 적용되는 벤치마크로, 제로인 방법론에서 산출 대상으로 두는 지수들이 그 “대상 범주”입니다. 
