...
## Question
금융 용어로 FXR과 CurrencyReturn이 서로 다른 역할을 하는 이유는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "금융 용어로 FXR과 CurrencyReturn이 서로 다른 역할을 하는 이유는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "금융 용어로 FXR과 CurrencyReturn이 서로 다른 역할을 하는 이유는 무엇인가?",
...
## Assistant Response
```text
- FXR(예: FXR_t^R)은 헤지드 지수 산출에서 통화 관련 항을 만드는 “입력 요소”로 쓰이며, HedgedIndexR_t와 ForwardReturn_t 계산에 직접 포함됩니다.   
- CurrencyReturn은 FXR의 비율( CurrencyReturn_t = FXR_t^R / FXR_0^R )로 정의돼, “통화로 인한 수익률 성분”으로 취급됩니다.   
- 그래서 HReturn_t에서 ForwardReturn_t에서 CurrencyReturn_t를 빼서( HReturn_t = ForwardReturn_t - CurrencyReturn_t ) 통화 성분을 분리하려는 역할 차이가 생깁니다.
```
...
## Assistant Response
```text
- FXR은 헤지드 지수 산출에서 사용하는 환율 관련 입력값으로, HedgedIndexR의 구성 항들(ForwardReturn 계산과 CurrencyReturn 계산)에서 기반이 되는 변수로 쓰입니다 .  
- CurrencyReturn은 그 FXR을 기준일 대비 비율로 “환율(통화) 자체의 수익률 성분”으로 정리해 만든 파생 수익률 지표로 정의됩니다( CurrencyReturn_t = FXR_t^R / FXR_0^R ) .  
- 그래서 역할이 갈립니다: Hedged 지수의 Hedge Return(HReturn_t)은 ForwardReturn에서 CurrencyReturn을 빼서( HReturn_t = ForwardReturn_t - CurrencyReturn_t ) ForwardReturn 안에 포함된 통화 성분을 분리/제거하는 데 CurrencyReturn이 직접 사용되고, FXR은 그 분리의 재료(기초값)로 기능합니다 .
```
...
## Assistant Response
```text
- 두 개념은 KRW Hedged 지수의 수익률을 환 관련 항으로 분해하는 공통 맥락에서 쓰이는데, Hedge Return을 만들기 위해 ForwardReturn에서 특정 구성요소를 떼어내는 구조입니다 .  
- FXR은 HedgedIndexR_t에서 환 변동분을 형성하는 “입력 항”으로 등장하며, (FXR_t^R/FXR_0^R) 형태로 지수 수익률 계산에 직접 반영됩니다 .  
- CurrencyReturn은 FXR을 이용해 “그 환 관련 입력 항을 수익률 항으로 정리한 결과”로, CurrencyReturn_t = FXR_t^R/FXR_0^R로 정의된 뒤 HReturn_t = ForwardReturn_t - CurrencyReturn_t에서 차감되는 역할을 합니다 .
```
...
## Assistant Response
```text
- 공통 맥락은 KRW Hedged 지수 산출에서 FX 관련 항을 분해해 쓰는 것입니다.   
- FXR은 HedgedIndexR_t 식에서 직접 쓰이는 기초 입력(환율 레벨/지수)이고, 여기서는 FXR_t^R/FXR_0^R 형태로 나타나 통화 변화 성분을 구성합니다.   
- CurrencyReturn은 FXR로부터 “수익률” 형태(정규화된 비율)로 계산된 결과값이며, ForwardReturn_t에서 CurrencyReturn_t를 빼서 HReturn_t(헤지 리턴)를 구하는 분해에 사용됩니다.
```
