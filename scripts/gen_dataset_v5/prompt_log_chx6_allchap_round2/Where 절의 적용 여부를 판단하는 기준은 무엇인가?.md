...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Where 절의 적용 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Where 절의 적용 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 수식에 사용된 기호/변수(예: ZCI_BM_i, ω_j, BM_r_j,i 등)의 의미를 명확히 정의하기 위해, 수식 뒤에 Where 절을 둔다   
- 또 다른 수식에서도 Where 절이 CE_{i,j}·R_{i,j}·σ_{i,j}·λ_{i,j}처럼 수식 구성요소의 변수 정의를 제공하는 형태로 사용된다
```
...
## Assistant Response
```text
Where 절은 수식에서 사용한 변수(기호)가 무엇을 뜻하는지 “정의”하기 위한 구문이다.   
- 해당 수식에 변수(예: r_{i,F}, J_{i-1}, J_{i-2})가 존재하고, 그 의미를 문장으로 풀어 제시해야 하는 경우 Where 절을 둔다.   
- 다른 수식에서도 분모·분자 또는 표기된 기호들(NAV, D_F, P_i, D_i 등)에 대해 Where 절로 각각의 정의를 나열할 때 적용한다.   
- 즉, “변수 정의가 필요한 수식”에 Where 절이 붙는 형태로 적용된다.
```
...
## Assistant Response
```text
Where 절은 펀드평가 방법론에서 “수식에 사용된 변수(기호)가 무엇을 의미하는지”를 정의하기 위한 목적이다. 

- 수식에 등장하는 항목(예: TR_t^B, S_{t-1,i}, S_{t,i} 등)의 의미를 풀어 쓰기 위해 Where 절을 둔다. 
- 따라서 해당 수식에 사용된 변수들에 대한 정의가 필요한 경우에 Where 절이 해당 산식 바로 아래에 적용된다.
```
...
## Assistant Response
```text
Where 절은 수식에서 사용된 기호(변수)의 의미를 정의하기 위한 부분임. 

- Where 절은 해당 수식에 등장한 변수(예: r_{i,F}, J_{i-1}, J_{i-2} 등)의 정의를 제시하는 경우에 적용됨 
- 같은 원리로 순유출입금액 수식에서도 M_t, S_t, P_{t-1} 등 기호가 나온 뒤 Where로 각 변수를 정의함 
- 배당시가배당률 수식에서도 S_i, ω_i 같은 기호를 Where로 정의한 형태로 적용됨
```
