...
## Question
Lehman Brother’s 지수인 경우의 산출식 적용은 일반 경우와 어떻게 다른가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Lehman Brother’s 지수인 경우의 산출식 적용은 일반 경우와 어떻게 다른가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Lehman Brother’s 지수인 경우의 산출식 적용은 일반 경우와 어떻게 다른가?",
...
## Assistant Response
```text
- 공통 적용(조건): KRW Hedged 지수는 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)에서 시작하며, HedgedIndexR_t = (I_t/I_0) × (FXR_t^R/FXR_0^R) − 1 + H_t × HReturn_t, HReturn_t = ForwardReturn_t − CurrencyReturn_t 구조를 그대로 사용합니다   
- 일반 경우(전환 규칙/조건): 헤지비율 H_t를 “임의의 비율”로 상수로 두는 경우에는 H_t를 0.5, 0.9, 1 등처럼 고정값으로 적용합니다(해당 상수 적용은 특정 Hedged 지수 산출에서 사용)   
- Lehman Brother’s 지수인 경우(전환 규칙/조건): Lehman Brother’s 지수인 경우에는 H_t를 상수로 두지 않고, Yield_t(%)를 이용해 H_t = (1 + (Yield_t/100)/2)^(1/6)로 산출해 앞의 HedgedIndexR_t에서 그대로 사용합니다
```
...
## Assistant Response
```text
- 적용 대상/맥락: KRW Hedged 지수 산출에서 핵심 입력은 헤지비율 H_t이며, H_t 값에 따라 최종 산출식(HedgedIndex_t 산출)이 달라집니다. 

- 적용 순서(공통): (1) 해당 t일의 HedgedIndexR_t를 구성 (2) HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 갱신   
  (여기서 HedgedIndexR_t 안에 “+ H_t × HReturn_t” 항이 포함됩니다.) 

- 적용 조건(일반 경우): BM을 “Lehman Brother’s 지수”로 보지 않는 경우, H_t는 임의 비율을 상수로 적용합니다(예: H_t = 0.5, 0.9, 1 등). 

- 전환 규칙(Lehman Brother’s 지수인 경우): BM이 “Lehman Brother’s 지수”인 경우, H_t를 상수로 두지 않고 수식으로 산출합니다.  
  H_t = (1 + (Yield_t / 100) / 2)^(1/6)
```
...
## Assistant Response
```text
- 적용 순서(공통): KRW Hedged 지수는 H_t가 결정된 후 HedgedIndexR_t에 반영해 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 산출합니다.   
- 적용 조건(일반 경우): Lehman Brother’s 지수가 아닌 경우에는 Hedge Ratio H_t를 “임의의 비율”로 상수(예: 0.5, 0.9, 1 등)로 두고, 이를 HedgedIndexR_t의 H_t × HReturn_t 항에 그대로 적용합니다.   
- 적용 조건(Lehman Brother’s 지수인 경우): H_t는 Yield_t(전월 말~해당 t일 기준의 Yield to Worst)를 이용해 H_t = (1 + (Yield_t/100)/2)^(1/6) 로 계산한 값을 사용합니다.   
- 전환 규칙: 벤치마크가 “Lehman Brother’s US Aggregate”로 분류되는 경우에만 H_t를 Yield 기반으로 전환하고, 그 외 지수(예: MSCI ACWI, JACI, JACI IG 등)에서는 상수 비율 기반으로 유지합니다.
```
...
## Assistant Response
```text
- 적용 조건: 환헤지(BM) 산출 중 KRW Hedged 지수 산출에서 헤지비율 H_t를 정하는 단계에서, 산출 대상이 Lehman Brother’s 지수인 경우에는 아래 전환 규칙을 적용합니다.   
- 적용 순서(차이 발생 구간): (1) H_t 산출 → (2) HReturn_t(헤지 리턴 누적수익률) 산출 → (3) HedgedIndexR_t(헤지 적용 누적수익률) 반영 → (4) HedgedIndex_t를 산출합니다.   
- 전환 규칙(Lehman Brother’s): H_t는 “Yield_t(전월 말일~해당 t일의 Yield to Worst)”로부터  
  H_t = (1 + (Yield_t/100)/2)^(1/6) 로 계산해 사용합니다.   
- 일반 경우의 적용 방식: Lehman Brother’s가 아닌 지수는 H_t를 “임의의 비율”로 상수 처리(예: H_t = 0.5, 0.9, 1)하여 사용합니다.
```
