...
## Question
현재 산출 대상 Hedged 지수의 헤지 방식 여부를 결정하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 헤지 방식 여부를 결정하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 헤지 방식 여부를 결정하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 투자설명서상 ‘필요한 경우 환헷지를 할 수 있다’면 환헷지를 하는 경우로 인정   
- ‘지금은 환헷지 수단이 없지만, 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다’면 투자하는 지역/통화 및 상황에 따라 판단(인도=현재 환헷지 불가, 러시아=현재 환헷지 가능, 브라질=ADR이면 USD와 환헷지 인정, 브라질 Local이면 환헷지 불가)   
- 환헷지 여부에 따라, 환헷지를 하는 경우 로컬지수 적용, 환헷지를 하지 않는 경우 원화환산 지수 적용(단, 적극적으로 밝힌 경우만 인정)
```
...
## Assistant Response
```text
이 기준은 “KRW Hedged 지수”를 어떤 헤지 방식(즉, 지수 기반을 무엇으로 둘지)으로 산출할지를 판단하기 위한 것이다. 

- 환에 대한 노출을 피하고 100% 환헤지를 할 수 있는 전제라면, Local 지수를 BM으로 적용하는 것이 타당하다고 본다. 
- 하지만 현실적으로는 환헤지 수단 부재, 헤지 비용(또는 Premium) 발생, 투자자별 헤지비율 차이 때문에 100% 전제가 그대로 적용되기 어렵다고 판단한다. 
- 이에 따라 현재 산출 대상 Hedged 지수는 “임의로 부여한 헤지비율”에 따라 각각 산출하는 방식으로 정리된다.
```
...
## Assistant Response
```text
헤지 방식 여부는 ‘현재 산출 대상 Hedged 지수’가 어떤 지수인지에 따라 헤지비율 H_t를 어떤 방식으로 정할지 판단하기 위한 기준입니다. 

- MSCI ACWI, MSCI WI, MSCI EM 및 JACI/JACI IG 산출 시: 임의로 부여한 헤지비율을 H_t로 적용(예: H_t=0.5, 0.9, 1) 
- Lehman Brother’s US Aggregate 산출 시: H_t를 Yield_t /100을 이용한 정해진 수식으로 계산 
- 즉, 산출 대상 지수 유형이 곧 헤지비율 산정(헤지 방식) 선택의 기준입니다.
```
...
## Assistant Response
```text
해당 Hedged 지수에서 헤지비율(H_t)을 어떤 방식으로 정할지 여부를 판단하기 위한 기준은 “현재 산출 대상 Hedged 지수의 종류(지수명)”입니다. 

- MSCI ACWI/WI/EM 및 JACI, JACI IG가 산출 대상인 경우: 임의로 부여한 헤지비율을 적용(예: 50%, 100%, 95% 등)하며, H_t를 상수로 설정합니다.  
- Lehman Brother’s US Aggregate가 산출 대상인 경우: Yield to Worst(Yield_t)를 이용해 H_t를 계산합니다.
```
