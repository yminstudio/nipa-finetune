...
```json
{
  ...
  "qa_type": "definition",
  "question_template": "Definition_Template_A1"
}
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 임의로 부여한 헤지비율에 따라 각각 산출하는 대상 지수(들)입니다. 

- MSCI ACWI, MSCI WI, MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등)   
  (MSCI WI, MSCI EM은 공무원연금 지수 제공목적으로 생성 추가) 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 KRW Hedged 지수를 산출할 때 “대상으로 삼는” 헤지 지수들로, 각 지수에 임의의 헤지비율(예: 50%, 100% 등)을 적용해 산출하는 것을 뜻합니다. 

- MSCI ACWI: 임의 헤지비율에 따라 산출   
- MSCI WI, MSCI EM: 임의 헤지비율에 따라 산출   
- Lehman Brother’s US Aggregate, JACI, JACI IG: (각각) 임의 헤지비율에 따라 산출(예: 95%, 100% 등)
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 KRW Hedged 지수 산출을 위해, 아래 지수들을 “임의로 부여한 헤지비율에 따라 각각 산출”하는 것으로 정의됩니다. 

- MSCI ACWI: 임의로 부여한 헤지비율에 따라 각각 산출   
- MSCI WI, MSCI EM: 임의로 부여한 헤지비율에 따라 각각 산출   
- JACI, JACI IG: 임의로 부여한 헤지비율에 따라 각각 산출   
- Lehman Brother’s US Aggregate: Hedged 지수 산출 대상
```
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI 및 JACI IG를 기반으로 하되, 각각에 “임의로 부여한 헤지비율(예: 50%, 100% 등)”을 적용해 산출하는 Hedged 지수를 의미합니다 .  
- Hedged 지수는 시점 t의 값이 `HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)`로 정의됩니다 .  
- 여기서 `HedgedIndexR_t`는 `I_t/I_0 × (FXR_t^R/FXR_0^R) - 1 + H_t × HReturn_t`, `HReturn_t = ForwardReturn_t - CurrencyReturn_t`로 정의됩니다 .
```
