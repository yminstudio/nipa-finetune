...
## Question
현재 산출 대상 Hedged 지수의 산출에 사용되는 기본 지수의 선정 원칙은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 산출에 사용되는 기본 지수의 선정 원칙은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 산출에 사용되는 기본 지수의 선정 원칙은 무엇인가?",
...
## Assistant Response
```text
- KRW Hedged 지수는 개별 제공업체의 KRW Hedged 지수(선정 후 구입/맞춤요청 포함)가 “제공되지 않는 경우”가 많고 비용이 큰 점을 이유로, 그 대신 일반적으로 공개되는 USD지수를 기본 지수로 사용한다.   
- 산출에 사용되는 기본 지수는 USD지수(It)를 두고, 전월 말일의 USD지수(I0)를 기준으로 HedgedIndex의 변화를 산출한다.   
- 현재 산출 대상 Hedged 지수(예: MSCI ACWI/WI/EM, Lehman Brother’s US Aggregate, JACI/JACI IG)는 이 기본 USD지수를 바탕으로 헤지비율에 따라(임의로 부여된 헤지비율) 각각 산출한다.
```
...
## Assistant Response
```text
KRW Hedged 지수의 산출을 위해 기본 지수를 고르는 기준은 “환헤지 성과를 벤치마크로 적절히 반영할 수 있느냐(가용성·비용 및 헤지비용 반영)”를 판단하기 위한 것입니다. 

- 제공업체가 KRW Hedged 지수를 산출·제공하더라도 해당 지수가 제한적이고, 산출/구입에 드는 비용이 적지 않다는 점을 감안합니다.   
- 또한 지수 산출에서 편의상 100% 헤지비율을 적용하는 경우가 일반적이어서, 투자자별 원하는 헤지비율을 반영하려면 Customized 요청이 필요하고 이 역시 비용이 크게 발생할 수 있다는 점을 감안합니다.   
- 이에 대한 대안으로 “일반적으로 공개되는 USD지수”를 기본 지수로 선정해, 이를 바탕으로 자체적으로 KRW Hedged와 유사한 지수를 산출합니다.
```
...
## Assistant Response
```text
헤지드 지수 산출에서 “기본 지수” 선정은 KRW 헤지 성과를 기준으로 성과측정이 가능하도록 하기 위한 것입니다. 

- KRW Hedged 지수를 BM으로 쓰는 것이 적절하더라도, 지수 산출·제공업체가 일부 KRW Hedged 지수만 산출/제공하고 있어 활용 범위가 제한되며, 설령 산출·구입하더라도 비용이 적지 않다고 봅니다.   
- 이러한 제약을 피하기 위해, 일반적으로 공개되는 USD 지수를 기본으로 하여 Forward 거래를 통한 헤지비용을 감안해 KRW Hedged와 유사한 지수를 자체 산출하는 방식으로 기본 지수를 정합니다.   
- 이에 따라 현재 산출 대상 Hedged 지수의 기본 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI 및 JACI IG로 설정합니다.
```
...
## Assistant Response
```text
KRW Hedged 지수를 BM으로 적용해 성과를 비교·측정하기 위해, 그 산출에 쓰일 기본 지수를 “합당한 BM” 관점에서 정한다. 

- 환에 대한 노출을 피하고 100% 환헤지를 한다면 Local지수를 BM으로 적용하는 것이 타당하지만, 통화별 환헤지 수단 등의 부재로 현실적으로 어렵다. 또한 Local 지수는 환헤지에 따른 비용(또는 Premium)을 반영하지 못하고, 헤지 비율도 투자자마다 다르다고 본다.   
- 그래서 일반적으로 공개되는 USD지수를 기본지수로 사용하여, Forward 거래를 통한 헤지비용을 감안한 “KRW Hedged와 유사한 지수”를 자체 산출하는 방식을 택한다.   
- 이에 따라 현재 산출 대상 Hedged 지수는 MSCI ACWI/WI/EM, Lehman Brother’s US Aggregate, JACI(JACI IG) 등이며, 임의로 부여한 헤지비율에 따라 각각 산출한다.
```
