...
## Question
현재 산출 대상 Hedged 지수에 포함되는 지수의 목록은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 목록은 무엇인가?",
  "qa_type": "application",
  "question_template": "Application_Template_A1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수에 포함되는 지수의 목록은 무엇인가?",
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 Hedged 지수 산출에 적용되는 BM/지수로서, 아래 지수들이 포함됩니다.   
...
## Assistant Response
```text
제로인 방법론에서 “현재 산출 대상 Hedged 지수”는 헤지비율을 적용해 산출하는 대상 지수로, 아래 지수들이 포함됩니다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수(= KRW Hedged 지수 산출에 사용)는 임의로 부여한 헤지비율에 따라 해당 지수를 각각 산출하는 “산출 대상 지수” 목록을 의미합니다.   

해당 지수는 MSCI ACWI, MSCI WI, MSCI EM, Lehman Brother’s US Aggregate, JACI, JACI IG 입니다.
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수에 포함되는 지수는 다음과 같다. 
