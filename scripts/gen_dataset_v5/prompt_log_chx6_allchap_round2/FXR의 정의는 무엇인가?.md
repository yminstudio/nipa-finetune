...
```json
{
  ...
  "target_total": 100,
  "generated_question": "FXR의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- FXR_t^R는 KRW Hedged 지수 산출에서 환율에 따른 항을 반영하기 위해 사용되는 값이며, CurrencyReturn_t = FXR_t^R / FXR_0^R로 정의된다   
- 이 CurrencyReturn_t는 HReturn_t = ForwardReturn_t − CurrencyReturn_t에 포함되어 최종 헤지 수익률에 반영된다
```
...
## Assistant Response
```text
FXR는 KRW Hedged 지수 산출에서 통화(환율) 성과를 반영하기 위해 사용하는 값이며, 통화수익률은 FXR의 기준값 대비 변화로 계산한다. 
...
## Assistant Response
```text
- FXR_t^R는 KRW Hedged 지수 산출에서 통화(커런시) 수익률을 계산하기 위해 사용하는 환율 변수이며, CurrencyReturn_t를 FXR_t^R / FXR_0^R로 정의합니다.   
- 또한 HedgedIndexR_t 식에서 I_t / I_0 × (FXR_t^R / FXR_0^R) 항으로 포함되어, 환헤지 수익률 구성요소의 일부로 사용됩니다.   
- 따라서 FXR_t^R는 해당 시점(t)의 통화 관련 환율 값, FXR_0^R는 기준 시점의 통화 관련 환율 값으로 쓰입니다.
```
...
## Assistant Response
```text
- 제로인 방법론에서 FXR는 환헷지 지수 산출에 사용되는 값으로, CurrencyReturn_t를 FXR로 정의합니다. 즉 CurrencyReturn_t = FXR_t^R / FXR_0^R 입니다.   
- 또한 ForwardReturn_t 산출식에도 FXR_0^R, FXR_t^R가 함께 포함됩니다.
```
