...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Where의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- “Where”는 수식에서 사용된 기호(변수)가 무엇을 의미하는지 “정의”를 나타내는 표기이며, 이후에 각 항목의 의미를 나열합니다.   
- 예를 들어 복합지수 수익률 산식에서 “Where” 다음에 \(ZCI\_BM_i\), \(\omega_j\), \(BM\_r_{j,i}\)의 정의가 제시됩니다.
```
...
## Assistant Response
```text
Where는 수식에 사용된 기호(항목)들의 의미를 “정의(약어/변수 설명)”로 제시하는 표기입니다.   
제로인 방법론에서는 예를 들어 복합지수 1일 수익률 수식에서 Where 뒤에 ZCI_BM_i, ω_j, BM_r_j,i의 정의를 둡니다.   
또한 HedgedIndexR_t, HReturn_t, ForwardReturn_t 등과 같이 수식 구성요소들도 Where 뒤에서 정의됩니다.
```
...
## Assistant Response
```text
펀드평가 방법론의 수식에서 “Where”는 앞의 수식에 사용된 기호(변수)들의 의미를 적어두는 구간을 뜻합니다.   
즉, “Where” 다음에 이어지는 항목들은 각 변수(예: $I_0$, $I_t$, $H_t$ 등)의 정의입니다.   
또한 “즉, …”로 설명한 뒤에 “Where”로 해당 설명과 연관된 변수 정의를 함께 제시하는 형태로 사용됩니다.
```
...
## Assistant Response
```text
- “Where”는 수식에서 사용된 기호(변수)들의 의미를 이어서 정의(설명)하기 위해 쓰는 표기입니다.   
- 예를 들어 해당 헤지드 지수 수익률 관련 수식에서는 “Where” 뒤에 $I_0$, $I_t$, $FFR_0$, $FFR_t$, $OddDay_t$, $T$, $H_t$ 등의 변수가 각각 무엇을 뜻하는지 제시합니다.
```
