...
## Question
HReturn과 ForwardReturn의 정의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HReturn과 ForwardReturn의 정의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HReturn과 ForwardReturn의 정의 차이는 무엇인가?",
...
## Assistant Response
```text
- HReturn은 ForwardReturn에서 CurrencyReturn을 차감한 값으로 정의됩니다: HReturn_t = ForwardReturn_t − CurrencyReturn_t   
- ForwardReturn은 KRW/USD 1 Month Forward 환율(FFR0, FFRt)과 USD지수/기준 환산 성분(FFR0, FXR0^R, FXR_t^R, OddDay_t/T)으로 산출되는 누적수익률 항으로 정의됩니다   
- 즉, HReturn은 ForwardReturn에서 CurrencyReturn 성분을 제거한 “헤지 관련 수익률”로 표현되고, ForwardReturn은 그 “기본(전방) 환율 기반 수익률” 항입니다
```
...
## Assistant Response
```text
- 둘 다 KRW Hedged 지수 산출에서 사용하는 “환헤지 관련 수익률” 항목으로, HReturn은 ForwardReturn과 CurrencyReturn을 이용해 정의됩니다.   
- ForwardReturn은 1개월 Forward 환율(FFR)과 환산용 환율( FXR^R ), 그리고 해당 월의 잔여일수(OddDay_t) 비중을 반영해 계산되는 “Forward 기반 수익률”입니다.   
- HReturn은 ForwardReturn에서 CurrencyReturn을 차감한 값으로, 즉 “ForwardReturn에서 환율(스팟 환산) 효과를 제외한 헤지 성격의 수익률”로 정의됩니다.
```
...
## Assistant Response
```text
- KRW Hedged 지수 산출에서 두 값은 모두 환헤지 관련 수익률 구성요소로 쓰이며, HReturn은 ForwardReturn을 기반으로 정리됩니다.   
- ForwardReturn은 Forward 환율(1 Month Forward) 변화와 잔여일수 가중을 반영해 계산한 “환헤지에 따른 포워드 효과 수익률”입니다.   
- HReturn은 ForwardReturn에서 CurrencyReturn(환율의 통화(환) 자체 변화분)을 차감한 값으로, “헤지에서 발생하는 부분(순수 헤지 기여분)”을 의미합니다.
```
...
## Assistant Response
```text
- (공통 맥락) HReturn과 ForwardReturn은 KRW Hedged 지수의 산출식에서 HedgedReturn(HReturn) 구성요소로 같이 쓰입니다.   
- ForwardReturn은 KRW/USD 1 Month Forward 환율(FFR)과 기준 spot 환율( FXR^R )을 활용해 정의되는 “Forward 거래로부터의 수익률”입니다(ForwardReturn_t 식).   
- HReturn은 ForwardReturn에서 CurrencyReturn(순수 통화/환율 변동분)인 CurrencyReturn_t를 차감해 정의됩니다. 즉 HReturn_t = ForwardReturn_t - CurrencyReturn_t 입니다.
```
