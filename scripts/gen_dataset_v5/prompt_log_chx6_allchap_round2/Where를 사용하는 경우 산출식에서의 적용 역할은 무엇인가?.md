...
## Question
Where를 사용하는 경우 산출식에서의 적용 역할은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Where를 사용하는 경우 산출식에서의 적용 역할은 무엇인가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "Where를 사용하는 경우 산출식에서의 적용 역할은 무엇인가?",
...
## Assistant Response
```text
- 산출식(예: CD 관련 듀레이션/수익률 등)을 먼저 제시한 뒤, Where를 사용해 그 산출식에 포함된 각 기호/항목이 계산에 쓰일 값(또는 하위 산출식)으로 연결되도록 지정한다    
- Where에 지정된 항목(예: r, r_{i,P}, NAV 등)은 해당 산출식을 계산할 때 바로 대입되는 입력값으로 적용한다   
- Where 안에서 또 다른 산출식이 이어지는 경우(예: ForwardReturn_t, CurrencyReturn_t 등), 하위 산출식의 Where까지 먼저 적용해 필요한 값을 만든 뒤 상위 산출식에 다시 반영하는 순서로 전환한다
```
...
## Assistant Response
```text
- 적용 순서: 산출식에 포함된 기호(예: 펀드 주간 로그수익률 평균, 무위험이자율 주간 로그수익률 평균, 표준편차 등)를 계산에 쓰기 전에, 식 아래의 Where 항목에서 해당 기호의 의미를 먼저 확인한다.   
- 적용 조건: 산출식 내 각 기호는 Where에서 정의된 값(무슨 수익률/무슨 기간/무슨 통계량인지)에 해당할 때에만 그 정의대로 대입하여 계산한다.   
- 전환 규칙: 무위험이자율(무위험이자율 주간 로그수익률)은 제로인에서 CD91물을 사용해 산출한다.    
- 적용 확장: Where는 다른 성과/비율 산출식에서도 동일하게, 산출식에 등장하는 변수 각각을 “어떤 값으로 적용할지”를 지정하는 역할로 사용된다.
```
...
## Assistant Response
```text
- 적용 대상/맥락: 산출식에 기호(항목)가 등장하는 경우, 그 기호가 어떤 값/통계량을 의미하는지와 어떤 데이터(대상)를 써서 계산할지를 “Where”로 지정한다. 예) 주간 로그수익률 평균, 무위험이자율 주간 로그수익률 평균, 주간 로그수익률 표준편차 등   
- 적용 순서: (1) 산출식에 있는 기호를 Where에 나온 정의와 1:1로 연결하고, (2) Where에서 요구한 방식(평균/표준편차, 특정 수익률 등)으로 각 항의 값을 먼저 산출한 뒤, (3) 그 값을 산출식의 분자/분모 위치에 그대로 대입한다   
- 적용 조건: Where에 명시된 산출 대상/사용항목을 조건으로 삼아야 한다. 예) 무위험이자율은 CD91물을 사용   
- 전환 규칙: Where에서 하위항을 또 다른 산식으로 정의하면, 하위항을 먼저 계산해 상위 산식의 항으로 “전환(대입)”한다. 예) HedgedIndexR_t의 Where에서 HReturn_t=ForwardReturn_t-CurrencyReturn_t로 정의된 경우, 그 관계식에 따라 순차 계산 후 적용
```
...
## Assistant Response
```text
Where를 사용하는 경우, 산출식에서의 적용 역할은 “앞의 수식에 등장한 기호들이 무엇을 의미하는지”를 정해 계산에 그대로 사용할 수 있게 만드는 것입니다.  

- 적용 순서: (1) 수식을 먼저 확인하고, (2) 수식에 나온 각 기호가 Where 뒤에서 대응 의미(예: r, J, 평균, 표준편차 등)로 매핑됩니다.  
- 적용 조건: Where는 해당 수식에서 사용된 기호(변수) 전부에 대해 정의가 제공된 경우에, 그 기호에 대한 값 산출/대입에 사용됩니다. 
- 적용 전환 규칙: 수식이 바뀌면 Where도 함께 바뀌므로, 다음 수식의 계산에는 그 수식에 딸린 Where의 정의를 적용합니다. (즉, 동일 기호라도 수식 단위로 정의를 갈아끼움)  
- 적용 결과: Where에서 정의된 변수들로 수식을 해석·구현한 뒤, 해당 기호가 요구하는 값(일자 수익률, 지수 값, 평균/분산 관련 값 등)을 사용해 결과를 계산합니다.
```
