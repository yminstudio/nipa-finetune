...
## Question
KRW Hedged 지수 산출 대상의 범위에 포함되는 지수군은 어떤 지수들인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "KRW Hedged 지수 산출 대상의 범위에 포함되는 지수군은 어떤 지수들인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A6"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "KRW Hedged 지수 산출 대상의 범위에 포함되는 지수군은 어떤 지수들인가?",
...
## Assistant Response
```text
KRW Hedged 지수 산출 대상의 범위(현재 산출 대상 Hedged 지수)에 포함되는 지수군은 다음과 같습니다. 

- MSCI 계열: MSCI ACWI, MSCI WI, MSCI EM 
- Lehman Brother’s: US Aggregate 
- JACI 계열: JACI, JACI IG
```
...
## Assistant Response
```text
KRW Hedged 지수 산출의 “산출 대상”에 포함되는 Hedged 지수군은 아래와 같습니다. 

- MSCI ACWI / MSCI WI / MSCI EM (임의의 헤지비율에 따라 각각 산출) 
- Lehman Brother’s US Aggregate 
- JACI / JACI IG (임의의 헤지비율에 따라 각각 산출)
```
...
## Assistant Response
```text
KRW Hedged 지수 산출 대상 범위에 포함되는 ‘현재 산출 대상 Hedged 지수’는 다음 지수군을 의미합니다. 

- MSCI 계열: MSCI ACWI, MSCI WI, MSCI EM 
- Lehman Brother’s 계열: Lehman Brother’s US Aggregate 
- JACI 계열: JACI, JACI IG
```
...
## Assistant Response
```text
KRW Hedged 지수 산출 대상의 범위(현재 산출 대상 Hedged 지수)는 아래 지수군을 포함합니다. 

- MSCI 계열: MSCI ACWI, MSCI WI, MSCI EM (각각 임의의 헤지비율에 따라 산출)   
- Lehman Brother’s 계열: Lehman Brother’s US Aggregate   
- JACI 계열: JACI, JACI IG (각각 임의의 헤지비율에 따라 산출)
```
