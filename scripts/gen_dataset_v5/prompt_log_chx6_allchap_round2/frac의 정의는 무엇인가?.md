...
## Assistant Response
```text
- 문서 내에서 frac는 수식에서 \frac 형태로 나타나며, 분수 형태의 계산 표기로 사용됩니다    
- 다만, ‘frac’라는 용어(표기 자체)에 대한 별도의 정의는 문서에 명시되어 있지 않습니다
```
...
## Assistant Response
```text
- 수식에서 “frac”은 분수(나눗셈) 형태를 나타내는 표기이며, 일반적으로 “분자/분모”를 뜻합니다.  
- 정의로는 \frac{A}{B} = A를 B로 나눈 값입니다.  
- 예를 들어 샤프지수는 S^F = \frac{\overline{R^F}-\overline{R^{rf}}}{\sigma^F}처럼 분수 형태로 사용됩니다.
```
...
## Assistant Response
```text
- 제로인 펀드평가 방법론에서 기호 “frac”(수식의 \frac{분자}{분모} 표기)는 분수 표기이며, “분자 값을 분모로 나눈 값(분자/분모)”을 의미합니다.  
- 예를 들어 수정샤프지수는 MS^F = \frac{\overline{R^{F}} - \overline{R^{rf}}}{\left(\sigma^{F}\right)^{\frac{|\overline{{R}^{F}} - \overline{{R}^{rf}}|}{\overline{{R}^{F}} - \overline{{R}^{rf}}}} 처럼 분자(초과수익)와 분모(위험 관련 항)의 비율로 나타냅니다.
```
...
## Assistant Response
```text
- 여기서 frac은 수식에서 “분자/분모” 형태로 값을 나타내며, 결과적으로 “분자 ÷ 분모(비율)”을 의미하는 표기입니다.   
- 예를 들어 기간중 양비중은 PP^F = \frac{n_P^F}{n} 으로, n_P^F를 n으로 나눈 값(비중)으로 정의해 사용합니다.   
- 또한 샤프지수는 S^F = \frac{\overline{R^F}-\overline{R^{rf}}}{\sigma^F} 처럼, (초과수익)에서 표준편차를 나눈 형태로 frac 표기가 사용됩니다.
```
