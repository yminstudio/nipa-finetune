...
## Question
현재 산출 대상 Hedged 지수의 구성 지수군은 MSCI 계열 지수를 포함하는가를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "현재 산출 대상 Hedged 지수의 구성 지수군은 MSCI 계열 지수를 포함하는가를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "현재 산출 대상 Hedged 지수의 구성 지수군은 MSCI 계열 지수를 포함하는가를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 현재 산출 대상 Hedged 지수의 구성 지수군에는 MSCI 계열 지수로서 MSCI ACWI, MSCI WI, MSCI EM이 포함된다.   
- 따라서 MSCI 계열 지수를 포함하는지의 판단 기준은 ‘현재 산출 대상 Hedged 지수’에 MSCI ACWI/WI/EM이 구성 지수로 포함되어 있는지 여부다.
```
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수의 “구성 지수군”이 무엇인지에 따라, 그 목록에 MSCI 계열 지수가 포함되는지를 확인하는 것이 판단 기준입니다. 

- 구성 지수군은 “현재 산출 대상 Hedged 지수”에 열거된 지수들로 정해집니다.   
- 해당 목록에 MSCI ACWI, MSCI WI, MSCI EM이 포함되어 있으므로, MSCI 계열 지수를 포함하는 것으로 판단합니다.
```
...
## Assistant Response
```text
구성 지수군이 “MSCI 계열 지수”를 포함하는지는 제로인이 정의한 “현재 산출 대상 Hedged 지수”에 MSCI 계열 지수가 포함되어 있는지를 보는 기준으로 판단합니다. 

- “현재 산출 대상 Hedged 지수” 목록에 MSCI ACWI, MSCI WI, MSCI EM이 포함되어 있으므로, 이 지수들이 구성 지수군에 해당하면 MSCI 계열 지수를 포함합니다. 
- 반대로 “현재 산출 대상 Hedged 지수” 목록에 포함된 다른 지수(Lehman Brother’s US Aggregate, JACI/JACI IG)만으로 구성되는 경우에는 MSCI 계열 지수를 포함하지 않는 것으로 판단합니다.
```
...
## Assistant Response
```text
MSCI 계열 지수가 “현재 산출 대상 Hedged 지수”의 구성 지수군에 포함되는지 여부를 판단하기 위한 기준은, Hedged 지수 산출에 실제로 사용하도록 정해 둔 구성 지수 목록에 MSCI 계열이 들어있는지입니다.   

- “현재 산출 대상 Hedged 지수” 항목에 MSCI ACWI, MSCI WI, MSCI EM이 포함되는지 확인   
- 해당 항목에는 MSCI ACWI, MSCI WI, MSCI EM이 포함되어 있으므로 MSCI 계열 지수는 구성 지수군에 포함됩니다.
```
