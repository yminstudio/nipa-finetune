...
## Question
MSCI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MSCI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?",
  "qa_type": "application",
  "question_template": "Application_Template_A6"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "MSCI 계열 지수들 중 현재 산출 대상에 해당하는 지수는 어떤 것들이 있는가?",
...
## Assistant Response
```text
- 헤지비율을 임의로 부여(예: 50%, 100%)하는 조건에서 MSCI ACWI를 현재 산출 대상 Hedged 지수로 적용   
- 헤지비율을 임의로 부여(예: 50%, 100%)하는 조건에서 MSCI WI를 현재 산출 대상 Hedged 지수로 적용   
- 헤지비율을 임의로 부여(예: 50%, 100%)하는 조건에서 MSCI EM을 현재 산출 대상 Hedged 지수로 적용
```
...
## Assistant Response
```text
- 적용 순서(선택): KRW Hedged 지수 산출에서 ‘현재 산출 대상 Hedged 지수’에 해당하는 MSCI 계열은 MSCI ACWI, MSCI WI, MSCI EM   
- 적용 조건(산출 방식): 위 MSCI 지수들은 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등)   
- 전환 규칙(0→t 산출): HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t)로 t일 지수로 전환하며, HedgedIndexR_t = (I_t/I_0)×(FXR_t^R/FXR_0^R) - 1 + H_t×HReturn_t
```
...
## Assistant Response
```text
해외투자 성과측정을 위한 KRW Hedged 지수 산출에서, “현재 산출 대상 Hedged 지수”에 포함되는 MSCI 계열은 아래 3개입니다. 

- 적용 순서: 현재 산출 대상 Hedged 지수 단계에서 포함 / 적용 조건: MSCI ACWI는 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) / 전환 규칙: 헤지비율이 달라질 때마다 해당 비율로 산출대상이 분리됨   
- 적용 순서: 현재 산출 대상 Hedged 지수 단계에서 포함 / 적용 조건: MSCI WI는 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) / 전환 규칙: 헤지비율이 달라질 때마다 해당 비율로 산출대상이 분리됨   
- 적용 순서: 현재 산출 대상 Hedged 지수 단계에서 포함 / 적용 조건: MSCI EM은 임의로 부여한 헤지비율에 따라 각각 산출(예: 50%, 100% 등) / 전환 규칙: 헤지비율이 달라질 때마다 해당 비율로 산출대상이 분리됨
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에 적용되는 “현재 산출 대상 Hedged 지수” 중, MSCI 계열에 해당하는 지수는 아래 3종입니다.   
- 적용 순서: 현재 산출 대상 Hedged 지수로부터 MSCI 계열 지수를 먼저 선정  
- 적용 조건: 각 지수는 “임의로 부여한 헤지비율(예: 50%, 100%)”에 따라 각각 산출   
- 전환 규칙: 해당 항목(현재 산출 대상 Hedged 지수)에서 MSCI 간 전환 규칙은 별도로 제시되어 있지 않습니다.
