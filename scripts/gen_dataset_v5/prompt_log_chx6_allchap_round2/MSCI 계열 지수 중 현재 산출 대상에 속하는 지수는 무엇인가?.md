...
## Question
MSCI 계열 지수 중 현재 산출 대상에 속하는 지수는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MSCI 계열 지수 중 현재 산출 대상에 속하는 지수는 무엇인가?",
  "qa_type": "definition",
  "question_template": "Definition_Template_A7"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 현재 산출 대상 Hedged 지수",
  "question": "MSCI 계열 지수 중 현재 산출 대상에 속하는 지수는 무엇인가?",
...
## Assistant Response
```text
정의: 현재 산출 대상 Hedged 지수에 포함되는 MSCI 계열 지수는 아래와 같다.   
- MSCI ACWI   
- MSCI WI   
- MSCI EM
```
...
## Assistant Response
```text
KRW Hedged 지수 산출에서 ‘현재 산출 대상 Hedged 지수’로 지정된 MSCI 계열 지수는 다음과 같다. 
...
## Assistant Response
```text
현재 산출 대상 Hedged 지수는 헤지비율을 임의로 부여해(예: 50%, 100% 등) 각각 산출하는 지수이며, 이 중 MSCI 계열 지수는 다음과 같다. 
...
## Assistant Response
```text
제로인 방법론에서 ‘현재 산출 대상 Hedged 지수’는 임의로 부여한 헤지비율에 따라 각각 산출하는 지수를 뜻하며, 그중 MSCI 계열 지수는 다음과 같습니다. 
