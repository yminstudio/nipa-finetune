...
## Question
HedgedIndex와 Lehman Brother’s 지수인 경우의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "HedgedIndex와 Lehman Brother’s 지수인 경우의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 산출식",
  "question": "HedgedIndex와 Lehman Brother’s 지수인 경우의 차이는 무엇인가?",
...
## Assistant Response
```text
- HedgedIndex: 헤지비율(H_t)을 임의의 비율로 부여해 적용합니다(예: H_t=0.5, 0.9, 1).   
- Lehman Brother’s 지수: 헤지비율(H_t)을 Yield_t로부터 산출해 적용합니다.   
- 두 경우 모두 헤지비율(H_t)과 Hedge Return(ForwardReturn−CurrencyReturn)의 결합이 KRW Hedged 누적수익률에 반영되는 구조입니다.
```
...
## Assistant Response
```text
공통 맥락으로 두 경우 모두 KRW Hedged Index의 산출은 HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 형태로 진행됩니다.   
차이는 헤지비율 H_t를 “어떻게 정하느냐”에 있습니다.

- HedgedIndex(일반 적용)인 경우: H_t를 임의로 부여한 상수(예: 0.5, 0.9, 1)로 둡니다.   
- Lehman Brother’s 지수인 경우: H_t를 Yield_t(전월 말일~해당 t일의 Yield to Worst)로부터 계산합니다.   
...
## Assistant Response
```text
두 경우 모두 KRW Hedged 누적수익률을 반영해 KRW Hedged 지수를 산출한다는 공통 맥락이 있습니다. 

- 차이 1(헤지비율 H_t 산정 방식): 일반적인 HedgedIndex는 헤지비율 H_t를 임의의 비율(예: 0.5, 0.9, 1 등)로 가정해 적용합니다.   
- 차이 2(Lehman Brother’s의 H_t): Lehman Brother’s 지수인 경우에는 H_t를 Yield to Worst(Yield_t)로부터 정해지는 식으로 계산해 적용합니다.
```
...
## Assistant Response
```text
두 지수 모두 KRW Hedged 지수 산출식에서 헤지비율 H_t가 핵심 입력으로 들어가며, KRW Hedged 누적수익률의 항 중 H_t × HReturn_t 형태로 반영됩니다 .

- HedgedIndex(헤지비율을 임의로 적용하는 경우): H_t를 상수(예: 0.5, 0.9, 1 등)로 두고, 해당 값이 그대로 사용됩니다   
- Lehman Brother’s 지수인 경우: H_t를 Yield_t(Yield to Worst)로부터 계산해 산출하며, H_t = (1 + (Yield_t/100)/2)^(1/6)로 정해집니다   
- 결과적으로, 같은 HReturn_t 구조라도 H_t를 “고정값”으로 쓰는지 “Yield 기반 산출값”으로 쓰는지에서 차이가 납니다
```
