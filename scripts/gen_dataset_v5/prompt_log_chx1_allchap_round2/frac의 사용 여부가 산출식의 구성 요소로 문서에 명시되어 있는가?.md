...
## Question
frac의 사용 여부가 산출식의 구성 요소로 문서에 명시되어 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "frac의 사용 여부가 산출식의 구성 요소로 문서에 명시되어 있는가?",
  "qa_type": "criteria",
  "question_template": "ct_criteria_06"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.2. 기간별 투자비용",
  "question": "frac의 사용 여부가 산출식의 구성 요소로 문서에 명시되어 있는가?",
...
## Assistant Response
```text
- 문서의 산출식에 분수 표기용 \frac가 산출식 구성요소로 사용되어 있습니다.   
- 예를 들어 adjESG wgt_i, HedgedIndexR_t, D\_{CD} 등의 산출식에 \frac 표기가 포함되어 있습니다.
```
...
## Assistant Response
```text
질문은 산출식의 작성 기준에서 ‘\frac를 써야/쓰지 말아야 한다’처럼 사용 여부가 별도 규칙(구성 요소)으로 명시돼 있는지 확인하려는 것입니다.

- 문서에는 실제 산출식들이 분수 형태를 \frac로 표기한 예들이 있습니다(예: Hedged 지수 산출식의 분수 표기). 
- 또한 그룹 수익률 산출식 등에서도 \frac 형태의 분수 표기가 사용됩니다. 
- 다만 ‘산출식 구성 요소로서 \frac 사용 여부를 명시한다(반드시 사용/금지 등)’는 규칙 문구는 확인되지 않습니다.
```
...
## Assistant Response
```text
- 질문은 “산출식에 분수 표기에서 ‘\frac(·)(·)’가 사용되는지(=산출식 구성 요소로 문서에 명시되는지)”를 판단하려는 것입니다.  
- 문서의 산출식에서 ‘\frac’ 표기가 분수 형태로 직접 사용됩니다. 예를 들어 HedgedIndexR_t 식에 $\frac{I_t}{I_0}$, $\frac{FXR_t^R}{FXR_0^R}$ 등이 포함됩니다   
- 또 다른 예로 채권의 듀레이션 산출식에서 $D_{CD}$에 $\frac{\frac{\text{잔존일수}}{365}}{(1+\frac{r}{365})}$처럼 ‘\frac’가 중첩 형태로 사용됩니다
```
...
## Assistant Response
```text
문서에서 ‘산출식의 구성 요소’로서 ‘frac(수식의 \frac 표기) 사용 여부’를 별도로 정해 두었는지를 묻는 질문입니다.

- 문서의 산출식 표기는 분수 형태를 \frac로 작성하고 있습니다(예: MS^F, S^F 등).  
- 다만 해당 산출식에서 제공되는 설명은 \frac 사용 여부가 아니라, 각 기호(예: \overline{R}^F, \sigma^F 등)에 대한 정의로 구성됩니다. 
- 따라서 ‘frac 사용 여부’ 자체가 산출식 구성요소로 명시된 기준(판단 규칙) 형태로는 드러나지 않습니다.
```
