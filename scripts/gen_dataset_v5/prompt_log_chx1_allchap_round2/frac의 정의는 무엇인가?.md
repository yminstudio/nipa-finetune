...
```json
{
  ...
  "target_total": 100,
  "generated_question": "frac의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- 문서에서 frac은 수식에서 \frac{분자}{분모} 형태로 나타나는 “분수(비율) 표기”로 사용됩니다.   
- 예를 들어 추정 주식매도회전율(TR_t^S) 산식처럼 \frac{\,\cdot\,}{\,\cdot\,} 형태로 분자/분모 값을 나누어 계산하는 식에 그대로 포함되어 있습니다.
```
...
## Assistant Response
```text
- LaTeX에서 frac(보통 \frac로 표기됨)는 분수 형태를 만드는 명령입니다.  
- \frac{분자}{분모}처럼 쓰며, 첫 번째 중괄호 내용은 분자, 두 번째 중괄호 내용은 분모로 표시됩니다.  
- 따라서 수식에서는 (분자)/(분모) 가로줄이 있는 분수로 나타납니다.
```
...
## Assistant Response
```text
- 여기서 “frac”은 수식 표기에 쓰이는 기호로, “분자/분모”를 뜻합니다.  
- 정의: \frac{A}{B} = A를 B로 나눈 값(= A ÷ B)을 의미합니다.
```
...
## Assistant Response
```text
수식 표기에서 frac은 분수(나눗셈)를 나타내는 표기입니다.  
즉 \frac{분자}{분모}는 “분자 ÷ 분모”를 의미하며, 예를 들어 D_{CD} = \frac{\frac{\text{잔존일수}}{365}}{(1 + \frac{r}{365})} 처럼 사용됩니다 .
```
...
## Assistant Response
```text
- 본 문서에서 수식에 쓰인 “frac”은(라텍스 표기 기준) 분수 형태를 뜻하는 기호인 “\frac{분자}{분모}”를 말합니다.   
- 정의: \frac{A}{B} = A를 B로 나눈 값을 나타내며, A는 분자, B는 분모입니다.   
- 예: \lambda = \frac{E}{2 \times \sigma^2} 처럼 “\frac{E}{2 \times \sigma^2}”는 “E / (2 \times \sigma^2)”를 의미합니다.
```
