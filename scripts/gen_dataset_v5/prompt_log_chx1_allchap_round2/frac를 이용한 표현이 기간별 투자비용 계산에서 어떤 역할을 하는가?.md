...
## Question
frac를 이용한 표현이 기간별 투자비용 계산에서 어떤 역할을 하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "frac를 이용한 표현이 기간별 투자비용 계산에서 어떤 역할을 하는가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.2. 기간별 투자비용",
  "question": "frac를 이용한 표현이 기간별 투자비용 계산에서 어떤 역할을 하는가?",
...
## Assistant Response
```text
- 1년 투자비용은 총신탁보수율과 선취수수료율을 합해 산출한다.   
- 3년 투자비용은 먼저 1년투자비용을 만든 뒤, 추가 항목으로 선취수수료율을 분수 형태(선취수수료율/3)로 나눈 값을 더한다.   
- 즉 frac(÷3) 표현은 선취수수료율을 3년 투자비용 계산에 반영할 때 기간에 맞게 나누어(÷3) 전환하는 역할을 한다.   
- 또한 비용효율성 산정에서는 선취수수료율을 평가기간으로 나누는 형태((총신탁보수율 * 평가기간 + 선취수수료율) / 평가기간)로 같은 전환 구조가 나타난다.
```
...
## Assistant Response
```text
- 적용 대상/맥락: 기간별 투자비용 산출에서 “선취수수료율을 기간에 맞게 반영”하는 방식으로 frac(예: /3) 형태의 나눗셈이 사용됩니다.   
- 적용 순서: 1) 1년 투자비용 = 총신탁보수율 + 선취수수료율을 먼저 계산합니다.   2) 3년 투자비용 = 1년투자비용 + 선취수수료율/3 로 계산합니다.   
- 적용 조건: frac(/3)은 3년 투자비용처럼 “3년 기간”으로 전환해 기간별 투자비용을 만들 때, 선취수수료율을 해당 기간에 대응시키기 위해 사용됩니다.   
- 전환 규칙: 1년→3년 전환 시 선취수수료율을 그대로 추가하지 않고, 기간 길이(3)로 나눈 값(선취수수료율/3)만 추가하여 기간별(연 단위) 투자비용 값으로 환산합니다.
```
...
## Assistant Response
```text
- 맥락: 기간별 투자비용 산식에서 3년 투자비용은 먼저 1년 투자비용을 만들고, 그 다음 선취수수료율을 기간 길이에 맞게 반영해 더하는 방식으로 구성됩니다. 
- 적용 순서: (1) 1년 투자비용 = 총신탁보수율 + 선취수수료율을 산출합니다. 
- 적용 조건: (2) 3년 투자비용을 계산할 때는 1년 투자비용에 추가항을 붙이며, 그 추가항으로 선취수수료율을 3년 기준으로 환산한 값이 사용됩니다. 
- 전환 규칙: (3) 선취수수료율은 3년 투자비용 추가항에서 $\frac{선취수수료율}{3}$ 형태로 분할되어 반영됩니다(즉, 3년 투자비용 = 1년 투자비용 + $\frac{선취수수료율}{3}$).
```
...
## Assistant Response
```text
- 적용 대상/맥락: 신탁보수율과 선취수수료율을 “기간별 투자비용”으로 바꾸는 계산에서, \frac{…}{…} 형태(분수 표현)가 기간 단위로 비용을 환산·배분하는 역할을 합니다.    
- 적용 순서(1년 투자비용): 1년 투자비용을 (총 신탁보수율 * 평가기간 + 선취수수료율) / 평가기간 으로 산출하며, 이때 분모(평가기간)로 나누는 \frac{…}{평가기간}이 비용을 평가기간 단위로 전환합니다.   
- 적용 순서(3년 투자비용): 3년 투자비용은 1년투자비용 + \frac{선취수수료율}{3} 형태로 구성하며, \frac{선취수수료율}{3}가 선취수수료율을 3년 동안 분할(배분)해 추가 반영하는 전환 규칙입니다.   
- 적용 조건/전환 규칙: 평가기간은 1년, 2년, 3년이며, 기간별 투자비용은 이 평가기간 개념에 따라 (1년은 분모로 나눠 전환, 3년은 1년투자비용에 분할 몫을 더하는 방식)으로 산출됩니다.
```
...
## Assistant Response
```text
- 적용대상/맥락: 기간별 투자비용 중 3년 투자비용을 산출할 때, 선취수수료율을 분수 형태로 나타낸 frac(선취수수료율/3)가 “추가로 반영하는 값”을 지정하는 역할을 합니다   
- 적용 순서: 먼저 1년 투자비용을 계산합니다(1년투자비용 = 총신탁보수율 + 선취수수료율)   
- 적용 순서: 그 다음 3년 투자비용을 구할 때, 위의 1년 투자비용에 frac로 표현된 선취수수료율/3을 더합니다(3년투자비용 = 1년투자비용 + 선취수수료율/3)   
- 전환 규칙: 기간을 1년에서 3년으로 전환할 때, 선취수수료율을 3으로 나눈 값(선취수수료율/3)을 추가 항으로 사용해 기간별 투자비용을 달리 계산합니다
```
