...
## Question
년투자비용 산출식에 frac가 사용된다는 기준에 부합하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "년투자비용 산출식에 frac가 사용된다는 기준에 부합하는가?",
  "qa_type": "criteria",
  "question_template": "ct_criteria_02"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.2. 기간별 투자비용",
  "question": "년투자비용 산출식에 frac가 사용된다는 기준에 부합하는가?",
...
## Assistant Response
```text
- 문서에 따르면 1년투자비용은 “총신탁보수율 + 선취수수료율”로 산출되어, 1년 투자비용 산출식에 분수표기(\frac)가 사용된다는 기준에는 부합하지 않습니다.   
- 한편 “3년투자비용” 산출식에는 “선취수수료율/3” 형태로 \frac가 사용됩니다.
```
...
## Assistant Response
```text
‘1년 투자비용’ 산출식에서 분수 표기를 나타내는 LaTeX의 frac(예: \frac{A}{B})가 쓰이는지 여부를 보려는 기준입니다. 

- 1년 투자비용은 “총신탁보수율 + 선취수수료율”로 제시되어 frac 표기가 없습니다.   
- 한편 3년 투자비용에는 “선취수수료율/3” 항이 \frac{선취수수료율}{3} 형태로 frac이 사용됩니다.   
- 따라서 질문의 대상이 ‘1년 투자비용 산출식’이라면 기준(frac 사용)에 부합하지 않습니다.
```
...
## Assistant Response
```text
해당 기준은 “연(1년) 투자비용” 산출식에 분수 표기인 frac(예: \frac{…}{…})가 실제로 들어가는지 여부를 판단하기 위한 것입니다. 

문서에서는 1년 투자비용을 “총신탁보수율 + 선취수수료율” 형태로 제시하거나, “(총 신탁보수율 * 평가기간 + 선취수수료율) / 평가기간”으로 제시하며, frac 표기는 사용되지 않습니다.  

따라서 “년(1년) 투자비용 산출식에 frac가 사용된다”는 기준에는 부합하지 않습니다.
```
...
## Assistant Response
```text
- 판단 기준: “년(1년) 투자비용 산출식에 LaTeX의 frac(\frac)이 사용되는지” 여부를 보려는 것입니다.  
- 문서의 1년 투자비용 산출식은 “총신탁보수율 + 선취수수료율”로 제시되어 frac(\frac) 형태가 쓰이지 않습니다.   
- 반면 frac(\frac)은 3년 투자비용에서 “선취수수료율/3” 항에 사용됩니다.   
- 따라서 “년투자비용 산출식에 frac가 사용된다”는 기준은 1년 투자비용 산식 기준으로는 부합하지 않습니다.
```
