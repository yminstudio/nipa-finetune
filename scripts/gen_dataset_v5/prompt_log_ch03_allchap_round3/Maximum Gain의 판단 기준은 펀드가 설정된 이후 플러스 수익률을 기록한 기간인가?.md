...
## Question
Maximum Gain의 판단 기준은 펀드가 설정된 이후 플러스 수익률을 기록한 기간인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Gain의 판단 기준은 펀드가 설정된 이후 플러스 수익률을 기록한 기간인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "13) 최대이익 ; Maximum Gain",
  "question": "Maximum Gain의 판단 기준은 펀드가 설정된 이후 플러스 수익률을 기록한 기간인가?",
...
## Assistant Response
```text
Maximum Gain은 펀드가 설정된 이후 “이익(플러스)”이 발생한 구간에서 가장 성과가 좋았던 수준을 판단하기 위한 지표입니다. 

- 판단 기준은 설정 이후 플러스 수익률을 기록한 기간들(주간 실현수익률 r_i^F)을 대상으로 합니다.   
- 그 기간들 중 최고의 수익률을 Maximum Gain으로 정의합니다( Max_G^F = max(r_i^F, 0) ).
```
