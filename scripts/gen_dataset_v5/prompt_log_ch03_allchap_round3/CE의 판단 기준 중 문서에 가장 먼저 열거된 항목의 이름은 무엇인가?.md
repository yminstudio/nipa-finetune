...
## Question
CE의 판단 기준 중 문서에 가장 먼저 열거된 항목의 이름은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CE의 판단 기준 중 문서에 가장 먼저 열거된 항목의 이름은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "6) 확실성등가 ; Certainty Equivalence(CE)",
  "question": "CE의 판단 기준 중 문서에 가장 먼저 열거된 항목의 이름은 무엇인가?",
...
## Assistant Response
```text
CE(확실성 등가) 기반 성과등급 산출에서 “CE순위”를 만들기 위한 대상 펀드를 가르는 판단 기준의 첫 항목은,   
- 변경전 지표 %순위(CE순위) 산출 대상 펀드
```
