...
## Question
Beta가 나타내는 민감도는 어떤 두 요소 간의 관계를 비교하는 지표인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Beta가 나타내는 민감도는 어떤 두 요소 간의 관계를 비교하는 지표인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) 베타 ; Beta",
  "question": "Beta가 나타내는 민감도는 어떤 두 요소 간의 관계를 비교하는 지표인가?",
...
## Assistant Response
```text
- 베타(Beta)는 시장 변화에 대한 펀드수익률의 민감도를 나타내는 지표입니다   
- 따라서 시장수익률(시장으로 간주한 KOSPI200)과 펀드수익률 간의 상호변동관계를 비교합니다   
- 즉, “시장(지수) 수익률이 변할 때 펀드 수익률이 얼마나 함께 움직이는지”의 관계를 보는 비교축입니다
```
