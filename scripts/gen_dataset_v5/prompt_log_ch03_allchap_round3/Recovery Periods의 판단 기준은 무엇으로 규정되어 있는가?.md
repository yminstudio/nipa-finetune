...
## Question
Recovery Periods의 판단 기준은 무엇으로 규정되어 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Recovery Periods의 판단 기준은 무엇으로 규정되어 있는가?",
  "qa_type": "criteria",
  "question_template": "RecoveryPeriods_criteria_definition"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "question": "Recovery Periods의 판단 기준은 무엇으로 규정되어 있는가?",
...
## Assistant Response
```text
Recovery Periods는 “Maximum Drawdown이 발생한 뒤 펀드가 회복(잔여)하는 데 걸리는 기간”을 판단하기 위한 지표입니다. 

- Recovery Periods 판단 기준(정의): 기간 동안 Maximum Drawdown 발생 후의 잔여기간   
- 산식 기준: RP^F = Recovery Period End Date(총기간) − Maximum Drawdown Start Date
```
