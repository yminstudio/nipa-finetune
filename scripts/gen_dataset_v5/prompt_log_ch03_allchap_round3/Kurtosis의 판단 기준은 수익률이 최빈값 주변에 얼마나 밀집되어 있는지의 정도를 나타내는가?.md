...
## Question
Kurtosis의 판단 기준은 수익률이 최빈값 주변에 얼마나 밀집되어 있는지의 정도를 나타내는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Kurtosis의 판단 기준은 수익률이 최빈값 주변에 얼마나 밀집되어 있는지의 정도를 나타내는가?",
  "qa_type": "criteria",
  "question_template": "criteria_kurtosis_dense_near_mode_1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 첨도 ; Kurtosis",
  "question": "Kurtosis의 판단 기준은 수익률이 최빈값 주변에 얼마나 밀집되어 있는지의 정도를 나타내는가?",
...
## Assistant Response
```text
- 커트시스(첨도)는 펀드 수익률 분포의 모양(특히 최빈값 주변의 형태)을 어떻게 보는지를 판단하기 위한 지표입니다.   
- 커트시스는 “수익률이 최빈값 주변에 얼마나 밀집되어 있는지”의 정도를 나타내는 기준으로 설명됩니다.   
- 커트시스 값이 0보다 크면 최빈값 주변에 자료가 더 많이 몰려 봉우리(정점)가 더 뾰족한 형태가 되고, 0보다 작으면 봉우리 경사가 완만해집니다.
```
