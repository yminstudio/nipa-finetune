...
## Question
하락편차와 Downside Deviation은 같은 개념인가?
...
```json
{
  ...
  "section": "2) 하락편차 ; Downside Deviation",
  "subsection": "",
  "seed_title": "2) 하락편차 ; Downside Deviation",
  "record_target": 3,
  ...
  "target_total": 100,
  "generated_question": "하락편차와 Downside Deviation은 같은 개념인가?",
  "qa_type": "comparison",
  "question_template": "동의어_비교_하락편차"
}
```
...
[핵심 명사]
) 하락편차 ; Downside Deviation, 하락편차 ; Downside Deviation, 하락편차, Downside, Deviation, 수익률이, 벤치마크, BM, 대비, 하락한, 시기의, 표준편차
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 하락편차 ; Downside Deviation",
  "question": "하락편차와 Downside Deviation은 같은 개념인가?",
...
## Assistant Response
```text
- 제로인에서 “하락편차”는 “Downside Deviation(하방편차/하락편차)”와 같은 의미로 사용됩니다.   
- 둘 다 “벤치마크(BM) 대비 하락한 시기”의 수익률을 대상으로 표준편차(변동성)를 계산해, 마이너스 수익률에 대한 위험지표로 해석합니다.   
- 따라서 질문의 관점에서는 개념이 동일하고, 실무적으로는 용어(하락편차 vs Downside Deviation) 표기 차이에 가깝습니다.
```
