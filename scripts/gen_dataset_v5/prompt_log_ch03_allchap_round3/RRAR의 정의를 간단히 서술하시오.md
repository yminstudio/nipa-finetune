...
## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0028
...
```json
{
  ...
  "section": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "subsection": "",
  "seed_title": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "record_target": 3,
  ...
  "target_total": 100,
  "generated_question": "RRAR의 정의를 간단히 서술하시오.",
  "qa_type": "definition",
...
[핵심 명사]
RRAR, 상대위험, 조정후, 수익률, Relative, Risk, Adjusted, Return, 샤프지수의, 산출식에서, 무위험수익률, 대신
...
## Assistant Response
```text
RRAR(상대위험 조정후 수익률)은 샤프지수 산출에서 무위험수익률 대신 펀드(동일유형) 평균수익률을 사용하여, 상대적으로 얻은 초과수익을 그 초과수익의 변동성으로 나누어 나타내는 지표입니다.   

RRAR = ( $\overline{R^F} - \overline{R^P}$ ) / $\sigma^{F-P}$   

여기서 $\overline{R^F}$는 펀드의 주간 로그수익률 평균, $\overline{R^P}$는 동일유형의 주간 로그수익률 평균이며, $\sigma^{F-P}$는 동일유형 초과수익률의 표준편차입니다.
```
