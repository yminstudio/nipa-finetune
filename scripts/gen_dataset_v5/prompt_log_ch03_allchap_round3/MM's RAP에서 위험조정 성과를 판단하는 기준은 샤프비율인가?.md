...
## Question
MM's RAP에서 위험조정 성과를 판단하는 기준은 샤프비율인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "MM's RAP에서 위험조정 성과를 판단하는 기준은 샤프비율인가?",
  "qa_type": "criteria",
  "question_template": "criteria-샤프비율_적용여부"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "9) MM’s RAP ; Modigliani & Modigliani ‘s Risk Adjusted Performance",
  "question": "MM's RAP에서 위험조정 성과를 판단하는 기준은 샤프비율인가?",
...
## Assistant Response
```text
- MM’s RAP은 시장(또는 벤치마크)의 위험 수준(BM의 표준편차)이 주어졌을 때, 위험을 반영한 펀드의 가상 수익률(위험조정 성과)을 판단하기 위한 지표입니다.   
- MM’s RAP의 산식에는 (펀드 평균수익률−무위험이자율) / 펀드 표준편차 형태가 들어가며, 이는 샤프지수의 구성과 동일한 “샤프비율(초과수익/변동성)” 요소를 사용합니다.    
- 다만 MM’s RAP은 그 샤프비율 형태에 시장(또는 BM) 표준편차(σ^M)를 곱해 최종 값을 산출합니다.
```
