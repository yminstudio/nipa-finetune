...
## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0019

## Question
Recovery Periods를 판단하는 기준의 핵심 판단 축은 무엇인가?
...
```json
{
  ...
  "section": "17) Recovery Periods",
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "record_target": 3,
  ...
  "target_total": 100,
  "generated_question": "Recovery Periods를 판단하는 기준의 핵심 판단 축은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "RecoveryPeriods_criteria_core"
}
```
...
[핵심 명사]
) Recovery Periods, Recovery Periods, Recovery, Periods, 기간, 동안, Maximum, Drawdown이, 발생한, 후의, 잔여기간, RP
...
```json
{
  ...
  "subsection": "",
  "seed_title": "17) Recovery Periods",
  "question": "Recovery Periods를 판단하는 기준의 핵심 판단 축은 무엇인가?",
...
## Assistant Response
```text
Recovery Periods는 Maximum Drawdown이 발생한 이후의 “잔여기간(얼마나 더 버티고 회복/마무리까지 시간이 걸렸는지)”을 판단하기 위한 지표입니다.   
- Maximum Drawdown Start Date(최대손실 구간의 시작 시점)   
- Recovery Period End Date(측정의 총기간에서의 종료 시점)   
- 위 두 날짜의 차이로 결정되는 Recovery Period 길이(RP^F)
```
