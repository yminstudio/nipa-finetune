...
## Question
Calmar Ratio의 판단 기준에서 분자는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Calmar Ratio의 판단 기준에서 분자는 무엇인가?",
  "qa_type": "criteria",
  "question_template": "calmar_ratio_criteria_numerator"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "12) Calmar Ratio",
  "question": "Calmar Ratio의 판단 기준에서 분자는 무엇인가?",
...
## Assistant Response
```text
Calmar Ratio는 최대 누적손실(최대낙폭) 대비 연간 수익률이 얼마나 좋은지를 판단하기 위한 지표입니다. 

분자(=Annualized Rate of Return)는 단순수익률의 연환산수익률(로그수익률이 아닌 연환산)입니다.
```
