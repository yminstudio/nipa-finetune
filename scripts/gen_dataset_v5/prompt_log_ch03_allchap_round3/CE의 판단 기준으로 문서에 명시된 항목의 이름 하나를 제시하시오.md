...
## Question
CE의 판단 기준으로 문서에 명시된 항목의 이름 하나를 제시하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CE의 판단 기준으로 문서에 명시된 항목의 이름 하나를 제시하시오.",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "6) 확실성등가 ; Certainty Equivalence(CE)",
  "question": "CE의 판단 기준으로 문서에 명시된 항목의 이름 하나를 제시하시오.",
...
## Assistant Response
```text
- CE(Certainty Equivalence)는 제로인이 펀드등급(태극마크)을 산출하기 위한 기초값(효용값, 위험조정 성과)을 판단하기 위한 지표입니다.   
- 문서에 명시된 CE 판단 기준 항목의 이름: 변경전 지표 %순위(CE순위)
```
