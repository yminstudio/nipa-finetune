...
## Question
RRAR의 판단 기준은 무위험수익률 대신 상대위험을 사용하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "RRAR의 판단 기준은 무위험수익률 대신 상대위험을 사용하는가?",
  "qa_type": "criteria",
  "question_template": "rrar_criteria_replacement_basis"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "question": "RRAR의 판단 기준은 무위험수익률 대신 상대위험을 사용하는가?",
...
## Assistant Response
```text
RRAR은 위험을 감안해 수익을 평가하기 위한 상대위험 조정 수익률입니다. 

- RRAR의 분모(위험)는 무위험수익률이 아니라 동일유형 초과수익률의 표준편차인 상대위험(σ^{F-P})을 사용합니다.   
- 또한 분자는 무위험수익률이 아닌 “펀드 평균수익률 − 동일유형 평균수익률”을 사용합니다.   
- 따라서 RRAR은 무위험수익률을 그대로 두지 않고, 위험 측면에서는 상대위험(동일유형 초과수익률의 변동성)을 사용한다고 볼 수 있습니다.
```
