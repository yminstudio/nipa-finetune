...
## Question
RRAR의 산출식에서 무위험수익률 대신 사용하는 값은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "RRAR의 산출식에서 무위험수익률 대신 사용하는 값은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "rrar_criteria_calc_basis"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "8) 상대위험 조정후 수익률 ; Relative Risk Adjusted Return(RRAR)",
  "question": "RRAR의 산출식에서 무위험수익률 대신 사용하는 값은 무엇인가?",
...
## Assistant Response
```text
RRAR은 샤프지수처럼 위험을 고려한 “상대적” 수익성과를 보기 위한 지표입니다. 

- RRAR의 산출식에서 무위험수익률 대신 사용하는 값은 펀드전체의 평균수익률입니다. 
- 문서의 기호로는 펀드 주간 로그수익률 평균인 \overline{R^F} (연환산 포함 값)입니다.
```
