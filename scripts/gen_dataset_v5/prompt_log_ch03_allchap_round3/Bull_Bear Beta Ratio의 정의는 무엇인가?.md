...
## Question
Bull/Bear Beta Ratio의 정의는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Bull/Bear Beta Ratio의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "11) Bull/Bear Beta Ratio",
  "question": "Bull/Bear Beta Ratio의 정의는 무엇인가?",
...
## Assistant Response
```text
- Bull/Bear Beta Ratio는 상승베타(Bull Beta)를 하락베타(Bear Beta)로 나눈 값입니다.   
- 수식은 BR^{F,M} = BL_B^{F,M} / BR_B^{F,M} 입니다.   
- 값이 클수록 상승장에서는 상대적으로 변동폭이 큰 펀드이고, 값이 작을수록 하락장에서는 변동폭이 큰 펀드입니다.
```
