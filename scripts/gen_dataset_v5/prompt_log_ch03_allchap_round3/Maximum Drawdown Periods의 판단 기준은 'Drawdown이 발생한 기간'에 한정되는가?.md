...
## Question
Maximum Drawdown Periods의 판단 기준은 'Drawdown이 발생한 기간'에 한정되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Drawdown Periods의 판단 기준은 'Drawdown이 발생한 기간'에 한정되는가?",
  "qa_type": "criteria",
  "question_template": "criteria_question_template_01"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "16) Maximum Drawdown Periods",
  "question": "Maximum Drawdown Periods의 판단 기준은 'Drawdown이 발생한 기간'에 한정되는가?",
...
## Assistant Response
```text
Maximum Drawdown Periods는 “최대손실 구간이 얼마나 오래 지속됐는지”를 판단하기 위한 지표입니다. 

- 판단 기준은 Maximum Drawdown이 발생한 구간의 시작일과 종료일을 잡는 데에 한정됩니다.   
- 따라서 기간 산정은 Maximum Drawdown의 Start Date~End Date(발생 기간) 사이의 길이(주, 월)로 결정됩니다.   
- “Drawdown이 발생한 기간”을 벗어난 다른 구간을 추가로 포함하는 기준은 제시되어 있지 않습니다.
```
