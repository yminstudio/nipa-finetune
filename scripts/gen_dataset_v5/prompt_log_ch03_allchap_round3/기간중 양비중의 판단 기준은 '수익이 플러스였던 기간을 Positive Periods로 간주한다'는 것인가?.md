...
## Question
기간중 양비중의 판단 기준은 '수익이 플러스였던 기간을 Positive Periods로 간주한다'는 것인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "기간중 양비중의 판단 기준은 '수익이 플러스였던 기간을 Positive Periods로 간주한다'는 것인가?",
  "qa_type": "criteria",
  "question_template": "기준_양수수익간주"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "15) 기간중 양비중 ; Positive Periods",
  "question": "기간중 양비중의 판단 기준은 '수익이 플러스였던 기간을 Positive Periods로 간주한다'는 것인가?",
...
## Assistant Response
```text
기간중 양비중은 전체 기간에서 플러스 수익이 발생한 “횟수(기간)”가 얼마나 자주 나타났는지를 판단하기 위한 지표입니다. 

- 네, 기준의 핵심은 “수익이 플러스였던 기간(주간)”을 Positive Periods로 간주한다는 취지에 해당합니다.   
- 구체적으로 Positive Periods는 기간중 펀드의 주간 실현수익률 r_i^F가 r_i^F > 0 인 기간의 비중입니다.
```
