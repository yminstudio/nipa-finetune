...
```json
{
  ...
  "target_total": 100,
  "generated_question": "정보비율(IR)의 정의를 한 문장으로 서술하시오.",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 정보비율 ; Information Ratio(IR)",
  "question": "정보비율(IR)의 정의를 한 문장으로 서술하시오.",
...
## Assistant Response
```text
- 정보비율(IR)은 적극적인 투자활동의 결과 나타나는 초과수익률을 적극적인 활동에 따른 수익률의 표준편차로 나눈 비율로, Reward-to-Variability Ratio(RVR)라고도 한다.   
- 펀드의 벤치마크를 가정하는 경우 IR은 IR^{F,M} = ( R̄^F − R̄^M ) / TE^{F,M} 로 표시한다.
```
