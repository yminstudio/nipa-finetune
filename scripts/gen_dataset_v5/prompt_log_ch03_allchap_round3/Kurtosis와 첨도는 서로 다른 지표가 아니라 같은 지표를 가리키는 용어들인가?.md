...
## Question
Kurtosis와 첨도는 서로 다른 지표가 아니라 같은 지표를 가리키는 용어들인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Kurtosis와 첨도는 서로 다른 지표가 아니라 같은 지표를 가리키는 용어들인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 첨도 ; Kurtosis",
  "question": "Kurtosis와 첨도는 서로 다른 지표가 아니라 같은 지표를 가리키는 용어들인가?",
...
## Assistant Response
```text
- 제로인 방법론에서는 “첨도 ; Kurtosis”처럼 한 쌍의 표기로 제시되어, 첨도와 Kurtosis가 같은 지표(같은 척도)를 가리키는 용어로 사용됩니다.   
- 즉 수익률 분포가 최빈값 주변에 더 몰려(정점이 뾰족) 있는지, 아니면 더 퍼져(정점이 평평) 있는지를 설명하는 Kurtosis 값 자체를 첨도 관점에서 해석하는 구조입니다.
```
