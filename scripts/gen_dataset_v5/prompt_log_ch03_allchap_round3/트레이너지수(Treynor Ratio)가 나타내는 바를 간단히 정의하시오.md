...
## Question
트레이너지수(Treynor Ratio)가 나타내는 바를 간단히 정의하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "트레이너지수(Treynor Ratio)가 나타내는 바를 간단히 정의하시오.",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) 트레이너지수 ; Treynor Ratio",
  "question": "트레이너지수(Treynor Ratio)가 나타내는 바를 간단히 정의하시오.",
...
## Assistant Response
```text
- 트레이너지수(Treynor Ratio)는 펀드의 체계적 위험 1단위당 무위험 초과수익률을 나타내는 위험조정 성과지표입니다.   
- 샤프지수가 총위험(표준편차)을 쓴 것과 달리, 트레이너지수는 분산투자로 줄일 수 있다는 관점에서 비체계적 위험은 제외하고 체계적 위험인 베타를 사용합니다.   
- 제로인 방법론 기준의 정의식은 S^F = (평균 펀드수익률 − 평균 무위험이자율) / 베타(펀드-시장) 입니다.
```
