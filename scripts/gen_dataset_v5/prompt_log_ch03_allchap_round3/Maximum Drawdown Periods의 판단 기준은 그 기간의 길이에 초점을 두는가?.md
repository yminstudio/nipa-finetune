...
## Question
Maximum Drawdown Periods의 판단 기준은 그 기간의 길이에 초점을 두는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Maximum Drawdown Periods의 판단 기준은 그 기간의 길이에 초점을 두는가?",
  "qa_type": "criteria",
  "question_template": "criteria_question_template_02"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "16) Maximum Drawdown Periods",
  "question": "Maximum Drawdown Periods의 판단 기준은 그 기간의 길이에 초점을 두는가?",
...
## Assistant Response
```text
- Maximum Drawdown Periods는 최대낙폭(Maximum Drawdown)이 나타난 ‘구간’을 얼마나 오래(길게) 지속했는지를 판단하기 위한 지표입니다.   
- 판단 기준은 Maximum Drawdown Start Date와 Maximum Drawdown End Date 사이의 기간(주, 월)입니다.   
- 즉, 구간의 길이가 길수록 Maximum Drawdown Periods 값이 커집니다(예: 6주 - 3주 = 3주).
```
