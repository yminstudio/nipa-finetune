...
## Question
Average Gain의 판단 기준에서 이익이 음수일 경우의 처리가 문서에 명시되어 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Average Gain의 판단 기준에서 이익이 음수일 경우의 처리가 문서에 명시되어 있는가?",
  "qa_type": "criteria",
  "question_template": "criteria_negative_gain_handling"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "10) 평균이익 ; Average Gain",
  "question": "Average Gain의 판단 기준에서 이익이 음수일 경우의 처리가 문서에 명시되어 있는가?",
...
## Assistant Response
```text
Average Gain은 펀드가 “이익을 볼 경우”의 평균 주간 이익을 판단하기 위한 기준입니다.   
Avg Gain 계산식에서 각 주의 수익률 r_i^F는 max(r_i^F, 0)을 사용하므로, 이익이 음수(r_i^F<0)인 경우에는 해당 값이 0으로 처리되어 평균에 음수로 반영되지 않습니다.
```
