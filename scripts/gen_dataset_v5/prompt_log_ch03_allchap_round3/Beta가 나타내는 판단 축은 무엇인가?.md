...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Beta가 나타내는 판단 축은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_axis_definition"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) 베타 ; Beta",
  "question": "Beta가 나타내는 판단 축은 무엇인가?",
...
## Assistant Response
```text
베타(Beta)는 시장수익률 변화에 대한 펀드수익률의 민감도(상호변동관계)를 판단하기 위한 축입니다. 

- β=1: 시장 수익률과 동일한 민감도를 가짐   
- β>1: 시장 수익률보다 민감하게 움직임(위험이 큼)   
- 시장이 1% 변할 때 펀드수익률이 몇 % 변하는지를 나타내는 지표로 해석됨(시장=KOSPI200)
```
