...
## Question
Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것은 VaR 계산의 어떤 판단 기준에 해당하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것은 VaR 계산의 어떤 판단 기준에 해당하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_VaR_정규분포가정_판단기준"
}
```
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것은 VaR 계산의 어떤 판단 기준에 해당하는가?",
...
## Assistant Response
```text
- VaR 계산에서 “얼마나 큰 손실을 어느 수준에서 볼 것인가(유의수준/신뢰수준)”를 정하기 위한 통계적 전제에 해당합니다.  
- Delta Normal Method에서는 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정합니다.   
- 이 정규분포 가정은 VaR 산출식에서 유의수준(α)이 적용될 때 사용하는 분포(정규성) 가정에 해당합니다.
```
