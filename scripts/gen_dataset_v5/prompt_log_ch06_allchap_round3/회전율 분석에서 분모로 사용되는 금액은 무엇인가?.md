...
```json
{
  ...
  "target_total": 100,
  "generated_question": "회전율 분석에서 분모로 사용되는 금액은 무엇인가?",
  "qa_type": "definition",
  "question_template": "def_template_회전율_분모"
}
```
...
[핵심 명사]
. 회전율 분석, 일별로 발생한 주식의 매매 내역이 있을 경우, 회전율, 분석, 순자산에, 대한, 주식매매의, 빈번한, 정도를, 나타내는, 지표로서, 결제액의
