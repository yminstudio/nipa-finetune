...
## Question
Delta Normal Method의 특징에서 수익률은 어떤 분포를 갖는다고 가정하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징에서 수익률은 어떤 분포를 갖는다고 가정하는가?",
  "qa_type": "definition",
  "question_template": "DeltaNormal_Definition_1"
}
```
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징에서 수익률은 어떤 분포를 갖는다고 가정하는가?",
...
## Assistant Response
```text
Delta Normal Method에서는 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정합니다.
```
