...
## Question
VaR 계산식에서 alpha와 Significance의 관계를 비교하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 alpha와 Significance의 관계를 비교하시오.",
  "qa_type": "comparison",
  "question_template": "comparison_template_1"
}
```
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 alpha와 Significance의 관계를 비교하시오.",
...
## Assistant Response
```text
VaR 계산식에서 α는 유의수준(Level of Significance)으로 정의되므로, α와 Significance(유의수준)는 같은 의미로 쓰입니다.   
또한 문서에서는 (1−α)×100을 신뢰수준으로 함께 제시하므로, α(유의수준)가 커질수록 신뢰수준은 (1−α)만큼 낮아지는 관계입니다.
```
