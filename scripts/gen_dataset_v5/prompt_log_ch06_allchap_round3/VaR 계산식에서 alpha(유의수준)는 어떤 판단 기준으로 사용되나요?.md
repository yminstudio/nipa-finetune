...
## Question
VaR 계산식에서 alpha(유의수준)는 어떤 판단 기준으로 사용되나요?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 alpha(유의수준)는 어떤 판단 기준으로 사용되나요?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 alpha(유의수준)는 어떤 판단 기준으로 사용되나요?",
...
## Assistant Response
```text
VaR 계산식에서 alpha(α)는 VaR이 어느 정도의 “신뢰도 수준”을 기준으로 위험을 산출할지를 정하는 판단 기준입니다.   

제로인 방법론에서는 α를 유의수준(Level of Significance)으로 두며, 동시에 (1-α)×100을 신뢰수준으로 해석합니다.
```
