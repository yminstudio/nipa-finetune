...
## Question
KOSPI 변동성의 측정 방법에서 Exponentially Weighted Moving Average 용어가 사용되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "KOSPI 변동성의 측정 방법에서 Exponentially Weighted Moving Average 용어가 사용되는가?",
  "qa_type": "criteria",
  "question_template": "criteria_EWMA_term_usage"
}
```
...
```json
{
  ...
  "subsection": "[KOSPI 변동성의 측정 방법]",
  "seed_title": "[KOSPI 변동성의 측정 방법]",
  "question": "KOSPI 변동성의 측정 방법에서 Exponentially Weighted Moving Average 용어가 사용되는가?",
...
## Assistant Response
```text
KOSPI 변동성의 측정 방법에서 ‘Exponentially Weighted Moving Average(이하 EWMA)’ 용어가 실제로 사용되는지 여부를 기준으로 판단합니다.  
제로인 방법론에서는 KOSPI 변동성의 측정 방법으로 EWMA (Exponentially Weighted Moving Average)를 사용한다고 명시합니다 .  
또한 EWMA 기반의(지수 가중을 반영하는) 변동성(분산) 산출 식으로 제시됩니다 .
```
