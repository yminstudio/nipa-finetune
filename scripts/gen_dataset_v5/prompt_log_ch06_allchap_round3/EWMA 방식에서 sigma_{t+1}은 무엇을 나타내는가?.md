...
## Question
EWMA 방식에서 sigma_{t+1}은 무엇을 나타내는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "EWMA 방식에서 sigma_{t+1}은 무엇을 나타내는가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "[KOSPI 변동성의 측정 방법]",
  "seed_title": "[KOSPI 변동성의 측정 방법]",
  "question": "EWMA 방식에서 sigma_{t+1}은 무엇을 나타내는가?",
...
## Assistant Response
```text
EWMA 방식에서 $\\sigma_{t+1}$(식에서는 $\\sigma^2_{t+1|t}$로 나타남)은 “다음 시점($t+1$)의 변동성”을 추정하기 위한 값으로, $t$ 시점까지의 수익률 제곱($r_t^2$)을 지수 가중으로 반영해 계산한 **조건부 분산 추정치**를 뜻합니다.
```
