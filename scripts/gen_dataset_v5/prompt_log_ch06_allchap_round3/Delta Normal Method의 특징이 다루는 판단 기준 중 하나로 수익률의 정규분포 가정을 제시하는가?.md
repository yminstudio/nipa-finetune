...
## Question
Delta Normal Method의 특징이 다루는 판단 기준 중 하나로 수익률의 정규분포 가정을 제시하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징이 다루는 판단 기준 중 하나로 수익률의 정규분포 가정을 제시하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_DeltaNormal_정규분포가정_판단기준"
}
```
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징이 다루는 판단 기준 중 하나로 수익률의 정규분포 가정을 제시하는가?",
...
## Assistant Response
```text
Delta Normal Method는 VaR(Value at Risk) 산출을 위한 가정을 제시합니다.   

- 그 특징 중 하나로, 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정합니다.
```
