...
## Question
Delta Normal Method의 특징에서 KOSPI 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준으로 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징에서 KOSPI 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준으로 포함되는가?",
  "qa_type": "criteria",
  "question_template": "criteria_DeltaNormal_KOSPI_정규분포_판단기준"
}
```
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징에서 KOSPI 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준으로 포함되는가?",
...
## Assistant Response
```text
Delta Normal Method로 VaR를 산출할 때 적용하는 핵심 전제(특징)가 무엇인지 판단하기 위한 기준입니다. 

- Delta Normal Method의 특징에 “기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정”이 포함됩니다. 
- 따라서 “KOSPI 수익률이 정규분포를 갖는다고 가정”하는 내용은 판단 기준(특징)으로 포함됩니다.
```
