...
## Question
문서의 Where 표기에서 alpha의 의미는 어떤 판단 기준으로 해석되나요?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "문서의 Where 표기에서 alpha의 의미는 어떤 판단 기준으로 해석되나요?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "문서의 Where 표기에서 alpha의 의미는 어떤 판단 기준으로 해석되나요?",
...
## Assistant Response
```text
alpha(α)는 VaR(Value at Risk) 산출에서 “얼마나 신뢰할 수 있는(손실구간의 상단을 어느 수준에서 볼지)”를 정하기 위한 값입니다.   
- Where에서 α는 유의수준(Level of Significance)으로 해석합니다.   
- 또는 (1−α)×100을 신뢰수준으로 해석합니다.
```
