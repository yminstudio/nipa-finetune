...
## Question
Top10 종목 집중도 계산을 시작할 때 먼저 고려해야 할 순서는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 계산을 시작할 때 먼저 고려해야 할 순서는 무엇인가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 계산을 시작할 때 먼저 고려해야 할 순서는 무엇인가?",
...
```text
- 사모펀드는 제외한 뒤   
- 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드 중 FoFs, 자펀드를 제외한 펀드로 정하고, 이 펀드를 집중도 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 설정   
- 이렇게 정한 기준값을 펀드규모 관계없이 전펀드에 적용   
- 적용 전환 규칙: 상한값보다 많은(>) 집중수를 가진 펀드는 ‘평균이상’, 상한~하한에 해당하는 펀드는 ‘평균수준’, 하한값보다 적은(<) 펀드는 ‘평균이하’
```
...
## Assistant Response
```text
Top10 종목 집중도 값을 구할 때는 먼저 “기준점”을 정한 뒤, 그 기준으로 각 펀드를 구간에 배치하는 순서로 진행합니다 .

- 적용 순서 1: 사모펀드를 제외하고, 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드( FoFs, 자펀드 제외)로 한정 
- 적용 순서 2: 기준점 계산 대상을 내림차순 정렬한 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 ‘집중도’를 기준점으로 설정 
- 적용 순서 3: 이렇게 정한 기준값을 펀드 규모와 무관하게 전 펀드에 적용 
- 전환 규칙: 상한값보다 많은(>) 집중수면 ‘평균이상’, 상한~하한이면 ‘평균수준’, 하한값보다 적은(<) 집중수면 ‘평균이하’
```
...
## Assistant Response
```text
Top10 종목 집중도 “기준점(레벨 분류용)”을 먼저 정하기 위한 시작 순서는 아래와 같습니다.   
- 사모펀드를 제외한 뒤, 기준점 계산에 쓰일 펀드 집합을 정합니다(순자산액 100억 이상 평가 공모펀드: FoFs, 자펀드 제외).   
- 해당 집합에서 Top10 종목 집중도를 내림차순 정렬하고, 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 각각 기준점으로 확정합니다.   
- 확정된 기준값은 펀드 규모 관계없이 전 펀드에 적용합니다.   
- 전환(분류) 규칙은 집중수 기준으로 결정합니다: 상한값보다 많은(>) 집중수면 ‘평균이상’, 상한~하한 구간이면 ‘평균수준’, 하한값보다 적은(<) 집중수면 ‘평균이하’.
```
...
## Assistant Response
```text
Top10 종목 집중도는 먼저 “기준값”을 만든 뒤, 그 기준값으로 개별 펀드를 분류하는 순서로 시작합니다. 

- 적용 순서: 기준값 산출 대상 펀드를 확정한다(사모펀드 제외). 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드이며, FoFs/자펀드는 제외한다.   
- 적용 순서: 기준값을 계산한다. 기준점 계산 대상 펀드를 Top10 종목 집중도(집중수) 기준 내림차순으로 정렬한 뒤, 33.1%(상한값)과 67.0%(하한값)에 해당하는 펀드의 집중도를 기준값으로 둔다.   
- 적용 조건: 이렇게 산출한 기준값은 펀드 규모 관계없이 전펀드에 적용한다.   
- 전환 규칙: 전펀드의 Top10 종목 집중수(집중도)를 기준값과 비교해 등급을 전환한다. 상한값보다 많은(>) 경우 ‘평균이상’, 상한~하한 구간이면 ‘평균수준’, 하한값보다 적은(<) 경우 ‘평균이하’로 분류한다.
```
