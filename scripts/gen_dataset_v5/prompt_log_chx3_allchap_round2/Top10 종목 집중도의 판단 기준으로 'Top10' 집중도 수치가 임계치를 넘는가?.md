...
## Question
Top10 종목 집중도의 판단 기준으로 'Top10' 집중도 수치가 임계치를 넘는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도의 판단 기준으로 'Top10' 집중도 수치가 임계치를 넘는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도의 판단 기준으로 'Top10' 집중도 수치가 임계치를 넘는가?",
...
## Assistant Response
```text
- 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외) 대상으로 내림차순 정렬 후 33.1%(상한값)에 해당하는 펀드의 Top10 종목 집중도를 임계치로 둔다   
- 해당 펀드의 Top10 종목 집중도 수치가 상한값보다 큰(>) 경우 임계치를 넘는 것으로 판단한다(‘평균이상’)
```
...
## Assistant Response
```text
Top10 종목 집중도 기준은 펀드의 ‘Top10’ 집중도 수치가 동종 펀드 분포에서 어느 수준 이상인지(임계치 초과 여부) 판단하기 위한 것입니다. 

- 임계치(기준점)는 내림차순 정렬 후 33.1%(상한값)에 해당하는 집중도를 사용합니다. 
- 해당 펀드의 Top10 집중도 수치가 상한값보다 큰(>) 경우 “임계치를 넘는다”로 판단합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도가 펀드의 집중도 “임계치”를 초과하는지(= 평균이상 해당 여부)를 판단하기 위한 기준입니다. 

- 임계치(상한값)는 Top10 종목 집중도를 기준점 산출 대상 펀드에서 기준점으로 잡은 33.1% 지점의 집중도(“상한값”)입니다. 
- Top10 집중도 수치가 상한값보다 큰 경우(>), 임계치를 넘는 것으로 판단합니다(“평균이상”). 
- 참고로 상한~하한 사이이면 임계치를 넘었다고 보지 않으며(“평균수준”), 하한보다 작으면에도 임계치 미초과로 봅니다.
```
...
## Assistant Response
```text
Top10 종목 집중도 수치가 “임계치(상한)”를 넘는지 여부는, 펀드의 집중도가 기준점 대비 높은 수준인지 판단하기 위한 기준입니다.   
- 기준점(임계치 상한값)은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)를 내림차순 정렬한 뒤 33.1%(상한값)에 해당하는 펀드의 Top10 집중도로 정합니다.   
- Top10 집중도 수치가 이 상한값보다 큰(>) 경우 “임계치를 넘는” 것으로 판단합니다.
```
