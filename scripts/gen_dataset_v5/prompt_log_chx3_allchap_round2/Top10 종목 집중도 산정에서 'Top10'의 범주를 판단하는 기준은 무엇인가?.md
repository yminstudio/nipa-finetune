...
## Question
Top10 종목 집중도 산정에서 'Top10'의 범주를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 산정에서 'Top10'의 범주를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_basic"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 산정에서 'Top10'의 범주를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- Top10 종목 집중도 ‘범주’는 기준점으로 설정한 상한값(33.1%)·하한값(67.0%)과의 비교로 나뉜다.   
- 기준점은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)의 Top10 종목 집중도를 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도로 산정한다.   
- 적용 결과는 (상한값보다 많은 집중수) ‘평균이상’, (상한~하한) ‘평균수준’, (하한값보다 적은 집중수) ‘평균이하’로 구분한다.   
- 사모펀드는 Top10 종목 집중도 산정에서 제외한다.
```
...
## Assistant Response
```text
Top10 종목 집중도에서 ‘Top10’의 범주(평균이상/평균수준/평균이하)를 가르는 기준은 특정 구간의 집중도 임계값입니다.   
- 사모펀드는 제외하고, 기준점(임계값) 산정은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)를 대상으로 합니다.   
- Top10 종목 집중도를 내림차순 정렬한 뒤, 33.1%(상한값)과 67.0%(하한값)에 해당하는 집중도를 기준점으로 둡니다.   
- 기준 적용 시 집중도가 상한값보다 큰 경우 ‘평균이상’, 상한~하한이면 ‘평균수준’, 하한값보다 작으면 ‘평균이하’로 범주를 판단합니다.
```
...
## Assistant Response
```text
‘Top10 종목 집중도’의 범주(평균이상/평균수준/평균이하)를 판단하기 위한 기준은, 펀드의 Top10 종목 집중도(집중수)가 기준점(분위 구간의 집중도)보다 큰지/같은지/작은지 여부입니다. 

- 기준점은 순자산액 100억 이상 평가 공모펀드( FoFs, 자펀드 제외 )를 내림차순 정렬한 뒤, 33.1%(상한값), 67.0%(하한값)에 해당하는 ‘집중도’를 사용합니다. 
- 집중수가 상한값보다 많은(>) 경우 ‘평균이상’, 상한~하한 구간은 ‘평균수준’, 하한값보다 적은(<) 경우 ‘평균이하’로 범주를 정합니다. 
- 기준값은 펀드 규모 관계없이 전 펀드에 동일하게 적용합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도에서 ‘Top10’의 범주는 펀드의 Top10 집중도 수준이 기준 분포의 상/중/하 어느 구간에 해당하는지로 판단합니다. 

- 사모펀드는 제외하고, 기준점 산정은 순자산액 100억 이상 평가 공모펀드 중 FoFs와 자펀드를 제외한 펀드의 집중도를 사용합니다. 
- 해당 펀드들을 집중도 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도를 기준점으로 합니다. 
- 집중도가 상한값보다 크면 ‘평균이상’, 상한~하한이면 ‘평균수준’, 하한값보다 작으면 ‘평균이하’로 범주를 나눕니다.
```
