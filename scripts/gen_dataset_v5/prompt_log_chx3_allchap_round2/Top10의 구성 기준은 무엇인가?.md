...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10의 구성 기준은 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
Top10(Top10 종목 집중도)의 구성 기준은 “집중도(집중수)를 기준점 대비로 구분”하는 방식입니다. 

- 사모펀드를 제외하고, 기준점 산출은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)에서 합니다. 
- 기준점은 해당 펀드들을 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도로 둡니다. 
- 적용은 각 펀드의 집중수를 상한값 초과(평균이상), 상한~하한(평균수준), 하한값 미만(평균이하)으로 구분합니다.
```
...
## Assistant Response
```text
Top10(Top10 종목 집중도)은 펀드의 “Top10 종목 집중도” 수준을 ‘평균이상/평균수준/평균이하’로 구분하기 위한 기준입니다. 

- 사모펀드는 제외하며, 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)입니다.   
- 해당 대상의 집중도를 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 둡니다.   
- 기준값은 펀드규모와 무관하게 전펀드에 적용하며, 상한값보다 많은(>) 집중수는 ‘평균이상’, 상한~하한은 ‘평균수준’, 하한값보다 적은(<) 집중수는 ‘평균이하’로 분류합니다.
```
...
## Assistant Response
```text
Top10의 “구성 기준”은 Top10 종목 집중도를 등급화(평균이상/평균수준/평균이하)하기 위한 산출·적용 대상과 기준점 구간으로 정의됩니다. 

- 사모펀드는 제외하고, 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)입니다. 
- 기준점은 해당 펀드들의 집중도를 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 “집중도”를 사용합니다. 
- 이렇게 정한 기준값은 펀드 규모와 무관하게 전 펀드에 적용하며, 상한값보다 많은(>) 집중수는 ‘평균이상’, 상한~하한은 ‘평균수준’, 하한값보다 적은(<) 집중수는 ‘평균이하’로 분류합니다.
```
...
## Assistant Response
```text
Top10의 “구성 기준”은 Top10 종목 집중도(상위 10개 종목의 집중 정도)를 기준점과 구간으로 나누어 정하는 방식입니다. 

- 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드이며, FoFs와 자펀드는 제외하고 사모펀드는 제외합니다. 
- 내림차순 정렬 후 상위 33.1%(상한값), 상위 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 삼습니다. 
- 집중수 기준으로 상한값보다 많은 경우 ‘평균이상’, 상한~하한은 ‘평균수준’, 하한값보다 적은 경우 ‘평균이하’로 구분합니다.
```
