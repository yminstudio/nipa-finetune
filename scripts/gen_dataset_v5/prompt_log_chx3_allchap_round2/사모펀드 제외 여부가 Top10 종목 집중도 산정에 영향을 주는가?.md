...
## Question
사모펀드 제외 여부가 Top10 종목 집중도 산정에 영향을 주는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "사모펀드 제외 여부가 Top10 종목 집중도 산정에 영향을 주는가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "사모펀드 제외 여부가 Top10 종목 집중도 산정에 영향을 주는가?",
...
## Assistant Response
```text
- 제로인 방법론의 Top10 종목 집중도는 사모펀드를 제외하고 산정한다.   
- 따라서 사모펀드 “제외” 여부는 Top10 종목 집중도 산정에 반영된다(사모펀드는 산정 대상에서 제외됨).
```
...
## Assistant Response
```text
- Top10 종목 집중도 산정에서는 “사모펀드 제외”가 전제조건으로 명시돼 있어, 사모펀드는 집중도 산정(및 이후 기준점 설정) 과정에 반영되지 않습니다.   
- 또한 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)로 한정되므로, 사모펀드 제외는 기준점 도출에 직접적으로 영향을 줍니다.   
- 이렇게 산정된 기준값은 펀드 규모와 무관하게 전펀드에 적용되기 때문에, 기준점이 “사모펀드가 제외된 분포”를 바탕으로 결정된다는 점에서 영향이 발생합니다.
```
...
## Assistant Response
```text
- 제로인의 Top10 종목 집중도 산정은 사모펀드를 제외한 상태에서 진행된다.   
- 또한 기준점(집중도 ‘평균이상/수준/이하’ 구분)을 정할 때도 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)를 대상으로 한다.   
- 따라서 사모펀드 제외 여부는 산정 대상이 달라지는 요소이므로, 사모펀드를 포함해 계산하면 제로인의 동일한 기준에 따른 Top10 종목 집중도 결과가 달라질 수 있다.
```
...
## Assistant Response
```text
- 제로인의 Top10 종목 집중도 산정은 “사모펀드 제외”가 전제이며, 기준점 계산 대상도 순자산 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)로 한정됩니다   
- 따라서 사모펀드를 산정에 “포함”할 경우, 제로인 방식의 산정 대상 정의와 달라져 집중도 산정 결과가 달라질 수 있습니다(방법론은 제외로 고정).
```
