...
## Question
억이상 펀드의 제외 여부와 Top10 종목 집중도 산정의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "억이상 펀드의 제외 여부와 Top10 종목 집중도 산정의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "억이상 펀드의 제외 여부와 Top10 종목 집중도 산정의 차이는 무엇인가?",
...
## Assistant Response
```text
- 억이상 펀드의 제외 여부: 순자산액 100억이상 평가 공모펀드(FoFs, 자펀드 제외)만 기준점 계산 대상이 되고, 그 기준값은 펀드규모 관계없이 전펀드에 적용됩니다.   
- Top10 종목 집중도 산정: 사모펀드는 제외하며, 기준점은 ‘100억이상 평가 공모펀드(FoFs, 자펀드 제외)’의 Top10 종목 집중도를 33.1%(상한), 67.0%(하한)로 구해 설정합니다.   
- 따라서 “억이상 제외 여부”는 기준점 산정에 쓰는 펀드 범위(대상 한정)이고, “Top10 종목 집중도 산정”은 그 범위에서 집중도를 계산해 평균이상/수준/이하로 나누는 데 쓰입니다.
```
...
## Assistant Response
```text
- 공통 맥락으로는 둘 다 “순자산액 100억 이상” 기준으로 기준점(33.1% 상한값, 67.0% 하한값)을 정하고, 기준은 펀드규모 관계없이 전펀드에 적용합니다(종목수: 33.1/67 구간, Top10 집중도: 33.1/67 구간).    
- 100억이상 펀드(종목수 기준)의 제외 범위는 “평가 공모펀드 중 FoFs, 자펀드 제외”로만 제시됩니다.   
- Top10 종목 집중도는 위 제외(평가 공모펀드 중 FoFs, 자펀드 제외)에 더해 “사모펀드 제외”가 추가로 명시됩니다. 즉, Top10 산정에서 제외 범위가 더 넓습니다.
```
...
## Assistant Response
```text
- 100억 이상 펀드는 ‘기준점 계산 대상’으로만 쓰이며(순자산액 100억 이상 평가 공모펀드), 그 기준값은 펀드 규모와 무관하게 전 펀드에 적용되어 최종 단계에서 100억 미만이 자동 제외되지는 않습니다.   
- Top10 종목 집중도는 추가로 ‘사모펀드 제외’가 있고, 기준점 계산도 ‘순자산액 100억 이상 평가 공모펀드에서 FoFs·자펀드 제외’된 집합의 Top10 집중도를 기준점으로 삼습니다.   
- 정리하면, 둘 다 100억 이상을 기준점 형성용 표본으로 쓰지만, Top10 집중도 산정에서는 사모펀드 및 FoFs·자펀드가 더 포함되지 않습니다.
```
...
## Assistant Response
```text
- 100억 이상 펀드의 “제외 여부”는 기준점 계산 시 순자산액 100억 이상 평가 공모펀드만을 대상으로 하며, FoFs와 자펀드는 제외합니다 .  
- Top10 종목 집중도 산정도 동일하게 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)로 두되 , 여기에 추가로 사모펀드를 제외합니다 .  
- 따라서 차이는 “무엇을 제외하느냐”에서 사모펀드 제외가 Top10에 더해지고, 산정 대상 지표는 100억 이상 기준점의 ‘집중도(Top10)’라는 점입니다 .
```
