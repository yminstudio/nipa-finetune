...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도의 정의는 무엇인가?",
...
## Assistant Response
```text
- Top10 종목 집중도는 사모펀드를 제외한 펀드에서의 ‘Top10 종목’ 집중도를 뜻하며, 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)의 집중도를 대상으로 산출한다 .  
- 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 삼는다 .
```
...
## Assistant Response
```text
- Top10 종목 집중도는 평가대상 펀드들을 ‘집중도’ 값 기준으로 비교·분류하기 위해 사용하는 지표입니다.   
- 제로인 방법론에서는 이 집중도에 대해 순자산 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)를 대상으로 집중도를 내림차순 정렬하고, 33.1%(상한값)·67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 삼습니다.
```
...
## Assistant Response
```text
- Top10 종목 집중도는 펀드의 집중도를 수치로 나타내는 지표로, “Top10(상위 10개) 종목”에 해당하는 집중도를 의미합니다.   
- 제로인 방법론에서는 이 “집중도” 값을 공모펀드(사모펀드 제외)에 적용해 기준점 설정에 활용합니다.
```
...
## Assistant Response
```text
- Top10 종목 집중도는 펀드의 상위 10개 종목 집중 정도를 수치화한 “집중도”로, 기준점 산정 시 집중도를 내림차순으로 정렬해 사용합니다.   
- 기준점 계산 대상은 사모펀드를 제외한 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)입니다.   
- 기준점은 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 두며, 상한값보다 큰(>) 집중수는 ‘평균이상’, 상한~하한은 ‘평균수준’, 하한값보다 작은(<) 집중수는 ‘평균이하’로 적용합니다.
```
