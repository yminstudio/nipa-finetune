...
## Question
Top10 종목 집중도 산정에서 제외 대상으로 명시된 범주를 판단 기준으로 삼는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 산정에서 제외 대상으로 명시된 범주를 판단 기준으로 삼는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 산정에서 제외 대상으로 명시된 범주를 판단 기준으로 삼는가?",
...
## Assistant Response
```text
- 예. Top10 종목 집중도 기준점 계산에서 사모펀드는 제외한다.   
- 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드로 한정하며, FoFs와 자펀드는 제외한다.
```
...
## Assistant Response
```text
Top10 종목 집중도에서 “제외 대상으로 명시된 범주”는 등급을 가르는 비교 기준이라기보다, 해당 지표 산정의 대상에서 빼는 필터로 쓰입니다. 

- 제외 대상 범주: 사모펀드   
- 실제 판단(등급) 기준: 100억이상 평가 공모펀드의 Top10 종목 집중도 기준점(상한 33.1%, 하한 67.0%)과 비교해 “평균이상/평균수준/평균이하”를 구분
```
...
## Assistant Response
```text
Top10 종목 집중도에서 “제외 대상으로 명시된 범주”는 집중도 수준(평균이상/수준/이하)을 나누는 기준이 아니라, 집중도 산정(특히 기준점 산정)의 대상 여부를 가르는 필터로 사용됩니다. 

- 제외: 사모펀드   
- 기준점 계산 대상 한정 기준: 순자산액 100억 이상 평가 공모펀드 중 FoFs와 자펀드는 제외
```
...
## Assistant Response
```text
Top10 종목 집중도에서 “제외 대상”은 집중도 수준을 구분하는 판단 기준이 아니라, 애초에 산정 대상에서 빼기 위한 범주이다. 

- 사모펀드는 Top10 종목 집중도 산정에서 제외한다.   
- 판단(분류) 기준은 제외 대상이 아니라, 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외) 집중도를 기준점으로 삼는다.   
- 기준점(33.1% 상한, 67.0% 하한) 대비로 집중수(또는 집중도)를 “평균이상/평균수준/평균이하”로 구분한다.
```
