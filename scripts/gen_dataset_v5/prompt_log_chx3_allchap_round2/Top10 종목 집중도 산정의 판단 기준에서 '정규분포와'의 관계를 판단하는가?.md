...
## Question
Top10 종목 집중도 산정의 판단 기준에서 '정규분포와'의 관계를 판단하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 산정의 판단 기준에서 '정규분포와'의 관계를 판단하는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 산정의 판단 기준에서 '정규분포와'의 관계를 판단하는가?",
...
## Assistant Response
```text
- Top10 종목 집중도 산정의 판단 기준에는 “100억이상 펀드의 경우 정규분포와 유사”라는 전제가 명시돼 있습니다.   
- 다만 ‘정규분포와의 관계’를 별도로 추가 판정하는 기준은 제시돼 있지 않으며, ‘평균이상/평균수준/평균이하’ 구분은 집중도를 33.1%(상한), 67.0%(하한) 기준점과 비교해 정합니다.
```
...
## Assistant Response
```text
- Top10 종목 집중도를 ‘평균이상/평균수준/평균이하’로 구분하기 위한 판단 기준입니다.   
- 이 판단 기준에서 ‘정규분포와의 관계’를 별도로 검증하거나 측정해 판정하지는 않고, “100억 이상 펀드의 경우 정규분포와 유사”하다는 전제를 둡니다.   
- 그 전제 하에 집중도 기준점은 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도로 설정되고, 상한값 초과는 평균이상, 상한~하한은 평균수준, 하한값 미만은 평균이하로 판단합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도는 집중도 수준을 ‘평균이상/평균수준/평균이하’로 나누기 위한 기준을 정하기 위한 것입니다.   
100억 이상 펀드의 경우 Top10 종목 집중도 기준점 산정에서 ‘정규분포와 유사’하다는 전제를 둡니다.   
이 전제 하에 상한/하한(33.1%, 67.0%)에 해당하는 집중도를 기준점으로 삼아 수준을 구분합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도 “수준(평균이상/평균수준/평균이하)”을 구분하기 위한 판단 기준은, 집중도 분포의 기준점을 정하는 데에 쓰입니다. 

100억 이상 펀드의 경우 Top10 종목 집중도 분포가 “정규분포와 유사”하다고 두고, 그에 따라 내림차순 정렬 후 33.1%(상한값)·67.0%(하한값)에 해당하는 집중도를 기준점으로 삼습니다.
```
