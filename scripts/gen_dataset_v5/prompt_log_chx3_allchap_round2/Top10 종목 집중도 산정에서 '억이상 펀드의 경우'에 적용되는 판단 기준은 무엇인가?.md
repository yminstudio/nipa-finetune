...
## Question
Top10 종목 집중도 산정에서 '억이상 펀드의 경우'에 적용되는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 산정에서 '억이상 펀드의 경우'에 적용되는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 산정에서 '억이상 펀드의 경우'에 적용되는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- Top10 종목 집중도는 100억이상 펀드의 경우 정규분포와 유사하게 보고, 기준점은 순자산액 100억이상 평가 공모펀드(FoFs, 자펀드 제외) 집중도를 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도로 설정한다   
- 판단 기준은 상한값보다 많은(>) 집중수를 ‘평균이상’, 상한~하한 집중수는 ‘평균수준’, 하한값보다 적은(<) 집중수는 ‘평균이하’로 구분한다
```
...
## Assistant Response
```text
Top10 종목 집중도를 ‘평균이상/평균수준/평균이하’ 중 어디에 해당하는지 판단하기 위한 기준입니다. 

- 100억 이상 펀드의 기준점은 사모펀드를 제외하고, 순자산액 100억 이상 평가 공모펀드 중 FoFs를 제외한 펀드의 Top10 종목 집중도를 대상으로 산정합니다.   
- 해당 펀드들을 집중도 내림차순으로 정렬한 뒤, 33.1%(상한값)와 67.0%(하한값)에 해당하는 집중도를 기준점으로 둡니다.   
- 판정은 집중도 기준으로 상한값 초과(>)이면 ‘평균이상’, 상한~하한이면 ‘평균수준’, 하한값 미만(<)이면 ‘평균이하’로 구분합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도를 100억이상 펀드의 집중도 분포 기준점과 비교해 ‘평균 이상/수준/이하’로 구분하기 위한 판단 기준입니다. 

- 기준점 산정: 순자산액 100억이상 평가 공모펀드(FoFs, 자펀드 제외)를 집중도 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도를 기준점으로 둡니다. 
- 적용 판단: 펀드의 집중수가 상한값보다 많은(>) 경우 ‘평균이상’, 상한~하한 사이인 경우 ‘평균수준’, 하한값보다 적은(<) 경우 ‘평균이하’로 분류합니다.
```
...
## Assistant Response
```text
Top10 종목 집중도에서 ‘100억 이상 펀드’는 집중도가 평균 대비 어느 수준인지(평균이상/평균수준/평균이하)를 판단하기 위한 기준을 적용합니다. 

- 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드이며, FoFs와 자펀드는 제외하고 사모펀드는 제외합니다.   
- 해당 펀드들을 집중도 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 합니다.   
- 상한값보다 큰(>) 집중수는 ‘평균이상’, 상한~하한 구간은 ‘평균수준’, 하한값보다 작은(<) 집중수는 ‘평균이하’로 판단합니다.
```
