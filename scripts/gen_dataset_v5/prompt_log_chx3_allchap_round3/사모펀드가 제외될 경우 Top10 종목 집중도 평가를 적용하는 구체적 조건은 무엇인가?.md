...
## Question
사모펀드가 제외될 경우 Top10 종목 집중도 평가를 적용하는 구체적 조건은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "사모펀드가 제외될 경우 Top10 종목 집중도 평가를 적용하는 구체적 조건은 무엇인가?",
  "qa_type": "application",
  "question_template": "Application_제외적용조건"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "사모펀드가 제외될 경우 Top10 종목 집중도 평가를 적용하는 구체적 조건은 무엇인가?",
...
## Assistant Response
```text
- (적용) Top10 종목 집중도 평가는 사모펀드를 제외한 상태에서 적용   
- (적용 조건) 기준점(상·하한값) 산출 및 기준분포는 “순자산액 100억 이상”인 평가 공모펀드(FoFs, 자펀드 제외)를 대상으로 하며, 100억 이상 펀드는 정규분포와 유사하다고 보고 기준점을 설정   
- (적용 순서/조건) 공모펀드를 Top10 종목 집중도 내림차순으로 정렬한 뒤 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도를 기준값으로 사용   
- (전환 규칙) 기준값은 펀드 규모와 무관하게 전(사모 제외) 펀드에 그대로 적용하고, 집중수가 상한값보다 많은(>) 경우 평균이상 / 상한~하한( ) 구간인 경우 평균수준 / 하한값보다 적은(<) 경우 평균이하로 구분
```
