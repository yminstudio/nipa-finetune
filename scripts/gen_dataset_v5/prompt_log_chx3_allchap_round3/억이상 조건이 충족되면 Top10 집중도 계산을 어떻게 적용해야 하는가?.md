...
## Question
억이상 조건이 충족되면 Top10 집중도 계산을 어떻게 적용해야 하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "억이상 조건이 충족되면 Top10 집중도 계산을 어떻게 적용해야 하는가?",
  "qa_type": "application",
  "question_template": "Application_억이상적용"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "억이상 조건이 충족되면 Top10 집중도 계산을 어떻게 적용해야 하는가?",
...
## Assistant Response
```text
- 적용 대상(억이상 조건 충족 시): 사모펀드를 제외하고, 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외) 중 Top10 종목 집중도를 측정한다.   
- 적용 순서(기준점 산정): 100억 이상 펀드를 Top10 집중도 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 펀드의 집중도를 기준점으로 설정한다.   
- 적용 조건(전 펀드 적용): 위의 상한/하한 기준값은 펀드 규모와 무관하게 전 펀드에 동일하게 적용한다.   
- 전환 규칙(등급 부여): 해당 펀드의 Top10 집중도가 상한값보다 큰(>) 경우 ‘평균이상’, 상한~하한 사이이면 ‘평균수준’, 하한값보다 작은(<) 경우 ‘평균이하’로 전환한다.
```
