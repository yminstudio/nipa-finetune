...
## Question
Top10 종목 집중도 산정이 정규분포를 전제로 하는지 여부를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도 산정이 정규분포를 전제로 하는지 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도 산정이 정규분포를 전제로 하는지 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
Top10 종목 집중도 산정에서 “정규분포 전제(정규분포와 유사)”가 적용되는 대상 여부를 가리기 위한 기준은 순자산 규모입니다. 

- 순자산액 100억 이상인 펀드의 Top10 종목 집중도는 “정규분포와 유사”하다고 보아 적용 기준을 설정합니다.   
- 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)입니다.   
- 이 대상 펀드의 집중도 기준점은 내림차순 정렬 후 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도를 기준값으로 삼아 “평균이상/평균수준/평균이하”를 구분합니다.
```
