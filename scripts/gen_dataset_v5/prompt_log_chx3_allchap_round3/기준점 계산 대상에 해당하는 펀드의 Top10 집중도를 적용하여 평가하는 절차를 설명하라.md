...
## Question
기준점 계산 대상에 해당하는 펀드의 Top10 집중도를 적용하여 평가하는 절차를 설명하라.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "기준점 계산 대상에 해당하는 펀드의 Top10 집중도를 적용하여 평가하는 절차를 설명하라.",
  "qa_type": "application",
  "question_template": "Application_기준점대상적용"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "기준점 계산 대상에 해당하는 펀드의 Top10 집중도를 적용하여 평가하는 절차를 설명하라.",
...
## Assistant Response
```text
- 적용 대상 설정: 사모펀드는 제외하고, 기준점 계산 대상은 순자산액 100억 이상 평가 공모펀드 중 FoFs 및 자펀드를 제외한 펀드의 Top10 종목 집중도로 한정한다.   
- 기준점 산출(상한/하한): 해당 펀드들을 Top10 종목 집중도 내림차순으로 정렬한 뒤, 33.1%(상한값)와 67.0%(하한값)에 해당하는 펀드의 집중도를 각각 상한값/하한값 기준으로 삼는다.   
- 기준값 적용 범위: 위 기준값(상한값/하한값)은 펀드규모와 무관하게 전 펀드에 적용한다.   
- 전환(등급화) 규칙: 각 펀드의 Top10 종목 집중도와 기준값을 비교해, 상한값보다 많은(>) 경우 ‘평균이상’, 상한~하한(상한값~하한값)인 경우 ‘평균수준’, 하한값보다 적은(<) 경우 ‘평균이하’로 전환한다.
```
