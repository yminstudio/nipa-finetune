...
## Question
Top10 종목 집중도와 정규분포의 비교에서 어떤 차이가 주로 다뤄지는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 종목 집중도와 정규분포의 비교에서 어떤 차이가 주로 다뤄지는가?",
  "qa_type": "comparison",
  "question_template": "Comparison_Top10_vs_정규분포"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 종목 집중도와 정규분포의 비교에서 어떤 차이가 주로 다뤄지는가?",
...
## Assistant Response
```text
- Top10 종목 집중도를 정규분포와 “유사”하다고 보고, 상·하한을 나누는 비교 기준(33.1%, 67.0%)을 통해 집중도의 상대적 위치를 판단하는 방식이 주로 다뤄집니다.   
- 차이는 집중도(Top10 집중수)가 기준 상한값보다 큰지(‘평균이상’), 상한~하한 구간에 드는지(‘평균수준’), 하한값보다 작은지(‘평균이하’)로 정리됩니다.
```
