...
## Question
AUM이 특정 값인 펀드에 대해 운용규모 수준을 적용하는 절차를 간단히 설명하라.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "AUM이 특정 값인 펀드에 대해 운용규모 수준을 적용하는 절차를 간단히 설명하라.",
  "qa_type": "application",
  "question_template": "적용_템플릿_1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 운용규모 수준",
  "question": "AUM이 특정 값인 펀드에 대해 운용규모 수준을 적용하는 절차를 간단히 설명하라.",
...
## Assistant Response
```text
- 적용대상: 유형 내에서 운용규모 수준을 산출하며, 클래스펀드는 운용펀드를 기준으로 기준값 측정 및 레벨을 부여합니다.   
- 기준펀드 설정 및 정렬: 기준펀드 수 계산대상은 순자산총액 10억 이상 평가 공/사모펀드이며, 순자산총액 내림차순으로 정렬한 뒤 상위 펀드부터 펀드수를 카운트합니다.   
- 경계값(전환 기준) 산정: 최상위 5.0%, 15.0%, 30.0%, 50.0%에 해당하는 펀드의 순자산액을 각각 경계점(기준점)으로 설정합니다.   
- 수준 배정(전환 규칙): 위 경계값을 기준으로 해당 유형 전 펀드에 적용해 초대형급/대형급/중형급/소형급/초소형급의 5개 레벨 중 해당 운용규모 수준을 부여합니다.
```
