...
## Question
Top10 집중도와 Top10 정의 간의 차이를 비교해 설명하라
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Top10 집중도와 Top10 정의 간의 차이를 비교해 설명하라",
  "qa_type": "comparison",
  "question_template": "Comparison_Top10집중도와Top10정의차이"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "Top10 집중도와 Top10 정의 간의 차이를 비교해 설명하라",
...
## Assistant Response
```text
- 공통 맥락: Top10 관련 평가는 ‘Top10 종목 집중도’ 체계 안에서 진행되며, 사모펀드는 제외하고(…) 순자산액 100억 이상 평가 공모펀드(FoFs, 자펀드 제외)를 기준점 계산 대상으로 삼는다.  또 해당 기준값은 펀드 규모와 무관하게 전 펀드에 적용한다.   
- Top10 집중도: 내림차순 정렬 후 33.1%(상한)·67.0%(하한)에 해당하는 “집중도”를 기준점으로 두고, 그보다 높은/중간/낮은지에 따라 ‘평균이상/평균수준/평균이하’로 레벨을 나눈다.   
- Top10 정의: Top10 집중도를 어떤 펀드/데이터 범위에서 평가하는지(사모 제외, 100억 이상 FoFs·자펀드 제외, 기준값 적용대상 범위)를 규정하는 쪽에 더 가깝다.
```
