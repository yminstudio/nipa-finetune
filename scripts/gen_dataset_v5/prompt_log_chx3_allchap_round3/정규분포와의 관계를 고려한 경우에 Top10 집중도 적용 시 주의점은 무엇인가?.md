...
## Question
정규분포와의 관계를 고려한 경우에 Top10 집중도 적용 시 주의점은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "정규분포와의 관계를 고려한 경우에 Top10 집중도 적용 시 주의점은 무엇인가?",
  "qa_type": "application",
  "question_template": "Application_정규분포고려"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "4) Top10 종목 집중도",
  "question": "정규분포와의 관계를 고려한 경우에 Top10 집중도 적용 시 주의점은 무엇인가?",
...
## Assistant Response
```text
- 적용 맥락/주의: Top10 종목 집중도는 “순자산액 100억 이상 펀드”의 경우 정규분포와 유사하다고 보고 기준점을 설정하며, 사모펀드는 적용 대상에서 제외합니다.   
- 기준점 산출 순서: 기준점 계산 대상(순자산액 100억 이상 평가 공모펀드; FoFs·자펀드 제외)에서 Top10 종목 집중도를 내림차순 정렬한 뒤, 33.1%(상한값), 67.0%(하한값)에 해당하는 집중도를 기준점으로 둡니다.   
- 적용 범위(전환의 전제): 위에서 구한 기준값은 펀드 규모 관계없이 “전 펀드”에 동일하게 적용합니다.   
- 전환 규칙(등급 부여): 상한값보다 큰(>) 집중도는 ‘평균이상’, 상한~하한은 ‘평균수준’, 하한값보다 작은(<) 집중도는 ‘평균이하’로 구분합니다.
```
