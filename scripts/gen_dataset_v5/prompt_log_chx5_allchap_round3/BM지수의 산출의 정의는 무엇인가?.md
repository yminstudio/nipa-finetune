...
## Assistant Response
```text
BM지수 산출(성과 계산에 사용되는 BM의 일 수익률 산출)은 BM 종가지수를 전영업일(직전 BM 지수) 대비로 환산해 정의합니다. 

- BM의 일수익률 = (펀드의 기준일–1일 종가) / (펀드의 기준일–1일의 전영업일 종가)   
- 이를 식으로 쓰면, r_{i, BM} = J_{i-1} / J_{i-2} − 1 (J는 BM 종가지수)
```
