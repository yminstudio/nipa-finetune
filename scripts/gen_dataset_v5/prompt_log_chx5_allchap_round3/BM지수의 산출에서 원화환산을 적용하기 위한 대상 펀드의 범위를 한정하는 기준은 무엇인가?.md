...
## Question
BM지수의 산출에서 원화환산을 적용하기 위한 대상 펀드의 범위를 한정하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM지수의 산출에서 원화환산을 적용하기 위한 대상 펀드의 범위를 한정하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "BM지수의 산출에서 원화환산을 적용하기 위한 대상 펀드의 범위를 한정하는 기준은 무엇인가?",
...
## Assistant Response
```text
원화환산 BM지수 적용 범위는 “어떤 펀드를 환 기준(원화)으로 환산한 BM에 연결해 성과를 비교할지”를 정하기 위한 기준입니다. 

- 해외투자 펀드 중 환헷지를 하지 않는 펀드   
- 환헷지를 하지 않는 경우에 해당하며, 그 적용(원화환산 지수)은 적극적으로 밝힌 경우만 인정
```
