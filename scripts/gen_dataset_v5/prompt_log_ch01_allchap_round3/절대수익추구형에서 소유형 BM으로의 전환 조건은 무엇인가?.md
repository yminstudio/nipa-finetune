...
## Question
절대수익추구형에서 소유형 BM으로의 전환 조건은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "절대수익추구형에서 소유형 BM으로의 전환 조건은 무엇인가?",
  "qa_type": "application",
  "question_template": "BM 적용 - 전환조건"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "절대수익추구형에서 소유형 BM으로의 전환 조건은 무엇인가?",
...
## Assistant Response
```text
절대수익추구형(대유형) 펀드는 투자전략/특성에 따라 소유형으로 배정되고, 그 소유형에 해당하는 소유형 BM으로 적용이 전환됩니다. 

- 채권알파로 전환 조건: 투자설명서상 절대수익 추구 + 마켓타이밍 전략 중심 + 평균주식편입비가 10% 수준으로 추정   
  → 소유형 BM: KOSPI200 10% + 매경 BP 종합채권 1년 90% 
- 시장중립으로 전환 조건: 저평가주식 매수-고평가 선물 매도 등으로 시장포지션을 중립화하는 전략 + 절대수익 추구   
  → 소유형 BM: KOSPI200 5% + 매경 BP 종합채권 1년 95% 
- 공모주하이일드로 전환 조건: 투기등급채권 투자 가능하면서 공모주식에도 투자(공모주 우선배정권이 있었던 펀드)   
  → 소유형 BM: KOSPI200 5% + 매경 BP 회사채 I-BBB 종합 2년 95%
```
