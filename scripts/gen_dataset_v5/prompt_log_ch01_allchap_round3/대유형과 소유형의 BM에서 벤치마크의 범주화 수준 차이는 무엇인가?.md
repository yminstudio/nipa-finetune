...
## Question
대유형과 소유형의 BM에서 벤치마크의 범주화 수준 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대유형과 소유형의 BM에서 벤치마크의 범주화 수준 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_대유형_vs_소유형_BM"
}
```
...
```json
{
  ...
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "대유형과 소유형의 BM에서 벤치마크의 범주화 수준 차이는 무엇인가?",
...
## Assistant Response
```text
제로인은 펀드 BM을 “대유형(대분류)–소유형(소분류)” 체계에 따라 설정해 성과를 비교합니다. 

- 대유형 BM은 해당 대분류(예: 주식형, 채권형 등) 수준에서 정해지는 벤치마크입니다.   
- 소유형 BM은 그 대유형 안의 세부 분류(예: 일반주식, 중소형주식 등) 수준에서 정해지는 벤치마크로, 범주화가 더 세분화됩니다.   
- 다만 소유형 BM 중 일부는 “볼드체 BM”으로 표시되며, 이 경우 소유형이 속한 대유형의 BM과 동일하게 적용됩니다(즉 세분화가 대유형 수준으로 합쳐지는 케이스가 존재).
```
