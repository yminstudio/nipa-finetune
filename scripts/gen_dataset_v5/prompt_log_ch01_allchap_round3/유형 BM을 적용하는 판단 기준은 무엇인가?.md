...
```json
{
  ...
  "target_total": 100,
  "generated_question": "유형 BM을 적용하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM 설정기준 세부적용 방법-유형 BM 적용 판단 기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "유형 BM을 적용하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
유형 BM은 펀드별로 별도 BM(펀드 BM)을 붙이기 어려운 경우, 해당 펀드를 “유형” 기준으로 비교하는 BM입니다.   

유형 BM을 적용하는 판단 기준은 다음과 같습니다.

- 국내펀드의 경우: 테마펀드와 기타 유형(기타인덱스)은 예외로 두고, 그 외 국내 펀드는 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다.   
- 해외펀드의 경우: 원칙은 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 설정하되, 그 비교지수가 없으면 투자전략과 가장 유사한 BM을 찾습니다.   
- 해외펀드에서 유형 BM으로 전환되는 조건: 적합한 BM이 없거나 확신하기 힘든 경우, 또는 제로인이 보유한 지수 중 유사한 BM이 존재하지 않는 경우 유형 BM을 적용합니다.
```
