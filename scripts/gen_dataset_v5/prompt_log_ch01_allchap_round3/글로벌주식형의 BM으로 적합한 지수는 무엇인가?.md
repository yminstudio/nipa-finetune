...
```json
{
  ...
  "target_total": 100,
  "generated_question": "글로벌주식형의 BM으로 적합한 지수는 무엇인가?",
  "qa_type": "application",
  "question_template": "적용_글로벌주식형BM"
}
```
...
```json
{
  ...
  "subsection": "해외펀드의 유형별 BM",
  "seed_title": "해외펀드의 유형별 BM",
  "question": "글로벌주식형의 BM으로 적합한 지수는 무엇인가?",
...
## Assistant Response
```text
글로벌주식형(해외주식형) 펀드의 BM은 제로인 기준에 따라 “명시된 비교지수→대체→유형 BM” 순으로 정해집니다. 

- 적용 1: 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정합니다.   
- 적용 2: 명시된 비교지수가 없으면, 펀드 투자전략과 가장 유사한 BM을 설정합니다(제로인이 보유한 지수 기준).   
- 전환 규칙: 적합한 BM이 없거나 확신하기 힘들면 유형 BM을 적용합니다.   
- 유형 BM(정답): 글로벌주식형의 유형 BM은 MSCI ACWI입니다.
```
