...
## Question
소유형 BM 적용 여부가 벤치마크 판단 기준에 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "소유형 BM 적용 여부가 벤치마크 판단 기준에 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "소유형 BM 적용 여부가 벤치마크 판단 기준에 포함되는가?",
...
## Assistant Response
```text
질문은 제로인 벤치마크(BM) 설정 시 “소유형 BM을 적용하는지”가 판단 기준으로 따로 포함되는지 확인하는 것입니다. 

핵심은 다음과 같습니다.  
- 제로인 BM 설정의 판단 기준은 “유형 BM을 쓸지, 펀드 BM을 별도로 쓸지(별도 적용 여부)”에 두고 있습니다.   
- 국내펀드는(테마펀드/기타인덱스 제외) 별도의 펀드 BM을 두지 않고 “유형 BM”을 적용합니다.   
- 유형별 BM은 대유형-소유형-소유형 BM으로 매핑되어 제시되므로, “유형 BM 적용”이 곧 소유형 BM으로 연결됩니다.
```
