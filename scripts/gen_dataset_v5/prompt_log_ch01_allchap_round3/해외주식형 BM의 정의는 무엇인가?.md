...
```json
{
  ...
  "target_total": 100,
  "generated_question": "해외주식형 BM의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "definition_template_001"
}
```
...
## Assistant Response
```text
해외주식형 펀드는 운용성과를 BM(벤치마크) 대비로 비교·평가하므로, 여기서의 “해외주식형 BM”은 펀드 성과 비교에 쓰이는 기준이 무엇인지를 뜻합니다.  

해외주식형 BM의 정의는 “해외주식형 펀드의 성과를 비교하기 위해 적용되는 펀드 BM(또는 유형 BM)”입니다.   

- 해외(해외주식형 포함) 펀드의 BM은 원칙적으로 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정합니다.   
- 비교지수가 없으면 펀드의 투자전략과 가장 유사한 BM을 설정하되, 적합한 BM이 없거나 확신하기 어려우면 유형 BM을 적용합니다.   
- 해외주식형의 유형 BM은 하위 유형별로 정해져 있으며, 예를 들어 글로벌주식은 MSCI ACWI, 유럽주식은 MSCI EUROPE, 중국주식은 MSCI CHINA를 사용합니다.   
- 해외펀드의 환헷지 여부에 따라 BM 적용 지수가 달라져, 환헷지 시에는 로컬지수, 환헷지 미적용 시에는 원화환산 지수를 적용합니다(단, 적극적으로 밝힌 경우만 인정).
```
