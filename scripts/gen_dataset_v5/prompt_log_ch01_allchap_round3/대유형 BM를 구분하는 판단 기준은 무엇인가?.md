...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대유형 BM를 구분하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "대유형_BM_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "대유형 BM를 구분하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
제로인은 BM(벤치마크)을 유형별로 설정하기 위해, 먼저 펀드를 대유형으로 나눌 때의 “편입 가능 자산 비중/투자대상” 판단기준을 적용합니다. 국내펀드는 투자설명서상 국내증권에 2/3 이상 투자하도록 정해져 있거나 그럴 가능성이 높은 경우를 전제로 합니다 .

- 주식형: 약관상 최고 주식투자한도가 2/3 초과 
- 주식혼합형: 약관상 주식투자한도가 10%~2/3이며, 위험자산 40%~2/3 구간에서 평균주식편입비가 거의 변하지 않는 타입 
- 채권혼합형: 약관상 주식투자한도가 10%~2/3이며, 위험자산 11~39% 구간에서 평균주식편입비가 거의 변하지 않는 타입 
- 채권형: 주식투자가 불가능하고 채권, CP 등 채권형 자산에만 투자하는 시가평가 펀드 
- MMF형: 가중평균 잔존만기 90일 이내인 채권형 자산에 투자 
- 부동산형: 공개시장을 통하지 않고 협상을 통해 부동산 및 관련증권에 투자 
- 절대수익추구형: 투자설명서상 절대수익을 추구하며(마켓타이밍 등), 평균주식편입비가 10% 수준으로 추정되는 펀드 
- 특별자산: 투자설명서상 주식·채권·부동산 이외의 투자대상자산에 투자 
- 헤지: 주식·채권·파생상품 등 다양한 자산에 투자하여 절대수익을 추구하는 사모펀드 
- 기타: 펀드 간 비교가 어렵지만 구분의 필요가 존재하는 펀드(예: 라이프싸이클, 베어마켓, 단기매칭 등)
```
