...
## Question
해외주식형 BM과 해외주식혼합형 BM의 주요 비교 축은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "해외주식형 BM과 해외주식혼합형 BM의 주요 비교 축은 무엇인가?",
  "qa_type": "comparison",
  "question_template": "comparison_template_001"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) 해외펀드의 유형별 BM",
  "question": "해외주식형 BM과 해외주식혼합형 BM의 주요 비교 축은 무엇인가?",
...
## Assistant Response
```text
해외주식형과 해외주식혼합형의 BM을 비교할 때, 핵심은 “BM에 어떤 자산(주식만 vs 주식+채권)을 담는가”와 “그 구성(지수 형태·가중 방식)이 어떻게 정해지는가”입니다.  

- 해외주식형 BM: 지역(예: 글로벌, 유럽, 중국 등)별 해외주식 지수 단일 축으로 설정됩니다(예: MSCI ACWI, MSCI EUROPE, MSCI CHINA).   
- 해외주식혼합형 BM: 주식지수와 채권지수를 함께 담아 “복합지수” 형태로 만듭니다(예: MSCI … CR 50% + 매경 BP 종합채권 01Y 50%).   
- 복합지수 구성 방식(주식혼합 BM 공통 축): 각 시장 지수의 BM 수익률을 투자비중 ω로 가중합해 복합지수 BM 일수익률을 산출합니다.   
- 지수 형태의 차이(주식 vs 채권): 주식 부문은 Capital Return 지수, 채권 부문은 Total Return 지수(단, BM 수익률의 90%만 반영)로 적용합니다.
```
