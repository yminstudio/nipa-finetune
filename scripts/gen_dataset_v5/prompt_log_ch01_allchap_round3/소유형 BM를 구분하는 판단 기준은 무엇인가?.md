...
```json
{
  ...
  "target_total": 100,
  "generated_question": "소유형 BM를 구분하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "소유형_BM_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "소유형 BM를 구분하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
소유형 BM은 펀드의 ‘소유형’에 맞춰 벤치마크를 정할 때, 유형BM을 쓸지(소유형BM) 펀드 BM을 쓸지(별도 BM) 판단하는 기준에 의해 구분됩니다. 

- 국내펀드: 테마펀드와 기타인덱스를 제외하고는 별도의 펀드 BM을 적용하지 않고 유형 BM(= 소유형BM)을 적용합니다.   
- 해외펀드: 원칙적으로 펀드 BM을 별도 적용합니다. 이때 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 쓰며, 제로인이 보유하지 않은 경우 희망 BM과 가장 유사한 BM을 사용합니다. BM이 없거나 제로인이 보유한 지수 중 유사한 BM이 없는 경우에는 유형 BM을 적용합니다.   
- (해외) 환헷지 여부: 환헷지를 하는 경우 로컬 지수를 적용하고, 하지 않는 경우 원화환산 지수를 적용하되(적극적으로 밝힌 경우만 인정) 판단합니다.
```
