...
## Question
KOSPI200를 벤치마크로 삼는 구분 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "KOSPI200를 벤치마크로 삼는 구분 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "KOSPI200를 벤치마크로 삼는 구분 기준은 무엇인가?",
...
## Assistant Response
```text
KOSPI200를 벤치마크(BM)로 쓰는지 여부는 제로인 펀드평가에서 펀드의 “국내 유형(대유형)·소유형”에 따라 BM이 정해지는 방식으로 구분됩니다. 

- 주식형 중 일반주식(순수·자산배분), 테마주식, K200 인덱스, 기타인덱스(기타)라면 BM이 KOSPI200로 설정됩니다. 
- 주식혼합형 중 일반주식혼합, 공격적자산배분이라면 BM이 “KOSPI200 50% + (종합채권지수) 50%” 형태로 KOSPI200을 포함합니다. 
- 절대수익추구형(채권알파·시장중립·공모주하이일드) 및 헤지펀드(롱숏·IPO·이벤트드리븐)처럼 다른 유형에서도 BM 산정 시 KOSPI200 비중(예: 5% 또는 10%)을 포함하도록 구분합니다.
```
