...
## Question
KOSPI200는 국내펀드의 어떤 유형의 BM으로 적용되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "KOSPI200는 국내펀드의 어떤 유형의 BM으로 적용되는가?",
  "qa_type": "application",
  "question_template": "적용_KOSPI200_펀드유형_BM"
}
```
...
```json
{
  ...
  "subsection": "국내펀드의 유형별 BM",
  "seed_title": "국내펀드의 유형별 BM",
  "question": "KOSPI200는 국내펀드의 어떤 유형의 BM으로 적용되는가?",
...
## Assistant Response
```text
제로인에서는 국내에 투자하는 펀드가 평가될 때 유형별 BM을 기준으로 펀드 BM이 정해집니다.   

- 적용 조건: 국내 투자 펀드는 기본적으로 “유형BM=펀드BM”으로 설정하며, 테마 및 기타 유형은 투자설명서/규약의 비교지수가 있으면 그 비교지수를, 없으면 유형BM을 사용합니다.   
- KOSPI200는 국내펀드 유형 중 다음 소유형의 유형BM으로 적용됩니다: 주식형 일반주식(순수, 자산배분), 테마주식, KOSPI200 인덱스, 기타인덱스.
```
