...
## Question
대유형 BM과 소유형 BM 중 지역 커버리지가 더 넓은 쪽은 어느 쪽인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대유형 BM과 소유형 BM 중 지역 커버리지가 더 넓은 쪽은 어느 쪽인가?",
  "qa_type": "comparison",
  "question_template": "비교_지역커버리지"
}
```
...
```json
{
  ...
  "subsection": "해외펀드의 유형별 BM",
  "seed_title": "해외펀드의 유형별 BM",
  "question": "대유형 BM과 소유형 BM 중 지역 커버리지가 더 넓은 쪽은 어느 쪽인가?",
...
## Assistant Response
```text
해외펀드는 유형/소유형에 따라 BM이 정해지며, 이때 소유형 BM은 특정 지역(또는 지역 묶음) 지수로 구체화됩니다. 

- 대유형(예: 해외주식형) 아래에 글로벌주식, 유럽주식, 중국주식, 인도주식 등 여러 소유형이 놓입니다.   
- 소유형 BM은 그 소유형에 대응해 MSCI ACWI, MSCI EUROPE, MSCI CHINA, MSCI INDIA처럼 지역별로 구분됩니다.   
- 따라서 지역 커버리지는 “대유형 BM”이 “소유형 BM”보다 더 넓은 쪽입니다.
```
