...
## Question
펀드 BM을 별도 적용하는 여부가 벤치마크 판단 기준에 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 BM을 별도 적용하는 여부가 벤치마크 판단 기준에 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1.2 유형별 벤치마크",
  "question": "펀드 BM을 별도 적용하는 여부가 벤치마크 판단 기준에 포함되는가?",
...
## Assistant Response
```text
질문은 제로인 벤치마크(BM) 설정을 판단할 때 “펀드 BM을 별도로 적용할지 여부”가 판단 기준에 들어가는지에 관한 것입니다.

- 벤치마크 설정기준에서 “유형 BM, 펀드 BM의 별도 적용”이 구체적으로 규정되어 있어, 펀드 BM을 별도 적용하는 여부는 벤치마크 판단 기준에 포함됩니다 .  
- 국내펀드는(테마펀드·기타인덱스 제외) 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다 .  
- 해외펀드는 펀드 BM을 별도 적용하며, 설정 당시 운용사가 희망하는 BM을 우선 적용하고(제로인이 보유하지 않으면 유사 BM), BM이 없거나 유사 BM이 없으면 유형 BM으로 전환합니다 .  
- 또한 해외펀드는 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 두되, 명시된 비교지수가 없으면 전략과 유사한 BM, 그마저 없으면 유형 BM으로 설정합니다 .
```
