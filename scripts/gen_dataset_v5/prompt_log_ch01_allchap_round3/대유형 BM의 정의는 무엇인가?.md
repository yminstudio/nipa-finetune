...
## Assistant Response
```text
제로인에서는 펀드의 성과를 비교하기 위해 BM(벤치마크)을 ‘유형’에 따라 정하고, 그 유형이 대유형/소유형으로 구분됩니다. 

대유형 BM의 정의는 “대유형(상위 유형) 구분에 해당하는 벤치마크를 의미하며, 대유형 아래 소유형에 매칭된 ‘소유형 BM’을 통해 유형 BM이 구성되는 것”입니다. 

- 국내펀드의 경우 테마펀드·기타인덱스를 제외하고는 별도의 펀드 BM을 두지 않고 유형 BM을 적용합니다.   
- 유형별 BM은 ‘대유형–소유형–소유형 BM’의 대응관계로 제시됩니다(예: 주식형 내 소유형이 달라지면 소유형 BM도 달라짐).
```
