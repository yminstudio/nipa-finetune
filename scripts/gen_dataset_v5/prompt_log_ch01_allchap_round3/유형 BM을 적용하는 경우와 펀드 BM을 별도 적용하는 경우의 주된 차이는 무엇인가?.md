...
## Question
유형 BM을 적용하는 경우와 펀드 BM을 별도 적용하는 경우의 주된 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "유형 BM을 적용하는 경우와 펀드 BM을 별도 적용하는 경우의 주된 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_적용방식차이"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "유형 BM을 적용하는 경우와 펀드 BM을 별도 적용하는 경우의 주된 차이는 무엇인가?",
...
## Assistant Response
```text
제로인에서는 벤치마크(BM)를 “같은 유형의 기준”으로 둘지(유형 BM) “해당 펀드의 비교지수”로 둘지(펀드 BM) 자산/펀드 특성에 따라 달리 적용합니다. 

주된 차이는 아래와 같습니다.  
- 적용 대상(범위): 국내펀드는 테마펀드·기타인덱스를 제외하면 별도 펀드 BM 없이 유형 BM을 적용하고, 해외펀드는 펀드 BM을 별도 적용합니다.   
- BM의 기준(무엇을 쓰는가): 유형 BM은 유형별로 정해진 BM을 쓰는 반면, 펀드 BM은 투자설명서/규약(또는 설정 당시 운용사의 희망)에 따른 펀드의 비교지수를 기준으로 설정합니다.   
- 부재 시 처리(대체 경로): 펀드 BM이 제로인에 없으면 희망 BM과 가장 유사한 BM을 적용하고, BM이 없거나 유사 BM이 없으면 유형 BM으로 전환합니다.   
  (참고로 국내 테마/기타는 비교지수가 없으면 유형 BM으로 설정합니다.)
```
