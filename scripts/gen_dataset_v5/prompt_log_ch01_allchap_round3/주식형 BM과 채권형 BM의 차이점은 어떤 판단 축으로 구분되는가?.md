...
## Question
주식형 BM과 채권형 BM의 차이점은 어떤 판단 축으로 구분되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "주식형 BM과 채권형 BM의 차이점은 어떤 판단 축으로 구분되는가?",
  "qa_type": "comparison",
  "question_template": "BM 비교 - 차이 축"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "주식형 BM과 채권형 BM의 차이점은 어떤 판단 축으로 구분되는가?",
...
## Assistant Response
```text
주식형·채권형 펀드를 평가할 때는 벤치마크(BM)를 “자산별 적용지수” 방식으로 구분해 수익률을 계산합니다.   

주식형 BM과 채권형 BM의 차이는 다음 판단 축(수익률 정의/적용 방식)으로 구분됩니다.  
- 자산별 BM의 수익률 기준: 주식은 Capital Return 지수, 채권은 Total Return 지수를 사용   
- 채권형의 적용 비율: 채권은 BM 수익률을 100%가 아니라 90%만 반영   
- 구분 배경(형평성 판단): 주식 BM은 Total Return이 아니라 Capital Return이어서(배당수익이 별도로 발생) 주식이 BM을 초과할 가능성이 생길 수 있으므로 공정성을 위해 수익률 반영 방식을 다르게 합니다.
```
