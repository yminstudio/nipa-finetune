...
```json
{
  ...
  "target_total": 100,
  "generated_question": "주식형 BM를 구분하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "주식형_BM_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 국내펀드의 유형별 BM",
  "question": "주식형 BM를 구분하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
주식형 펀드의 BM은 “어떤 지수를 BM으로 채택하느냐”를 먼저 나누고, 그 기준에 따라 유형BM과 비교지수(펀드 BM) 중 무엇을 쓰는지가 달라집니다. 

- 국내 펀드의 경우, 테마펀드와 기타인덱스를 제외하면 별도의 펀드 BM을 두지 않고 유형 BM을 BM으로 설정합니다.   
- 국내 펀드의 경우, 테마 및 기타 유형은 투자설명서 또는 규약에 있는 비교지수를 BM으로 설정하며, 해당 비교지수가 없으면 유형 BM으로 설정합니다.   
- 해외 펀드의 경우, 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정하고, 없으면 투자전략과 가장 유사한 BM을 택하되 적합한 BM이 없거나 확신하기 어렵다면 유형 BM으로 설정합니다.
```
