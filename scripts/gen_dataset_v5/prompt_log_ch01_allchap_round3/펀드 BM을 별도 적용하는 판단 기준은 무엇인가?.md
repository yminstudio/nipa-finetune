...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 BM을 별도 적용하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "BM 설정기준 세부적용 방법-펀드 BM 별도 적용 판단 기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) BM의 설정기준 세부적용 방법",
  "question": "펀드 BM을 별도 적용하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
질문은 제로인 펀드평가에서 “펀드 BM(해당 펀드별로 정해 쓰는 비교지수)”을 유형 BM(동일 유형에 쓰는 BM) 대신 별도로 적용할지 판단하는 기준을 묻는 것입니다. 

- 국내에 투자하는 펀드는 테마펀드, 기타인덱스를 제외하면 별도의 펀드 BM을 적용하지 않고 유형 BM을 적용합니다. 
- 해외에 투자하는 펀드는 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 적용합니다(별도 적용). 
- 단, 투자설명서/규약에 비교지수가 없으면 펀드의 투자전략과 가장 유사한 BM을 펀드 BM으로 설정하되, 적합한 BM이 없거나 확신하기 어려우면 유형 BM으로 적용합니다. 
- 또한 설정 당시 운용사가 희망하는 BM을 펀드 BM으로 두고, 제로인이 보유하지 않은 경우는 희망 BM과 가장 유사한 BM을 사용하며, BM이 없거나 유사한 BM이 없으면 유형 BM을 적용합니다.
```
