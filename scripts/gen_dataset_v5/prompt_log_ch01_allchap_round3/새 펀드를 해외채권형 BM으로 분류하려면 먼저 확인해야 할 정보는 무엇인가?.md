...
## Question
새 펀드를 해외채권형 BM으로 분류하려면 먼저 확인해야 할 정보는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "새 펀드를 해외채권형 BM으로 분류하려면 먼저 확인해야 할 정보는 무엇인가?",
  "qa_type": "application",
  "question_template": "application_template_001"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) 해외펀드의 유형별 BM",
  "question": "새 펀드를 해외채권형 BM으로 분류하려면 먼저 확인해야 할 정보는 무엇인가?",
...
## Assistant Response
```text
신규 펀드를 해외채권형 BM으로 붙이려면, 먼저 펀드가 ‘해외채권형’ 요건에 해당하는지와 그 다음에 BM(비교지수/유형BM)을 어떤 기준으로 정하는지 확인합니다.  

- 1) 펀드 유형분류를 위한 투자설명서상 투자목적·투자전략을 확인합니다.   
- 2) 해외채권형 해당 여부를 확인합니다(약관상 주식투자가 불가능 + 2/3 이상을 해외 채권형 자산에 투자). 해당하면 ‘해외채권형’ 분류의 대상이 됩니다.   
- 3) 해외펀드의 펀드 BM은 투자설명서 또는 규약에 명시된 비교지수를 우선 확인해 그 비교지수를 펀드 BM으로 설정합니다.   
- 4) 전환 규칙: 비교지수가 없다면 펀드의 투자전략과 가장 유사한 BM을 설정하되, 적합한 BM이 없거나 확신하기 어려우면 유형 BM으로 전환합니다.   
- 5) 추가 전환 규칙: 설정 당시 희망 BM을 제로인이 보유하지 않으면 희망 BM과 가장 유사한 BM을 적용하고, BM이 없거나 유사한 보유 지수가 없으면 유형 BM을 적용합니다.
```
