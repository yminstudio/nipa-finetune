...
## Question
신용도의 구분에서 채권과 CP는 서로 같은 범주에 속하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "신용도의 구분에서 채권과 CP는 서로 같은 범주에 속하는가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "1) 신용도의 구분",
  "question": "신용도의 구분에서 채권과 CP는 서로 같은 범주에 속하는가?",
...
## Assistant Response
```text
- 제로인 신용도의 구분(스타일 분류를 위한 신용평가)에서는 CP는 제외됩니다.   
- 펀드 내 보유 채권과 CP의 평균신용등급은 별도로 표시됩니다.   
- 따라서 채권과 CP는 신용도의 구분에서 서로 같은 범주로 보지 않습니다.
```
...
## Assistant Response
```text
- 제로인의 채권 포트폴리오 “신용도의 구분”은 채권의 시가평가액 가중 신용점수를 기준으로 나뉩니다.   
- 이때 스타일 분류를 위한 신용평가에서는 CP는 제외합니다.   
- 또한 보유 채권과 CP의 평균신용등급은 별도로 표시하므로, 신용도의 구분 관점에서 채권과 CP는 같은 범주로 취급되지 않습니다.
```
...
## Assistant Response
```text
- 제로인 “신용도의 구분”에서 신용 위험(신용등급 구분)은 보유 채권(CP 등 단기 투자상품 제외)의 가중평균 부도율로 측정합니다.   
- 따라서 채권과 CP는 이 신용도 구분(스타일 분류 목적)의 동일 범주로 함께 취급되지 않습니다.   
- 다만 CP는 신용등급 평가에서 제외되더라도, 펀드 내 보유 채권과 CP의 평균신용등급은 별도로 표시합니다.
```
...
## Assistant Response
```text
신용도의 구분은 “채권 포트폴리오”의 신용위험(신용점수/부도율 등)으로 스타일을 나누는 맥락에서 적용됩니다.   
- 채권은 스타일 분류를 위한 신용평가에 반영됩니다(채권 포트폴리오 가중 신용점수 기준).   
- CP는 스타일 분류를 위한 신용평가에서는 제외됩니다.   
- 다만 펀드 내 보유 “채권”과 “CP”의 평균 신용등급은 각각 별도로 표시되며, 채권/CP은 신용등급별 배점 체계도 구분되어 있습니다.
```
