...
## Question
'시장의 규모별 대표 PER'와 'PBR' 사이의 비교 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "'시장의 규모별 대표 PER'와 'PBR' 사이의 비교 기준은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 가치특성의 구분",
  "question": "'시장의 규모별 대표 PER'와 'PBR' 사이의 비교 기준은 무엇인가?",
...
## Assistant Response
```text
- 시장의 규모별 대표 PER과 PBR은 모두 “해당 시장의 규모별 PER(또는 PBR)의 중앙값(Median)”을 대표치로 사용한다.   
- 대표치를 정할 때도 PER(PBR) 높은 종목부터 시가총액 누적비중 50%에 해당하는 종목의 PER(PBR) 값을 그룹의 대표 값으로 정한다.   
- 개별 종목의 상대값은 “종목 PER(PBR) / 해당 종목 사이즈의 대표 PER(PBR)”로 산출해, 비교 기준(나눗셈 기준)이 PER과 PBR에서 동일하다.
```
...
## Assistant Response
```text
- 공통 비교 기준은 “시장 규모별 대표값”을 고르는 방식으로, 각 규모 구간에서 PER(또는 PBR)의 중앙값(Median)을 대표치로 사용합니다.   
- 대표 PER은 PER 값을 기준으로, 대표 PBR은 PBR 값을 기준으로 종목을 내림차순 정렬한 뒤 “시가총액 누적비중 50%에 해당하는 종목”의 PER(PBR) 값을 대표치로 삼습니다.   
- 비교가 이루어지는 공통 집합(거래소+코스닥 전 종목, 우선주/뮤추얼펀드/관리종목 등 제외)도 동일합니다.
```
...
## Assistant Response
```text
- 공통 기준: 제로인 방법론에서는 거래되는 종목을 시장의 규모(대·중·소 등 구분)별로 묶고, 그 규모 안에서 PER 또는 PBR의 “대표치”를 정합니다.   
- ‘시장의 규모별 대표 PER’: 각 규모 구간에서 PER이 높은 종목부터 시가총액 누적비중 50%(Median)에 해당하는 종목의 PER을 대표값으로 삼습니다.   
- PBR: 대표 PER과 동일한 방식으로, 각 규모 구간에서 PBR이 높은 종목부터 시가총액 누적비중 50%(Median)에 해당하는 종목의 PBR을 대표값으로 삼습니다.
```
...
## Assistant Response
```text
시장의 규모별 대표 PER과 PBR은 “동일한 시장 규모 구분”이라는 공통 맥락에서 비교됩니다. 

- 각 규모(대형/중형/소형 등) 내에서 PER(또는 PBR) 값의 대표치를 정할 때, 해당 지표의 중앙값(Median)을 사용합니다.   
- 선정 방식도 동일하게, PER(PBR)이 높은 종목부터 시가총액 누적비중이 50%에 해당하는 지점의 PER(PBR)을 대표 값으로 삼습니다.   
- 따라서 비교 기준은 “규모 구분 + 50% 누적 기반 중앙값 선정 규칙”이 같고, 비교 대상만 ‘PER’ vs ‘PBR’로 달라지는 것입니다.
```
