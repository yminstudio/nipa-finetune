...
## Question
대표 PER, PBR를 결정할 때 시장 규모 구분의 판단 기준으로 삼는 지표는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대표 PER, PBR를 결정할 때 시장 규모 구분의 판단 기준으로 삼는 지표는 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "가) 시장의 규모별 대표 PER, PBR 계산",
  "seed_title": "가) 시장의 규모별 대표 PER, PBR 계산",
  "question": "대표 PER, PBR를 결정할 때 시장 규모 구분의 판단 기준으로 삼는 지표는 무엇인가?",
...
## Assistant Response
```text
- 시장 규모 구분의 판단 지표는 “시가총액”입니다.   
- 규모별 대표 PER/PBR를 정할 때는, 각 규모 내에서 “시가총액 누적비중 50%”에 해당하는 종목의 PER(PBR)을 대표값으로 사용합니다.   
- 규모별 대표 PER/PBR는 해당 규모에서의 “중앙값(Median)”을 대표치로 삼습니다.
```
...
## Assistant Response
```text
대표 PER, PBR을 시장 규모별로 정할 때 “규모 구분”과 관련해 기준이 되는 지표는 시가총액입니다. 

- 시장(거래소·코스닥)에서 규모별로 PER(또는 PBR) 대표치를 정하며, 대표치 선정 과정에서 시가총액을 누적해 판단합니다. 
- PER(PBR)이 높은 종목부터 나열한 뒤, 해당 그룹 내에서 “시가총액 누적비중 50%”에 해당하는 종목의 PER(PBR)를 대표 값으로 삼습니다(이는 중앙값(Median) 기준과 동일한 취지입니다). 
- 따라서 시장 규모 구분 판단을 뒷받침하는 핵심 지표는 “시가총액(및 그 누적비중)”입니다.
```
...
## Assistant Response
```text
대표 PER, PBR을 “시장 규모별”로 나눠 대표값을 정할 때, 시장 규모(대형/중형/소형 등) 구분의 판단 기준으로 삼는 지표는 다음과 같습니다. 

- 개별 주식의 시가총액(주식의 시장 시가총액)을 기준으로 규모를 구분합니다.   
- 납입자본금은 시장 규모 구분의 판단 기준으로 사용하지 않습니다.   
- 거래소 상장주와 코스닥 등록주(제외대상: 뮤추얼펀드, 우선주, 관리종목 등 제외)를 시가총액 내림차순으로 정렬한 뒤 구간(예: 상위 5%, 차순위 15% 등)으로 대·중·소(또는 5*5 세부 구간)를 정합니다.
```
...
## Assistant Response
```text
대표 PER, PBR을 시장 규모별로 산출할 때, 시장 규모 구분(대·중·소)을 나누는 판단 기준 지표는 다음과 같습니다.

- 주식 규모 구분 지표는 시가총액(납입자본금은 사용하지 않음)입니다.   
- 거래소 상장주와 코스닥 등록주 전체를 시가총액이 큰 순서로 나열한 뒤, 그 순위를 기준으로 규모를 정합니다(우선주/뮤추얼펀드/관리종목/구조조정 대상기업 등은 제외).   
- 시가총액 상위 5%를 대형주, 그 다음 15%를 중형주, 나머지를 소형주로 구분합니다.   
- 이렇게 구분된 규모별 그룹에서 PER(또는 PBR) 대표치는 각 규모의 중앙값(Median)으로 정합니다.
```
