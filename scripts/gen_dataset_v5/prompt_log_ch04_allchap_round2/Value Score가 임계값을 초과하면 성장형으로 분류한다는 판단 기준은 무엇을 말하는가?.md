...
## Question
Value Score가 임계값을 초과하면 성장형으로 분류한다는 판단 기준은 무엇을 말하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Value Score가 임계값을 초과하면 성장형으로 분류한다는 판단 기준은 무엇을 말하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_ValueScore_임계값초과_성장형"
}
```
...
```json
{
  ...
  "subsection": "라) 펀드의 가치특성 구분",
  "seed_title": "라) 펀드의 가치특성 구분",
  "question": "Value Score가 임계값을 초과하면 성장형으로 분류한다는 판단 기준은 무엇을 말하는가?",
...
## Assistant Response
```text
- 펀드의 가치특성 점수(Value Score)가 2.25를 초과하면 성장형으로 분류한다.
```
...
## Assistant Response
```text
이 판단 기준은 펀드의 주식 ‘가치-성장’ 스타일(가치특성)을 Value Score로 나누기 위한 것입니다. 

- Value Score는 펀드의 상대 PER + 펀드의 상대 PBR로 산출합니다.   
- Value Score가 2.25를 초과하면 성장형으로 분류합니다.   
- (참고) Value Score가 1.75 미만이면 가치형, 1.75~2.25 사이는 혼합형입니다.
```
...
## Assistant Response
```text
- 이 기준은 펀드 주식의 가치특성(성장형/혼합형/가치형) 분류를 위한 판단 기준이다.   
- Value Score는 “펀드의 상대 PER + 펀드의 상대 PBR”로 산출한다.   
- Value Score가 2.25를 초과하면 성장형으로 분류한다.
```
...
## Assistant Response
```text
- 펀드의 주식 포트폴리오 ‘가치특성(성장형/혼합형/가치형)’을 구분하기 위한 판단 기준이다.   
- 이때 Value Score가 2.25를 초과하면 성장형으로 분류한다.   
- Value Score는 펀드의 상대 PER과 상대 PBR을 더해 산출한 값이다.
```
