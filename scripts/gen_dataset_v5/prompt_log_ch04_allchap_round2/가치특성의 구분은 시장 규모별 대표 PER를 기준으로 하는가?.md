...
## Question
가치특성의 구분은 시장 규모별 대표 PER를 기준으로 하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "가치특성의 구분은 시장 규모별 대표 PER를 기준으로 하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_per"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 가치특성의 구분",
  "question": "가치특성의 구분은 시장 규모별 대표 PER를 기준으로 하는가?",
...
## Assistant Response
```text
- 가치특성 구분은 Value Score를 기준으로 하며, Value Score에는 펀드의 상대 PER이 포함됩니다.   
- 상대 PER은 종목 PER을 “해당 종목의 규모”에 대한 시장 규모별 대표 PER로 나눈 값이므로, 가치특성 구분은 시장 규모별 대표 PER을 기준으로 합니다.
```
...
## Assistant Response
```text
- 가치특성 구분은 펀드의 “가치(저평가)–성장(고평가)” 성향을 판단하기 위한 분류입니다.   
- 네, 시장 규모별 대표 PER(PBR)을 기준으로 합니다. 시장 규모별 대표 PER(PBR)은 각 규모의 중앙값(Median)을 대표치로 사용합니다.   
- 구분은 이 대표 PER(PBR) 대비 각 종목의 상대 PER(PBR)을 산출하고, Value Score(상대 PER + 상대 PBR)를 기준으로(예: Value Score < 1.75는 가치형) 나뉩니다.
```
...
## Assistant Response
```text
가치특성 구분은 펀드가 보유한 종목의 PER·PBR이 “시장 규모에 비해 어느 정도 싼지/비싼지”를 상대적으로 판단하기 위한 것입니다. 

- 시장에서는 거래되는 주식의 규모별로 대표 PER(또는 PBR)을 정하며, 대표 PER은 각 규모에서 PER의 중앙값(Median) 방식으로 산정합니다.   
- 개별 종목의 상대 PER는 “종목의 PER ÷ 해당 종목의 규모별 대표 PER”로 계산해, 가치특성 판단에 직접 반영됩니다.   
- 최종적으로는 Value Score를 “상대 PER + 상대 PBR”로 만든 뒤, 그 값으로 가치형/혼합형/성장형을 구분합니다.
```
...
## Assistant Response
```text
가치특성 구분은 개별 주식이 “시장 규모 대비” 저평가/고평가에 해당하는지를 상대적으로 판단하기 위한 기준입니다. 

- 가치특성의 기본값은 시장의 규모별 대표 PER(PBR)입니다. 규모별 PER(또는 PBR)은 중앙값(Median)을 대표치로 쓰고, 그 대표값을 “규모 내 PER(PBR) 상위~누적 50%” 구간 기준으로 정합니다.   
- 개별 종목의 PER은 해당 종목이 속한 규모의 대표 PER(PBR)로 나눈 상대 PER(PBR)로 변환합니다.   
- 이후 펀드의 가치특성 점수(Value Score)를 상대 PER + 상대 PBR로 산출하고, 그 값에 따라 가치형/혼합형/성장형으로 분류합니다.
```
