...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CP등급별 예상부도율의 정의를 간단히 서술하시오.",
  "qa_type": "definition",
  "question_template": "CP등급별 예상부도율 정의"
}
```
...
```json
{
  ...
  "subsection": "[CP등급별 예상부도율]",
  "seed_title": "[CP등급별 예상부도율]",
  "question": "CP등급별 예상부도율의 정의를 간단히 서술하시오.",
...
## Assistant Response
```text
- CP등급별 예상부도율은 CP의 신용등급별로 부도율을 “예상”한 값이다.   
- 이때 각 신용등급별 부도율은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용한다.   
- CP 신용등급(A1, A2, A3, B, C, D이하)별로 예상부도율(%)이 제시된다.
```
...
## Assistant Response
```text
- CP등급별 예상부도율은 CP의 신용등급(A1, A2, A3, B, C, D이하)마다 부여되는 예상(추정) 부도율을 의미한다.   
- 이 예상부도율은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용한다.   
- 따라서 CP 신용등급별 예상부도율 표에 각 등급의 예상부도율(%) 값이 정해져 있다.
```
...
## Assistant Response
```text
- CP 등급별 예상부도율은 CP의 신용등급(A1, A2, A3, B, C, D이하 등)에 대응하여 제시되는 예상 부도율(%)이다.   
- 각 신용등급별 부도율(따라서 CP 등급별 예상부도율)은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용한다.
```
...
## Assistant Response
```text
- 제로인에서 CP 등급별 예상부도율은 CP의 각 신용등급에 대해 사용되는 예상(추정) 부도율을 뜻한다.   
- 각 신용등급별 부도율(예상부도율)은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 등급별로 제시한 것이다.   
- 따라서 CP는 자신의 CP 신용등급에 해당하는 예상부도율 값을 참고해 부도위험을 산정/반영한다.
```
