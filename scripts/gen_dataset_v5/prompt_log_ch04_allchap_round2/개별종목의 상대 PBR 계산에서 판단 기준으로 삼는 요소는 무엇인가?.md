...
## Question
개별종목의 상대 PBR 계산에서 판단 기준으로 삼는 요소는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "개별종목의 상대 PBR 계산에서 판단 기준으로 삼는 요소는 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "나) 개별종목의 상대 PER, PBR 계산",
  "seed_title": "나) 개별종목의 상대 PER, PBR 계산",
  "question": "개별종목의 상대 PBR 계산에서 판단 기준으로 삼는 요소는 무엇인가?",
...
```text
- 종목의 PBR   
- 해당 종목 사이즈(규모)의 대표 PBR   
- 해당 종목 사이즈의 대표 PBR은 시장 규모별 PBR의 중앙값(Median)
```
...
## Assistant Response
```text
개별종목의 상대 PBR은 해당 종목이 “규모(시가총액) 기준으로 볼 때” 자산가치 대비 어느 정도의 밸류에이션 위치에 있는지를 판단하기 위한 지표입니다. 

- 종목의 PBR(개별 값)   
- 해당 종목의 규모 구분(대형/중형/소형)   
- 규모별 대표 PBR(해당 규모에서 PBR의 중앙값(Median)을 대표 값으로 사용하며, 시장 내 시가총액 누적비중 50%에 해당하는 종목의 PBR을 대표로 정함)
```
...
## Assistant Response
```text
상대 PBR은 ‘해당 종목이 같은 규모의 시장에서 볼 때 PBR이 어느 정도인지’를 판단하기 위한 기준입니다. 

- 개별 종목의 PBR 자체
- 해당 종목이 속한 규모(대형·중형·소형)의 대표 PBR
- 대표 PBR은 규모별 PBR의 중앙값(Median)을 사용해 산출(해당 규모 내에서 PBR이 높은 종목부터 시가총액 누적비중 50%에 해당하는 종목의 PBR을 대표값으로 설정) 
- 상대 PBR = 종목 PBR / (해당 종목 사이즈의 대표 PBR)
```
...
## Assistant Response
```text
개별종목의 상대 PBR은 같은 규모 내에서 해당 종목의 PBR이 시장의 기준 대비 상대적으로 높은지/낮은지를 판단하기 위한 지표다. 

- 종목의 PBR   
- 종목이 속하는 규모 구분(대형, 중형, 소형)   
- 해당 규모의 대표 PBR(시장 규모별 PBR 중앙값, 즉 PBR을 내림차순으로 나열했을 때 시가총액 누적비중 50%에 해당하는 종목의 PBR)
```
