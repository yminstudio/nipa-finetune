...
## Question
대표 PER, PBR 계산에서 시장 규모 구간을 설정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대표 PER, PBR 계산에서 시장 규모 구간을 설정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "가) 시장의 규모별 대표 PER, PBR 계산",
  "seed_title": "가) 시장의 규모별 대표 PER, PBR 계산",
  "question": "대표 PER, PBR 계산에서 시장 규모 구간을 설정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 시장 규모 구간은 거래소+코스닥 전 종목(우선주, 뮤추얼펀드, 관리종목 등 제외)을 시가총액 기준으로 구분한다   
- 3*3 분류 기준: 전체 종목수 상위 5%는 대형주, 차순위 15%는 중형주, 나머지는 소형주로 설정한다   
- 5*5 분류 기준(시가총액 내림차순 누적 백분위): 상위 1% 초대형주, 상위 1%~5% 대형주, 상위 5%~20% 중형주, 상위 20%~50% 소형주, 상위 50% 미만 초소형주로 설정한다   
- 대표 PER/PBR 산정에서는 각 규모 구간별로 PER(PBR) 높은 종목부터 시가총액 누적비중 50%에 해당하는 PER(PBR)를 대표치(중앙값, Median)로 정한다
```
...
## Assistant Response
```text
대표 PER, PBR을 시장의 “규모(시가총액) 구간”별로 나눠 계산할 때의 판단 기준은 다음과 같습니다.

- 거래소와 코스닥의 전체 종목(우선주, 뮤추얼펀드, 관리종목 등 제외)을 시가총액이 큰 종목부터 순서대로 나열하고, 그 순위 누적 비중(상위 구간)을 기준으로 규모 구간을 설정합니다 
- 3*3 분류의 경우: 상위 5%를 대형주, 차순위 15%를 중형주, 나머지를 소형주로 둡니다 
- 5*5 분류의 경우: 상위 1%를 초대형주, 1%~5%를 대형주, 5%~20%를 중형주, 20%~50%를 소형주, 50% 미만을 초소형주로 둡니다 
- 각 규모 구간(예: 대형/중형/소형) 안에서는 PER(또는 PBR)을 높은 순으로 정렬한 뒤, 해당 규모 구간 내 시가총액 누적비중이 50%에 도달하는 종목의 PER(PBR)을 대표값(중앙값)으로 정합니다
```
...
## Assistant Response
```text
대표 PER/PBR을 구하기 위해 “시장 규모 구간(대·중·소, 또는 초대형~초소형)”은 해당 시장에서 종목들을 시가총액 기준으로 몇 % 구간에 해당하는지로 나누어 설정합니다. 

- 대상 종목은 거래소+코스닥의 전체 종목(우선주, 뮤추얼펀드, 관리종목 등은 제외)으로 하되, 시가총액이 큰 순서로 정렬합니다. 
- 3*3(대형/중형/소형) 구간은 “전체 대상 종목수” 기준 상위 5%를 대형, 차순위 15%를 중형, 나머지를 소형으로 정합니다. 
- 5*5(초대형/대형/중형/소형/초소형) 구간은 “전체 대상 종목수” 기준 상위 1% 이내를 초대형, 상위 1%~5%를 대형, 상위 5%~20%를 중형, 상위 20%~50%를 소형, 상위 50% 미만을 초소형으로 정합니다. 
- 이렇게 나뉜 “규모별 그룹” 각각에 대해, 그 그룹에서 PER(PBR) 대표치를 산정합니다(대표치 산정 로직은 별도 단계).
```
...
## Assistant Response
```text
대표 PER, PBR에서 “시장 규모 구간”은 해당 시장의 종목을 시가총액 기준으로 나눈 다음, 그 구간별로 대표 PER(PBR)을 뽑기 위한 경계로 사용됩니다. 

- 시장 규모 구간은 종목의 시가총액을 기준으로 전체 종목을 내림차순 정렬한 뒤, 전체 대상 종목수 대비 상위 백분율 경계로 나눕니다.  
- 3*3(대·중·소) 경계: 대형=상위 5%, 중형=다음 15%(5~20%), 소형=그 외 구간입니다. 
- 5*5 경계: 초대형=상위 1% 이내, 대형=1~5%, 중형=5~20%, 소형=20~50%, 초소형=50% 미만입니다. 
- 구간 설정에 쓰는 대상은 거래소+코스닥 전 종목이지만, 우선주·뮤추얼펀드·관리종목(등) 및 구조조정 대상기업 등은 제외합니다.  
- 이렇게 나뉜 규모별 구간에서 PER(PBR) 대표값은 중앙값(Median)으로 정합니다.
```
