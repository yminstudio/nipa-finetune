...
## Question
시장 규모에 따른 대표 PER와 PBR 계산의 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "시장 규모에 따른 대표 PER와 PBR 계산의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "가) 시장의 규모별 대표 PER, PBR 계산",
  "seed_title": "가) 시장의 규모별 대표 PER, PBR 계산",
  "question": "시장 규모에 따른 대표 PER와 PBR 계산의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 시장 규모별 대표 PER(또는 PBR) 선정 시, 각 규모에서 PER(또는 PBR)의 중앙값(Median)을 대표치로 사용합니다.   
- 규모별 종목을 PER(PBR) 기준 내림차순으로 정렬한 뒤, PER(PBR)가 높은 종목부터 시가총액 누적비중이 50%에 도달하는 종목의 PER(PBR)을 대표 값으로 정합니다(=Median 해당).   
- 대표 PER(PBR) 산출 대상은 거래소·코스닥 전 종목이며, 우선주·뮤추얼펀드·관리종목 등은 제외합니다.   
- 규모 구분은 시장의 3*3(대형/중형/소형 및 초대형·초소형 등) 또는 5*5 구분 체계를 사용해, 각 규모 그룹 내에서 위의 Median(시가총액 누적비중 50%) 기준으로 대표 PER·PBR을 정합니다.
```
...
## Assistant Response
```text
시장 규모에 따른 대표 PER·PBR은 “해당 시장에서 종목 규모(대형/중형/소형 등)별로 PER(또는 PBR)의 대표 수준을 정하는 것”을 판단 기준으로 합니다. 

- 규모별 대표 PER(PBR)은 각 규모에서 PER(또는 PBR)의 중앙값(Median)을 대표치로 사용합니다.   
- 산정 대상은 거래소+코스닥의 전 종목이며, 우선주·뮤추얼펀드·관리종목 등은 제외합니다.   
- 해당 규모에 속한 종목을 PER(PBR)가 높은 종목부터 내림차순으로 나열합니다.   
- 나열한 뒤 “시가총액 누적비중 50%”에 해당하는 종목의 PER(PBR)를 그 규모 그룹의 대표 값으로 정합니다.
```
...
## Assistant Response
```text
시장(거래소+코스닥)에서 주식의 “규모별” 대표 PER(또는 PBR)를 정할 때의 판단 기준은 다음과 같습니다. 

- 규모별 PER(또는 PBR) 대표치는 해당 규모의 중앙값(Median)을 사용합니다.   
- 대표 Median은 PER(PBR)를 높은 종목부터 정렬했을 때, 시가총액 누적비중이 50%(=그 규모의 중간 지점)에 해당하는 종목의 PER(PBR)로 정합니다.   
- 기준 산정 대상은 거래소와 코스닥시장의 전 종목이며, 우선주·뮤추얼펀드·관리종목 등은 제외합니다.
```
...
## Assistant Response
```text
- 시장 규모별 대표 PER/PBR은 각 규모 구간에서 PER(또는 PBR)의 중앙값(Median)으로 정한다   
- 대표값은 거래소와 코스닥의 전체 종목을 대상으로 하되, 우선주·뮤추얼펀드·관리종목 등은 제외한다   
- 규모 구간 내 종목들에서 PER(또는 PBR) 높은 순으로 보았을 때, 시가총액 누적비중이 50%에 해당하는 종목의 PER(또는 PBR)을 대표값으로 사용한다   
- 이 기준은 시장의 규모를 나눈 각 구간(예: 대형/중형/소형 등)마다 동일하게 적용된다
```
