...
```json
{
  ...
  "target_total": 100,
  "generated_question": "채권 및 CP의 신용등급별 배점의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "[채권 및 CP의 신용등급별 배점]",
  "seed_title": "[채권 및 CP의 신용등급별 배점]",
  "question": "채권 및 CP의 신용등급별 배점의 정의는 무엇인가?",
...
## Assistant Response
```text
- 채권 및 CP의 신용등급별 배점은 “채권 신용등급명(또는 CP 신용등급명)”에 대응되는 배점 값을 부여하는 것으로 정의된다.   
- 각 신용등급은 부여되는 “신용등급 부여”(최상등급(1), 상등급(2), 상중등급(3), 중등급(4), 하등급(5)) 구분과 함께 배점 체계로 연결된다.
```
...
## Assistant Response
```text
채권 및 CP의 신용등급별 배점은 각 신용등급(채권: AAA+~C-미만, CP: A1+~D이하)에 대응해 부여되는 수치 점수(배점)이다. 

- 채권 배점 예: AAA+·AAA0·AAA- = 0, AA+ = 0.3, CCC+·CCC0·CCC- = 20, CC+·CC0·CC- = 30, C+·C0·C- = 50, C-미만 = 100.   
- CP 배점 예: A1+·A1·A1- = 0, A2+ = 0.5, A2 = 1, A3+ = 2, A3 = 2.5, A3- = 3, B+ = 5, B = 7, B- = 9, C+·C·C- = 30, D+·D·D이하 = 100.
```
...
## Assistant Response
```text
- 채권 및 CP의 신용등급별 배점은 각 자산의 신용등급명(채권 AAA+, AA+, … / CP A1+, A2+, …)에 대응해 부여되는 수치(배점)로 정의된다.   
- 배점은 ‘신용등급 부여’의 등급군(최상등급(1), 상등급(2), 상중등급(3), 중등급(4), 하등급(5))에 따라 정해진다.   
- 예를 들면, 채권 AAA+과 CP A1+은 배점 0(최상등급(1)), 채권 AA+은 배점 0.3(상등급(2)), 채권 BBB+는 배점 2.1(중등급(4)), 채권 BB+는 배점 3.5(하등급(5))이다.
```
...
## Assistant Response
```text
- 채권 및 CP의 신용등급별 배점이란, 채권 신용등급명과 CP 신용등급명 각각에 대해 표에 따라 부여되는 숫자 점수(배점) 값을 말한다.   
- 이 배점은 신용등급 부여 구간(최상등급(1), 상등급(2), 상중등급(3), 중등급(4), 하등급(5))에 따라 정해지며, 예를 들어 채권 AAA+~AAA- 및 CP A1+~A1-는 최상등급(1)으로 배점 0이 부여된다.   
- 그 외 등급도 표에 제시된 “신용등급명-배점” 매핑값을 그대로 사용한다.
```
