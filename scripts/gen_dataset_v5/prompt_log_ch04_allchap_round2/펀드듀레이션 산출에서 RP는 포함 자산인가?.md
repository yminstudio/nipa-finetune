...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드듀레이션 산출에서 RP는 포함 자산인가?",
  "qa_type": "criteria",
  "question_template": "펀드듀레이션_포함자산_검토_템플릿1"
}
```
...
```json
{
  ...
  "subsection": "가) 펀드듀레이션",
  "seed_title": "가) 펀드듀레이션",
  "question": "펀드듀레이션 산출에서 RP는 포함 자산인가?",
...
## Assistant Response
```text
- 예. 펀드듀레이션 산출의 대상 자산에 RP가 포함되어 있습니다.   
- 펀드듀레이션은 편입 종목별 시가평가액을 가중치로 하여 각 종목의 수정듀레이션을 합산하는데, 이때 RP도 해당 산출에 포함됩니다( RP는 잔존만기에 상관없이 D = 1/365 적용).
```
...
## Assistant Response
```text
펀드듀레이션 산출에서 RP가 포함되는지는 제로인에서 듀레이션 계산에 적용하는 대상 자산을 기준으로 판단합니다. 

- 네, RP는 펀드듀레이션 산출의 대상 자산에 포함됩니다.   
- RP의 듀레이션 값(D)은 잔존만기에 상관없이 D = 1/365로 적용합니다.   
- 펀드듀레이션은 펀드 내 편입 각 자산의 시가평가액 가중평균으로 산출하며, 이때 RP도 포함됩니다( D_F = Σ P_i × D_i / NAV ).
```
...
## Assistant Response
```text
펀드듀레이션 산출에서 “포함 자산” 여부는 듀레이션 계산 시 제로인에서 정의한 대상 자산 범위를 기준으로 판단합니다. 

- RP는 펀드듀레이션 산출의 대상 자산에 포함됩니다. 
- RP는 잔존만기에 상관없이 D=1/365를 적용합니다. 
- 최종 펀드듀레이션은 각 종목의 시가평가액 가중 DU레이션을 NAV로 나누는 방식으로 반영됩니다(여기에도 RP가 “대상 자산”으로 들어갑니다).
```
...
## Assistant Response
```text
펀드듀레이션 산출에서 RP가 포함 자산인지 여부는 “듀레이션 계산 시 반영하는 대상 자산” 기준으로 판단합니다.

- 제로인 펀드듀레이션 산출의 대상 자산에 RP가 포함됩니다.   
- 듀레이션은 펀드 내 편입종목별 시가평가액 가중으로 합산해 구하며, RP도 그 항목으로 반영됩니다.   
- RP는 잔존만기에 상관없이 듀레이션 값 D = 1/365를 적용합니다.
```
