...
## Question
개별종목의 상대 PER, PBR 계산에서 공통적으로 적용되는 판단 축은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "개별종목의 상대 PER, PBR 계산에서 공통적으로 적용되는 판단 축은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "나) 개별종목의 상대 PER, PBR 계산",
  "seed_title": "나) 개별종목의 상대 PER, PBR 계산",
  "question": "개별종목의 상대 PER, PBR 계산에서 공통적으로 적용되는 판단 축은 무엇인가?",
...
```text
- 종목의 규모(대형, 중형, 소형) 구분   
- 규모별 대표 PER(PBR) 대비 해당 종목의 PER(PBR) 상대 위치
```
...
## Assistant Response
```text
해당 종목의 PER·PBR 수준이 동종(동일 규모) 대비 어느 쪽에 위치하는지, 즉 가치특성을 상대적으로 보기 위한 공통 판단 축은 ‘규모(시가총액) 구분을 통한 대표 PER·PBR 대비’입니다. 

- 판단 축 1: 종목을 시장의 규모별(대형/중형/소형)로 나누어 상대 비교의 기준집단을 설정   
- 판단 축 2: 각 규모별 대표 PER(PBR)을 기준으로 삼음   
- 판단 축 3: 종목의 상대 PER(PBR)= 종목 PER(PBR) / 해당 종목 사이즈의 대표 PER(PBR)
```
...
## Assistant Response
```text
종목의 상대 PER, 상대 PBR이 ‘규모 차이’에 의해 왜곡되지 않도록, 어떤 기준 축으로 정규화할지를 판단하기 위한 것입니다. 

- 공통 판단 축: 종목의 규모 구분(대형, 중형, 소형)   
- 적용 기준: 해당 종목이 속한 규모의 대표 PER(또는 대표 PBR) 대비로 나누어 상대값을 산출
```
...
## Assistant Response
```text
개별종목의 상대 PER·PBR이 “절대 수준”이 아니라 “동일한 시장 규모 대비 상대적 수준”을 판단하기 위한 공통 축은 종목의 규모 구분입니다. 

- 판단 축: 종목이 속한 규모(대형, 중형, 소형)   
- 기준: 해당 규모의 대표 PER(PBR)을 분모로 두고, 종목의 PER(PBR)로 나눠 상대 PER(PBR)을 산출   
- 목적: 규모별 PER(PBR) 차이가 크고 영향도 다를 수 있어, 규모별로 상대 비교가 가능하도록 함
```
