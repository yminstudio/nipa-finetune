...
## Question
개별종목의 상대 PER 계산에서 판단 기준으로 사용되는 요소는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "개별종목의 상대 PER 계산에서 판단 기준으로 사용되는 요소는 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "나) 개별종목의 상대 PER, PBR 계산",
  "seed_title": "나) 개별종목의 상대 PER, PBR 계산",
  "question": "개별종목의 상대 PER 계산에서 판단 기준으로 사용되는 요소는 무엇인가?",
...
```text
- 종목의 PER   
- 해당 종목 사이즈(대형, 중형, 소형)의 대표 PER   
- 대표 PER은 시장 규모별 PER의 중앙값(Median)을 대표치로 사용
```
...
## Assistant Response
```text
개별종목의 상대 PER은 해당 종목의 PER 수준이 “자기 규모 구간”의 전형적인(대표) PER 대비 어느 정도인지를 판단하기 위한 기준입니다. 

- 종목의 PER(분자)
- 해당 종목이 속하는 규모(대형/중형/소형 등) 구분
- 해당 규모의 대표 PER(분모): 시장 규모별 PER의 대표치로 중앙값(Median)을 사용
```
...
## Assistant Response
```text
개별종목의 상대 PER은 해당 종목의 PER이 “어느 규모 구간에서의 대표 PER 대비” 어느 정도 수준인지 판단하기 위한 기준입니다. 

- 상대 PER의 분자: 해당 개별종목의 PER 
- 상대 PER의 분모: 해당 종목이 속하는 규모(대형, 중형, 소형)별 대표 PER 
- 규모별 대표 PER 산정 기준: 시장의 규모별 PER 중앙값(Median)으로, 시가총액 누적비중 50%에 해당하는 종목의 PER을 대표값으로 사용
```
...
## Assistant Response
```text
개별종목의 상대 PER은 해당 종목의 밸류(가치) 수준을 규모별로 상대 비교하기 위한 기준입니다. 

- 종목의 PER(해당 종목이 가지는 PER 값)   
- 종목이 속한 “규모(대형·중형·소형)”에 대응되는 대표 PER   
- 대표 PER은 해당 규모 집단에서의 PER 중앙값(Median)을 대표치로 사용   
- 상대 PER은 “종목 PER ÷ 해당 규모 대표 PER” 형태로 산출되므로, 위 두 값이 판단 기준 요소입니다
```
