...
## Question
시장 규모별 대표 PER 계산과 PBR 계산의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "시장 규모별 대표 PER 계산과 PBR 계산의 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_지표차이"
}
```
...
```json
{
  ...
  "subsection": "가) 시장의 규모별 대표 PER, PBR 계산",
  "seed_title": "가) 시장의 규모별 대표 PER, PBR 계산",
  "question": "시장 규모별 대표 PER 계산과 PBR 계산의 차이는 무엇인가?",
...
## Assistant Response
```text
- 시장 규모별 대표 PER과 대표 PBR은 “규모별로 해당 지표의 중앙값(Median)”을 대표값으로 쓰는 점은 동일합니다.   
- 다만 대표값을 정하는 대상 지표가 다릅니다: 대표 PER은 PER 기준, 대표 PBR은 PBR 기준으로 각 규모 집합의 종목을 내림차순 정렬해 누적합니다.   
- 누적 기준(해당 규모에서 시가총액 누적 비중 50%에 해당하는 종목의 PER(PBR) 선택)도 동일하며, 결과적으로 “50% 지점에서의 PER vs PBR”이 각각 대표값이 됩니다.   
- 대상 모집단(거래소·코스닥 전 종목, 우선주·뮤추얼펀드·관리종목 등 제외)과 규모 구간을 나누는 방식도 동일합니다.
```
...
## Assistant Response
```text
시장 규모별 대표 PER과 대표 PBR은 “시장 규모별 대표값”을 정하는 방식 자체는 동일하고, 대표로 삼는 지표가 PER이냐 PBR이냐만 다릅니다. 

- 대표값(대상): 거래소+코스닥 전체 종목 중 해당 규모(대형/중형/소형 등)에 속하는 종목의 PER(또는 PBR)을 사용합니다.   
- 대표값(산정): 각 규모별로 PER(또는 PBR)을 내림차순 정렬한 뒤, PER(PBR)이 높은 종목부터 시가총액 누적비중이 50%에 해당하는 구간의 PER(PBR)을 대표값으로 정합니다(규모별 중앙값 개념).   
- 핵심 차이: “대표 PER”은 PER을, “대표 PBR”은 PBR을 위 절차에 그대로 대입해 대표값을 산출합니다(절차는 PER/PBR 동일).   
- 이후 공통 구조: 이후 개별종목은 동일한 방식으로 “상대 PER(PBR) = 종목 PER(PBR) / 해당 규모 대표 PER(PBR)”을 계산합니다.
```
...
## Assistant Response
```text
시장 규모별 “대표 PER”과 “대표 PBR”은 계산 절차의 뼈대는 동일하고, 입력하는 지표(배수)가 PER이냐 PBR이냐만 달라집니다. 

- 시장 내 종목을 시장규모(대형/중형/소형 등, 3*3 또는 5*5 구분)별로 나눈 뒤, 각 규모 구간에서 PER(또는 PBR) 값을 기준으로 내림차순 정렬합니다.   
- PER “대표값”은 해당 규모 구간에서 PER이 높은 종목부터 시가총액 누적비중이 50%에 해당하는 시점의 PER을 대표치로 정합니다(대표치는 Median 취급).   
- PBR “대표값”도 동일하게, PBR이 높은 종목부터 시가총액 누적비중이 50%에 해당하는 시점의 PBR을 대표치로 정합니다(대표치는 Median 취급).   
- 즉, 차이는 “PER을 쓰는지 PBR을 쓰는지”뿐이며, 규모별 대표값을 정하는 누적비중 50% 및 Median 기반의 선정 로직은 동일합니다.
```
...
## Assistant Response
```text
제로인에서는 시장 규모별 “대표치”를 PER과 PBR 모두 같은 방식으로 산출하되, 대표값을 정할 때 쓰는 입력 지표가 PER인지 PBR인지가 핵심 차이입니다. 

- 대표 PER 계산: 거래소·코스닥 전 종목(우선주/뮤추얼펀드/관리종목/기타 제외 조건 반영) 중 해당 규모에 속한 종목을 PER 기준으로 내림차순 정렬한 뒤, 시가총액 누적비중이 50%에 해당하는 종목의 PER을 그 규모의 대표 PER로 정합니다(중앙값(Median) 개념).   
- 대표 PBR 계산: 대표 PER과 동일한 절차를 PBR에 대해 수행합니다. 즉, 해당 규모 종목을 PBR 내림차순으로 나열하고 시가총액 누적비중 50%에 해당하는 종목의 PBR을 대표 PBR로 정합니다(중앙값(Median) 개념).   
- 차이 요약: “어느 종목을 대표로 뽑는 규칙(규모별, PER/PBR 내림차순, 시총 누적 50%)”은 동일하며, 달라지는 것은 PER vs PBR 중 무엇을 정렬/대표값 산정 기준으로 쓰는지뿐입니다.
```
