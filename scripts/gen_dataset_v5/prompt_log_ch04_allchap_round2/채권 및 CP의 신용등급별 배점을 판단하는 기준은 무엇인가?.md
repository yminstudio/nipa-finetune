...
## Question
채권 및 CP의 신용등급별 배점을 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "채권 및 CP의 신용등급별 배점을 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "채권_CP_신용등급별_배점_기준"
}
```
...
```json
{
  ...
  "subsection": "[채권 및 CP의 신용등급별 배점]",
  "seed_title": "[채권 및 CP의 신용등급별 배점]",
  "question": "채권 및 CP의 신용등급별 배점을 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 채권 신용등급별 배점은 AAA(+/0/-) 0, AA(+/0/-) 0.3/0.6/0.9, A(+/0/-) 1.2/1.5/1.8, BBB(+/0/-) 2.1/2.4/2.7, BB(+/0/-) 3.5/5/7, B(+/0/-) 12, CCC+ 20으로 부여한다   
- CP 신용등급별 배점은 A1(+/0/-) 0, A2(+/0/-) 0.5/1/1.5, A3(+/0/-) 2/2.5/3, B+/B/B- 5/7/9로 부여한다
```
...
## Assistant Response
```text
채권 및 CP의 신용등급별 배점은 신용위험(신용도)을 반영하기 위해, 각 상품의 신용등급(채권 등급명/CP 등급명)에 미리 정해진 점수를 그대로 배점으로 부여하는 기준이다. 

- 채권 AAA+·AAA0·AAA-와 CP A1+·A1·A1-는 배점이 0(최상등급)이다. 
- 채권 AA+·AA0·AA-는 0.3·0.6·0.9, CP A2+·A2는 0.5·1로 상등급(2)에 해당한다. 
- 채권 B+·B0·B-는 모두 12(하등급)로 배점이 커지며, CP도 하등급(5)에 해당하는 등급(예: B+ 5, B- 9)별로 정해진 배점이 적용된다.
```
...
## Assistant Response
```text
- 채권 및 CP의 신용등급별 배점은 해당 자산의 신용등급(채권 등급명, CP 등급명)을 기준으로 배점표에서 등급명에 매칭되는 점수를 부여하는 방식으로 결정된다.   
- 채권은 AAA+~AAA- 배점이 0으로 부여되고, AA+는 0.3, BB0는 5, CCC+는 20처럼 신용등급이 낮아질수록 배점이 커지도록 정해져 있다.   
- CP는 A1+~A1- 배점이 0으로 부여되고, A2+는 0.5, B+는 5, C+는 50, D+ 및 C-미만은 100으로 부여되며, 각 등급은 최상(1)~하(5) 등급으로 함께 구분된다.
```
...
## Assistant Response
```text
- 채권 및 CP의 “신용등급별 배점”은, 해당 상품의 신용등급이 신용점수(배점)에 어떻게 대응되는지를 정해 두는 매칭을 기준으로 판단한다.   
- 채권은 KIS 채권평가사의 신용등급(무등급 제외)과 채권 신용등급-배점 매칭표에 따라 배점을 부여한다(예: AAA+·AAA0·AAA- 0, AA+ 0.3, AA0 0.6, BBB- 2.7, B+ 12, CCC+~CCC- 20, C+~C- 50, C-미만 100).    
- CP는 KIS 채권평가사의 CP 신용등급(무등급 제외)과 CP 신용등급-배점 매칭표에 따라 배점을 부여한다(예: A1+·A1·A1- 0, A2+ 0.5, A2 1, A2- 1.5, A3+ 2, A3 2.5, A3- 3, B+ 5, B 7, B- 9, C+·C0 30, C- 50, D+·D·D이하 100).
```
