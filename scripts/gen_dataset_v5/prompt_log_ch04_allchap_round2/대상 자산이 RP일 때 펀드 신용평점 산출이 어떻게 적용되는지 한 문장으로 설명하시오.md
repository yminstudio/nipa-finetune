...
## Question
대상 자산이 RP일 때 펀드 신용평점 산출이 어떻게 적용되는지 한 문장으로 설명하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "대상 자산이 RP일 때 펀드 신용평점 산출이 어떻게 적용되는지 한 문장으로 설명하시오.",
  "qa_type": "application",
  "question_template": "적용_RP사례"
}
```
...
```json
{
  ...
  "subsection": "나) 펀드 신용평점",
  "seed_title": "나) 펀드 신용평점",
  "question": "대상 자산이 RP일 때 펀드 신용평점 산출이 어떻게 적용되는지 한 문장으로 설명하시오.",
...
## Assistant Response
```text
- 펀드 신용평점 산출 시 RP도 대상 자산으로 보고, RP 종목 i의 시가평가액 P_i를 포트폴리오에 반영한다.   
- RP, 콜, 보통예금, 정기예금은 AAA0 등급을 부여하고, 이에 해당하는 DF_i(부도율)를 적용한다.   
- 이후 펀드 신용평점 DF_F는 DF_F = (∑ P_i × DF_i) / NAV로 산출한다.
```
...
## Assistant Response
```text
RP가 포함된 펀드의 신용평점은 RP에 부여된 신용등급(=부도율)을 다른 편입자산과 동일한 방식으로 시가평가액 비중 가중평균에 반영해 산출합니다.   
- 먼저 펀드 내 RP를 포함한 편입 종목 i의 시가평가액을 P_i로 두고, 펀드 NAV로 정규화할 준비를 합니다.   
- RP, 콜, 보통예금, 정기예금은 신용등급을 AAA0으로 부여하여(따라서 DF_i도 그에 대응하는 값으로) 종목 i의 부도율을 정합니다.   
- 이후 펀드 신용평점(DF_F)은 DF_F = (Σ P_i × DF_i) / NAV로 계산하되, RP에서 정한 DF_i가 RP의 P_i만큼 가중되어 합산됩니다.
```
...
## Assistant Response
```text
- 펀드 내 편입자산 중 RP가 “대상 자산”에 해당하면, RP에는 AAA0 신용등급을 부여한다.   
- 펀드 신용평점은 각 종목의 시가평가액 가중으로 부도율을 평균내는 방식으로 산출하며, DF_F = (Σ P_i × DF_i) / NAV 이다.   
- 전환 규칙은 RP(AAA0 신용등급) → 그에 해당하는 DF_i(부도율)로 매핑한 뒤, DF_i를 Σ P_i × DF_i에 반영해 NAV로 나누어 펀드 수준의 DF_F를 결정하는 것이다.
```
...
## Assistant Response
```text
펀드 신용평점 산출에서 펀드에 편입된 RP는 AAA0 등급을 부여받은 종목의 부도율(DF_i)로 전환한 뒤, RP의 시가평가액(P_i) 가중치를 포함해 모든 대상 종목의 P_i×DF_i 합을 NAV로 나눈 값(DF_F)으로 산출한다.
```
