...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CP등급별 예상부도율을 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "CP등급별_예상부도율_판단기준"
}
```
...
```json
{
  ...
  "subsection": "[CP등급별 예상부도율]",
  "seed_title": "[CP등급별 예상부도율]",
  "question": "CP등급별 예상부도율을 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- CP의 신용등급을 A1, A2, A3, B, C, D이하로 구분한 값에 따라 예상부도율을 정한다.   
- 예상부도율(%)은 A1 0.0, A2 1, A3 2.5, B 7, C 30, D이하 100 이다.   
- 따라서 CP등급별 예상부도율 판단 기준은 “CP 신용등급→해당 예상부도율”의 매핑표이다.
```
...
## Assistant Response
```text
CP의 신용등급에 대응되는 예상부도율(부도 위험 수준)을 정하기 위한 기준이다.  

- 예상부도율은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값(등급별 예상부도율)으로 제시된다   
- CP는 KIS 채권평가사에서 제공하는 신용등급(무등급 제외)을 사용해 CP 등급을 정하고, 그 등급에 해당하는 예상부도율을 매칭한다   
- CP 등급별 예상부도율은 A1, A2, A3, B, C, D이하 각각에 대해 고정된 수치로 구분해 제시한다
```
...
## Assistant Response
```text
CP 등급별 예상부도율은 CP 신용등급에 해당하는 부도율 값을 “어떻게 정했는지(추정 근거)”와 “어떤 등급에 어떤 값이 매칭되는지(매핑)”로 판단합니다. 

- 추정 근거: 과거 10년간 신용평가 등급별 부도율로부터 각 신용등급의 부도율(예상부도율)을 추정해 사용합니다. 
- 등급-값 매핑 기준: CP 등급별 예상부도율은 아래처럼 등급에 따라 고정 매핑됩니다.   
  - A1: 0.0% / A2: 1.0% / A3: 2.5% / B: 7.0% / C: 30% / D이하: 100%
- 따라서 판단 기준은 “해당 CP가 어떤 CP 등급(A1~D이하)에 해당하는가”이며, 그 등급에 연결된 예상부도율 값을 사용합니다.
```
...
## Assistant Response
```text
CP등급별 예상부도율은 “CP 신용등급”에 해당하는 등급의 예상부도율 값을 사용해 판단합니다. 

- 신용등급별 부도율(예상부도율)은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용합니다.   
- CP의 예상부도율은 아래 등급-값 매핑을 기준으로 합니다(예: A1 0.0%, A2 1.0%, A3 2.5%, B 7.0%, C 30.0%, D이하 100.0%).
```
