...
## Question
가치특성의 구분은 시장 규모별 대표 PBR를 기준으로 하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "가치특성의 구분은 시장 규모별 대표 PBR를 기준으로 하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_pbr"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "2) 가치특성의 구분",
  "question": "가치특성의 구분은 시장 규모별 대표 PBR를 기준으로 하는가?",
...
## Assistant Response
```text
- 가치특성의 구분은 시장 규모별 대표 PBR를 사용해 계산한 상대 PBR(해당 종목 사이즈의 대표 PBR 대비 종목의 PBR)과 상대 PER을 합산한 Value Score를 기준으로 합니다.   
- 따라서 시장 규모별 대표 PBR가 가치특성 분류의 판단 기준에 포함됩니다.
```
...
## Assistant Response
```text
펀드의 가치특성(가치형/혼합형/성장형)을 구분하기 위한 기준은 Value Score를 사용하는 것입니다.   
시장 규모별 대표 PBR은 해당 규모별 PER(또는 PBR) 중앙값(Median)으로 정하며, 종목의 PBR을 “해당 종목의 규모별 대표 PBR”로 나눈 상대 PBR이 Value Score 계산에 반영됩니다.    
Value Score는 “상대 PER + 상대 PBR”이며, Value Score가 1.75 미만이면 가치형, 1.75~2.25는 혼합형, 2.25 초과는 성장형으로 분류합니다.
```
...
## Assistant Response
```text
가치특성 구분은 펀드의 가치형/혼합형/성장형을 판별하기 위한 것입니다. 

- 기준이 되는 핵심 값은 시장 규모별 대표 PBR입니다(시장 규모별 대표 PER(PBR)을 중앙값(Median) 기준으로 정함).   
- 각 종목의 상대 PBR는 “해당 종목 PBR ÷ 해당 종목 사이즈의 대표 PBR”로 계산하며, 이 상대 PBR는 상대 PER과 함께 Value Score에 반영됩니다.    
- 최종적으로 Value Score 값의 구간에 따라 가치형/혼합형/성장형으로 분류합니다.
```
...
## Assistant Response
```text
가치특성 구분은 펀드의 투자성향(가치/성장)을 판단하기 위한 Value Score를 산출하는 데 목적이 있습니다.   
- 네, 시장 규모별 대표 PBR은 ‘상대 PBR’ 계산에 사용됩니다. 시장의 규모별 대표 PER(PBR)는 각 규모의 중앙값(Median)으로 정해지고, 개별종목 PBR은 그 대표 PBR로 나누어 상대 PBR을 구합니다.   
- 다만 가치특성의 구분은 상대 PBR만이 아니라 상대 PER과 함께 더해 Value Score를 만들고, 그 Value Score의 크기로 가치형/혼합형/성장형을 나눕니다(예: Value Score < 1.75는 가치형, 2.25 초과는 성장형).
```
