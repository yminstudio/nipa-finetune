...
## Question
펀드의 가치특성 점수(Value Score)의 정의를 한 문장으로 서술하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 가치특성 점수(Value Score)의 정의를 한 문장으로 서술하시오.",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "다) 펀드의 가치특성 점수(Value Score) 계산",
  "seed_title": "다) 펀드의 가치특성 점수(Value Score) 계산",
  "question": "펀드의 가치특성 점수(Value Score)의 정의를 한 문장으로 서술하시오.",
...
## Assistant Response
```text
- 펀드의 가치특성 점수(Value Score)는 펀드의 상대 PER + 펀드의 상대 PBR로 정의된다.   
- 이때 상대 PER(PBR)는 펀드 내 주식 중 누적편입비 40~60%에 해당하는 종목들의 상대 PER(PBR)를 편입비 가중 평균한 값이다.
```
...
## Assistant Response
```text
펀드의 가치특성 점수(Value Score)는 해당 펀드가 보유한 주식의 상대 PER과 상대 PBR을 합산해 산출하는 값이며, 정의는 “Value Score = 펀드의 상대 PER + 펀드의 상대 PBR”이다.
```
...
## Assistant Response
```text
- 펀드의 가치특성 점수(Value Score)는 펀드 내 가치특성을 나타내기 위해 상대 PER과 상대 PBR을 결합해 산출합니다.   
- Value Score = 펀드의 상대 PER + 펀드의 상대 PBR 입니다.
```
...
## Assistant Response
```text
- 펀드의 가치특성 점수(Value Score)는 해당 펀드의 주식 포트폴리오에서 산출한 상대 PER과 상대 PBR을 합산한 값으로, Value Score = 펀드의 상대 PER + 펀드의 상대 PBR이다.
```
