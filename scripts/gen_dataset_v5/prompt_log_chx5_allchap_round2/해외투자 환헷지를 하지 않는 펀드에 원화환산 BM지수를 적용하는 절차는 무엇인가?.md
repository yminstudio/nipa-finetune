...
## Question
해외투자 환헷지를 하지 않는 펀드에 원화환산 BM지수를 적용하는 절차는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "해외투자 환헷지를 하지 않는 펀드에 원화환산 BM지수를 적용하는 절차는 무엇인가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "해외투자 환헷지를 하지 않는 펀드에 원화환산 BM지수를 적용하는 절차는 무엇인가?",
...
## Assistant Response
```text
- 해외펀드의 펀드 BM을 먼저 설정한다. 투자설명서/규약에 명시된 비교지수를 펀드 BM으로 적용하고, 비교지수가 없으면 펀드 투자전략과 가장 유사한 BM으로 설정하되, 적합한 BM이 없거나 확신하기 힘든 경우 유형 BM을 적용한다.   
- 환헷지 구분을 확인한다. 환헷지를 하지 않는 해외펀드에 대해서는 로컬지수가 아니라 원화환산 지수(단, 적극적으로 밝힌 경우만 인정)를 BM으로 적용한다.   
- 원화환산 BM지수를 산출한다. 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율, 그리고 기산일 지수는 해당 BM의 첫 지수로 둔다.   
- 산출된 원화환산 BM지수를 BM 수익률 산출에 사용해 적용한다. BM의 일수익률은 (펀드 기준일–1일) 종가 / (펀드 기준일–1일)의 전영업일 종가로 계산한다.
```
...
## Assistant Response
```text
- 1) 해외펀드의 펀드 BM을 먼저 정한다: 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 두고, 없으면 펀드 투자전략과 가장 유사한 BM(적합한 BM이 없거나 확신이 어려운 경우 유형 BM)으로 정한다.   
- 2) 환헷지 구분을 확정한다: 해외투자에서 환헷지를 하지 않는 것으로 “적극적으로 밝힌 경우”에 한해 원화환산 BM지수를 적용한다(환헷지를 하는 경우는 로컬지수 적용).   
- 3) 전환 규칙으로 원화환산 BM지수를 산출한다: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율.   
- 4) 기산일을 맞춘다: 기산일 지수는 해당 BM의 첫 지수로 둔다.   
- 5) 위 절차로 산출한 원화환산 BM지수를 해당 펀드의 BM으로 적용해 성과를 비교한다.
```
...
## Assistant Response
```text
- 적용 조건 확인: 해외펀드 중 환헷지를 하지 않는 경우에는 원화환산 지수를 BM으로 적용하되, 환헷지 미실시가 ‘적극적으로 밝힌 경우’에 한해 인정합니다.   
- 적용 순서(1) 펀드 BM 확정: 투자설명서 또는 규약에 명시된 비교지수를 펀드 BM으로 설정하고, 명시가 없으면 펀드 투자전략과 가장 유사한 BM을 우선, 적합한 BM이 없거나 확신하기 어려우면 유형 BM을 설정합니다.   
- 적용 순서(2) 원화환산 BM지수 산출: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 산출하며, 기산일 지수는 해당 BM의 첫 지수로 둡니다.   
- 적용 순서(3) BM 수익률 산출 및 적용: 해외펀드는 지역별로 BM 적용일을 정한 뒤(예: 아시아·호주 당일, 그 외 당일-1), BM 일수익률은 r(i,BM)=J(i-1)/J(i-2)-1로 계산하여 원화환산 BM 기준으로 성과 계산에 반영합니다.
```
...
## Assistant Response
```text
- 해외펀드의 비교지수(펀드 BM)를 먼저 정한다(설명서·규약에 명시된 비교지수 우선, 없으면 유사 BM 또는 유형 BM).   
- 환헷지 여부를 확인한다: 해외투자 펀드가 “환헷지를 하지 않는 경우”에는 원화환산 BM지수를 적용하되, 투자설명서상 이를 적극적으로 밝힌 경우만 인정한다(환헷지를 하는 경우에는 로컬지수 적용).   
- 원화환산 BM지수를 산출한다: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율, 그리고 기산일 지수는 해당 BM의 첫 지수로 둔다.   
- 산출된 원화환산 BM지수를 펀드 성과평가에 적용한다: 펀드 기준일의 BM 적용일은 ‘펀드 기준일 - 1일’로 두고, 이 적용일의 BM 지수로 BM 수익률을 산출한다.
```
