...
## Question
기산일 지수와 BM지수 산출 기준 중 어느 것이 지수의 기본 기준이 되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "기산일 지수와 BM지수 산출 기준 중 어느 것이 지수의 기본 기준이 되는가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "기산일 지수와 BM지수 산출 기준 중 어느 것이 지수의 기본 기준이 되는가?",
...
```text
- BM지수 산출 기준이 기본이 됩니다.  
- 기산일 지수는 “해당 BM의 첫 지수”로 정의되어, BM 지수가 먼저 산출되고 그 첫 값이 기산일 지수로 잡힙니다.
```
...
## Assistant Response
```text
- 두 항목은 원화환산 BM지수 등을 만들거나 BM 수익률을 계산할 때 “출발점”을 정하는 데 같이 쓰입니다.  
- BM지수 산출 기준은 BM의 적용일과 BM 일수익률 산식을 통해(예: (기준일-1일) 종가를 (기준일-1일) 전영업일 종가와 비교) BM의 기준 흐름을 먼저 정합니다.   
- 기산일 지수는 그렇게 산출되는 해당 BM에서 “첫 지수(시작 값)”로 두는 값입니다(기산일 지수 := 해당 BM의 첫 지수).   
- 따라서 기본 기준이 되는 것은 BM지수 산출 기준이고, 기산일 지수는 그 BM의 첫 값을 지칭하는 방식입니다.
```
...
## Assistant Response
```text
- 공통 맥락: 제로인에서는 BM을 먼저 산출(적용)한 뒤, 그 BM의 “첫 값”을 기준으로 기산일 지수를 잡습니다.   
- BM지수 산출 기준(기본 적용): 펀드 기준일과 시장 BM의 일자를 맞추기 위해 BM 적용일을 정하는 방식 등, BM 지수의 산출/적용 기준이 출발점이 됩니다.   
- 기산일 지수: “해당 BM의 첫 지수”로 정의되어, BM지수 산출 기준(그 BM 값의 첫 시점)이 만들어낸 결과를 그대로 기준으로 삼습니다.
```
...
## Assistant Response
```text
- BM지수 산출 기준이 기본이 됩니다. BM지수의 수익률/지수값을 계산하는 출발점이 ‘해당 BM’ 자체이기 때문입니다.   
- 기산일 지수는 “해당 BM의 첫 지수”로 정의되어, BM지수 산출 결과를 기준으로 첫 값을 잡는 개념입니다.
```
