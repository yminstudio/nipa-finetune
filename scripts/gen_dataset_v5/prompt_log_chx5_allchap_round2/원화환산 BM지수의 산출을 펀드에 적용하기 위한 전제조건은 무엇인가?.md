...
## Question
원화환산 BM지수의 산출을 펀드에 적용하기 위한 전제조건은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출을 펀드에 적용하기 위한 전제조건은 무엇인가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출을 펀드에 적용하기 위한 전제조건은 무엇인가?",
...
## Assistant Response
```text
- 해외투자 펀드 중, 환헷지를 하지 않는 펀드에 원화환산 BM지수 산출을 적용하기 위해 사용   
- 해당 Local(or USD)지수를 기준으로 원화환산 BM지수를 산출   
- 산출식은 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 적용   
- 기산일 지수는 해당 BM의 첫 지수로 설정
```
...
## Assistant Response
```text
- 적용 대상: 해외투자 펀드 중 ‘환헷지를 하지 않는 펀드’에 원화환산 BM지수 산출을 적용하기 위한 전제조건이 됩니다.   
- 환헷지 구분 인정 조건: 해외펀드에서 환헷지를 하지 않는 경우 원화환산 지수로 적용하되, “적극적으로 밝힌 경우만” 인정합니다.    
- 적용 순서: (1) 펀드의 BM을 먼저 설정한 뒤, (2) 해외펀드 기준으로 환헷지 여부를 판단하고, (3) 환헷지를 하지 않는 것으로 인정되는 경우 원화환산 BM지수를 선택·적용합니다.   
- 전환(지수 선택) 규칙: 환헷지를 하는 것으로 인정되면 로컬지수를 BM으로 적용하고, 환헷지를 하지 않는 것으로 인정되면 원화환산 BM지수를 적용합니다.    
- 전환된 원화환산 BM지수 산출/기산일: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율, 기산일 지수는 해당 BM의 첫 지수로 둡니다.
```
...
## Assistant Response
```text
- 적용대상 확인: 해외투자 펀드 중에서 ‘환헷지를 하지 않는’ 경우에 한해 원화환산 BM지수 산출을 펀드 BM으로 적용하기 위해, 원화환산 지수 적용이 “적극적으로 밝힌 경우만 인정”됨을 전제로 합니다   
- 적용 순서 1) 환헷지 여부로 산출·적용 대상을 구분합니다(환헷지 미실시 → 원화환산 BM 적용)   
- 적용 순서 2) 입력 지수 선택: 해당 Local(or USD)지수를 산출식의 지수항으로 사용합니다   
- 전환 규칙 1) 원화환산 산출: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율   
- 전환 규칙 2) 기산일: 기산일 지수는 해당 BM의 첫 지수로 둡니다
```
...
## Assistant Response
```text
- 1) 적용 대상 펀드를 먼저 확정: 해외투자 펀드 중 환헷지를 하지 않는 펀드에 원화환산 BM지수 산출을 적용하기 위해 사용합니다.   
- 2) 환헷지에 따른 전환 규칙: 환헷지를 하는 경우에는 로컬지수를 BM으로 적용하고, 환헷지를 하지 않는 경우에는 원화환산 지수를 적용하되 ‘적극적으로 밝힌 경우만’ 인정합니다.   
- 3) 산출 적용 조건(입력요소): 해당 펀드에 사용할 Local(or USD) 지수와 (원화/로컬) 환율을 확보한 후, 원화환산 BM지수를 지수×(원화/로컬)환율로 산출합니다.   
- 4) 기산일 전제: 기산일 지수는 해당 BM의 첫 지수로 둡니다.
```
