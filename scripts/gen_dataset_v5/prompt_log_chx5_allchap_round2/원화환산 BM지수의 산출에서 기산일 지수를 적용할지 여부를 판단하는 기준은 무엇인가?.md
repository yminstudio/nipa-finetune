...
## Question
원화환산 BM지수의 산출에서 기산일 지수를 적용할지 여부를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출에서 기산일 지수를 적용할지 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출에서 기산일 지수를 적용할지 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수 산출에서 기산일 지수는 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 수행되는 경우에 적용한다.   
- 기산일 지수는 원화환산 대상 BM의 첫 지수로 정의한다.
```
...
## Assistant Response
```text
기산일 지수 적용 여부는 원화환산 BM지수 산출의 기준점(시작값)을 무엇으로 둘지 정하기 위한 기준이다. 

- 원화환산 BM지수 산출에서는 기산일 지수를 “해당 BM의 첫 지수”로 둔다. 
- 따라서 기산일 지수는 해당 BM의 최초 지수로 설정되는 값이므로, 원화환산 BM지수 산출 시 이를 기준으로 적용한다.
```
...
## Assistant Response
```text
기산일 지수 적용 여부는 원화환산 BM지수를 계산할 때 “기초값(기산일 지수)”을 무엇으로 둘지 결정하기 위한 기준입니다.

- 원화환산 BM지수 산출에 쓰는 해당 Local(or USD) BM의 첫 지수를 기산일 지수로 설정합니다.   
- 만약 사용되는 BM이 복합지수라면, 기산일은 포함된 BM 중 최단기간 존재하는 BM의 최초일을 기준으로 합니다.
```
