...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출 정의는 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수는 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위하여 산출하며, 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율이다.   
...
## Assistant Response
```text
- 원화환산 BM지수는 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 산출합니다.   
- 원화환산 BM지수 = 해당 Local(or USD) 지수 × (원화/로컬) 환율   
- 기산일 지수는 해당 BM의 첫 지수로 정의합니다.
```
...
## Assistant Response
```text
- 원화환산 BM지수는 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 산출하는 지수입니다.   
- 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율   
- 기산일 지수는 해당 BM의 첫 지수입니다.
```
