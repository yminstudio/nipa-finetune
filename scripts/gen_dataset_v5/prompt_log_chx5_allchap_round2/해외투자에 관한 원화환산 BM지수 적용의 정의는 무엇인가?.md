...
## Question
해외투자에 관한 원화환산 BM지수 적용의 정의는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "해외투자에 관한 원화환산 BM지수 적용의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "definition_term_specific"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "해외투자에 관한 원화환산 BM지수 적용의 정의는 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수 적용은 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위하여 산출한 BM지수를 말한다.   
- 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율이다.   
- 기산일 지수는 해당 BM의 첫 지수로 정의한다.
```
...
## Assistant Response
```text
- 해외투자 펀드 중 환헷지를 하지 않는 경우, 해당 펀드의 BM을 원화 기준으로 환산하기 위해 원화환산 BM지수를 적용합니다.   
- 원화환산 BM지수의 정의는 “원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율”입니다.   
- 이때 원화환산 BM지수의 기산일은 해당 BM의 첫 지수입니다.
```
...
## Assistant Response
```text
해외투자에서 원화환산 BM지수 적용은, 환헷지를 하지 않는 해외투자 펀드에 대해 원화 기준의 BM을 만들기 위해 사용하는 적용 정의이다. 

- 정의: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율   
- 기산일 지수: 해당 BM의 첫 지수
```
...
## Assistant Response
```text
- 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 산출하는 지수가 원화환산 BM지수이다.   
- 원화환산 BM지수의 정의는 “해당 Local(or USD)지수 × (원화/로컬)환율”로 계산하는 것이다.   
- 기산일 지수는 해당 BM의 첫 지수로 본다.
```
