...
## Question
원화환산 BM지수의 산출을 펀드에 적용하기 위한 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출을 펀드에 적용하기 위한 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출을 펀드에 적용하기 위한 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 해외펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 원화환산 BM지수를 산출한다.   
- 환헷지 여부로 BM을 구분하며, 환헷지를 하지 않는 경우에 원화환산 지수를 적용하되 ‘적극적으로 밝힌 경우만’ 인정한다.   
- 환헷지로 인정되는지 여부는 투자설명서/규약 문구와 투자지역 상황에 따라 판단한다(예: ‘필요한 경우 환헷지를 할 수 있다’는 환헷지로 인정, ‘현재 환헷지 수단이 없지만 후에…’는 지역별로 현재 환헷지 가능 여부를 판단).
```
...
## Assistant Response
```text
해외펀드에서 원화환산 BM지수(원화환산 지수)를 BM으로 적용할지 판단하기 위한 기준은 “환헷지 여부(및 그 인정 방식)”입니다.   

- 해외투자 펀드 중 환헷지를 하지 않는 펀드에 원화환산 BM지수 산출을 적용합니다.   
- 환헷지를 하는 경우에는 로컬지수를 적용합니다.   
- 환헷지를 하지 않는 경우 원화환산 지수를 적용하되, “적극적으로 밝힌 경우만 인정”합니다.
```
...
## Assistant Response
```text
원화환산 BM지수의 적용은 “해외펀드가 환위험에 대해 환헷지를 하는지 여부”를 기준으로 판단하기 위한 것입니다. 

- 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 원화환산 BM지수를 산출합니다.   
- 환헷지를 하지 않는 경우에는 원화환산 지수를 BM으로 적용하되, “적극적으로 밝힌 경우만” 인정합니다.   
- (대조기준) 환헷지를 하는 경우에는 로컬지수를 적용합니다.
```
