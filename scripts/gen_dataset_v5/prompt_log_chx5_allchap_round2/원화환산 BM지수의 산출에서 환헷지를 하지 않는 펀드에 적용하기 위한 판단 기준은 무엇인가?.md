...
## Question
원화환산 BM지수의 산출에서 환헷지를 하지 않는 펀드에 적용하기 위한 판단 기준은 무엇인가?
...
```json
{
  ...
  "call_requested_question_count": 56,
  "call_allowed_types": [
    "criteria"
  ],
  "call_type_targets": {
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출에서 환헷지를 하지 않는 펀드에 적용하기 위한 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
[질문 유형 목표]
criteria 56개, definition 0개, comparison 0개, application 0개
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출에서 환헷지를 하지 않는 펀드에 적용하기 위한 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수는 해외투자 펀드 중 ‘환헷지를 하지 않는 펀드’에 적용하기 위하여 산출한다.   
- 해외펀드의 환헷지 구분은 투자설명서상 환헷지 가능 여부를 기준으로 하며, ‘환헷지를 하지 않는 경우’에만 원화환산 지수 적용(단, 적극적으로 밝힌 경우만 인정)한다.   
- 투자설명서에 ‘필요한 경우 환헷지를 할 수 있다’라고 되어 있으면 환헷지를 하는 경우로 인정한다.   
- ‘지금은 환헷지 수단이 없지만 후에 환헷지 수단이 생기는 경우 환헷지를 할 수 있다’는 문구는 투자하는 지역의 통화 및 상황에 따라 판단하며, 예: 인도는 현재 환헷지 할 수 없음, 러시아는 현재 환헷지 가능, 브라질은 ADR 위주 투자면 USD와 환헷지로 인정(다만 브라질 Local 투자면 환헷지 할 수 없음).
```
...
## Assistant Response
```text
원화환산 BM지수의 산출이 “환헷지를 하지 않는 해외펀드”에 적용되는지를 가리기 위한 판단 기준입니다. 

- 환헷지를 하지 않는 경우, 원화환산 지수 적용 
- 다만 환헷지 미실시로 “적극적으로 밝힌 경우만” 비헷지로 인정 
- 투자설명서 문구가 “필요한 경우 환헷지를 할 수 있다”면 환헷지 하는 경우로 인정 
- “지금은 환헷지 수단이 없지만 후에 가능”은 투자 지역의 통화·상황에 따라 현재 환헷지 가능 여부로 판단(예: 인도=현재 환헷지 불가→비헷지, 러시아=현재 환헷지 가능→환헷지)
```
...
## Assistant Response
```text
원화환산 BM지수 적용을 위해, 펀드가 “환헷지를 하는 경우”인지 “환헷지를 하지 않는 경우”인지 여부를 판단하기 위한 기준입니다. 

- 해외펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 원화환산 BM지수를 산출합니다. 
- 환헷지를 하지 않는 경우에는 원화환산 지수를 적용하되, “적극적으로 밝힌 경우만 인정”합니다. 
- 투자설명서에 “필요한 경우 환헷지를 할 수 있다”처럼 환헷지 가능 의사가 명시되면 환헷지를 하는 경우로 판단합니다. 
- “지금은 환헷지 수단이 없지만 후에 환헷지 수단이 생기면 할 수 있다”처럼 조건부인 경우는 투자지역의 통화 및 상황에 따라 판단하며, 문서의 예시는 인도(현재 환헷지 불가), 러시아(현재 환헷지 가능), 브라질(Local에 투자=환헷지 불가 / ADR=USD와 환헷지로 인정)입니다.
```
