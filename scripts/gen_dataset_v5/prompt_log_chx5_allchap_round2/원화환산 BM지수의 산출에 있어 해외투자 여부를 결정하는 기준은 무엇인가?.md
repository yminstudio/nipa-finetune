...
## Question
원화환산 BM지수의 산출에 있어 해외투자 여부를 결정하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출에 있어 해외투자 여부를 결정하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출에 있어 해외투자 여부를 결정하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수는 “해외투자 펀드” 중에서 “환헷지를 하지 않는 펀드”에 적용하기 위해 산출한다.   
- 해외펀드에서 환헷지를 하는 경우에는 로컬지수를 적용하고, 환헷지를 하지 않는 경우에는 원화환산 지수를 적용한다(단, 적극적으로 밝힌 경우만 인정).
```
...
## Assistant Response
```text
원화환산 BM지수의 산출/적용이 “해외투자 펀드 중 환위험을 어떻게 다루는지”를 판단하기 위한 것이라면, 해외투자 여부 및 원화환산 대상 여부는 아래 조건으로 정해집니다. 

- 해당 펀드가 “해외투자 펀드”에 해당하는 경우 
- 그중 “환헷지를 하지 않는 펀드”에 적용하기 위해 원화환산 BM지수를 산출 
- 반대로 “환헷지를 하는 경우”에는 원화환산이 아니라 로컬지수를 적용
```
...
## Assistant Response
```text
원화환산 BM지수는 해외통화 노출을 환율로 반영해야 하는 비교지수 선택 여부를 판단하기 위한 것이다. 

- 원화환산 BM지수 산출 대상은 “해외투자 펀드”이다.   
- 그중에서도 “환헷지를 하지 않는 펀드”에 적용하기 위해 산출한다.
```
