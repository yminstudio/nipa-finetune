...
## Question
원화환산 BM지수의 산출이 적용되기 위한 펀드의 정의는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 BM지수의 산출이 적용되기 위한 펀드의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 BM지수의 산출이 적용되기 위한 펀드의 정의는 무엇인가?",
...
## Assistant Response
```text
- 원화환산 BM지수의 산출이 적용되는 펀드의 정의는 “해외투자 펀드 중, 환헷지를 하지 않는 펀드”입니다.   
- 이러한 펀드에 적용하기 위해 원화환산 BM지수를 산출합니다.
```
...
## Assistant Response
```text
- 원화환산 BM지수의 산출은 해외투자 펀드 중 환헷지를 하지 않는 펀드에 적용하기 위해 산출합니다.   
- 따라서 “원화환산 BM지수 적용 펀드”는 해외투자 펀드이면서 환헷지를 하지 않는(적극적으로 밝힌 경우만 인정) 펀드로 정의됩니다.
```
...
## Assistant Response
```text
- 원화환산 BM지수의 산출이 적용되는 펀드는 “해외투자 펀드”이며, 그중에서도 “환헷지를 하지 않는 펀드”를 대상으로 합니다.   
- 정의형으로 정리하면, 환헤지 비적용(비환헤지)인 해외투자 펀드에 대해 원화환산 BM지수 산출이 적용됩니다.
```
...
## Assistant Response
```text
- 원화환산 BM지수의 산출이 적용되는 펀드는 해외투자 펀드 중 환헷지를 하지 않는 펀드이다.   
- 환헷지를 하지 않는 경우에는 원화환산 지수를 적용하며, 이 구분은 적극적으로 밝힌 경우만 인정한다.
```
