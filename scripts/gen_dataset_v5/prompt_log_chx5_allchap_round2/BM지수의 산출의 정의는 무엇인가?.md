...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM지수의 산출의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- BM지수의 산출(정의)은 BM의 일수익률을, 해당 기간의 BM 종가지수를 이용해 구하는 것으로 규정합니다.   
- BM의 일수익률은 r_{i,BM} = (J_{i-1}/J_{i-2}) - 1 이며, 여기서 J_{i-1}은 i-1일 BM 종가지수, J_{i-2}는 (i-1일의 전영업일) BM 종가지수입니다.
```
...
## Assistant Response
```text
- 제로인에서 BM지수(벤치마크 지수)는 펀드 성과측정에 쓰기 위해, 기준가/종가 개념에 맞춰 지수값을 이용해 수익률(성과)로 환산되며, BM 일수익률은  r_{i,BM} = J_{i-1}/J_{i-2} - 1  (여기서 J_{i-1}: i-1일 BM 종가지수, J_{i-2}: i-1일의 전영업일 BM 종가지수)로 정의합니다 .  
- 해외투자에서 환헷지를 하지 않는 경우의 원화환산 BM지수는  원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율 로 정의합니다 .  
- 환헷지 반영을 위해 사용하는 KRW Hedged 지수는  HedgedIndex_t = HedgedIndex_0 × (1 + HedgedIndexR_t) 로 정의하며, HedgedIndexR_t는 지수 움직임( I_t / I_0 )과 환 관련 항(ForwardReturn, CurrencyReturn 등)을 반영해 산출합니다 .
```
...
## Assistant Response
```text
- 펀드 기준가의 기준일에 대응하여 BM을 적용하기 위해, BM 적용일은 “펀드 기준일 − 1일”로 정의합니다.   
- 이때 BM의 일 수익률은 “(펀드 기준일 − 1일)의 BM 종가지수 / (펀드 기준일 − 1일의 전영업일) BM 종가지수 − 1”로 정의됩니다.    
- 각 기간 동안의 BM 수익률은 위와 같이 산출된 BM의 일수익률을 기준으로, 일별 BM 수익률을 시간가중하여 산출하는 것으로 정의됩니다.
```
