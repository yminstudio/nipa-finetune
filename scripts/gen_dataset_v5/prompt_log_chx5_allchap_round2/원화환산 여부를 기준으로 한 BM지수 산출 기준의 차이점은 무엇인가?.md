...
## Question
원화환산 여부를 기준으로 한 BM지수 산출 기준의 차이점은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 여부를 기준으로 한 BM지수 산출 기준의 차이점은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 여부를 기준으로 한 BM지수 산출 기준의 차이점은 무엇인가?",
...
## Assistant Response
```text
- 환헷지를 하지 않는 경우에는 원화환산 BM지수를 산출하며, 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 환산해 사용합니다.   
- 환헷지를 하는 경우에는 로컬지수를 BM으로 적용하며, 원화환산 과정을 적용하는 방식이 아닙니다.   
- 원화환산 BM지수의 기산일 지수는 해당 BM의 첫 지수로 둡니다.
```
...
## Assistant Response
```text
- 공통 맥락: 해외펀드의 BM은 환헷지 여부에 따라 적용되는 지수 형태가 달라집니다. 환헷지를 하는 경우에는 로컬지수를 적용하고, 환헷지를 하지 않는 경우에는 원화환산 지수를 적용합니다.   
- 원화환산 BM지수: 해당 Local(or USD) 지수에 (원화/로컬) 환율을 곱해 원화 기준으로 환산해 산출합니다.   
- KRW Hedged 지수: 환에 대한 노출을 피하기 위해 헤지비율을 반영해, USD 지수를 바탕으로 Forward 거래를 통한 헤지비용을 감안하는 방식으로 별도 산출합니다.
```
...
## Assistant Response
```text
- 해외펀드의 경우 환헷지 여부에 따라 BM을 원화환산 기준으로 잡을지(원화환산 지수) 또는 원화환산 없이 다른 기준지수로 잡을지가 달라집니다.   
- 환헷지를 하지 않는 경우: 원화환산 BM지수를 적용하며, 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 산출합니다.   
- 환헷지를 하는 경우: 로컬지수를 적용하며, 원화환산 지수처럼 환율을 곱하는 방식의 산출을 BM 기준으로 사용하지 않습니다.
```
...
## Assistant Response
```text
- 원화환산 BM지수는 해외투자 펀드 중 환헷지를 하지 않는 경우에 적용하며, BM을 원화로 만들기 위해 해당 Local(or USD)지수에 “원화/로컬 환율”을 곱해 산출합니다.   
- (대비) KRW Hedged 지수는 환에 대한 노출을 피하고(100% 환헤지 가정) 성과를 비교하기 위한 BM으로, 단순 환율 곱이 아니라 USD지수 기반에 Forward 거래 및 헤지비용을 반영하는 방식으로 산출합니다.    
- 따라서 원화환산 여부(환헷지 반영 방식) 관점에서는 “환율을 곱해 원화로 환산하는지”와 “헤지비용까지 포함해 KRW Hedged로 재산출하는지”가 핵심 차이입니다.
```
