...
## Question
원화환산 여부에 따른 BM지수 산출의 차이점은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "원화환산 여부에 따른 BM지수 산출의 차이점은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "X.5. 원화환산 BM지수의 산출",
  "question": "원화환산 여부에 따른 BM지수 산출의 차이점은 무엇인가?",
...
## Assistant Response
```text
- 환헷지를 하는 경우에는 BM을 로컬지수로 적용해 원화환산 과정을 반영하지 않습니다.   
- 환헷지를 하지 않는 경우에는 BM을 원화환산 지수로 적용하며, 원화환산 BM지수는 해당 Local(or USD)지수에 (원화/로컬) 환율을 곱해 산출합니다.   
- 원화환산 BM지수는 기산일 지수를 “해당 BM의 첫 지수”로 둡니다.
```
...
## Assistant Response
```text
- 해외펀드의 BM은 환헷지 여부에 따라 “원화환산 BM지수”를 쓸지(원화 표시) 또는 “로컬지수”를 쓸지(원화 환산 없이)로 달라집니다.   
- 원화환산 여부가 “있음(원화환산 BM지수 적용)”인 경우: 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 산출합니다(기산일 지수는 해당 BM의 첫 지수).   
- 원화환산 여부가 “없음(로컬지수 적용)”인 경우: 환헷지를 하는 경우 BM은 로컬지수를 그대로 적용합니다.
```
...
## Assistant Response
```text
- 공통 맥락: 해외펀드 BM은 환헷지 여부에 따라 “로컬지수 그대로 쓰는 경우”와 “원화로 환산해 쓰는 경우”로 구분됩니다.   
- 원화환산 BM지수(환헷지 미실시): 원화환산 BM지수 = 해당 Local(or USD)지수 × (원화/로컬)환율로 산출합니다(기산일 지수는 해당 BM의 첫 지수).   
- 원화환산 미실시(환헷지 실시): 로컬지수를 BM으로 적용하며, 원화/로컬 환율을 곱해 원화 기준으로 변환하는 방식이 아니라 로컬지수 자체를 사용합니다.
```
...
## Assistant Response
```text
- 해외펀드의 BM은 환헷지를 하는 경우 로컬지수를 적용하고, 환헷지를 하지 않는 경우 원화환산 지수를 적용합니다.   
- 원화환산 BM지수는 “해당 Local(or USD)지수 × (원화/로컬)환율”처럼 환율을 곱해 산출합니다.   
- 환율을 곱하지 않는(원화환산을 하지 않는) BM은 로컬지수를 그대로 적용하므로, 지수 산출에 환율 반영이 없습니다.
```
