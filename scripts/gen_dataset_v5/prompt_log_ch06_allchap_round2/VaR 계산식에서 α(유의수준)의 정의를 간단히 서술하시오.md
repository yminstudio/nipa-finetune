...
## Question
VaR 계산식에서 α(유의수준)의 정의를 간단히 서술하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 α(유의수준)의 정의를 간단히 서술하시오.",
  "qa_type": "definition",
  "question_template": "정의1"
}
```
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 α(유의수준)의 정의를 간단히 서술하시오.",
...
## Assistant Response
```text
- VaR 계산식에서 α(유의수준)는 유의수준(Level of Significance)으로 정의한다.   
- 이때 신뢰수준은 (1-α)×100으로 나타낸다.
```
...
## Assistant Response
```text
- VaR 계산식에서 α(유의수준)는 “유의수준(Level of Significance)”을 의미합니다.   
- 또한 α는 (1-α) × 100을 신뢰수준으로 나타내는 값으로 정의됩니다.
```
...
## Assistant Response
```text
- VaR 계산식에서 α(유의수준)는 유의수준(Level of Significance)을 의미한다.   
- α는 (1-α)×100으로 표시되는 신뢰수준과 대응된다.
```
...
## Assistant Response
```text
- VaR 계산식에서 α(유의수준)는 유의수준(Level of Significance)을 뜻한다.   
- 즉, 신뢰수준은 (1−α)×100으로 표현되며, α는 그 보완 관계로 정해진다.
```
