...
```json
{
  ...
  "target_total": 100,
  "generated_question": "EWMA의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "definition_EWMA"
}
```
...
## Assistant Response
```text
- EWMA(Exponentially Weighted Moving Average)는 과거 수익률 제곱에 지수 가중치를 부여해 변동성(분산)을 추정하는 방식으로, KOSPI 변동성의 측정에 사용합니다.   
- KOSPI 변동성은 다음과 같이 정의됩니다.  \n  σ²_{t+1|t} = (1-λ) ∑_{t=1}^{T} λ^{t-1} r_t² = λ σ²_{t|t-1} + (1-λ) r
```
...
## Assistant Response
```text
EWMA(Exponentially Weighted Moving Average)는 지수적으로 가중치가 감소하는 방식으로 과거 수익률의 제곱( r_t^2 )을 누적해 조건부 분산(변동성)을 추정하는 방법입니다. 

문서에서는 이를 KOSPI 변동성 측정에 사용하며, 다음처럼 정의/표현합니다. 

- σ²_{t+1|t} = (1-λ) · ∑_{t=1}^{T} λ^{t-1} r_t² 
- 또한 같은 값을 σ²_{t+1|t} = λ σ²_{t|t-1} + (1-λ) r 형태로도 나타냅니다.
```
...
## Assistant Response
```text
EWMA(Exponentially Weighted Moving Average)는 지수적으로 가중치를 부여한 이동평균 방식으로, (예: KOSPI) 수익률의 제곱을 가중평균하여 다음 시점의 변동성(분산)을 추정하는 방법입니다. 

- EWMA 분산 추정식:  \n  σ²_{t+1|t} = (1-λ) ∑_{t=1}^{T} λ^{t-1} r_t² 
- 같은 식을 재귀적으로 표현하면:  \n  σ²_{t+1|t} = λ σ²_{t|t-1} + (1-λ) r
```
...
## Assistant Response
```text
- EWMA(Exponentially Weighted Moving Average, 지수이동평균)는 과거 수익률 제곱에 지수적으로 가중치를 부여해 분산(변동성)을 추정하는 방식입니다.   
- 제로인에서는 KOSPI 변동성 측정에 EWMA를 사용하며, 분산을 다음과 같이 계산합니다.  \n  σ²(t+1|t) = (1-λ)∑(t=1~T) λ^(t-1) r_t² = λ σ²(t|t-1) + (1-λ) r
```
