...
## Question
VaR 계산식에서 α(유의수준)와 Significance의 관계를 비교하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 α(유의수준)와 Significance의 관계를 비교하시오.",
  "qa_type": "comparison",
  "question_template": "비교1"
}
```
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 α(유의수준)와 Significance의 관계를 비교하시오.",
...
## Assistant Response
```text
- VaR 계산식에서 α는 “유의수준(Level of Significance)”로 정의되므로, α와 Significance는 같은 의미(같은 레벨)를 나타냅니다.   
- 즉 Significance가 의미하는 유의수준의 값이 α이며, (1-α)×100이 신뢰수준으로 함께 연결됩니다.
```
...
## Assistant Response
```text
- VaR(델타노멀) 계산식에서 α는 “유의수준(Level of Significance)”으로 정의되므로, α와 Significance는 같은 값(동일 개념)입니다.   
- 같은 맥락에서 Significance(=α)는 “신뢰수준 = (1-α)×100”으로도 표현됩니다.
```
...
## Assistant Response
```text
- VaR 계산식에서 α는 “유의수준(Level of Significance)”으로 정의되어, α와 Significance는 같은 개념(동일한 값/표기)으로 대응됩니다.   
- 따라서 Significance를 설정한다는 것은 곧 α를 정하는 것이며, 문서에서는 (1-α)×100을 신뢰수준으로 함께 연결해 둡니다.   
- 결론적으로 α는 Significance의 수치 표현이고, Significance가 커질수록 (1-α)×100(신뢰수준)은 작아지는 관계로 정리됩니다.
```
...
## Assistant Response
```text
- VaR 계산식에서 α는 “유의수준(Level of Significance)”으로 정의되며, Significance 자체가 α를 의미하는 공통 맥락에서 같은 역할을 갖는다.   
- 즉, VaR 산출에 들어가는 α와 Significance는 서로 대응되는 값(동일한 개념/계수)으로 비교된다.   
- 또한 α는 신뢰수준과 반대방향 관계로 함께 정리되는데, 신뢰수준=(1−α)×100으로 제시된다.
```
