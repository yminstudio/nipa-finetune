...
## Question
VaR 계산식에서 Level(유의수준)의 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 Level(유의수준)의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 Level(유의수준)의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- VaR 계산식에서 Level(유의수준)은 α로 두며, 이를 유의수준(Level of Significance)으로 정의합니다   
- Level(유의수준) α는 (1-α)×100을 신뢰수준으로 볼 수 있다는 관계로 판단합니다
```
...
## Assistant Response
```text
- VaR 계산식에서 Level(유의수준)은 VaR 산출 시 적용되는 유의수준/신뢰수준을 정하기 위한 값이다.   
- 제로인 방법론에서는 VaR의 α를 “유의수준(Level of Significance)”으로 두며, 동시에 (1−α)×100을 “신뢰수준”으로 본다.   
- 따라서 Level(유의수준)의 판단 기준은 신뢰수준을 (1−α)×100 관계로 환산해 α를 설정하는 데에 있다.
```
...
## Assistant Response
```text
VaR 계산에서 Level(유의수준)은 “얼마나 보수적으로(또는 공격적으로) 손실 범위를 볼지”를 정하는 값입니다. 

- VaR 식에서 Level은 α로 두며, α를 유의수준(Level of Significance)으로 판단합니다.   
- 동일한 α는 신뢰수준과도 연결되어, (1-α)×100을 신뢰수준으로 봅니다.
```
...
## Assistant Response
```text
VaR에서 Level(유의수준)은 “얼마나 높은 신뢰수준에서(손실이 그 값을 초과할 가능성을 어느 수준으로 볼 것인지)” 위험을 평가하는지를 정하는 기준입니다. 

- Level(유의수준) = α로 두며,   
- α는 (1-α)×100을 신뢰수준으로 해석합니다.
```
