...
## Question
Delta-Normal Method에서 Delta와 Normal의 역할을 비교하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta-Normal Method에서 Delta와 Normal의 역할을 비교하시오.",
  "qa_type": "comparison",
  "question_template": "comparison_delta_normal_roles"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "6.5. 주식 포트폴리오의 VaR",
  "question": "Delta-Normal Method에서 Delta와 Normal의 역할을 비교하시오.",
...
## Assistant Response
```text
- Normal은 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정하는 역할이다.   
- Delta는 주식 포트폴리오 가치와 KOSPI 사이의 선형관계를 가정하며, 이때의 위험민감도(델타)가 시간에 따라 변할 수 있다는 점이 함께 언급된다.   
- 따라서 Normal은 수익률의 분포 가정을, Delta는 포트폴리오-시장 간 연계(선형관계/위험민감도)를 중심으로 VaR 계산의 기반이 된다.
```
...
## Assistant Response
```text
Delta-Normal Method는 KOSPI를 기초로 주식 포트폴리오의 VaR를 산출한다는 공통 맥락에서, ‘Normal’과 ‘Delta’가 서로 다른 가정을 담당한다. 

- Normal: KOSPI 수익률이 정규분포를 갖는다고 보는 가정이다.   
- Delta: 포트폴리오 가치와 KOSPI 사이의 선형관계를 전제로, 포트폴리오의 위험민감도(델타) 개념이 반영되는 축이다.   
- 정리하면, Normal은 수익률 분포의 형태를, Delta는 포트폴리오 위험이 시장( KOSPI )에 반응하는 민감도를 구분해 다룬다.
```
...
## Assistant Response
```text
Delta-Normal Method는 주식 포트폴리오의 VaR를 산출할 때, KOSPI(기초적 시장가격)와 포트폴리오 가치 사이의 관계를 전제로 한다. 

- Delta의 역할: 포트폴리오의 위험민감도(델타)를 반영하며, 포트폴리오의 위험민감도(델타)가 시간에 따라 변할 수 있다고 본다.   
- Normal의 역할: KOSPI 수익률이 정규분포를 갖는다고 가정해, VaR 계산에 필요한 분포 가정을 제공한다.
```
...
## Assistant Response
```text
Delta-Normal Method는 VaR 산출에서, “Normal”과 “Delta”가 각각 서로 다른 가정을 담당한다. 

- Normal의 역할: 기초적 시장가격(KOSPI) 수익률이 정규분포를 가진다고 가정해 분포(확률) 측면의 전제를 제공한다.   
- Delta의 역할: 주식 포트폴리오 가치와 KOSPI 사이의 선형관계를 가정하며, 그 과정에서 포트폴리오의 위험민감도(델타)가 시간에 따라 변할 수 있음을 전제로 한다.
```
