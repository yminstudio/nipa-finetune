...
## Question
VaR 계산식에서 Where 절이 제시하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 Where 절이 제시하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 Where 절이 제시하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- α : 유의수준(Level of Significance) 또는 (1-α)×100 신뢰수준  
- T : 포트폴리오 보유기간(1주)  
- V(Pt) : 포트폴리오의 변동성( KOSPI 변동성의 함수 )
```
...
## Assistant Response
```text
VaR 계산식에서 Where 절은 VaR(위험금액)을 어떤 기준(유의수준/신뢰수준, 보유기간, 변동성)으로 산출할지를 정하기 위한 것입니다. 

- α: 유의수준(Level of Significance)이며, (1-α)×100을 신뢰수준으로 봅니다.   
- T: 포트폴리오 보유기간(1주)입니다.   
- V(P_t): 포트폴리오의 변동성(KOSPI 변동성의 함수)입니다.
```
...
## Assistant Response
```text
VaR 계산에서 Where 절은 VaR을 산출할 때 필요한 “입력 판단 기준(구성요소)”를 정합니다. 

- α: 유의수준(Level of Significance)으로, (1-α)×100을 신뢰수준으로 봅니다.   
- T: 포트폴리오 보유기간(1주)입니다.   
- V(P_t): 포트폴리오의 변동성으로, KOSPI 변동성의 함수로 정의됩니다.
```
...
## Assistant Response
```text
VaR 계산식에서 Where 절은 VaR을 구성하는 입력값(수준/기간/변동성)을 무엇으로 정할지에 대한 기준을 제시합니다. 
