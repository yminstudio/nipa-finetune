...
## Question
왼쪽(left) 구간과 오른쪽(right) 구간의 추정 주식매수회전율(TR)을 비교할 때 사용하는 판단 축은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "왼쪽(left) 구간과 오른쪽(right) 구간의 추정 주식매수회전율(TR)을 비교할 때 사용하는 판단 축은 무엇인가?",
  "qa_type": "comparison",
  "question_template": "Comparison-TR-left-right"
}
```
