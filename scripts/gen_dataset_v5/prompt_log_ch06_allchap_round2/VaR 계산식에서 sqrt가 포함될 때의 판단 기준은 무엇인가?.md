...
## Question
VaR 계산식에서 sqrt가 포함될 때의 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "VaR 계산식에서 sqrt가 포함될 때의 판단 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[VaR 계산식]",
  "seed_title": "[VaR 계산식]",
  "question": "VaR 계산식에서 sqrt가 포함될 때의 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- VaR은 Delta Normal Method를 사용하며, 이때 VaR 계산식은 VaR = α √(T · V(P_t)) 형태로 정해져 있어 √가 포함됩니다.   
- 여기서 √( )의 판단 기준은 “포트폴리오 보유기간 T(=1주)과 포트폴리오 변동성 V(P_t, KOSPI 변동성의 함수)의 곱(T · V(P_t))”에 제곱근을 취하는 것입니다.   
- 계수 α는 유의수준(또는 신뢰수준과의 관계)이며, √는 α에 곱해지는 것이 아니라 √(T · V(P_t)) 구간에 적용됩니다.
```
...
## Assistant Response
```text
VaR 산출에서 sqrt가 들어가는 부분은 “보유기간과 변동성으로부터 VaR의 위험규모를 계산하는 방식이 맞는지”를 판단하기 위한 구조입니다. 

- VaR는 VaR = α √(T · V(P_t)) 형태로 결정되며, sqrt는 “T와 포트폴리오 변동성 V(P_t)의 곱”을 제곱근 형태로 반영하기 위해 포함됩니다.   
- 여기서 α는 유의수준(또는 (1-α)×100 신뢰수준)으로, sqrt 결과(위험규모)에 신뢰수준 수준을 반영하는 값입니다.   
- T는 포트폴리오 보유기간(1주), V(P_t)는 포트폴리오의 변동성(= KOSPI 변동성의 함수)으로 정의되어, sqrt 내부에 들어갈 대상이 이 둘인지 확인하는 것이 기준입니다.
```
...
## Assistant Response
```text
- sqrt는 VaR의 크기를 결정할 때 “보유기간과 포트폴리오 변동성”을 먼저 곱한 뒤, 그 결과값의 제곱근을 취해 산출한다는 판단 기준을 뜻합니다.   
- 적용 기준은 VaR = α × sqrt(T × V(Pt))에서 sqrt가 T × V(Pt) 전체에 대해 적용된다는 점입니다(여기서 T는 포트폴리오 보유기간 1주, V(Pt)는 포트폴리오 변동성).   
- 최종 VaR은 제곱근 처리된 T × V(Pt)에 유의수준 α(또는 신뢰수준)까지 곱해 결정됩니다.
```
...
## Assistant Response
```text
VaR 계산식에서 sqrt는 “보유기간 동안의 변동성(위험)을 제곱근 형태로 반영해 손실위험 크기를 산출”하기 위한 요소입니다. 

- sqrt 안의 판단 기준은 VaR = α · √(T · V(Pt))에서 (T · V(Pt)) 값을 쓰는 것입니다.   
- 여기서 α는 유의수준(= (1-α)×100 신뢰수준), T는 포트폴리오 보유기간(1주), V(Pt)는 포트폴리오 변동성( KOSPI 변동성의 함수 )입니다.   
- 이 sqrt를 포함한 VaR 산출은 Delta Normal Method의 가정(기초 시장 수익률 정규분포, 주식 포트폴리오 가치와 KOSPI의 선형관계) 하에서 적용됩니다.
```
