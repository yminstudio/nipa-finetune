...
## Question
Delta Normal Method의 특징 중 수익률이 어떤 분포를 갖는다고 가정하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징 중 수익률이 어떤 분포를 갖는다고 가정하는가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징 중 수익률이 어떤 분포를 갖는다고 가정하는가?",
...
## Assistant Response
```text
- Delta Normal Method의 특징은 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정하는 것입니다.   
- 즉, VaR 산출을 위해 KOSPI 수익률 분포를 “정규분포”로 설정합니다.
```
...
- 즉, “기초적 시장가격(KOSPI)의 수익률 분포 = 정규분포”로 설정합니다.
```
...
## Assistant Response
```text
Delta Normal Method에서는 VaR 산출을 위해 기초적 시장가격(KOSPI) 수익률의 분포를 가정합니다.  
기본 정의: KOSPI 수익률이 정규분포를 갖는다고 가정합니다.
```
...
```text
- Delta Normal Method에서는 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정합니다. 
- 즉, 수익률의 분포가 정규분포(Normal distribution)라는 전제가 특징입니다.
```
