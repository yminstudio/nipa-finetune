...
## Question
Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "[Delta Normal Method의 특징]",
  "seed_title": "[Delta Normal Method의 특징]",
  "question": "Delta Normal Method의 특징에서 수익률이 정규분포를 갖는다고 가정하는 것이 판단 기준인가?",
...
## Assistant Response
```text
- Delta Normal Method의 특징으로, 기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정합니다   
- 따라서 “수익률이 정규분포를 갖는다”는 가정은 Delta Normal Method의 특징(판단 기준)에 해당합니다
```
...
## Assistant Response
```text
Delta Normal Method에서 “수익률이 정규분포를 갖는다”는 내용은 결과를 평가/판단하는 ‘판단 기준’이라기보다, VaR을 계산하기 위해 두는 방법론상의 가정(특징)으로 제시됩니다. 

- Delta Normal Method의 특징으로 “기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정”합니다. 
- 함께 “주식 포트폴리오 가치와 KOSPI 사이의 선형관계”도 가정합니다. 
- 이 가정들 때문에 계산은 용이하지만, 가정이 현실에 맞지 않을 수 있고 위험민감도(델타)가 시간에 따라 변할 수 있다고 설명합니다.
```
...
## Assistant Response
```text
- Delta Normal Method의 “정규분포 가정”이 무엇을 위한 것인지부터 보면, VaR를 산출하기 위한 모형 전제를 말합니다.  
- 제로인 방법론에서 Delta Normal Method의 특징으로 “기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정”한다고 제시되어 있습니다.   
- 따라서 이것은 펀드 성과를 가르는 ‘판단 기준’이라기보다는, VaR 계산을 위한 전제(가정)에 해당합니다.
```
...
## Assistant Response
```text
Delta Normal Method에서 말하는 “정규분포 가정”이 무엇을 판단하는 기준인지 묻는 것이라면, 이는 평가(판단) 기준이라기보다 VaR 산출에 쓰이는 전제(방법론의 특징)에 해당합니다. 

해당 방법론의 특징으로 “기초적 시장가격(KOSPI) 수익률이 정규분포를 갖는다고 가정”한다고 제시되어 있습니다.
```
