...
```json
{
  ...
  "target_total": 100,
  "generated_question": "기간평균 Score의 정의는 무엇인가?",
  "qa_type": "definition",
  "question_template": "기간평가 정의"
}
```
...
## Assistant Response
```text
- 기간평균 Score는 정성평가점수를 제외한 기간별 평가지표에 대해 1년, 2년, 3년의 평가기간별로 산출된 백분위점수를 평균하여 산출한 값이다.   
- 각 평가기간별 백분위점수를 기준으로 하므로 펀드별 운용기간이 달라도 비교가능성이 높다.
```
...
## Assistant Response
```text
기간평균 Score는 정성평가점수를 제외한 기간별 평가지표에 대해, 1년·2년·3년의 각 평가기간별로 산출된 백분위점수를 평균하여 산출하는 점수입니다.
```
...
## Assistant Response
```text
- 기간평균 Score는 정성평가점수를 제외한 평가지표에 대해, 1년·2년·3년의 평가기간별로 산출된 백분위점수를 평균하여 산출한 점수입니다.   
- 기간평균 Score는 각 평가기간의 백분위점수에 기반하므로, 펀드별 운용기간이 다르더라도 비교가능성이 높도록 설계됩니다.
```
...
## Assistant Response
```text
기간평균 Score는 정성평가점수를 제외한 기간별 평가지표에 대해, 1년·2년·3년 평가기간별로 산출된 백분위점수를 평균하여 산출하는 점수입니다. 

따라서 기간평균 Score는 평가기간별 백분위점수들을 하나로 평균한 값으로 이해하면 됩니다.
```
