...
## Question
그룹의 일별 BM 수익률을 판단하는 기준은 무엇으로 정해져 있는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "그룹의 일별 BM 수익률을 판단하는 기준은 무엇으로 정해져 있는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "다) 그룹의 일별 BM 수익률",
  "seed_title": "다) 그룹의 일별 BM 수익률",
  "question": "그룹의 일별 BM 수익률을 판단하는 기준은 무엇으로 정해져 있는가?",
...
## Assistant Response
```text
- i일 P그룹의 일별 BM 수익률 r^{BM}_{i,P}는 (Σ GNAV_{i,j})/(Σ MDNAV_{i,j}) − 1로 정해져 있다   
- GNAV_{i,j}(기준NAV)는 MDNAV_{i,j}에 BM 일수익률 R^{BM}_{i,j}(= r^{BM}_{i,j}+1)를 곱해 산출한다   
- MDNAV_{i,j}(수정순자산총액)는 NAV_{i,j}를 i일 j펀드의 일수익률 구간인 R_{i,j}(= r_{i,j}+1)로 나눠 구한다
```
...
## Assistant Response
```text
그룹의 일별 BM 수익률은 그 그룹(동일 유형/피어그룹)에 속한 펀드들의 벤치마크 성과를 “기준 NAV( GNAV )로 가중해” 산출해 판단합니다. 

- i일 P그룹 일 BM 수익률은  (∑ GNAV(i,j) / ∑ MDNAV(i,j)) − 1 로 정해집니다.   
- 분자 GNAV(i,j)는 i일의 P그룹(유형) 내 j펀드의 기준 NAV(= 수정 NAV × 해당 펀드 BM 일수익률) 합입니다.    
- 분모 MDNAV(i,j)는 i일의 j펀드 수정 순자산총액(= NAV / 해당 펀드 일수익률) 합으로, 가중치 기준으로 사용됩니다.
```
...
## Assistant Response
```text
그룹의 일별 BM 수익률은 그룹이 BM 대비 일간 성과를 보였는지를 판단하기 위한 산출 기준입니다.   
- i일 P그룹의 일 BM수익률은 r^{BM}_{i,P} = (∑ GNAV_{i,j}) / (∑ MDNAV_{i,j}) − 1로 정해져 있습니다.   
- 여기서 GNAV_{i,j}는 i일 P그룹(유형)에 속한 j펀드의 기준NAV이며, GNAV_{i,j} = MDNAV_{i,j} × R^{BM}_{i,j} 입니다.   
- 분모의 MDNAV_{i,j}는 i일 j펀드의 수정순자산총액(수정NAV)으로, MDNAV_{i,j} = NAV_{i,j} / R_{i,j} (R_{i,j}: i일 j펀드 일수익률+1)로 정해집니다.
```
...
## Assistant Response
```text
그룹의 일별 BM 수익률을 “벤치마크(BM) 성과”로 판단하기 위한 기준은, 각 그룹(유형) 내 펀드들의 기준NAV(GNAV)와 수정순자산총액(MDNAV)을 이용해 일 BM수익률을 계산하는 것입니다. 

- i일 그룹(P)의 일 BM 수익률은 r^{BM}_{i,P} = (Σ GNAV_{i,j})/(Σ MDNAV_{i,j}) − 1로 정해져 있습니다.   
- GNAV_{i,j}(기준NAV)는 MDNAV_{i,j}에 해당 펀드의 BM 일수익률(= r^{BM}_{i,j}+1)을 곱해 산출합니다.   
- MDNAV_{i,j}(수정순자산총액)은 NAV_{i,j}를 i일 펀드의 일수익률 R_{i,j}(= r_{i,j}+1)로 나눈 값으로 정의됩니다.
```
