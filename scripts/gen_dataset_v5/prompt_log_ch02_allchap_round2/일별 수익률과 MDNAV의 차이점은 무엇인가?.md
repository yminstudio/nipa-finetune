...
```json
{
  ...
  "target_total": 100,
  "generated_question": "일별 수익률과 MDNAV의 차이점은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "나) 일별 수익률",
  "seed_title": "나) 일별 수익률",
  "question": "일별 수익률과 MDNAV의 차이점은 무엇인가?",
...
## Assistant Response
```text
- 일별 수익률 r_{i,P}: i일의 P그룹 “일 수익률”로, ΣNAV / ΣMDNAV − 1로 계산됩니다.   
- MDNAV_{i,j}: i일의 j펀드 “수정순자산총액”으로, 일별 수익률 산출에서 NAV와 함께 분모에 들어가는 값입니다.   
- MDNAV는 NAV를 i일의 j펀드 일수익률 R_{i,j}(= r_{i,j}+1)로 나눈 값(MDNAV_{i,j} = NAV_{i,j}/R_{i,j})입니다.
```
...
## Assistant Response
```text
- 일별 수익률은 i일의 그룹 성과로, i일 그룹(P)의 수익률 r_{i,P} = (∑ NAV) / (∑ MDNAV) − 1로 계산됩니다 .  
- MDNAV는 같은 i일의 각 j펀드에 대한 “수정순자산총액”으로, MDNAV_{i,j} = NAV_{i,j} / R_{i,j}이며 R_{i,j}는 i일 j펀드의 일수익률(= r_{i,j}+1)입니다 .  
- 즉, 일별 수익률은 결과값(수익률, 비율)이고, MDNAV는 그 수익률 계산에 사용되는 보정된 순자산금액(입력값)이라는 차이가 있습니다 .
```
...
## Assistant Response
```text
- 공통 맥락으로, 일별 수익률을 계산할 때 분모/가중에 MDNAV(수정 순자산총액)가 사용됩니다.   
- 일별 수익률은 i일의 성과를 “수익률(%)”로 나타내는 값이며, 그룹 기준 일별수익률은 (일별 NAV 합) / (일별 MDNAV 합) − 1로 산출됩니다.   
- MDNAV는 i일의 수정 순자산총액으로, NAV를 일수익률 반영분(R=일수익률+1)으로 나눠 보정한 값입니다(MDNAV = NAV / R).
```
...
## Assistant Response
```text
- 일별 수익률은 i일의 성과를 나타내는 ‘수익률 값’으로, 펀드의 경우 기준가 등락(분배율 반영)을 전영업일 기준가로 나눠 r로 산출합니다.   
- MDNAV는 i일 j펀드의 ‘수정 순자산총액(값)’이며, 펀드의 일수익률 R(= r+1)을 이용해 MDNAV = NAV / R로 정의됩니다.   
- 즉, 일별 수익률 r와 R가 MDNAV를 만드는 데 직접 쓰이는 반면, 그룹 일별 수익률 산출에서는 NAV(분자)와 MDNAV(분모)를 비교해 일별 수익률을 계산하는 구조입니다.
```
