...
## Question
펀드 ESG 스코어 산출에서 등급 산출 대상 유형으로 포함되는 자산 유형은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 ESG 스코어 산출에서 등급 산출 대상 유형으로 포함되는 자산 유형은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "나) 펀드 ESG 스코어의 산출"
}
```
...
```json
{
  ...
  "subsection": "나) 펀드 ESG 스코어의 산출",
  "seed_title": "나) 펀드 ESG 스코어의 산출",
  "question": "펀드 ESG 스코어 산출에서 등급 산출 대상 유형으로 포함되는 자산 유형은 무엇인가?",
...
## Assistant Response
```text
펀드 ESG 스코어의 등급 산출을 위해 고려하는 “대상 포트폴리오 자산”은 펀드 보유자산 중 ESG 평가점수를 집계할 수 있는 범위를 뜻합니다. 

- 포함 자산: 주식종목(우선주 포함)   
- 제외 자산: ETF(ETF 구성 종목을 배분할지 여부 결정 필요, 종목 vs 펀드)
```
...
## Assistant Response
```text
펀드 ESG 스코어(등급)를 산출하기 위해 포트폴리오에서 평가 대상으로 삼는 자산 유형은 “대상 포트폴리오 자산” 기준에 따릅니다. 

- 주식종목(우선주 포함)   
- 제외: ETF(ETF 구성 종목을 배분할지 여부는 별도 판단 필요로 명시)
```
...
## Assistant Response
```text
펀드 ESG 스코어(및 그에 따른 ESG 등급)를 산출할 때, 어떤 포트폴리오 자산을 평가대상으로 볼지를 정하기 위한 기준이다. 

- 주식종목(우선주 포함)   
- 제외: ETF(ETF 구성 종목을 배분할지 여부 결정 필요)
```
