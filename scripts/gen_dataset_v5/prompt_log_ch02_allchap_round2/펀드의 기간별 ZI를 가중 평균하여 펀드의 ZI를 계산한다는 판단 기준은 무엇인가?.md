...
## Question
펀드의 기간별 ZI를 가중 평균하여 펀드의 ZI를 계산한다는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 기간별 ZI를 가중 평균하여 펀드의 ZI를 계산한다는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_2"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "Step 4 : 각 펀드의 기간별 ZI 를 가중 평균하여 펀드의 ZI 를 계산합니다.",
  "question": "펀드의 기간별 ZI를 가중 평균하여 펀드의 ZI를 계산한다는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 펀드의 최종 ZI는 기간별 ZI(ZIᵢ,ⱼ)를 가중 평균한 값으로 계산한다: ZIᵢ = Σ(ωⱼ × ZIᵢ,ⱼ)   
- 기간별 가중치 ωⱼ는 “기간 길이에 비례함”을 판단 기준으로 한다   
- 가중 평균에 포함되는 기간 개수 n은 등급에 따라 정한다(1년 등급이면 n=1, 3년·5년·10년 등급이면 n=3)
```
...
## Assistant Response
```text
이 기준은 여러 기간(예: 1년, 3년 등)에서 산출된 ZI를 하나의 펀드 ZI로 종합하기 위한 계산 규칙입니다. 

- 펀드의 ZI는 기간별 ZI를 가중 평균한 값으로 계산하며, ZI_i = Σ(ω_j × ZI_{i,j}) 형태로 산정합니다. 
- 기간 가중치 ω_j는 “기간 길이에 비례함”으로 정해집니다. 
- 적용되는 기간 수 n은 등급/등급 구성에 따라 달라지며, 1년등급은 n=1, 3년·5년·10년 등급은 n=3으로 기간별 ZI를 함께 반영합니다.
```
...
## Assistant Response
```text
펀드의 성과를 서로 다른 기간 성과로부터 하나의 ZI로 종합하기 위한 판단 기준이다. 

- 펀드의 최종 ZI는 기간별 ZI를 가중 평균(가중합)해 계산하며, 산식은 ZI_i = Σ(ω_j × ZI_{i,j}) 이다.   
- 기간별 가중치 ω_j는 “기간 길이에 비례”한다.   
- 가중 평균에 포함하는 기간 수 n은 등급 기간에 따라 1년 등급은 n=1, 3년/5년/10년 등급은 n=3이다.
```
...
## Assistant Response
```text
펀드의 기간별 ZI를 가중 평균해 “펀드의 최종 ZI”를 산출하기 위한 기준은, 기간별 ZI 점수를 기간 가중치를 반영해 선형 결합하는 방식입니다. 

- 펀드의 ZI는 펀드 i에 대해 ZI_i = Σ(ω_j × ZI_{i,j})로 계산합니다.   
- 여기서 ω_j는 j기의 가중치이며, “기간 길이에 비례”하는 값입니다.   
- 적용되는 기간 개수 n은 1년 등급은 n=1, 3년·5년·10년 등급은 n=3입니다.
```
