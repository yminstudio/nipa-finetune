...
## Question
ZI Score 변환 개념에서 ZI 값이 Min 이상 Max 이하인지를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score 변환 개념에서 ZI 값이 Min 이상 Max 이하인지를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "ZI Score 변환 개념",
  "question": "ZI Score 변환 개념에서 ZI 값이 Min 이상 Max 이하인지를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- ZI Score 변환에서 ZI 값이 Min 이상 Max 이하인지 여부는, ZI 값이 “최소값(Min)부터 최대값(Max)까지”의 범위에 포함되는지로 판단한다.   
- Min은 Score 0, Max는 Score 100 구간의 양끝값으로 대응된다.
```
...
## Assistant Response
```text
ZI Score 변환에서 “ZI 값이 어느 구간에 속하는지”를 보려는 것입니다.   
ZI Score 변환 개념상 ZI 값은 최소값(Min)부터 최대값(Max)까지의 범위를 가지므로, ZI 값이 Min 이상 Max 이하인지로 판단합니다.   
또한 이 Min→Max 범위에서 Score가 0→100으로 정규화되며, 구간별 선형 보간 방식으로 Score가 계산됩니다.
```
