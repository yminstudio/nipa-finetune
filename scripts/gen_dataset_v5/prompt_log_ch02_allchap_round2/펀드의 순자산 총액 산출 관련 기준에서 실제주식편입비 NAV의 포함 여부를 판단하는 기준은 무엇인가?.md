...
## Question
펀드의 순자산 총액 산출 관련 기준에서 실제주식편입비 NAV의 포함 여부를 판단하는 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 순자산 총액 산출 관련 기준에서 실제주식편입비 NAV의 포함 여부를 판단하는 기준은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "바) 펀드의 순자산 총액 산출 관련",
  "seed_title": "바) 펀드의 순자산 총액 산출 관련",
  "question": "펀드의 순자산 총액 산출 관련 기준에서 실제주식편입비 NAV의 포함 여부를 판단하는 기준은 무엇인가?",
...
## Assistant Response
```text
- 순자산총액(NAV)은 제로인에서 보정 순자산총액을 의미하며, 순자산총액 사용은 보정 순자산총액(수정 NAV, MDNAV)을 적용하는 것으로 정한다.   
- 수정 NAV(MDNAV)는 그룹수익률 산출에 사용한다.   
- 실제주식편입비 NAV(SNAV’)는 ‘그룹의 실제편입비 수익률’ 산출시에만 사용하므로, 해당 수익률 산출에 들어갈 때에만 포함 여부를 “포함”으로 판단한다.
```
...
## Assistant Response
```text
- 실제주식편입비 NAV를 순자산 총액(NAV) 산출에 “포함해 사용할지”는, 그 NAV가 어떤 수익률 산출에 쓰이기 위한 것인지로 판단한다.   
- 실제주식편입비 NAV는 “그룹의 실제편입비(실제주식편입비) 수익률”을 산출할 때에 사용한다.   
- 산출은 수정순자산총액(MDNAV)에 실제편입비 일수익률 R’(= r’+1)을 곱해 실제주식편입비NAV를 만든다.
```
...
## Assistant Response
```text
실제주식편입비 NAV의 포함 여부를 판단하는 기준은, 그 NAV가 “어떤 수익률 산출에 쓰이는지” 여부를 보는 것입니다. 

- 실제주식편입비 NAV는 그룹의 “실제편입비 수익률(실제주식편입비 수익률)” 산출시에 사용합니다.   
- 반면 순자산총액(NAV)은 기본적으로 제로인에서 산출한 보정 순자산총액을 의미하며, 이 기준의 NAV를 사용합니다.   
- 따라서 실제편입비 수익률 산출에 해당할 때에만 실제주식편입비 NAV(예: 실제편입비 일수익률을 반영한 SNAV')를 적용하고, 그 외의 산출에서는 보정 순자산총액 기반 NAV를 사용합니다.
```
...
## Assistant Response
```text
실제주식편입비 NAV를 “순자산 총액 산출”에서 포함할지는, 그 NAV가 어디에 쓰이도록 정의됐는지를 기준(산출 목적)으로 판단합니다. 

- 실제주식편입비 NAV는 “그룹의 실제편입비 수익률” 산출시에만 사용되는 값으로 정의됩니다. 
- 실제주식편입비 NAV는 수정 NAV(MDNAV)에 “실제편입비 일수익률”을 곱해 산출하며( SNAV’ = MDNAV × R’ ), 이때 MDNAV는 수정순자산총액입니다. 
- 따라서 순자산 총액(NAV)은 보정 순자산총액을 의미하고(실제주식편입비 NAV 자체가 순자산 총액에 그대로 ‘포함’되는 기준이 아니라), 실제편입비 수익률 계산 단계에서 SNAV’로 반영됩니다.
```
