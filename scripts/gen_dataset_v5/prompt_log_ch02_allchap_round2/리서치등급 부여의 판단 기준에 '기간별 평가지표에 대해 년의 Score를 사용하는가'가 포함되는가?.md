...
## Question
리서치등급 부여의 판단 기준에 '기간별 평가지표에 대해 년의 Score를 사용하는가'가 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "리서치등급 부여의 판단 기준에 '기간별 평가지표에 대해 년의 Score를 사용하는가'가 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "다) 리서치등급 부여",
  "seed_title": "다) 리서치등급 부여",
  "question": "리서치등급 부여의 판단 기준에 '기간별 평가지표에 대해 년의 Score를 사용하는가'가 포함되는가?",
...
## Assistant Response
```text
- 리서치등급 부여 판단에는 포함됩니다. 정성평가점수를 제외한 기간별 평가지표에 대해 1년, 2년, 3년의 평가기간별 백분위점수를 평균해 기간평균 Score를 산출합니다.
```
...
## Assistant Response
```text
리서치등급에서 “기간별 평가지표”는 정성평가를 제외한 정량 지표의 성과를 기간(1년, 2년, 3년) 관점에서 반영하기 위한 것입니다.   
- 기간별 평가지표에 대해 1년·2년·3년 평가기간별로 산출된 백분위점수를 사용합니다.   
- 이 백분위점수들을 평균해 “기간평균 Score”를 산출하므로, 1년의 Score 사용 여부는 포함됩니다.   
- 참고로 정성평가점수는 기간평균을 산출하지 않고 가장 최근 시점에 근접한 점수를 반영합니다.
```
...
## Assistant Response
```text
리서치등급 부여에서 “기간별 평가지표 점수를 어떤 방식으로 집계해 점수로 반영하는지”를 판단합니다.  
- 정성평가점수를 제외한 기간별 평가지표에 대해, 평가기간별(1년, 2년, 3년)로 산출된 백분위점수를 평균하여 기간평균 Score를 산출합니다.   
- 따라서 기간별 평가지표에 대해 1년 Score를 포함한(1년·2년·3년의) 평가기간별 Score를 사용해 반영합니다.
```
...
## Assistant Response
```text
리서치등급의 기간평균 Score를 어떻게 산출하는지를 판단하기 위한 기준입니다. 

- 기간별 평가지표(정성평가점수 제외)에 대해 1년, 2년, 3년 평가기간별 백분위점수를 사용합니다.   
- 그리고 그 1년/2년/3년 백분위점수를 평균해 기간평균 Score를 산출합니다.   
- 따라서 “기간별 평가지표에 대해 년의 Score(1년·2년·3년 Score)를 사용하는가”는 리서치등급 판단 기준에 포함된 내용으로 볼 수 있습니다.
```
