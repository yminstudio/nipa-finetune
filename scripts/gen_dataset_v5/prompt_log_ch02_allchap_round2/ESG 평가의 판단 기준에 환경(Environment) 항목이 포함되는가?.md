...
## Question
ESG 평가의 판단 기준에 환경(Environment) 항목이 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ESG 평가의 판단 기준에 환경(Environment) 항목이 포함되는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "가) ESG평가란?",
  "seed_title": "가) ESG평가란?",
  "question": "ESG 평가의 판단 기준에 환경(Environment) 항목이 포함되는가?",
...
## Assistant Response
```text
- 제로인 펀드의 ESG 평가 판단 기준에는 환경(Environment) 항목이 포함됩니다.   
- ESG 평가는 환경(Environment), 사회책임(Social Responsibility), 지배구조(Governance) 등 지속가능성에 영향을 미치는 요소를 평가하는 것으로 정의되어 있습니다.
```
...
## Assistant Response
```text
ESG 평가는 기업의 지속가능성에 영향을 미치는 요소를 평가하기 위한 판단 기준으로, 환경(Environment) 항목이 포함됩니다. 

- 환경(Environment)  
- 사회책임(Social Responsibility)  
- 지배구조(Governance)
```
...
## Assistant Response
```text
ESG 평가는 기업의 지속가능성에 영향을 미치는 요소를 평가하기 위한 것이다. 

- 예, ESG 평가의 판단 기준에는 환경(Environment) 항목이 포함된다.   
- 제로인 방법론에서 ESG는 환경(Environment)·사회책임(Social Responsibility)·지배구조(Governance) 등으로 구성해 평가한다.
```
...
## Assistant Response
```text
ESG 평가는 기업의 지속가능성에 영향을 미치는 요소를 평가하기 위한 판단 기준입니다.   
이 판단 기준에는 환경(Environment) 항목이 포함되며, 환경(Environment)과 함께 사회책임(Social Responsibility), 지배구조(Governance)도 함께 제시됩니다.
```
