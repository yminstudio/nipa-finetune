...
## Question
일별 수익률의 정의에서 기준은 전일 NAV 대비 변화율로 정의되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "일별 수익률의 정의에서 기준은 전일 NAV 대비 변화율로 정의되는가?",
  "qa_type": "criteria",
  "question_template": "criteria_basis"
}
```
...
```json
{
  ...
  "subsection": "나) 일별 수익률",
  "seed_title": "나) 일별 수익률",
  "question": "일별 수익률의 정의에서 기준은 전일 NAV 대비 변화율로 정의되는가?",
...
## Assistant Response
```text
- 아니요. 제로인에서 일별 실현수익률은 i일 기준가를 i일 전영업일 기준가로 나눈 형태(분배율은 P0·Di 반영)로 정의됩니다.   
- 일별 그룹수익률은 i일 NAV와 수정순자산총액(MDNAV)을 사용해 산출하며, “전일 NAV 대비 변화율”로 정의되어 있지는 않습니다.
```
...
## Assistant Response
```text
- 펀드의 일별 수익률은 당일 기준가(운용/평가에 쓰이는 NAV 성격의 값)의 전영업일 대비 변화를 기준으로 산출하기 위한 값입니다.   
- 펀드 일별 실현수익률은 r_{i,F} = (P_i + P_0 × D_i) / P_{i-1} − 1 로, 분모에 전영업일 기준가(P_{i-1})가 들어가 전일 NAV 대비 변화율(비율에서 1을 차감) 형태로 정의됩니다.   
- BM의 일수익률도 ‘기준일 기준가 / 기준일의 전영업일 기준가’의 변화율(비율에서 1을 차감)로 정의됩니다.
```
...
## Assistant Response
```text
일별 수익률은 특정 날짜의 성과를 “전날 대비 얼마나 변했는지”로 판단하기 위한 지표입니다. 

- 펀드의 일별 실현수익률은 전영업일 펀드 기준가(P_{i-1})를 분모로 하여 변화율 형태로 정의됩니다: r_{i,F} = (P_i + P_0 × D_i) / P_{i-1} − 1.   
- 다만 분자에는 분배율(또는 상환률) D_i가 반영됩니다.   
- 그룹수익률의 경우에는 전일 NAV 대비 변화율처럼 단순화된 형태가 아니라, ΣNAV / ΣMDNAV − 1 구조로 산출됩니다.
```
...
## Assistant Response
```text
일별 수익률은 펀드의 기준가가 전일(전영업일) 대비 얼마나 변했는지를 기준으로 수익률을 판단하기 위한 것입니다. 

제로인의 일별 실현수익률은 전영업일 기준가(=NAV로 쓰이는 기준가/순자산가치) 대비 변화율 형태로, 분배가 있는 경우 이를 반영해 다음처럼 정의됩니다.   
- r_{i,F} = (P_i + P_0 × D_i) / P_{i-1} − 1   
따라서 “전일 NAV 대비 변화율”이 기본 뼈대이며, 분배율 D_i가 있으면 분배분이 분자에 추가됩니다.
```
