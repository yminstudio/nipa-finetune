...
## Question
펀드의 순자산 총액 산출 관련에서 '기준 NAV'와 '수정 NAV'의 차이를 판단하는 주된 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 순자산 총액 산출 관련에서 '기준 NAV'와 '수정 NAV'의 차이를 판단하는 주된 기준은 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "바) 펀드의 순자산 총액 산출 관련",
  "seed_title": "바) 펀드의 순자산 총액 산출 관련",
  "question": "펀드의 순자산 총액 산출 관련에서 '기준 NAV'와 '수정 NAV'의 차이를 판단하는 주된 기준은 무엇인가?",
...
## Assistant Response
```text
- 수정 NAV는 “그룹수익률 산출”에 사용되는 NAV로, 일수익률로 보정해 구한다( MDNAV = NAV / R ).   
- 기준 NAV는 “그룹의 BM 수익률 산출”에 사용되는 NAV로, 수정 NAV에 BM의 일수익률을 곱해 산출한다( GNAV = MDNAV × R^BM ).   
- 따라서 ‘기준 NAV vs 수정 NAV’의 차이는 주로 어느 수익률 산출에 쓰이는지(그룹수익률 vs BM 수익률)와, BM 일수익률을 반영해 계산하는지 여부에 의해 판단된다.
```
...
## Assistant Response
```text
- 두 NAV는 모두 순자산총액(NAV) 기반으로 만들어지며, 수정 NAV는 그룹수익률 산출에, 기준 NAV는 BM 수익률 산출에 사용된다   
- 차이를 가르는 주된 기준은 “어떤 일수익률을 적용해 NAV를 변환하느냐”이다   
- 수정 NAV는 펀드의 일수익률(R_{i,j})로 NAV를 나누어 조정하고, 기준 NAV는 BM의 일수익률(R^{BM}_{i,j})을 곱해 수정 NAV를 재조정한다
```
...
## Assistant Response
```text
- ‘기준 NAV’와 ‘수정 NAV’의 차이를 가르는 주된 기준은, 둘이 각각 어떤 수익률 산출(운용 성과 vs 벤치마크 성과)에 투입되는지입니다.   
- 수정 NAV는 그룹수익률 산출에 사용되고, 기준 NAV는 그룹의 BM 수익률 산출에 사용됩니다.   
- 이에 따라 기준 NAV는 수정 NAV에 BM의 일수익률을 곱해 산출되는 구조로 제시됩니다.
```
...
## Assistant Response
```text
두 값은 “순자산총액(NAV) 산출” 맥락에서 쓰이되, 차이는 계산에 어떤 목적/단계로 투입되느냐로 판단합니다. 

- 수정 NAV: 그룹(동일 Peer) 수익률을 산출할 때 사용되는 순자산총액입니다.   
- 기준 NAV: 그룹의 BM 수익률을 산출할 때 사용되는 기준가 기반 순자산총액입니다.   
- 즉, “그룹 수익률 단계에서 쓰이는가(수정 NAV)” vs “BM 수익률 단계에서 쓰이는가(기준 NAV)”가 주된 구분 기준입니다.
```
