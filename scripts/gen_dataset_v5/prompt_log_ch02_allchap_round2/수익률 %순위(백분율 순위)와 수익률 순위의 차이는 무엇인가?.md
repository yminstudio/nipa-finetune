...
## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0044

## Question
수익률 %순위(백분율 순위)와 수익률 순위의 차이는 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "수익률 %순위(백분율 순위)와 수익률 순위의 차이는 무엇인가?",
  "qa_type": "comparison",
  "question_template": "비교_수익률백분율순위_수익률순위"
}
```
...
[핵심 명사]
) 수익률 %순위(백분율 순위), 수익률 %순위, 백분율 순위, 수익률, 순위, 백분율, 모집단의, 크기를, 고려할, 필요, 없이, 특정
