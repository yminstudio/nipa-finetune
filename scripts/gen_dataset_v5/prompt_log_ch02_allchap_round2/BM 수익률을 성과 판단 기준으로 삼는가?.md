...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM 수익률을 성과 판단 기준으로 삼는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "다) BM 수익률",
  "seed_title": "다) BM 수익률",
  "question": "BM 수익률을 성과 판단 기준으로 삼는가?",
...
## Assistant Response
```text
- 제로인 펀드평가에서는 성과요인분석에서 BM 대비 초과수익률을 성과 판단 맥락에서 활용하며, 이 과정에 BM수익률이 포함됩니다   
- BM수익률은 자산배분효과(평균편입비수익률 – BM수익률) 등 초과수익률 분해 항목에 직접 사용됩니다   
- 또한 유형별 BM의 3년간 월간 수익률 표준편차를 위험(기준)으로 삼아 위험회피계수의 산정에 반영합니다
```
...
## Assistant Response
```text
제로인 방법론에서는 펀드의 성과를 BM 대비로 판단하기 위해 BM 수익률을 기준으로 사용합니다. 

- “BM 대비 초과수익(또는 초과손실)”이 성과의 기준이 되며, 그 구성요소 중 자산배분효과는 평균편입비수익률 – BM수익률로 정의됩니다. 
- BM 대비 위험 구간을 성과 관점에서 함께 보는데, 하락편차/상승편차는 펀드 수익률이 BM 대비 하락(또는 상승)한 시기의 표준편차로 정의됩니다. 
- 또한 “하락베타”도 BM(벤치마크) 수익률 대비로 정의되며, 계산에 시장(또는 BM) 주간 log 수익률(R^M_i)이 사용됩니다.
```
...
## Assistant Response
```text
BM 수익률은 펀드 수익률을 벤치마크(BM) 대비로 비교해, BM과의 “차이”를 반영하는 지표를 산출하기 위한 기준입니다. 

- 하락편차: 펀드 수익률이 BM 수익률보다 낮은 구간을 기준으로 하락 시기의 위험(표준편차)을 계산합니다.   
- 상승편차: 펀드 수익률이 BM 수익률을 초과하는 구간을 기준으로 상승 시기의 위험(표준편차)을 계산합니다.   
- 성과요인분석: “BM 대비 초과수익률”을 자산배분/타이밍/종목선정 효과로 분해할 때 BM수익률이 투입됩니다.   

한편 성과 평가는 수익률(Return) 항목을 “3M %Rank” 등으로 제시하고 있어, BM 수익률 자체가 그 지표의 단독 성과판단 기준으로 명시되지는 않습니다.
```
...
## Assistant Response
```text
BM 수익률은 펀드 성과를 “상대적으로(벤치마크 대비)” 판단하기 위한 기준으로 활용한다. 

- 성과 점수에서의 수익성(성과 판단의 핵심)은 평가기간 펀드의 수익률이 높은 순으로 동일 유형 내 백분위순위점수를 부여하는 방식이다. 
- 다만 BM 수익률은 BM 대비 초과수익률과 그 원인(자산배분효과/자산배분타이밍효과/종목선정효과) 분석을 위해 사용된다. 
- 또한 BM 대비 하락·상승 구간의 편차(하락편차/상승편차) 같은 위험지표 계산에도 BM 수익률이 기준으로 들어간다.
```
