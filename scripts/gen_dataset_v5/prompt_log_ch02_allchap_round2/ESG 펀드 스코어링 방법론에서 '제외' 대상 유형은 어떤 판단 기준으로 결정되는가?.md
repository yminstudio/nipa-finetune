...
## Question
ESG 펀드 스코어링 방법론에서 '제외' 대상 유형은 어떤 판단 기준으로 결정되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ESG 펀드 스코어링 방법론에서 '제외' 대상 유형은 어떤 판단 기준으로 결정되는가?",
  "qa_type": "criteria",
  "question_template": "ESG펀드스코어링_제외대상유형_기준"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "3) ESG 펀드 스코어링 방법론",
  "question": "ESG 펀드 스코어링 방법론에서 '제외' 대상 유형은 어떤 판단 기준으로 결정되는가?",
...
## Assistant Response
```text
- ESG 펀드 스코어(펀드 ESG 스코어 산출)에서 ‘제외’ 대상 유형은 ETF이며, 그 판단 기준은 “ETF 구성 종목을 배분할지 여부를 결정해야 한다(종목 vs 펀드 관점)”는 점이다
```
...
## Assistant Response
```text
펀드 ESG 점수 산출에 포함할 “대상 포트폴리오 자산”을 정하기 위한 기준이다.   
- 제외 대상 유형: ETF   
- 판단 기준: ETF의 경우 “ETF 구성 종목을 배분할지 여부”를 결정해야 하므로(종목 vs 펀드 처리 필요), 이를 이유로 제외한다.
```
...
## Assistant Response
```text
- ESG 펀드 스코어링에서 ‘제외’ 대상은 ESG 평가 점수 산출에 포함할 수 있는 포트폴리오 자산 범위를 정하기 위한 기준으로 결정된다.  
- 대상 포트폴리오 자산은 주식종목(우선주 포함)이다.   
- ETF는 제외 대상으로 하며, 그 판단 기준은 ETF 구성 종목을 펀드에 배분할지 여부를 결정해야 한다는 점(종목 vs 펀드 처리 방식의 결정 필요)이다.
```
...
## Assistant Response
```text
펀드 ESG 스코어를 산출할 때 어떤 포트폴리오 자산을 “계산 대상에서 제외”할지 정하는 기준입니다. 

- 제외 대상 유형은 ETF입니다.   
- 판단 근거는 ETF의 경우 “ETF 구성 종목을 펀드 내 보유 종목으로 배분할지 여부(종목 vs 펀드)”를 먼저 결정할 필요가 있기 때문입니다.
```
