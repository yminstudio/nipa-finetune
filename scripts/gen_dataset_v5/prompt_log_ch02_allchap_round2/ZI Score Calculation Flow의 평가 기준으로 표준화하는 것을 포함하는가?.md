...
## Question
ZI Score Calculation Flow의 평가 기준으로 표준화하는 것을 포함하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score Calculation Flow의 평가 기준으로 표준화하는 것을 포함하는가?",
  "qa_type": "criteria",
  "question_template": "ZI_ScoreCalculation_Flow_기준_2"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "<ZI Score Calculation Flow>",
  "question": "ZI Score Calculation Flow의 평가 기준으로 표준화하는 것을 포함하는가?",
...
## Assistant Response
```text
- 포함합니다. ZI Score Calculation Flow에서는 펀드의 기간별 성과를 계산한 뒤 이를 표준화하여 ZI Score를 산출합니다   
- 각 기간에 대해 CE(i,j)를 “표준화한” ZI(i,j) 점수(= ZI 점수)를 계산하는 것이 평가 기준의 핵심 단계로 제시되어 있습니다
```
...
## Assistant Response
```text
ZI Score Calculation Flow는 펀드의 기간별 위험조정 성과를 바탕으로 ZI Score를 산출하기 위한 절차입니다.   
포함 여부는 “예”이며, CE(i,j)를 표준화해 ZI(i,j)로 변환하는 과정이 들어갑니다.   
구체적으로는 ZI(i,j)를 ZI_{i,j} = (CE_{i,j} − 평균 CE) / (표준편차 CE) 형태로 계산해 표준정규분포로의 표준화와 동일한 취지의 표준화가 수행됩니다.
```
...
## Assistant Response
```text
ZI Score Calculation Flow는 펀드의 기간별 초과성과를 바탕으로 산출한 효용값(CE)을 “표준화된 ZI 점수”로 바꿔 성과를 평가/비교 가능하게 만드는 것을 판단합니다. 

- 각 기간 j에 대해 CE(i,j)를 표준화하여 ZI(i,j)로 산출하는 과정이 포함됩니다.   
- 표준화 기준은 (CE(i,j)에서 Peer Group 내 j기 평균 효용값을 빼고) Peer Group 내 j기 효용값의 표준편차로 나누는 형태로 정의됩니다.
```
...
## Assistant Response
```text
ZI Score를 산출하기 위해, 기간별 성과를 표준화하는 절차가 포함됩니다. 

- 기간별로 CE(i,j)를 계산하고, 이를 표준화해 ZI(i,j)를 산출합니다(“CE 값을 표준화한 ZI 점수”).   
- 이후 ZI 값을 100점으로 환산한 값이 ZI Score입니다.
```
