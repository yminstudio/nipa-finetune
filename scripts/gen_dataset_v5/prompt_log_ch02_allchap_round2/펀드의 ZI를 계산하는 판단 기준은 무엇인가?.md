...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드의 ZI를 계산하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_template_1"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "Step 4 : 각 펀드의 기간별 ZI 를 가중 평균하여 펀드의 ZI 를 계산합니다.",
  "question": "펀드의 ZI를 계산하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- ZI 계산의 기초값은 기간별 효용값(CE)이며, CE는 주간로그수익률의 평균 R_{i,j}에서 위험회피계수 λ_{i,j}가 반영된 위험(표준편차 σ_{i,j}의 제곱) λ_{i,j}×σ_{i,j}^2를 차감해 산정한다.   
- 기간별 ZI(ZI_{i,j})는 CE를 동종( Peer Group ) 내에서 표준화하며, CE_{i,j}에서 Peer Group 평균 효용값 \overline{CE}_j를 뺀 값을 Peer Group 내 효용값의 표준편차 σ_j로 나눈다.   
- 펀드의 최종 ZI(ZI_i)는 기간별 ZI_{i,j}를 기간 가중치 ω_j로 가중 평균해 결정한다.
```
...
## Assistant Response
```text
ZI는 펀드의 위험조정 성과를 동종 집단 대비 상대적 위치로 평가하기 위한 지표입니다. 

- 기간 j의 ZI는, 해당 펀드의 효용값(CE)에서 동종(해당 Peer Group) 평균 효용값을 뺀 값을 동종 Peer Group 효용값 표준편차로 나누어 산출합니다:  
  ZI_{i,j} = (CE_{i,j} − \overline{CE}_j) / σ_j 
- 효용값(CE)은 기간 j의 주간로그수익률 평균에서 위험회피계수 λ와 주간로그수익률 분산(σ_{i,j}^2)을 반영한 위험 부담을 차감해 계산합니다:  
  CE_{i,j} = R_{i,j} − λ_{i,j} × σ_{i,j}^2 
- 펀드의 최종 ZI는 기간별 ZI_{i,j}를 기간 가중치 ω_j로 가중 평균해 산출합니다:  
  ZI_i = Σ( ω_j × ZI_{i,j} )
```
