...
## Question
성능등급의 판단 기준으로 CE 식의 유도에 해당하는 항목의 명칭은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "성능등급의 판단 기준으로 CE 식의 유도에 해당하는 항목의 명칭은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_ce_induction_item"
}
```
...
```json
{
  ...
  "subsection": "[참고 : CE 식의 유도]",
  "seed_title": "[참고 : CE 식의 유도]",
  "question": "성능등급의 판단 기준으로 CE 식의 유도에 해당하는 항목의 명칭은 무엇인가?",
...
## Assistant Response
```text
- 확실성등가(CE, Certainty Equivalence)   
- 성능등급 산출을 위한 기초값으로, CE 식의 유도 항목으로 제시된다
```
...
## Assistant Response
```text
성능등급(태극마크)을 산출하기 위한 기초값인 CE를 유도할 때, 효용을 결정짓는 상수(위험을 반영하는 계수)를 구합니다.   
그에 해당하는 항목의 명칭은 위험회피계수(람다; λ)입니다.
```
