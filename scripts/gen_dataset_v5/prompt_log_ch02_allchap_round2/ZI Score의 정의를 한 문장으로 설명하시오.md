...
```json
{
  ...
  "target_total": 100,
  "generated_question": "ZI Score의 정의를 한 문장으로 설명하시오.",
  "qa_type": "definition",
  "question_template": "ZI_Score_Definition_Template"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "<ZI Score Calculation Flow>",
  "question": "ZI Score의 정의를 한 문장으로 설명하시오.",
...
## Assistant Response
```text
ZI Score는 제로인에서 CE를 표준화해 얻은 ZI 값을 Min~Max 범위에 대해 0~100점으로 환산(구간 기반 선형 보간)한 점수이며, ZI등급 산출에 쓰는 ZI값을 100점으로 해석 가능하게 만든 값입니다.
```
...
## Assistant Response
```text
- ZI값은 CE값을 표준화시킨 것으로, 해석의 직접성이 낮아 이를 100점으로 환산한 값을 사용합니다   
- ZI Score는 ZI값을 Min~Max 범위에서 0~100으로 정규화해 환산한 값입니다
```
...
## Assistant Response
```text
- ZI Score는 ZI 값(CE를 표준화한 ZI)을 최소값(Min)~최대값(Max) 범위에서 0~100점으로 정규화한 환산 점수이다.   
- ZI Score는 ZI 값이 증가할수록 점수도 증가하도록 구간 기반 선형 보간 방식으로 계산된다.
```
