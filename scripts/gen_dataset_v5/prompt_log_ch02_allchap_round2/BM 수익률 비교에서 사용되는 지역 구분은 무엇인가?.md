...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM 수익률 비교에서 사용되는 지역 구분은 무엇인가?",
  "qa_type": "comparison",
  "question_template": "BM_수익률_지역비교템플릿"
}
```
...
```json
{
  ...
  "subsection": "다) BM 수익률",
  "seed_title": "다) BM 수익률",
  "question": "BM 수익률 비교에서 사용되는 지역 구분은 무엇인가?",
...
## Assistant Response
```text
BM 수익률을 비교할 때는 시차를 반영하기 위해 투자지역별로 BM 적용일을 다르게 두는 지역 구분을 사용합니다. 

- 국내(국내 FoFs 포함) 
- 중국~일본, 호주 등 아시아 및 호주(인도는 별도) 
- 이외 지역
```
...
## Assistant Response
```text
해외펀드의 BM 수익률을 펀드 기준일에 맞춰 비교하기 위해, 지역에 따라 BM의 적용일(시차 반영)을 다르게 둡니다. 

- 국내 FoFs: 당일-1(국내 적용일의 전영업일 기준)   
- 중국~일본, 호주(아시아 및 호주): 당일(국내 적용과 동일)   
- 그 밖의 지역: 당일-1(국내 적용일의 전영업일 기준)
```
...
## Assistant Response
```text
BM 수익률 비교에서 해외펀드 등 BM 적용일(시차)을 달리하기 위해 사용되는 지역 구분은 다음과 같습니다. 
