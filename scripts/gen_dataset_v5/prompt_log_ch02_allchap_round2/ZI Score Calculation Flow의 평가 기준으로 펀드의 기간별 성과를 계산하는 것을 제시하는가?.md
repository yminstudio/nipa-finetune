...
## Question
ZI Score Calculation Flow의 평가 기준으로 펀드의 기간별 성과를 계산하는 것을 제시하는가?
...
{
  "chapter": "2. 펀드평가 방법론",
  "section": "<ZI Score Calculation Flow>",
  "subsection": "",
  "seed_title": "<ZI Score Calculation Flow>",
  "record_target": 3,
  ...
  "target_total": 100,
  "generated_question": "ZI Score Calculation Flow의 평가 기준으로 펀드의 기간별 성과를 계산하는 것을 제시하는가?",
  "qa_type": "criteria",
  "question_template": "ZI_ScoreCalculation_Flow_기준_1"
}
```
...
[핵심 명사]
ZI, Score, Calculation, Flow, 평가를, 위해, 펀드의, 기간별, 성과를, 계산하고, 이를, 표준화하여
...
```json
{
  ...
  "subsection": "",
  "seed_title": "<ZI Score Calculation Flow>",
  "question": "ZI Score Calculation Flow의 평가 기준으로 펀드의 기간별 성과를 계산하는 것을 제시하는가?",
...
## Assistant Response
```text
- ZI Score Calculation Flow에서는 펀드의 기간별 성과를 계산하고, 이를 표준화하여 ZI Score를 산출한다고 제시합니다.   
- 분석 기간은 1 Year, 3 Year, 5 Year로 두고, 각 기간에 대해 기간 j의 초과성과 CE(i,j)와 표준화 ZI(i,j)를 계산합니다.   
- 최종 ZI Score는 기간별 ZI(i,j)를 기간별 가중치 wj로 가중합하여 산출합니다.
```
...
## Assistant Response
```text
ZI Score 산출을 위해 펀드의 기간별 성과를 먼저 계산하는 절차가 제시됩니다.   
- 분석 기간(1 Year, 3 Year, 5 Year)에 대해 각 기간의 펀드 초과성과 CE(i,j)를 계산합니다.   
- 각 기간별 CE(i,j)를 표준화해 기간별 ZI(i,j)를 산출합니다.   
- 이후 기간별 ZI(i,j)를 기간별 가중치 w_j로 가중 평균해 최종 ZI Score(ZI_i)를 계산합니다.
```
