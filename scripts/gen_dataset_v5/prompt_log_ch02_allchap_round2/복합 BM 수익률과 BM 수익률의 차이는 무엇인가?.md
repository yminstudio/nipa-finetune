...
```json
{
  ...
  "target_total": 100,
  "generated_question": "복합 BM 수익률과 BM 수익률의 차이는 무엇인가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "라) 복합 BM 수익률",
  "seed_title": "라) 복합 BM 수익률",
  "question": "복합 BM 수익률과 BM 수익률의 차이는 무엇인가?",
...
## Assistant Response
```text
- BM 수익률은 BM(벤치마크) 일수익률을 BM 종가지수 값의 변화로 산출합니다(예: \(r_{i, BM}=\frac{J_{i-1}}{J_{i-2}}-1\)).   
- 복합 BM 수익률은 여러 시장 지수의 BM 일수익률을 투자비중(가중치) \(\omega_j\)로 가중합해 복합지수의 일수익률을 산출합니다(예: \(ZCI\_BM_i=\omega_1 BM\_r_{1,i}+...+\omega_n BM\_r_{n,i}\)).
```
...
## Assistant Response
```text
복합 BM 수익률과 BM 수익률은 모두 “BM의 일별 수익률(그리고 이를 기간 누적)”을 구한다는 공통점이 있습니다.  

- BM 수익률은 특정 BM(단일 지수)의 종가지수 흐름으로 일수익률을 산출하며, 예시는 r_i,BM = J_{i-1}/J_{i-2} − 1 형태입니다.   
- 복합 BM 수익률은 여러 시장 지수 BM을 사전에 정해진 비중(ω_j)으로 결합해 “복합지수”를 만들고, 그 복합지수의 일수익률을 ω 가중합으로 산출합니다.   
- 따라서 단일 BM(일반 BM)은 하나의 지수 변화 자체를 쓰는 반면, 복합 BM은 여러 BM을 비중으로 함께 반영해 계산합니다.
```
...
## Assistant Response
```text
- 공통 맥락에서 두 값 모두 BM(벤치마크)의 “일 수익률”을 기준으로 합니다.   
- BM 수익률은 특정 BM 지수의 일별 종가지수를 이용해 산출하며, 기준일의 전영업일 BM 종가지수를 분자로 사용합니다.   
- 복합 BM 수익률은 여러 시장 지수 BM의 일 수익률을 각 지수의 비중(ω)대로 가중평균(가중합)해 “복합지수”의 일 수익률로 산출합니다.
```
...
## Assistant Response
```text
- 공통 맥락: 제로인에서는 BM의 수익률을 일별 수익률로 산출한 뒤 기간 수익률 계산에 사용합니다.   
- BM 수익률: 단일 BM(지수)의 종가지수 값(J)을 사용해 일별 수익률을 산출합니다(예: r_{i,BM}=J_{i-1}/J_{i-2}-1).   
- 복합 BM 수익률: 여러 시장 BM을 미리 정한 비중(ωj)으로 가중 평균해 복합지수의 일별 수익률을 산출합니다(ZCI_BM_i=ω1×BM_r1,i+…+ωn×BM_rn,i).   
- 차이의 핵심: BM 수익률은 “하나의 BM 지수” 수익률, 복합 BM 수익률은 “복수 BM 지수의 비중 조합” 수익률입니다.
```
