...
## Question
복합 BM 수익률의 판단 기준에 주식의 포함 여부를 포함하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "복합 BM 수익률의 판단 기준에 주식의 포함 여부를 포함하는가?",
  "qa_type": "criteria",
  "question_template": "criteria_주식포함여부"
}
```
...
```json
{
  ...
  "subsection": "라) 복합 BM 수익률",
  "seed_title": "라) 복합 BM 수익률",
  "question": "복합 BM 수익률의 판단 기준에 주식의 포함 여부를 포함하는가?",
...
## Assistant Response
```text
- 복합 BM 수익률의 판단 기준은 복합지수를 구성하는 “각 시장 지수 BM의 비중(ω_j)”을 이용해 복합지수 1일 수익률을 산출하는 데 있습니다.   
- 따라서 주식의 포함 여부를 별도로 판단하는 기준은 제시되어 있지 않고, 주식/채권 등은 사전적으로 정해진 자산배분 비중(각 시장지수 BM의 비중) 안에서 반영되는 전제로 다뤄집니다.
```
...
## Assistant Response
```text
복합 BM 수익률은 혼합형 펀드의 전략적 자산배분에 따른 기준 성과(복합지수)를 보기 위한 것입니다.   
- 제로인에서는 복합 BM을 “각 시장 지수”를 투자비중(=자산배분 비중)에 따라 가중 평균해 산출합니다.   
- 이때 주식은 “주식과 채권 등”처럼 자산배분 구성요소로 언급되며, 주식 관련 시장지수도 그 비중에 따라 복합지수에 포함되어 계산됩니다.   
- 복합 BM 수익률(복합지수 1일 수익률) 자체는 각 시장 지수 BM의 비중(ω)과 해당 지수의 1일 수익률의 가중합으로 정의됩니다.
```
...
## Assistant Response
```text
복합 BM 수익률은 혼합형 펀드의 전략적 자산배분(각 시장의 비중)을 반영해 복합지수 수익률을 산출하기 위한 기준이다 .  
- 복합지수 1일 수익률은 각 시장 지수 BM의 비중(ω)을 가중치로 하여 시장 지수 BM 수익률(BM_r)에 곱해 더하는 방식으로 정의된다 .  
- 따라서 복합 BM 수익률의 산출/판단 기준에서 ‘주식의 포함 여부’를 별도로 체크하는 항목은 제시되지 않는다 .  
- 다만 자산별 적용지수는 주식은 Capital Return 지수, 채권은 Total Return 지수(90% 반영)로 정해져 있어, 주식은 별도의 “포함 여부” 판단이 아니라 주식 BM의 적용지수 선택을 통해 반영된다 .
```
...
## Assistant Response
```text
복합 BM 수익률은 혼합형 펀드의 사전 자산배분(주식과 채권 등 정해진 비중)을 반영하기 위한 판단입니다. 

- 복합지수는 펀드의 투자비중에 따라 각 시장 지수를 가중해 산출하므로, 주식이 그 비중에 포함되는지 여부가 복합 BM 산출에 반영됩니다.   
- 자산별로 적용되는 지수 형태도 구분되며, 주식이 포함될 경우 주식은 Capital Return 지수를 사용합니다.   
- 채권은 Total Return 지수(90% 적용)로 별도 반영되므로, 복합 BM 산출 시 “주식 포함 여부”는 자산구성에 따라 적용 방식에 영향을 줍니다.
```
