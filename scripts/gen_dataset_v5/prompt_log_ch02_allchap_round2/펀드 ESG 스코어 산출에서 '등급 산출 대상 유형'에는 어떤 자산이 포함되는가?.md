...
## Question
펀드 ESG 스코어 산출에서 '등급 산출 대상 유형'에는 어떤 자산이 포함되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "펀드 ESG 스코어 산출에서 '등급 산출 대상 유형'에는 어떤 자산이 포함되는가?",
  "qa_type": "definition",
  "question_template": "정의_등급산출대상유형_포함자산"
}
```
...
```json
{
  ...
  "subsection": "나) 펀드 ESG 스코어의 산출",
  "seed_title": "나) 펀드 ESG 스코어의 산출",
  "question": "펀드 ESG 스코어 산출에서 '등급 산출 대상 유형'에는 어떤 자산이 포함되는가?",
...
## Assistant Response
```text
- 정의: 펀드 ESG 스코어 산출에서 “등급 산출 대상 유형”은 대유형 및 소유형이다.   
- 해당 등급 산출에 쓰이는 대상 포트폴리오 자산은 주식종목(우선주 포함)이며, ETF는 제외된다.
```
...
## Assistant Response
```text
- 펀드 ESG 스코어의 등급 산출에서 ‘등급 산출 대상 유형’이란, 펀드를 분류해 그 분류 내에서 ESG 등급(상대 등급)을 매기기 위한 대상 유형을 뜻하며 ‘대유형’과 ‘소유형’이 해당됩니다.   
- 이 등급 산출을 위해 ESG 스코어 산출에 포함되는 대상 포트폴리오 자산은 ‘주식종목(우선주 포함)’입니다.   
- 다만 ‘ETF’는 제외됩니다.
```
...
## Assistant Response
```text
- “등급 산출 대상 유형”은 대유형 및 소유형에 해당하는 펀드들을 의미합니다.   
- 이 등급 산출을 위해 ESG 스코어에 포함되는 자산(대상 포트폴리오 자산)은 주식종목(우선주 포함)입니다.   
- 단, ETF는 제외됩니다.
```
...
## Assistant Response
```text
펀드 ESG 스코어 산출에서 등급 산출에 쓰이는 대상 포트폴리오 자산(등급 산출 대상)은 다음과 같습니다. 

- 주식종목(우선주 포함)
- ETF는 제외(ETF를 구성 종목 기준으로 배분할지 여부에 따라 결정 필요하다고 명시)
```
