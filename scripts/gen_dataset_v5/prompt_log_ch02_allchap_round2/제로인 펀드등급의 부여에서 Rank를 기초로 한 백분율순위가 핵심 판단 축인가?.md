...
## Question
제로인 펀드등급의 부여에서 Rank를 기초로 한 백분율순위가 핵심 판단 축인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "제로인 펀드등급의 부여에서 Rank를 기초로 한 백분율순위가 핵심 판단 축인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "다) 제로인 펀드등급의 부여",
  "seed_title": "다) 제로인 펀드등급의 부여",
  "question": "제로인 펀드등급의 부여에서 Rank를 기초로 한 백분율순위가 핵심 판단 축인가?",
...
## Assistant Response
```text
- 네. 제로인 펀드등급의 성과등급(Performance Rating)은 유형 내 ZI 백분율순위(%Rank)를 산출하고, 이를 기초로 정식 등급/가등급을 부여합니다.   
- 또한 ZI등급은 “ZI 순위에 대한 백분율 순위”를 기준으로 한다고 명시되어 있습니다.
```
...
## Assistant Response
```text
펀드등급은 위험조정수익률 기반의 표준화점수(ZI)를 바탕으로 “동일 유형 내 상대 위치”를 반영해 부여하기 위한 기준입니다. 

- 제로인 펀드등급의 부여는 ZI의 백분율순위(%Rank)를 기초로 합니다.   
- 따라서 등급을 결정하는 핵심 판단 축은 Rank를 바탕으로 한 백분율순위(%Rank)입니다.   
- 다만 ZI Score는 ZI값을 100점으로 환산한 값이며, “등급 기준”은 백분율순위(%Rank)입니다.
```
...
## Assistant Response
```text
펀드등급은 유형 내에서 펀드 성과를 상대적으로 비교해 등급을 매기기 위한 판단 체계입니다. 

- 제로인 펀드등급 부여는 ZI 백분율순위(%Rank)를 기초로 합니다. 
- ZI등급은 “ZI 순위에 대한 백분율 순위”를 기준으로 하며, ZI Score는 ZI 값을 100점으로 환산한 값이라 구분됩니다. 
- 따라서 Rank 기반 백분율순위(%Rank)는 등급 산정의 핵심 판단 축에 해당합니다.
```
...
## Assistant Response
```text
등급을 부여하기 위해 펀드의 유형 내 상대적 성과 위치(순위)를 판단하는 것이 핵심입니다.   
- 제로인 펀드등급은 “ZI 백분율순위(%Rank)”를 기초로 정해집니다.   
- 운용기간에 따라 3년·5년·10년 이상은 정식등급, 1년은 가등급을 부여하며, 이때 기준이 되는 축이 ZI 백분율순위(%Rank)입니다.
```
