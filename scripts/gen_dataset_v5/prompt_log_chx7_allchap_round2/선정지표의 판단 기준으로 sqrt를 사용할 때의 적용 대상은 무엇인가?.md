...
## Question
선정지표의 판단 기준으로 sqrt를 사용할 때의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표의 판단 기준으로 sqrt를 사용할 때의 적용 대상은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표의 판단 기준으로 sqrt를 사용할 때의 적용 대상은 무엇인가?",
...
## Assistant Response
```text
- 선정지표(TECO 지표) 계산에서 sqrt는 TE_{i,j}^T 값에 적용됩니다   
- TE_{i,j}^T의 “분모/분자 형태” 중, 펀드 i와 타겟펀드( j' ) 수익률의 차이를 t시점별로 제곱해 합산한 값( Σ (R_t^i − R_t^{j'})^2 /(n−1) )에 sqrt를 취합니다   
- 여기서 적용 대상이 되는 항은 R_t^i(펀드 i의 수익률), R_t^{j'}(타겟펀드의 수익률), n(데이터의 개수)입니다
```
...
## Assistant Response
```text
선정지표 중 TECO 산출에 쓰이는 TE(Tracking Error) 식에서 sqrt는 “펀드와 타겟의 수익률 차이”를 분산 형태로 만든 값에 적용됩니다. 

- sqrt의 적용 대상은  t=1~T 구간에서의 (펀드 i 수익률 R_t^i − 타겟펀드 수익률 R_t^{j'})의 제곱합입니다.   
- 그 제곱합은 데이터 개수 n-1로 나누어 평균화한 뒤 sqrt가 취해집니다.   
- 결과적으로 sqrt는 “펀드 i와 타겟펀드의 수익률 차이의 변동(표준편차에 해당하는 값)”을 만드는 데 사용됩니다.
```
...
## Assistant Response
```text
- 제로인 선정지표에서 sqrt는 TECO 지표 산출 과정 중 TE_{i,j}^T(추적오차 성격의 변동성 항)를 계산할 때 적용됩니다.   
- sqrt의 적용 대상은 측정기간 t=1~T 동안의 “펀드 i의 수익률 R_t^i”와 “타겟펀드의 수익률 R_t^{j'}”의 차이(R_t^i − R_t^{j'})를 제곱해 합한 값을 n-1로 나눈 값입니다.   
- 여기서 n은 해당 측정기간에 사용된 데이터의 개수입니다.
```
