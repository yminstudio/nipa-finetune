...
## Question
선정지표의 판단 기준으로 frac를 사용할 때의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표의 판단 기준으로 frac를 사용할 때의 적용 대상은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표의 판단 기준으로 frac를 사용할 때의 적용 대상은 무엇인가?",
...
## Assistant Response
```text
- 선정지표(TECO 지표) 산출에서 \frac{…}{…} 형태(분수)를 사용할 때의 적용 대상은 분자에 해당하는 Σ_{t=1}^{T} (R_t^i − R_t^{j′})^2 입니다.   
- 여기서 R_t^i 는 t시점의 펀드 i 수익률 입니다.   
- 또한 R_t^{j′} 는 t시점의 타겟펀드 수익률 입니다. filecite  
- \frac{…}{…} 형태의 분모는 n−1 이며, n 은 데이터의 개수 입니다.
```
...
## Assistant Response
```text
선정지표 판단 기준에서 \frac 를 쓰는 경우는 TECO 지표의 추적오차(TE_{i,j}^T) 산식에서 나타나는 분수 구조입니다. 

- 분자( \sum_{t=1}^{T} ( R_t^i - R_t^{j'} )^2 ) : t=1~T 구간에서 펀드 i 수익률 R_t^i 와 타겟펀드 j' 수익률 R_t^{j'} 의 차이를 제곱한 값의 합   
- 분모( n-1 ) : 데이터 개수 n 에서 1을 뺀 값   
- TE_{i,j}^T 로 계산된 값은 이후 TECO_{i,f}^T 산식의 입력(TECO_{i,f}^T = TE_{i,f}^T × (2 - ρ_{i,f}^T))으로 사용됩니다
```
...
## Assistant Response
```text
선정지표에서 ‘비중(분수)’ 형태의 판단기준을 쓸 때 적용 대상은 “해당 펀드의 기간 중 수익이 플러스/마이너스였던 기간”입니다. 

- 기간중 음수익비중: 전체 펀드 기간(‘주’로 환산) 중 마이너스 수익이 발생했던 기간의 비중 
- 기간중 양수익비중(Positive Periods): 전체 펀드 기간(‘주’로 환산) 중 플러스 수익이 발생했던 기간의 비중 
- 이때 플러스/마이너스 구분에 쓰는 수익은 펀드의 주간 실현수익률이며, 양수 구간은 i번째 주의 주간 실현수익률이 0보다 큰지로 해당 기간을 카운트해 비중을 산출합니다.
```
...
## Assistant Response
```text
- 제로인 선정지표에서 사용하는 TECO 지표는 계산식에 분수 형태(위의 frac에 해당하는 항)를 포함하며, 이는 TECO 산출에 쓰는 기간별 수익률 비교 결과를 표준화하기 위한 “합/데이터수” 구성입니다.   
- 분수( frac )의 적용 대상(분자): 1~T 시점에 대해 펀드 i의 수익률 R_t^i 와 타겟펀드의 수익률 R_t^{j'} 의 차이( R_t^i - R_t^{j'} )를 제곱해 합산한 값입니다.   
- 분수( frac )의 적용 대상(분모): 데이터의 개수 n이며, 해당 식에서는 n-1로 나눕니다.
```
