# 질문 프롬프트 로그

## Stage
questions

## Record ID
Zeroin 펀드평가 방법론 - only_text_structure_0005

## Question
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

## Model
gpt-5-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "record_target": 12,
  "requested_question_count": 14,
  "call_requested_question_count": 6,
  "call_allowed_types": [
    "definition",
    "comparison",
    "application"
  ],
  "call_type_targets": {
    "definition": 2,
    "comparison": 2,
    "application": 2
  },
  "target_total": 100,
  "generated_question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "qa_type": "comparison",
  "question_template": "comparison_operation_sqrt_sum"
}
```

## System Prompt
```text
당신은 제로인 방법론 기반 QA 데이터셋의 질문 생성기다.
반드시 한국어로만 답하고, 제공된 재료 범위 안에서만 질문을 만든다.
질문은 짧고 단일 쟁점 중심이어야 한다.
일반 금융상식, 투자조언, 문서 바깥 배경지식 질문은 금지한다.
업로드 문서를 보지 않고도 일반 금융 상식만으로 답할 수 있는 질문은 생성하지 않는다.
질문은 반드시 문서 안의 특정 정의, 기준, 비교, 적용 중 하나에 직접 매핑되어야 한다.
정답은 문서의 핵심 정보와 의미 연결을 위한 최소 설명만으로 성립해야 하며, 문서 밖 지식 확장을 요구하는 질문은 만들지 않는다.
정답이 자연스러운 문장으로 연결될 수는 있어도, 그 연결을 위해 새로운 정보가 필요해지는 질문은 만들지 않는다.
```

## User Prompt
```text
아래 재료만 사용해서 자연스러운 질문 후보를 생성해줘.

[단원 제목]
X.7. 유사펀드 측정

[절 제목]
5) 선정지표

[소절 제목]


[핵심 제목]
5) 선정지표

[핵심 명사]
) 선정지표, 선정지표, TECO, 지표, TE, sqrt, frac, sum, left, right, n-1, Where

[질문 유형 목표]
criteria 0개, definition 2개, comparison 2개, application 2개

[이번 호출에서 허용되는 질문 유형]
definition, comparison, application

[이번 호출의 생성 목표]
definition 2개, comparison 2개, application 2개를 우선 맞춘다.

규칙:
- 질문은 짧고 단일 쟁점 중심
- 문서 범위를 벗어난 질문 금지
- 제목을 그대로 복붙한 문장 금지
- 일반 금융상식, 투자조언, 시장전망 질문 금지
- 업로드 문서를 보지 않고도 일반 금융 상식만으로 답할 수 있는 질문은 생성하지 않는다
- 답이 표 전체 요약이나 장문 설명이 되도록 만드는 질문은 금지하고, 특정 기준 하나만 묻는 질문으로 만든다
- 질문 하나에는 하나의 판단축만 남긴다. 정의와 기준, 기준과 예외를 한 문장에 함께 묻지 않는다
- 정답은 문서의 핵심 정보와 의미 연결을 위한 최소 설명만으로 완결될 수 있어야 한다
- 정답이 자연스러운 문장으로 연결되더라도 새로운 정보 추가 없이 답할 수 있어야 한다
- 질문이 새로운 판단 기준, 질문에 없는 대상, 불필요한 비교를 답변에 끌어오게 만들면 폐기한다
- definition 질문은 정의문 하나로 직접 답할 수 있어야 하며 절차·비교·예외를 동시에 요구하지 않는다
- criteria 질문은 판단 기준만 묻게 하고 적용 절차나 다른 대상 비교를 함께 요구하지 않는다
- comparison 질문은 필요한 비교축만 묻게 하고 각 대상의 정의나 절차 설명까지 요구하지 않는다
- application 질문은 적용 순서나 전환 조건을 묻되 정의/비교를 한 문장에 함께 묻지 않는다
- 질문 유형은 definition, criteria, comparison, application 중 하나만 사용
- 허용되지 않은 질문 유형은 절대 생성하지 않는다
- 최대 6개까지만 생성

생성 후 자체 점검:
- 이 질문은 문서의 특정 표, 기준, 정의, 절차가 없으면 답하기 어려운가?
- 이 질문에 일반 금융 상식으로 그럴듯하게 답할 수 있으면 폐기한다
- 질문이 길거나 복합적이면 더 짧은 한 쟁점 질문으로 다시 쓴다
- 이 질문에 답하려고 할 때 질문에 없는 대상/비교/예외를 덧붙이게 되면 폐기한다

반환 형식:
순수 JSON 배열만 출력한다.
[
  {
    "question": "질문 내용",
    "qa_type": "definition|criteria|comparison|application",
    "question_template": "템플릿 이름"
  }
]

```

---

## Stage
answers

## Record ID
v5_q_0083

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "answer_policy": "structured_medium"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
반드시 한국어로만 답한다.
업로드된 문서에 근거가 없는 기준, 수치, 예외를 만들지 않는다.
질문에 필요한 최소 범위까지만 답한다.
문서 밖 일반 지식으로 빈칸을 메우지 않는다.
문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준에 한한다.
답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
답변은 짧고 구조적으로 유지한다.
문서의 핵심 용어와 판단 기준어는 가능하면 유지한다.
질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
새로운 판단 기준, 규칙, 예외, 질문에 없는 대상이나 비교는 추가하지 않는다.
출처/원문/문서/장/절을 직접 언급하지 않는다.
일반 금융 상식으로 확장하지 않는다.
```

## User Prompt
```text
[질문]
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

[답변 스타일]
구조 보존형 답변으로 3~5개 불릿까지 허용한다. 표/절차/산식의 핵심 항목만 순서대로 정리하고, 문맥 연결을 위한 최소 설명 외의 배경설명은 넣지 않는다.
- 답변 길이는 공백 포함 최대 1200자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이와 생략된 주어/관계 보완 수준에 한한다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 비교형: 답변은 질문에 직접 필요한 비교축의 차이만 제시한다
- 비교형: 각 대상의 정의, 절차, 예외 설명으로 확장하지 않는다
- 질문에 직접 필요한 기준만 남기고 예시, 배경설명, 장황한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
- sum: 기간/항에 대해 값을 누적하는 “합산(집계) 연산”이며, 예를 들어 TECO 식에서 \sum_{t=1}^{T} \left( R_t^i - R_t^{j'} \right)^2 처럼 항들을 더해줍니다   
- sqrt: sum으로 얻은 결과(분모·분자 포함)를 대상으로 “제곱근 취하는 변환(루트) 연산”이며, TECO는 sqrt{ … } 형태로 최종 값을 만듭니다   
- 연산 배치: 제로인 산식에서는 보통 “sum(제곱항 누적) → sqrt(최종 변환)” 순서로 구성되며, 표준편차도 \sigma^F=\sqrt{\frac{\sum_{i=1}^n (R^F_i-\overline{R^F})^2}{n-1}} 형태입니다   
- 예시로 동일 구조: 하락편차/상승편차 역시 \sqrt{\frac{\sum[\,\min/\max(\cdot)\,]^2}{(\,n^{F,BM}_D-1\,)}\times 52}처럼, 내부에서 \sum로 항을 합산한 뒤 바깥에서 sqrt로 변환합니다
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "structured_medium",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Stage
answers

## Record ID
v5_q_0083

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "answer_policy": "structured_medium"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

[답변 스타일]
구조 보존형 답변으로 3~5개 불릿까지 허용한다. 표/절차/산식이 무엇을 설명하는지 짧게 밝힌 뒤 핵심 항목만 순서대로 정리하고, 문맥 연결을 위한 최소 설명 외의 배경설명은 넣지 않는다.
- 답변 길이는 공백 포함 최대 1200자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 비교형: 답변은 질문에 직접 필요한 비교축의 차이만 제시한다
- 비교형: 각 대상의 정의, 절차, 예외 설명으로 확장하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Stage
answers

## Record ID
v5_q_0083

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "answer_policy": "structured_medium"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

[답변 스타일]
구조 보존형 답변으로 3~5개 불릿까지 허용한다. 표/절차/산식이 무엇을 설명하는지 짧게 밝힌 뒤 핵심 항목만 순서대로 정리하고, 문맥 연결을 위한 최소 설명 외의 배경설명은 넣지 않는다.
- 답변 길이는 공백 포함 최대 1200자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 비교형: 답변은 질문에 직접 필요한 비교축의 차이만 제시한다
- 비교형: 각 대상의 정의, 절차, 예외 설명으로 확장하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
표준편차(위험지표) 산식처럼 같은 계산식 안에서도 `sum`과 `sqrt`는 하는 일이 다릅니다. 

- `sum(Σ …)` : 여러 기간(또는 항) 값을 “합”으로 누적하는 연산 유형(분자에 등장)   
- `sqrt(√ …)` : `sum`으로 만든 값에 대해 “제곱근”을 취해 최종 지표 스케일로 바꾸는 연산 유형(표준편차에 외곽으로 등장)   
- `sum`은 항(예: (R_i − 평균)^2)들을 집계하는 단계이고, `sqrt`는 집계 결과를 변환(√)해 단일 값(예: 표준편차)으로 만드는 단계입니다.   
- 동일한 구조가 초과수익률 표준편차 산식에서도 `sqrt( … )` 밖에 `sum`이 포함되는 형태로 나타납니다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "structured_medium",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Stage
answers

## Record ID
v5_q_0083

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "answer_policy": "structured_medium"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

[답변 스타일]
구조 보존형 답변으로 3~5개 불릿까지 허용한다. 표/절차/산식이 무엇을 설명하는지 짧게 밝힌 뒤 핵심 항목만 순서대로 정리하고, 문맥 연결을 위한 최소 설명 외의 배경설명은 넣지 않는다.
- 답변 길이는 공백 포함 최대 1200자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 비교형: 답변은 질문에 직접 필요한 비교축의 차이만 제시한다
- 비교형: 각 대상의 정의, 절차, 예외 설명으로 확장하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
- sqrt: 제곱근(루트) 변환 연산입니다. 위험지표에서 분자에 sum으로 누적된 값(예: 제곱항의 합)을 (n-1) 등으로 나눈 뒤, 그 최종 결과에 제곱근을 취해 표준편차/편차 값으로 만듭니다(예: 표준편차는 √(Σ(… )²/(n-1))).   
- sum: 항들을 더해 “누적”하는 합산 연산입니다. 예를 들어 표준편차에서 Σ는 각 기간의 평균 대비 제곱편차(… )²를 전 기간에 대해 합칩니다.   
- 연산 유형의 배치(차이): sum은 “제곱항을 합해 크기를 만든다”에 해당하고, sqrt는 “그 합(나눈 값)의 제곱근으로 변환한다”에 해당합니다. 같은 구조가 하락편차/상승편차/트래킹에러 등에서도 반복됩니다(예: 하락편차 √(Σ[min(… ,0)]²/(n_D-1)*52), 트래킹에러 √(Σ(… )²/(n-1))).
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "structured_medium",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Assistant Response
```text
두 연산은 제로인 산식에서 “집계”와 “변환(비선형 스케일링)” 역할로 함께 쓰입니다. 예를 들어 표준편차/하락편차/트래킹에러 식에서 sum 안의 값에 sqrt가 적용됩니다.  

- sum: 여러 기간(또는 조건을 만족하는 기간)의 항을 더해 “누적값(예: 제곱항의 합, 조건부 제곱합)”을 만드는 연산(집계/합산)입니다. 
- sqrt: 이미 만들어진 “누적값(및 분모/가공 포함)”에 대해 제곱근을 취해 최종 지표의 단위/스케일을 정하는 연산(제곱근 변환)입니다.  
- 연산 유형 기준 차이의 핵심: sum이 먼저 항들을 합쳐 누적을 만들고, 그 결과에 sqrt를 적용해 표준편차/편차/트래킹에러 같은 최종 값을 산출합니다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "structured_medium",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```

---

## Stage
answers

## Record ID
v5_q_0083

## Model
gpt-5.4-nano

## API Base
https://api.openai.com/v1

## Extra
```json
{
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee",
  "chapter": "X.7. 유사펀드 측정",
  "section": "5) 선정지표",
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.",
  "answer_policy": "structured_medium"
}
```

## System Prompt
```text
제로인은 펀드평가사로 펀드평가의 방법론을 가지고 있습니다.
- 당신은 제로인 방법론에 근거해 답변하는 도메인 어시스턴트다.
- 반드시 한국어로만 답한다.
- 답변은 질문에 직접 필요한 범위를 넘기지 않되, 독립적으로 이해 가능하도록 필요한 짧은 전제 설명은 포함할 수 있다.
- 답변은 독립적으로 이해 가능해야 한다. 하지만 그 이해 가능성을 이유로 새 정보를 만들면 안 된다.
- 답변은 불필요하게 짧게 끊지 말고, 구조적으로 정리하되 필요한 연결 문장은 남긴다.
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소한의 설명만 허용한다.
- 이 최소 설명은 용어를 풀거나 생략된 주어와 관계를 잇는 수준, 또는 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다.
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다.
- 질문이 묻지 않은 다른 판단 기준으로 확장하지 않는다.
- 출처/원문/문서/장/절을 직접 언급하지 않는다.
- 일반 금융 상식으로 확장하지 않는다.
- **반드시 답변은 독립적으로 이해 가능해야 한다.**
```

## User Prompt
```text
[질문]
sqrt와 sum의 차이를 '연산 유형' 기준으로 비교하시오.

[답변 스타일]
구조 보존형 답변으로 3~5개 불릿까지 허용한다. 표/절차/산식이 무엇을 설명하는지 짧게 밝힌 뒤 핵심 항목만 순서대로 정리하고, 문맥 연결을 위한 최소 설명 외의 배경설명은 넣지 않는다.
- 답변 길이는 공백 포함 최대 1200자 이내
- 문서에 없는 내용은 추가하지 않되, 의미 연결을 위한 최소 설명만 허용한다
- 이 최소 설명은 용어 풀이, 생략된 주어/관계 보완, 질문의 대상을 짧게 다시 잡아 주는 수준에 한한다
- 답변은 독립적으로 이해 가능해야 하지만, 그 이해 가능성을 이유로 새 정보를 만들면 안 된다
- 답변은 자연스러운 문장으로 연결하되, 새로운 정보 추가 없이 기존 내용을 매끄럽게 표현한다
- 새로운 판단 기준, 규칙, 예외, 질문에 없는 대상/비교는 추가하지 않는다
- 비교형: 답변은 질문에 직접 필요한 비교축의 차이만 제시한다
- 비교형: 각 대상의 정의, 절차, 예외 설명으로 확장하지 않는다
- 질문에 직접 필요한 기준만 남기되, 새 정보가 아닌 범위에서 짧은 전제 설명은 허용한다
- 예시, 장황한 배경설명, 불필요한 열거는 줄인다
- 제목, 번호 섹션, 마크다운 헤더(예: ##, 1) )는 쓰지 않는다
```

---

## Assistant Response
```text
- 전제: 제로인 위험지표(표준편차, 하락편차, 트래킹오차 등)는 “여러 기간의 항을 더한 값(sum)”을 만든 뒤, 그 값을 “제곱근(sqrt)”으로 변환해 최종 지표를 산출하는 형태로 제시됩니다.    
- sum 연산 유형: 여러 관측치(예: 기간 i)의 값을 ‘더해서’(누적) 분자/계산의 집계 입력을 만드는 연산입니다.    
- sqrt 연산 유형: sum으로 집계된 결과(예: 분산 형태의 값)에 대해 ‘제곱근을 취해’ 최종 위험지표 값을 만드는 연산입니다.    
- 핵심 차이(연산 유형 기준): sum은 집계(덧셈) 연산, sqrt는 변환(제곱근) 연산이며, 제로인 수식에서는 대체로 sum이 먼저 계산되고 sqrt가 그 다음에 적용됩니다.
```

## Response Extra
```json
{
  "answer_status": "generated",
  "answer_issues": [],
  "answer_policy": "structured_medium",
  "vector_store_id": "vs_68a80414c938819189ac784ba37c10ee"
}
```
