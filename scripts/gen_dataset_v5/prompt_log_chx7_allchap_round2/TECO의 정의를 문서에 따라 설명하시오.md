...
```json
{
  ...
  "target_total": 100,
  "generated_question": "TECO의 정의를 문서에 따라 설명하시오.",
  "qa_type": "definition",
  "question_template": "definition_TECO"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "TECO의 정의를 문서에 따라 설명하시오.",
...
## Assistant Response
```text
- TECO의 정의: TECO_{i,f}^T는 “타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성(변동의 방향성 포함)”을 나타내는 지표로, 다음과 같이 정의된다. TECO_{i,f}^T = TE_{i,f}^T × (2 − ρ_{i,f}^T)   
- TE_{i,f}^T는 수익률 차이의 제곱평균을 통해 산출되는 값으로, TE_{i,f}^T = √( Σ_{t=1}^{T}(R_t^i − R_t^{j'})^2 / (n−1) )   
- ρ_{i,f}^T는 공분산/표준편차를 기반으로 한 상관계수로, ρ_{i,f}^T = σ_{i,f}^T / (σ_i^T σ_f^T)   
- TECO는 TE(변동 추이의 유사성)뿐 아니라 상관계수를 함께 활용하여 “방향성도 고려”한다.
```
...
## Assistant Response
```text
- TECO(=TECO 지표)는 타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성을 나타내는 지표이며, TE뿐만 아니라 상관계수로 변동의 “방향성”까지 함께 고려하는 지표임   
- 정의(계산식): TECO_{i,f}^T = TE_{i,f}^T × (2 - ρ_{i,f}^T)   
- TECO 지표는 0~100 사이의 값을 가짐
```
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드 간 수익률 변동 추이의 유사성(TE)뿐 아니라 변동의 방향성까지 함께 고려하는 지표로,  “타겟펀드와의 수익률 변동 추이와 변동의 방향성도 함께 고려”된다고 설명됩니다.  
- 정의식은 TECO_{i,f}^T = TE_{i,f}^T × (2 − ρ_{i,f}^T) 이며, TE와 상관계수(ρ)를 결합해 방향성 요소를 반영합니다.   
- 여기서 TE_{i,j}^T 는 두 펀드의 t시점 수익률 차이 제곱의 평균을 제곱근으로 한 값으로,  수익률 변동 추이의 “유사성”을 수치화합니다.  
- ρ_{i,f}^T 는 공분산을 각 펀드 수익률의 표준편차로 나눈 형태의 상관계수로 정의되며,  TECO에 방향성 정보를 더합니다.  
- TECO 지표는 0~100 사이의 값을 가집니다(현재는 수정 예정 문구로 0~10점 사이로 표기됨).
```
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드 간 수익률 변동 추이의 유사성을 나타내는 지표이며, 변동의 ‘방향성’까지 함께 고려하기 위해 TE와 상관계수(ρ)를 결합해 산출합니다.   
- TE(추적오차 성격의 유사성 기초값)는 타겟펀드 대비 각 시점 수익률 차이를 제곱해 평균내어 제곱근으로 정의됩니다: TE_{i,j}^T = √{ Σ_{t=1}^{T} (R_t^i − R_t^{j’})^2 /(n−1) }.   
- ρ(상관계수)는 수익률의 공분산을 각 펀드 수익률의 표준편차로 나눈 값으로 정의됩니다.   
- 정의(핵심): TECO_{i,f}^T = TE_{i,f}^T × (2 − ρ_{i,f}^T).
```
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드의 수익률 변동 추이의 “유사성”을 나타내는 지표로, 변동의 방향성까지 함께 고려하기 위해 TE(변동 추이의 차이)뿐 아니라 상관계수(방향성)도 활용한다   
- TECO의 정의(산식)는 TE와 상관계수를 결합한 값으로,  
  TECO_{i,f}^T = TE_{i,f}^T × (2 - ρ_{i,f}^T) 이다   
- 여기서 TE_{i,f}^T는 두 펀드 수익률의 차이를 제곱해 평균한 값의 제곱근으로 정의된다: TE_{i,f}^T = √( Σ_{t=1}^{T}(R_t^i - R_t^{f'})^2 / (n-1) )   
- ρ_{i,f}^T는 공분산과 각 수익률의 표준편차로 계산되는 상관계수이며(정의식은 공분산/표준편차 곱), TECO는 0~100 사이의 값을 갖는다
```
