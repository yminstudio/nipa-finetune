...
## Question
선정지표의 판단 기준으로 Where를 사용할 때의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표의 판단 기준으로 Where를 사용할 때의 적용 대상은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표의 판단 기준으로 Where를 사용할 때의 적용 대상은 무엇인가?",
...
- $R_t^{j'}$: t시점의 타겟펀드 수익률   
- $n$: 데이터의 개수   
- $\sigma_{i,f}^T$, $\sigma_i^T$, $\sigma_f^T$: 펀드 i와 타겟펀드 수익률의 공분산 및 표준편차   
- $R_i^t$, $R_f^t$, $\overline{R_i}$, $\overline{R_f}$: 측정기간 내 각 시점 수익률 및 평균 수익률
```
...
## Assistant Response
```text
선정지표(TECO)에서 “Where”는 지표에 포함된 각 기호(예: 수익률, 표준편차, 공분산)가 무엇을 의미하는지 “계산에 실제로 투입되는 대상”을 한정해 주는 용도입니다. 

- 펀드 i: Where에서 “펀드 i의 수익률( t시점 )”, “펀드 i의 평균 수익률”, “펀드 i의 수익률 표준편차” 등에 적용 
- 타겟펀드: Where에서 “타겟펀드의 수익률( t시점 )”, “타겟펀드의 평균 수익률”, “타겟펀드의 수익률 표준편차” 등에 적용 
- t시점의 수익률 비교 대상: Where에서 펀드 i의 수익률과 타겟펀드의 수익률을 t시점 단위로 함께 사용 
- 데이터 수 n: Where에서 분모 및 각 통계량(분산/공분산 등) 산출에 들어가는 “데이터의 개수”에 적용
```
...
## Assistant Response
```text
선정지표(TECO 지표) 수식에서 Where는 “수식에 들어간 기호(변수)를 무엇으로 해석/대입하는지”를 정하는 적용 대상입니다. 

- $R_t^i$ : t시점의 펀드 i의 수익률 
- $R_t^{j'}$ (또는 $R_f^t$) : t시점의 타겟(타켓) 펀드의 수익률 
- $n$ : 데이터의 개수 
- $\sigma_{i,j}^T$, $\sigma_i^T$ 및 $\sigma_f^T$ : (해당 측정기간 $T$ 기준) 펀드 i와 타겟 간 공분산/각 표준편차 
- $\overline{R_i}$, $\overline{R_f}$ : 측정기간 동안의 펀드 i/타겟 평균 수익률
```
...
## Assistant Response
```text
선정지표(TECO 지표)에서 식 뒤에 나오는 “Where”는, 해당 지표를 계산할 때 사용하는 값들이 어떤 대상(펀드/타겟펀드)과 어떤 통계항목에 해당하는지를 지정하는 데 적용됩니다. 

- t시점의 값: R_t^i(펀드 i의 수익률), R_t^{j'}(타겟펀드의 수익률) 
- 측정 입력: n(데이터 개수) 
- 측정기간의 통계량(공분산/표준편차): σ_{i,f}^T(펀드 i와 타겟펀드 수익률 공분산), σ_i^T(펀드 i 수익률 표준편차), σ_f^T(타겟펀드 수익률 표준편차) 
- 측정기간 평균 및 수익률 기호: R_i^t(펀드 i 수익률), R_f^t(타겟펀드 수익률), \overline{R_i}(펀드 i 평균 수익률), \overline{R_f}(타겟펀드 평균 수익률)
```
...
## Assistant Response
```text
선정지표(TECO) 산식에서 Where는 지표 계산에 쓰이는 변수가 “어떤 대상의 어떤 값”을 뜻하는지를 정의합니다. 

- t시점의 비교 대상은 펀드 i와 타겟펀드(j′)입니다. 
- R_t^i: t시점의 펀드 i 수익률 
- R_t^{j′}: t시점의 타겟펀드 수익률, 그리고 n: 데이터의 개수 
- 공분산·표준편차의 Where에서는 펀드 i와 타겟펀드의 수익률 관계(공분산, 각 표준편차)를 정의합니다. 
- 평균수익률(평균값)도 측정기간 동안의 펀드 i와 타겟펀드의 평균 수익률로 적용됩니다.
```
