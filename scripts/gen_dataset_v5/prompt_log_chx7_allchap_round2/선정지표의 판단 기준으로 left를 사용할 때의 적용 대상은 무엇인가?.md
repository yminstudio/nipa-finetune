...
## Question
선정지표의 판단 기준으로 left를 사용할 때의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표의 판단 기준으로 left를 사용할 때의 적용 대상은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표의 판단 기준으로 left를 사용할 때의 적용 대상은 무엇인가?",
...
## Assistant Response
```text
- left(하락 쪽, Lower)는 원금보존성(원금보존성/원금손실률) 측정지표인 Lower Partial Moments에 적용합니다   
- Lower Partial Moments에서 Lower는 하락위험도(downside Risk)를 의미하며, 상승분을 제외한 하락분만을 사용합니다   
- 원금보존성은 Lower Partial Moments를 적용하되 Target return을 0으로 간주하고 α를 1로 하여 계산합니다
```
...
## Assistant Response
```text
- 선정지표에서 left(하락 측, Lower Partial Moments의 ‘Lower’에 해당)를 판단 기준으로 사용할 때의 적용 대상은 “원금보존성 / 원금손실률” 평가 항목입니다.   
- 이때 left는 하락위험도(downside risk)를 의미하며, “상승분은 제외하고 하락분만”을 사용해 손실 관련 변동을 측정하는 데 적용됩니다.   
- 원금보존성을 보기 위해서는 left를 하락분 계산에 적용하되, Target return을 0으로 간주하고(알파=1) 계산하며 값이 클수록(손실폭이 작을수록) 더 높은 점수를 부여하는 방식으로 연결됩니다.
```
...
## Assistant Response
```text
원금보존성/원금손실률을 평가하는 선정지표에서, “left”에 해당하는 기준은 Lower Partial Moments의 Lower(하락위험도)로 해석됩니다. 

- 적용 대상: 원금보존성 / 원금손실률 평가에 사용되는 Lower Partial Moments 
- 판단 기준(“left”): Lower = 하락위험도(downside Risk), Partial = 상승분을 제외한 하락분만 사용 
- 원금보존성 산출 시 적용: Lower Partial Moments를 적용하되 Target return을 0으로 간주하고 α를 1로 설정
```
...
## Assistant Response
```text
선정지표(기간수익률 및 펀드지표의 %순위) 판단 기준에서 “left(좌동)”이 적혀 있으면, 해당 항목은 위에 기재된 기존 판단 기준을 그대로 적용한다. 

- 적용 대상 1: 평가대상 펀드(모펀드, 종류형펀드 제외)   
- 적용 대상 2: 기간 내 성과의 단절이 없는 펀드
```
