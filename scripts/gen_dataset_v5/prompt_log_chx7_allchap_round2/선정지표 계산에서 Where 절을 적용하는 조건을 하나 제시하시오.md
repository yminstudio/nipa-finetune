...
## Question
선정지표 계산에서 Where 절을 적용하는 조건을 하나 제시하시오.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표 계산에서 Where 절을 적용하는 조건을 하나 제시하시오.",
  "qa_type": "application",
  "question_template": "application_where_condition"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표 계산에서 Where 절을 적용하는 조건을 하나 제시하시오.",
...
## Assistant Response
```text
- 적용 순서: TECO 지표 산식에서 $R_t^i, R_t^{j'}$를 계산하기 직전에 Where 절로 변수 정의를 적용한다.   
- 적용 조건(Where 절): $R_t^i$는 “t시점의 펀드 i의 수익률”, $R_t^{j'}$는 “t시점의 타겟펀드의 수익률”이어야 한다.   
- 적용 전환: Where 절로 정의된 두 값은 이후 $ (R_t^i - R_t^{j'})^2$ 항에 그대로 대입되어 합산된다.
```
...
## Assistant Response
```text
- 대상은 “배당주식시가배당률”을 계산할 때의 주식 종목 집합으로 잡는다.   
- Where 절(변수 정의/산식 적용)에 해당하는 조건: 주식종목 중 시가배당이 존재하는 종목만을 대상으로 한다.   
- 전환 규칙: 이렇게 선정된 “배당 종목 집합” 내에서 각 종목의 비중(해당 집합 기준 비중)을 ω_i로 사용한다.
```
...
## Assistant Response
```text
- 선정지표 중 TECO 지표를 계산할 때는, TECO 산출식에 포함된 기호(펀드 수익률, 타겟펀드 수익률, 데이터 개수 등)를 식 바로 아래에서 Where 절로 정의한 뒤 
- Where 절은 “산출식에 등장하는 각 변수/기호를 뜻하는 값”을 정리해야 할 때 적용한다 
- 적용 순서는 (1) TECO(예: TE, ρ, TECO) 계산식 확인 → (2) 계산식에 쓰인 기호에 대해 Where 절 정의 → (3) Where 절 정의값을 산출식에 대입하여 TECO로 전환한다
```
...
## Assistant Response
```text
- 선정지표(TECO 지표) 점수화 단계에서 TECO_score를 계산할 때, 먼저 TECO_{i,f}^T 값을 구한다.   
- TECO_{i,f}^T_score 전환식 적용 시, 분모의 max(TECO_{i,f}^T)는 “TECO 지표값이 높은 순으로 상위 5%에 속하는 관측치”는 제외한 상태에서 산출한다.   
- 상위 5%에 속한 관측치는, 점수화 계산 과정에서 음(-)의 값이 나올 수 있어도 0점으로 처리한다.   
- 위와 같은 조건을 만족한 max를 사용해 TECO_{i,f}^T_score를 최종적으로 전환한다.
```
...
## Assistant Response
```text
- 적용 순서: TECO 선정지표 산출을 위해 TE_{i,j}^T를 먼저 계산한 뒤(합: t=1~T), 이어서 TECO_{i,f}^T = TE_{i,f}^T × (2 − ρ_{i,f}^T)로 전환합니다.   
- 적용 조건(Where 절): TE_{i,j}^T/ρ_{i,f}^T 계산에서 Where로 정해진 값들을 사용하며, R_t^i는 “t시점의 펀드 i 수익률”, R_t^{j'}(또는 R_f^t)는 “t시점의 타겟(타겟펀드) 수익률”, n은 “데이터의 개수”입니다.   
- 전환 규칙: 최종 TECO_{i,f}^T_score는 100 − 100 × (TECO_{i,f}^T / max(TECO_{i,f}^T)) 형태로 점수로 변환합니다.
```
