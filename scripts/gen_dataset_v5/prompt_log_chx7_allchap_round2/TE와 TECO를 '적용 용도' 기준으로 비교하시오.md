...
```json
{
  ...
  "target_total": 100,
  "generated_question": "TE와 TECO를 '적용 용도' 기준으로 비교하시오.",
  "qa_type": "comparison",
  "question_template": "comparison_purpose_TE_TECO"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "TE와 TECO를 '적용 용도' 기준으로 비교하시오.",
...
## Assistant Response
```text
- TE: 펀드와 시장(또는 BM)의 추적오차(Tracking Error)로서 정보비율(IR)에서 분모로 사용됨(펀드의 초과수익과 변동의 비율 산정)   
- TECO: 타겟펀드와 대상펀드의 “유사성”을 보기 위한 선정지표로 사용됨(TECO 지표의 산출식이 선정지표로 제시)   
- TECO는 TE(수익률 변동 추이의 유사성)뿐만 아니라 상관계수도 함께 활용해 변동의 방향성까지 고려하는 용도에 해당   
- TECO 활용: 펀드닥터 유사펀드 제시, 매니저별 운용펀드 파악, 신규펀드 리스크 등급 부여에 활용됨
```
...
## Assistant Response
```text
TE와 TECO는 모두 수익률의 변동과 연관된 지표이지만, “적용 용도”가 다릅니다.

- TE(추적오차): 펀드의 기간수익률이 벤치마크 수익률과 “어느 정도 차이”가 나는지 변동성으로 측정하는 용도로 사용됩니다 .
- TE(추적오차): 정보비율(IR)에서 분모로 사용되어, 초과수익 대비 ‘추적오차(=TE)’를 함께 반영하는 용도입니다 .
- TECO(유사성 지표): 타겟펀드와 대상펀드 사이의 수익률 변동 “추이 유사성”을 보는 선정지표로 사용됩니다 .
- TECO(유사성 지표): TE뿐 아니라 상관계수를 함께 반영해 변동의 방향성까지 고려하며, 그 결과(점수/순위)를 활용해 유사펀드 제시, 매니저별 운용펀드 파악, 신규펀드 리스크 등급 부여에 사용됩니다  .
```
...
## Assistant Response
```text
- TE는 타겟펀드와 대상펀드 사이의 수익률 변동 추이의 ‘유사성’을 나타내는 지표로 적용되며, 수익률의 차이를 제곱해 평균한 값으로 TE를 계산합니다.    
- TECO는 TE(수익률 변동 추이 유사성)에 더해 상관계수를 활용해 ‘방향성’까지 함께 고려하는 용도로 적용됩니다.   
- TECO는 따라서 타겟펀드와의 수익률 변동 추이 및 변동의 방향성을 동시에 반영한 ‘유사성 지표’로 활용되며, 유사펀드 제시/매니저별 운용펀드 파악/신규펀드 리스크 등급 부여에 이용됩니다.
```
...
## Assistant Response
```text
- TE(추적오차)는 일정기간 펀드 수익률과 대응 벤치마크 수익률의 차이가 어느 정도 변동하는지를 측정하는 지표이며, 제로인에서는 펀드 성과의 정보비율(IR) 산출 시 분모의 “추적오차(TE)”로 활용합니다.    
- TECO는 타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성을 나타내는 “유사성 지표”로, TE에 더해 상관계수를 함께 반영하여 변동의 방향성까지 고려하는 용도로 쓰입니다.   
- 적용 용도(활용처) 관점의 차이: TE는 벤치마크 대비 변동(추적오차) 수준을 성과지표 계산에 반영하는 데 초점이 있고, TECO는 유사펀드 제시/매니저 운용펀드 파악/신규펀드 리스크 등급 부여 등 “유사성 판단”에 사용됩니다.
```
...
## Assistant Response
```text
- TE: 타겟펀드(또는 목표펀드)와 대상펀드 사이의 수익률 “변동 추이의 유사성”을 나타내는 기초 지표로 쓰이며, 펀드 수익률의 시점별 차이를 제곱해 평균한 값의 제곱근으로 정의된다 .  
- TECO: TE에 더해 상관계수(ρ)를 함께 반영해, 변동 추이의 유사성뿐 아니라 “변동의 방향성”까지 고려하는 지표로 활용된다 . (TECO = TE × (2 − ρ))   
- 적용 용도(활용처) 기준 차이: TE는 TECO 산식에서 사용되는 구성요소(기초지표)로 제시되는 반면, TECO는 유사성 지표로서 매월 유사펀드 제공, 매니저별 운용펀드 파악, 신규펀드 리스크 등급 부여 등에 직접 활용 방안이 제시된다 .  
- 결론적으로 “무엇을 보느냐”의 적용 목적이 다르며, TE는 유사성(추이), TECO는 유사성+방향성(추이 및 상관)을 함께 보는 용도에 해당한다 .
```
