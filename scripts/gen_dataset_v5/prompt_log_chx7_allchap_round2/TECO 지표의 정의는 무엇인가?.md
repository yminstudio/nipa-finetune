...
```json
{
  ...
  "target_total": 100,
  "generated_question": "TECO 지표의 정의는 무엇인가?",
  "qa_type": "definition",
...
## Assistant Response
```text
- TECO 지표는 타겟펀드와 대상펀드의 수익률 변동 추이 유사성을 TE와 상관계수를 함께 반영한 지표로, 다음과 같이 정의된다.   
  - TECO_{i,f}^T = TE_{i,f}^T × (2 - ρ_{i,f}^T) 
- TE_{i,f}^T(추적오차 기반 유사성, TE)는 다음과 같이 정의된다.   
  - TE_{i,j}^T = √( Σ_{t=1}^{T} (R_t^i - R_t^{j'})^2 / (n-1) ) 
- ρ_{i,f}^T(상관계수)는 다음과 같이 정의된다.
```
...
## Assistant Response
```text
TECO 지표는 대상펀드와 타겟펀드의 “수익률 변동 추이의 유사성”을 TE(추적오차 성격)와 상관계수(방향성)로 결합해 만든 지표입니다. 

- TECO 정의(결합식): TECO_{i,f}^T = TE_{i,f}^T × (2 − ρ_{i,f}^T) 
- TE(변동 추이 유사성) 정의: TE_{i,j}^T = √( Σ_{t=1}^{T} (R_t^i − R_t^{j'})^2 / (n−1) ) 
- ρ(상관계수) 정의: ρ_{i,f}^T = σ_{i,f}^T / (σ_i^T σ_f^T) (공분산/각 표준편차 기반)
```
...
## Assistant Response
```text
TECO 지표는 타겟펀드와 대상펀드 사이의 수익률 “변동 추이의 유사성”을 나타내되, 상관계수를 함께 사용해 “변동의 방향성”까지 고려하는 지표입니다 .

- TE(변동 추이 유사성, 비유사도 기반):  
  TE_{i,f}^T = √( [∑_{t=1}^{T}( R_t^i − R_t^{f} )^2 ] / (n−1) )   
  (R_t^i: t시점 펀드 i 수익률, R_t^{f}: t시점 타겟펀드 수익률, n: 데이터 개수)

- ρ(상관계수, 방향성 반영):  
  ρ_{i,f}^T = σ_{i,f}^T / (σ_i^T σ_f^T) 로 정의(수익률 공분산/표준편차 기반) 

- TECO(지표 정의):  
  TECO_{i,f}^T = TE_{i,f}^T × ( 2 − ρ_{i,f}^T ) 

- (점수화) TECO_{i,f}^T_score:  
  TECO_{i,f}^T_score = 100 − 100 × ( TECO_{i,f}^T / max(TECO_{i,f}^T) )
```
...
## Assistant Response
```text
TECO 지표는 타겟펀드와 대상펀드 간의 수익률 변동 추이 “유사성(TE)”과 “방향성(상관계수 ρ)”을 함께 반영한 지표이며, TECO는 다음처럼 정의됩니다. 

- TE(추적오차 성격의 변동 추이 유사성)  
  $TE_{i,j}^T=\sqrt{\dfrac{\sum_{t=1}^{T}\left(R_t^i-R_t^{j'}\right)^2}{n-1}}$ 
- 상관계수(방향성)  
  $ \rho_{i,f}^T=\dfrac{\sigma_{i,f}^T}{\sigma_i^T \sigma_f^T}$ (공분산/표준편차로 산출) 
- TECO 지표값(유사성·방향성 결합)  
  $TECO_{i,f}^T = TE_{i,f}^T \times (2-\rho_{i,f}^T)$ 
- (점수화) TECO 점수  
  $TECO_{i,f}^T_{score}=100-100\times\left(\dfrac{TECO_{i,f}^T}{\max(TECO_{i,f}^T)}\right)$
```
...
## Assistant Response
```text
TECO 지표는 타겟펀드와 대상펀드의 수익률 변동 추이 “유사성”을 나타내며, 변동의 방향성까지 함께 반영한 지표입니다 .  

- TECO의 정의(산식): TECO_{i,f}^T = TE_{i,f}^T × (2 - ρ_{i,f}^T)   
- TE(추적오차 성격의 변동 차이) 정의: TE_{i,f}^T = √( Σ_{t=1}^{T} (R_t^i - R_t^{f'})^2 / (n-1) )   
- ρ(상관계수) 정의: ρ_{i,f}^T = σ_{i,f}^T / (σ_i^T σ_f^T)   
- TECO 값의 범위: TECO 지표는 0~100 사이의 값을 가집니다
```
