...
## Question
선정지표의 판단 기준으로 sum를 사용할 때의 적용 대상은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "선정지표의 판단 기준으로 sum를 사용할 때의 적용 대상은 무엇인가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5) 선정지표",
  "question": "선정지표의 판단 기준으로 sum를 사용할 때의 적용 대상은 무엇인가?",
...
## Assistant Response
```text
- TECO 지표의 판단 기준에서 sum는 t=1~T 범위의 “t시점 수익률”을 대상으로 합니다. 즉, t시점의 펀드 i 수익률 R_t^i 와 t시점의 타겟펀드 수익률 R_t^{j'}의 차이 (R_t^i − R_t^{j'})^2 를 합산합니다   
- 같은 TECO 산출에 포함된 상관계수(ρ)에서는 sum가 t=1~T 범위의 “평균 대비 수익률(편차)”을 대상으로 합니다. (R_i^t − \overline{R_i})(R_f^t − \overline{R_f}) 와 (R_i^t − \overline{R_i})^2, (R_f^t − \overline{R_f})^2 를 각각 합산합니다   
- 위 sum에 사용되는 구간의 개수는 n=데이터의 개수로 정의됩니다
```
...
## Assistant Response
```text
선정지표의 판단 기준에서의 sum는 TECO 지표를 계산할 때, 제곱항을 구성하는 수익률 값들을 시간 관측 구간 전체에 대해 누적(합산)하기 위한 것입니다. 

- sum의 적용 대상: 측정기간 내 시점 t별 값( t = 1 ~ T )   
- sum에 들어가는 항: 각 시점 t에서의 펀드 수익률 R_t^i 와 타겟펀드 수익률 R_t^{j'}의 차이( R_t^i − R_t^{j'} ) 제곱   
- 요약 의미: 측정기간(T) 동안의 t별 “초과/차이”를 전부 합산해 TECO의 분자(제곱합)를 만듭니다.   
- 관련 변수: 데이터 개수 n은 동일한 측정기간의 관측치 개수로, sum으로 누적된 값의 분모( n−1 )와 함께 사용됩니다.
```
...
## Assistant Response
```text
선정지표(유사펀드 선정)에서 TECO 지표를 계산할 때, Σ( sum )는 “측정기간의 개별 시점별 값”을 대상으로 적용됩니다. 

- Σ_{t=1}^{T} 로 표기된 sum의 적용 대상은 t=1부터 T까지의 시점(측정기간 내 시점들)   
- 각 시점 t에서 펀드 i의 수익률 R_t^i 와 타겟펀드의 수익률 R_t^{j'} 의 차이 (R_t^i - R_t^{j'})를 계산   
- 그 차이를 제곱한 값들을 시점별로 합산하는 방식으로 sum이 사용됨   
- 합산된 결과는 데이터의 개수 n으로 정규화(분모에 n-1이 사용)함
```
...
## Assistant Response
```text
- 선정지표(TECO)의 계산식에서 Σ(sum)는 측정기간 내 관측치의 합(인덱스 t에 대한 누적)으로 사용됩니다.   
- 구체적으로 TECO 식의 Σ는 t=1~T 구간에서 펀드 i의 수익률 R_t^i 와 타겟펀드(또는 j′)의 수익률 R_t^{j′} 간 차이( R_t^i − R_t^{j′} )의 제곱을 누적합니다.   
- TECO에 포함되는 상관계수 ρ 계산에서도 Σ는 동일하게 t=1~T 구간의 관측치에 대해, (R_i^t−평균)×(R_f^t−평균) 형태의 값을 누적합니다.   
- 즉, Σ의 적용 대상은 “기간 내 시점(t=1~T)의 수익률 데이터(펀드/타겟의 값)”입니다.
```
