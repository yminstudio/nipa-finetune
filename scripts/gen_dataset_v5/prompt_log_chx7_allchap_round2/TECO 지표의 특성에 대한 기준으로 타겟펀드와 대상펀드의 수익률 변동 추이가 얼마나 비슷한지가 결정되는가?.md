...
## Question
TECO 지표의 특성에 대한 기준으로 타겟펀드와 대상펀드의 수익률 변동 추이가 얼마나 비슷한지가 결정되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "TECO 지표의 특성에 대한 기준으로 타겟펀드와 대상펀드의 수익률 변동 추이가 얼마나 비슷한지가 결정되는가?",
  "qa_type": "criteria",
  "question_template": "TECO_특성_criteria"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "6) TECO 지표의 특성",
  "question": "TECO 지표의 특성에 대한 기준으로 타겟펀드와 대상펀드의 수익률 변동 추이가 얼마나 비슷한지가 결정되는가?",
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드의 수익률 변동 추이의 유사성을 나타내는 TE를 바탕으로 하며, 동시에 상관계수(ρ)를 활용해 변동의 방향성까지 함께 고려함으로써 유사성이 어떻게 판단되는지를 결정합니다   
- 유사성(TE)은 각 시점의 수익률 차이(Rt^i − Rt^{j′})를 제곱해 합산한 값의 제곱근 형태로 산출되는 “수익률 변동 추이의 유사성 지표”로 정의됩니다   
- 방향성까지 반영한 TECO 값은 TECO = TE × (2 − ρ) 형태로, TE의 유사성에 상관계수(ρ)의 효과를 결합해 결정됩니다   
- 최종적으로 TECO는 0~100 사이의 값을 가져(현재는 0~10점 사이로 표기 예정) TECO 값/점수 체계를 통해 “얼마나 비슷한지”를 표현합니다
```
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드 간 수익률 변동 추이의 유사성을 “TE(변동 추이의 차이)”와 “상관계수(변동의 방향성)”를 함께 반영해 결정합니다.   
- TE는 양 펀드의 시점별 수익률 차이(Rt^i − Rt^{j’}) 제곱의 평균을 루트로 계산하므로, 변동 추이가 더 비슷할수록(차이가 작을수록) TE가 작아지는 방식입니다.   
- TECO는 TE에 (2 − ρ_{i,f}^T)를 곱해 만들며, 여기서 ρ_{i,f}^T는 두 펀드 수익률의 상관계수입니다. 즉, 변동 방향까지 유사한지 여부가 TECO에 반영됩니다.   
- 최종 “유사성” 평가는 TECO 값을 기준으로 점수화(TECO_score)된 0~100(현재 0~10) 범위의 결과로 정리됩니다.
```
...
## Assistant Response
```text
- TECO는 타겟펀드와 대상펀드의 “수익률 변동 추이 유사성”을 TE로 먼저 측정합니다. TE는 두 펀드의 시점별 수익률 차이를 제곱해 기간 동안 누적해 산출합니다.   
- 여기에 더해 TECO는 상관계수도 함께 사용합니다. 즉 단순히 변동의 크기가 비슷한지뿐 아니라, 두 펀드의 변동이 같은 방향으로 함께 움직이는지(방향성)까지 고려합니다.   
- 구체적으로 TECO는 TE에 (2 − 상관계수) 항을 곱해 결합하므로, “변동 추이의 거리(TE)”와 “변동의 동조성(상관계수)”이 동시에 반영됩니다.   
- 따라서 타겟펀드와 대상펀드의 수익률 변동 추이가 얼마나 비슷한지는, TE로 나타나는 변동 추이 유사성과 상관계수로 나타나는 변동 방향성의 결합 결과인 TECO로 결정됩니다.
```
...
## Assistant Response
```text
- TE는 타겟펀드와 대상펀드의 주간 수익률 차이( R_t^i 와 타겟의 수익률 R_t^{j'} )를 제곱해 평균 내고 제곱근을 취함으로써, 두 펀드의 수익률 “변동 추이”가 얼마나 가깝게 일치하는지를 유사성(크기) 측면에서 나타냅니다   
- TECO는 이 TE에 더해 상관계수 ρ를 함께 반영합니다. 즉 TECO를 TE × (2 − ρ) 형태로 결합하여, 단순히 얼마나 비슷하게 움직였는지뿐 아니라 변동의 “방향성”까지 고려해 비슷함을 판단합니다   
- 따라서 TECO 지표의 “특성” 기준으로 타겟펀드와 대상펀드의 수익률 변동 추이가 얼마나 비슷한지는, ① 변동의 일치 정도(TE)와 ② 변동 방향의 동행 정도(ρ)를 함께 본 결과로 결정됩니다   
- TECO 값은 0~100 범위의 값을 가지며(현재는 0~10점으로 운영), 이 값이 곧 위 기준에 따른 유사성 수준을 의미합니다
```
