...
## Question
TECO 지표의 특성 판단의 한 가지 기준으로 수익률 변동 추이의 유사성을 선택하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "TECO 지표의 특성 판단의 한 가지 기준으로 수익률 변동 추이의 유사성을 선택하는가?",
  "qa_type": "criteria",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "6) TECO 지표의 특성",
  "question": "TECO 지표의 특성 판단의 한 가지 기준으로 수익률 변동 추이의 유사성을 선택하는가?",
...
## Assistant Response
```text
- 예. TECO 지표는 타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성을 나타내는 지표인 TE를 활용해 특성을 판단한다   
- 다만 TECO는 수익률 변동 추이의 유사성(TE)뿐 아니라 상관계수도 함께 사용해 변동의 방향성도 고려한다
```
...
## Assistant Response
```text
- 예. TECO 지표의 특성 판단에서 수익률 변동 추이의 유사성은 TE(TE 지표)가 나타내는 핵심 구성요소로 포함됩니다.   
- TECO는 타겟펀드와 대상펀드 사이의 “수익률 변동 추이의 유사성”을 TE로 확인하고, 여기에 상관계수까지 함께 활용해 변동의 방향성도 고려합니다.   
- 따라서 TECO의 특성 판단 기준은 수익률 변동 추이 유사성(TE)뿐 아니라 방향성(상관계수)까지 동시 반영하는 형태라고 정리할 수 있습니다.
```
...
## Assistant Response
```text
- 예, TECO 지표의 특성(유사펀드) 판단에서 “타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성”은 TE가 나타내는 핵심 구성요소로 포함됩니다.   
- TE는 두 펀드의 시점별 수익률 차이를 제곱해 측정하는 방식으로, 시간에 따른 수익률 흐름의 유사도를 반영합니다.   
- 다만 TECO는 유사성(TE)만 쓰는 것이 아니라 상관계수도 함께 활용하여 변동의 방향성까지 고려합니다.   
- TECO 지표 값은 0~100 사이(현재는 0~10점 사이)로 제시됩니다.
```
...
## Assistant Response
```text
- 그렇다. TECO 지표의 특성을 판단할 때, 타겟펀드와 대상펀드 사이의 수익률 변동 추이의 유사성을 나타내는 TE를 기준(한 가지 요소)로 선택한다.   
- TECO는 이 유사성(TE)뿐 아니라 상관계수도 함께 활용해, 수익률 변동 추이의 “방향성”까지 고려하도록 구성되어 있다.   
- 따라서 TECO의 특성 판단은 “수익률 변동 추이 유사성”을 포함하는 방향으로 정의되어 있다.
```
