...
## Question
BM대비 성과요인 분석의 판단 기준은 펀드수익률과 BM수익률의 차이인가?
...
```json
{
  ...
  "section": "5.1. BM대비 성과요인 분석",
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "record_target": 34,
  "requested_question_count": 36,
  "call_requested_question_count": 20,
  "call_allowed_types": [
    "criteria"
  ],
  "call_type_targets": {
  ...
  "target_total": 100,
  "generated_question": "BM대비 성과요인 분석의 판단 기준은 펀드수익률과 BM수익률의 차이인가?",
  "qa_type": "criteria",
  "question_template": "criteria_04"
}
```
...
[핵심 명사]
. BM대비 성과요인 분석, 자산배분효과 : 평균편입비수익률 – BM수익률, 자산배분타이밍효과 : 실제편입비수익률 – 평균편입비수익률, 종목선정효과 : 펀드수익률 – 실제편입비수익률, BM대비, 성과요인, 분석, 특정, 기간동안, 초과수익, 또는, 초과손실

[질문 유형 목표]
criteria 20개, definition 0개, comparison 0개, application 0개
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "BM대비 성과요인 분석의 판단 기준은 펀드수익률과 BM수익률의 차이인가?",
...
## Assistant Response
```text
- BM대비 성과요인 분석의 출발점(판단 기준)은 BM대비 초과수익률이며, 이는 펀드수익률과 BM수익률의 차이로 제시됩니다.   
- BM대비 초과수익률은 자산배분효과, 자산배분타이밍효과, 종목선정효과의 합으로 분해해 원인을 봅니다.
```
...
## Assistant Response
```text
BM대비 성과요인 분석은 “BM대비 초과수익(또는 초과손실)”이 왜 발생했는지를 판단하기 위한 것입니다.   

- 판단의 출발점은 펀드의 BM대비 초과수익률이며, 이를 자산배분효과·자산배분타이밍효과·종목선정효과로 분해합니다.   
- 이때 각 효과는 단순히 “펀드수익률–BM수익률” 한 가지 차이로만 정의되지 않고, 예를 들어 자산배분효과는 “평균편입비수익률 – BM수익률”, 종목선정효과는 “펀드수익률 – 실제편입비수익률”처럼 기준수익률 간의 차이로 산정됩니다.   
- 따라서 전체 판단 기준은 “BM대비 초과수익률” 자체(상회/하회 성과)이고, 구체적인 성과요인은 그 초과수익률을 구성하는 요소별 차이로 봅니다.
```
...
## Assistant Response
```text
BM대비 성과요인 분석은 특정 기간 동안의 BM대비 초과수익(또는 초과손실)이 왜 발생했는지를 판단하기 위한 것입니다. 

- 판단의 출발점(기준)은 펀드수익률과 BM수익률의 “단순 차이” 자체라기보다, BM대비 초과수익률이며 이를 자산배분효과+자산배분타이밍효과+종목선정효과로 분해합니다.   
- 이때 자산배분효과는 평균편입비수익률 – BM수익률, 자산배분타이밍효과는 실제편입비수익률 – 평균편입비수익률, 종목선정효과는 펀드수익률 – 실제편입비수익률처럼 각 구간의 “차이”로 정의됩니다.   
- 따라서 펀드수익률과 BM수익률의 차이만을 단일 기준으로 보아야 하는 구조는 아니고, 그 차이를 구성 효과로 나누어 해석하는 방식입니다.
```
...
## Assistant Response
```text
BM대비 성과요인 분석은 특정 기간 동안 BM 대비 초과수익(또는 초과손실)의 원인을 설명하기 위한 것입니다.   
- 판단의 기준(분해 대상)은 “BM대비 초과수익률”이며, 이는 자산배분효과 + 자산배분타이밍효과 + 종목선정효과의 합으로 정합니다.   
- 따라서 단순히 ‘펀드수익률–BM수익률’ 한 가지 차이만을 기준으로 삼기보다는, 효과별로 서로 다른 수익률 차이를 사용합니다.   
- 자산배분효과는 평균편입비수익률 – BM수익률, 자산배분타이밍효과는 실제편입비수익률 – 평균편입비수익률, 종목선정효과는 펀드수익률 – 실제편입비수익률로 정의됩니다.
```
