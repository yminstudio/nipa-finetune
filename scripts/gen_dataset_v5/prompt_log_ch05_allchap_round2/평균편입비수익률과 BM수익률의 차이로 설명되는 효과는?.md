...
```json
{
  ...
  "target_total": 100,
  "generated_question": "평균편입비수익률과 BM수익률의 차이로 설명되는 효과는?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "평균편입비수익률과 BM수익률의 차이로 설명되는 효과는?",
...
## Assistant Response
```text
- 평균편입비수익률과 BM수익률의 차이(평균편입비수익률 – BM수익률)는 자산배분효과를 설명합니다.   
- 이 차이가 (-)이면 자산배분 실패로 해석됩니다.
```
...
## Assistant Response
```text
- BM대비 초과수익의 성과요인분해에서 평균편입비수익률과 BM수익률의 차이(평균편입비수익률 – BM수익률)는 자산배분효과를 설명합니다.   
- 따라서 평균편입비수익률이 BM수익률보다 낮게 나타나면 자산배분 실패(예: 상승장에서 평균편입비를 낮게 가져간 경우)로 해석됩니다.
```
...
## Assistant Response
```text
BM대비 초과수익률을 원인별로 나눌 때, “평균편입비수익률 − BM수익률”의 차이는 자산배분효과를 의미합니다.   
평균적인 편입비가 BM의 배분 대비 어떤 수익률 결과를 냈는지를 보여주는 축입니다.   
예를 들어 상승장에서 평균편입비를 낮게 가져갔다면 평균편입비수익률이 BM보다 낮아져 자산배분효과는 (-)로 나타납니다.
```
...
## Assistant Response
```text
BM대비 초과수익률을 성과요인으로 나눌 때, 평균편입비수익률과 BM수익률의 차이는 자산배분효과를 의미합니다. 

- 자산배분효과 = 평균편입비수익률 – BM수익률   
- 즉, 평균적인 편입비가 BM 대비 “낮게” 가져가면(예: 상승장) 자산배분효과는 (-)로 나타나 자산배분 실패로 해석됩니다.
```
