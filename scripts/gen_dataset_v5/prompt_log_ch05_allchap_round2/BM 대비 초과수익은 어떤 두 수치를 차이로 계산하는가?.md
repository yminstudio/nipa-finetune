...
## Question
BM 대비 초과수익은 어떤 두 수치를 차이로 계산하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM 대비 초과수익은 어떤 두 수치를 차이로 계산하는가?",
  "qa_type": "application",
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "BM 대비 초과수익은 어떤 두 수치를 차이로 계산하는가?",
...
## Assistant Response
```text
- BM 대비 초과수익률은 펀드수익률에서 BM수익률을 차감한 값(펀드수익률 − BM수익률)으로 계산한다.
```
...
## Assistant Response
```text
- 적용 조건: 특정 기간 동안의 “BM 대비 초과수익(또는 초과손실)”을 산출할 때   
- 적용 순서: 같은 기간에 (1) 펀드수익률, (2) BM수익률을 각각 구한다   
- 전환 규칙: BM 대비 초과수익률은 두 수치의 차이로 계산하되, 펀드수익률에서 BM수익률을 차감한다
```
...
## Assistant Response
```text
- 특정 기간의 BM 대비 초과수익(또는 초과손실)은 해당 기간의 펀드수익률과 BM수익률을 비교하는 값입니다 
- 적용 순서: ① 같은 기간의 펀드수익률을 정함 ② 같은 기간의 BM수익률을 정함 ③ 두 수치를 “차이”로 계산
- 전환 규칙: BM 대비 초과수익률 = 펀드수익률 − BM수익률
```
...
## Assistant Response
```text
- 적용 대상/전제: 특정 기간의 BM대비 초과수익(초과수익률)을 구할 때, 같은 기간의 펀드수익률과 BM수익률을 사용합니다.   
- 적용 순서: 펀드수익률에서 BM수익률을 차감합니다. 즉, BM 대비 초과수익률 = 펀드수익률 - BM수익률입니다.   
- 전환 규칙(분해 시): 위 차이는 자산배분효과(평균편입비수익률 - BM수익률) + 자산배분타이밍효과(실제편입비수익률 - 평균편입비수익률) + 종목선정효과(펀드수익률 - 실제편입비수익률)로 표현해 합산됩니다.
```
