...
## Question
다음 자산이 펀드 신용평점 산출의 대상 자산에 해당하는지 판단하시오: CD.
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "다음 자산이 펀드 신용평점 산출의 대상 자산에 해당하는지 판단하시오: CD.",
  "qa_type": "application",
  "question_template": "적용_대상자산판단"
}
```
...
```json
{
  ...
  "subsection": "나) 펀드 신용평점",
  "seed_title": "나) 펀드 신용평점",
  "question": "다음 자산이 펀드 신용평점 산출의 대상 자산에 해당하는지 판단하시오: CD.",
...
## Assistant Response
```text
- 적용 순서: 펀드 신용평점 산출 시, 대상 자산에 포함된 종목을 편입비(시가평가액) 기준으로 가중해 부도율(신용평점)을 산출한다   
- 적용 조건: CD는 펀드 신용평점 산출의 대상 자산에 해당한다   
- 전환 규칙: CD는 AAA0 등급을 부여한다
```
