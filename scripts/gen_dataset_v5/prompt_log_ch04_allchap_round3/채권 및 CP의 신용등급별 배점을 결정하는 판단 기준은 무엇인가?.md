...
## Question
채권 및 CP의 신용등급별 배점을 결정하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "채권 및 CP의 신용등급별 배점을 결정하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_decision"
}
```
...
```json
{
  ...
  "subsection": "[채권 및 CP의 신용등급별 배점]",
  "seed_title": "[채권 및 CP의 신용등급별 배점]",
  "question": "채권 및 CP의 신용등급별 배점을 결정하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
채권 및 CP의 신용등급별 배점은 신용위험(부도 가능성)을 점수로 반영하기 위한 기준이다. 

- 신용 위험은 보유 채권(부도율 가중)으로 측정하며, 신용등급별 부도율은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용한다. 
- 이때 채권은 AAA~CCC이하, CP는 A1~D이하의 “신용등급별 예상부도율” 체계를 사용한다. 
- 배점은 해당 신용등급을 기준으로 채권 및 CP 등급별로 미리 정해진 “배점표(배점, 신용등급 부여)” 값에 따라 부여된다(최상등급~하등급).
```
