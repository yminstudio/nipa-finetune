...
## Question
개별종목의 상대 PER, PBR 계산에서 비교 축으로 규모별을 사용하는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "개별종목의 상대 PER, PBR 계산에서 비교 축으로 규모별을 사용하는가?",
  "qa_type": "comparison",
...
```json
{
  ...
  "subsection": "나) 개별종목의 상대 PER, PBR 계산",
  "seed_title": "나) 개별종목의 상대 PER, PBR 계산",
  "question": "개별종목의 상대 PER, PBR 계산에서 비교 축으로 규모별을 사용하는가?",
...
## Assistant Response
```text
- 네. 개별종목의 상대 PER, 상대 PBR은 종목 PER(PBR)을 “해당 종목 사이즈의 대표 PER(PBR)”로 나누는 방식으로 계산합니다.   
- 이때 비교 축(분모의 기준)은 규모별(대형, 중형, 소형)로 나뉘어 사용합니다.   
- 규모별 대표 PER(PBR)은 각 규모에서 PER(PBR) 중앙값을 대표치로 정해 산출합니다.
```
