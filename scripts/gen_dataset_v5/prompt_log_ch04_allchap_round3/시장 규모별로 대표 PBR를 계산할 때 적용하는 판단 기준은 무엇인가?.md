...
## Question
시장 규모별로 대표 PBR를 계산할 때 적용하는 판단 기준은 무엇인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "시장 규모별로 대표 PBR를 계산할 때 적용하는 판단 기준은 무엇인가?",
  "qa_type": "criteria",
  "question_template": "criteria_market_size_per_mean_pbr"
}
```
...
```json
{
  ...
  "subsection": "가) 시장의 규모별 대표 PER, PBR 계산",
  "seed_title": "가) 시장의 규모별 대표 PER, PBR 계산",
  "question": "시장 규모별로 대표 PBR를 계산할 때 적용하는 판단 기준은 무엇인가?",
...
## Assistant Response
```text
- 시장에서 거래되는 주식의 규모별 대표 PBR을 고르기 위해, 각 규모 구간에서 PBR의 중앙값(Median)을 대표치로 사용합니다   
- 대상은 거래소+코스닥의 전 종목이며(우선주, 뮤추얼펀드, 관리종목 등 제외), 규모별 해당 종목들을 집계합니다   
- 규모 구간 내에서 PBR을 높은 종목부터 내림차순으로 나열한 뒤, 누적 시가총액 비중이 50%에 도달하는 지점(= Median 구간)의 PBR을 대표 PBR로 정합니다
```
