...
```json
{
  ...
  "target_total": 100,
  "generated_question": "CP등급별 예상부도율의 정의는 무엇인가?",
  "qa_type": "definition",
...
```json
{
  ...
  "subsection": "[CP등급별 예상부도율]",
  "seed_title": "[CP등급별 예상부도율]",
  "question": "CP등급별 예상부도율의 정의는 무엇인가?",
...
## Assistant Response
```text
- 제로인에서 말하는 CP등급별 예상부도율은 CP의 신용등급(A1, A2, A3, B, C, D이하)에 대해 부여되는 예상부도율(%) 값입니다.   
- 이때 신용등급별 부도율(예상부도율)은 과거 10년간 신용평가 등급별 부도율로부터 추정된 값을 사용합니다.   
- 따라서 CP등급별 예상부도율은 각 등급에 대응하는 연간 부도위험 수준(예상치)을 수치로 정리한 것입니다.
```
