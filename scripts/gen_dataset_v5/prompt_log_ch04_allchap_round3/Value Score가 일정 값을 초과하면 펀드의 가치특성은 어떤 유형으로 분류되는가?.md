...
## Question
Value Score가 일정 값을 초과하면 펀드의 가치특성은 어떤 유형으로 분류되는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "Value Score가 일정 값을 초과하면 펀드의 가치특성은 어떤 유형으로 분류되는가?",
  "qa_type": "criteria",
  "question_template": "가치특성_임계값초과_성장형분류"
}
```
...
```json
{
  ...
  "subsection": "라) 펀드의 가치특성 구분",
  "seed_title": "라) 펀드의 가치특성 구분",
  "question": "Value Score가 일정 값을 초과하면 펀드의 가치특성은 어떤 유형으로 분류되는가?",
...
## Assistant Response
```text
펀드의 가치특성(성장형/혼합형/가치형)을 Value Score로 구분합니다.  
Value Score 값이 2.25를 초과하면 성장형으로 분류됩니다.
```
