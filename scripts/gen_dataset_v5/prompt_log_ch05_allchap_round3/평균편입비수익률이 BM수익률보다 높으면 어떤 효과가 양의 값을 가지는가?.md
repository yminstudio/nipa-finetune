...
## Question
평균편입비수익률이 BM수익률보다 높으면 어떤 효과가 양의 값을 가지는가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "평균편입비수익률이 BM수익률보다 높으면 어떤 효과가 양의 값을 가지는가?",
  "qa_type": "application",
  "question_template": "적용템플릿_자산배분효과_수익률차이"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "평균편입비수익률이 BM수익률보다 높으면 어떤 효과가 양의 값을 가지는가?",
...
## Assistant Response
```text
- 적용대상: BM 대비 초과수익률 분해에서의 자산배분효과(평균편입비수익률 − BM수익률)   
- 적용조건: 평균편입비수익률이 BM수익률보다 높음(평균편입비수익률 > BM수익률)   
- 전환규칙: 자산배분효과 = 평균편입비수익률 − BM수익률 이므로, 조건을 대입하면 자산배분효과가 0보다 큼   
- 결론: 따라서 “자산배분효과”가 양의 값(+)을 가집니다.
```
