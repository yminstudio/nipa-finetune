...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM대비 성과요인 분석의 목적은 무엇인가?",
  "qa_type": "application",
  "question_template": "적용템플릿_BM대비_성과요인분석_목적"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "BM대비 성과요인 분석의 목적은 무엇인가?",
...
## Assistant Response
```text
특정 기간 동안 BM대비 초과수익(또는 초과손실)의 “원인”을 설명하기 위해 BM대비 성과요인 분석을 수행합니다. 

- 적용 조건: 특정 기간을 정하고, 해당 기간의 BM 대비 초과수익(또는 초과손실)을 대상으로 합니다. 
- 적용 순서: BM대비 초과수익률을 자산배분효과, 자산배분타이밍효과, 종목선정효과 3가지로 나눠 원인을 분해합니다. 
- 전환 규칙(분해): BM대비 초과수익률 = 자산배분효과 + 자산배분타이밍효과 + 종목선정효과 로 전환합니다. 
- 전환 규칙(산출): 자산배분효과 = 평균편입비수익률 – BM수익률, 자산배분타이밍효과 = 실제편입비수익률 – 평균편입비수익률, 종목선정효과 = 펀드수익률 – 실제편입비수익률로 각각 계산합니다.
```
