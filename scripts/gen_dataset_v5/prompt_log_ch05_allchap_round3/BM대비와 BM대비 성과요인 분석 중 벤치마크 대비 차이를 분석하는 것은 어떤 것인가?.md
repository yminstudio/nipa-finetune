...
## Question
BM대비와 BM대비 성과요인 분석 중 벤치마크 대비 차이를 분석하는 것은 어떤 것인가?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM대비와 BM대비 성과요인 분석 중 벤치마크 대비 차이를 분석하는 것은 어떤 것인가?",
  "qa_type": "comparison",
  "question_template": "비교템플릿_BM대비_성과요인분석_대_BM대비"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "BM대비와 BM대비 성과요인 분석 중 벤치마크 대비 차이를 분석하는 것은 어떤 것인가?",
...
## Assistant Response
```text
벤치마크 대비 “차이(=BM대비 초과수익/초과손실)”의 원인을 쪼개어 보는 것은 BM대비 성과요인 분석입니다. BM대비 초과수익률을 자산배분효과, 자산배분타이밍효과, 종목선정효과의 합으로 분해해 설명합니다.
```
