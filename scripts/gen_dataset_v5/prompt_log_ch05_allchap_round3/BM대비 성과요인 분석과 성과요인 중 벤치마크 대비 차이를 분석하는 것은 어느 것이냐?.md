...
## Question
BM대비 성과요인 분석과 성과요인 중 벤치마크 대비 차이를 분석하는 것은 어느 것이냐?
...
```json
{
  ...
  "target_total": 100,
  "generated_question": "BM대비 성과요인 분석과 성과요인 중 벤치마크 대비 차이를 분석하는 것은 어느 것이냐?",
  "qa_type": "comparison",
  "question_template": "비교템플릿_BM대비_성과요인분석_대성과요인"
}
```
...
```json
{
  ...
  "subsection": "",
  "seed_title": "5.1. BM대비 성과요인 분석",
  "question": "BM대비 성과요인 분석과 성과요인 중 벤치마크 대비 차이를 분석하는 것은 어느 것이냐?",
...
## Assistant Response
```text
- BM대비 성과요인 분석은 특정 기간의 BM대비 초과수익(또는 초과손실) 원인을 자산배분효과, 자산배분타이밍효과, 종목선정효과로 나누어 설명하는 분석입니다.   
- ‘성과요인 중 벤치마크 대비 차이’는 펀드 수익률과 BM 수익률의 차이(=BM대비 초과수익률)에 해당하며, 그 차이를 위 3가지 성과요인으로 분해해 분석하는 것이 바로 BM대비 성과요인 분석입니다.
```
