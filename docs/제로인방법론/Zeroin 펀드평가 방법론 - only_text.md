...
펀드 평가를 위해 각 펀드의 기간별 성과를 계산하고 이를 표준화하여 ZI Score를 산출한다.
...
CE(i,j) : 펀드 i 의 기간 j 초과성과
ZI(i,j) : CE 값을 표준화한 ZI 점수
...
* w_j 는 기간별 가중치
* j 는 기간 인덱스 (1Y, 3Y, 5Y)
* 기간별 ZI 점수를 가중 평균하여 최종 ZI Score 산출
...
ZI 값은 최소값(Min)부터 최대값(Max)까지의 범위를 갖는다.

ZI Score는 다음과 같은 구간 기반 변환 방식으로 계산된다.
...
* ZI 값이 증가할수록 Score가 증가한다.
* Min → Max 범위에서 Score는 0 → 100으로 정규화된다.
* 구간별 선형 보간 방식으로 Score가 계산된다.
...
펀드 성과 분석에서는 다음 두 가지 수익률 분포가 사용된다.
...
* 단순 수익률은 큰 변동에서 분포 왜곡이 발생할 수 있다.
* 로그 수익률은 통계 분석에서 안정적인 분포 특성을 가진다.
...
리서치 등급 역시 동일한 퍼센트 분포 기준을 사용한다.

| 등급  | 리서치등급 | % Rank | 누적 % Rank |
| --- | ----- | ------ | --------- |
| 1등급 | ★★★★★ | 10     | 10        |
| 2등급 | ★★★★☆ | 23     | 33        |
| 3등급 | ★★★☆☆ | 34     | 67        |
...
* 펀드 성과를 백분위 기준으로 분류한다.
* 동일한 분포 기준(10/23/34/23/10)을 적용한다.
...
수익률 데이터는 분포 형태에 따라 여러 구간으로 나누어 분석된다.
...
* 수익률 분포를 기반으로 성과 위치를 판단한다.
* ZI Score 계산 시 분포 기반 표준화가 수행된다.
---
